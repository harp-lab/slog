...
| reflection | [slog-reflection.md](slog-reflection.md) | RF0–RF5 | RF0 done; RF1 slices 0/0.1/1/2 shipped ([rf1-contract.md](rf1-contract.md) as-built: plan determinism, tie-group fix, ProgramModel, ABI-2 emitter + airtightness gate + daemon cohort decoder — 167/167 under `SLOG_PLAN_ABI=2`; RF0 harness reads both shapes); 2026-08-05 pre-flip hardening (flavored-cohort dedup, fold-kind variants, gates extended); REMAINING: flip the default + migrate fixture consumers + recount, then slices 3 (attribute vocabulary) and 4 (plan goldens of record) |
