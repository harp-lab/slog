...
with Tom same day — see §12).  Status: DESIGNED, ready to execute.  This document supersedes the `(vec T)` roadmap item (the one
remaining Phase-1 deliverable in docs/primitives.md §12) and the cons-backed
`[T]` as the blessed sequence type.  It builds on docs/primitives.md (the
