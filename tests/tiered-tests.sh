...
if [ -z "$FACTS_HASH" ] || [ -z "$REC_HASH" ] || [ ! -f "build/$REC_HASH.cpp" ]; then
  ...
  CXX_FLAGS="-std=c++20 -fPIC -Idaemon -ffp-contract=off"
  native_cxx "${NATIVE_PLUGIN_FLAGS[@]}" $(ls build/$REC_HASH.cpp build/$REC_HASH.p*.cpp 2>/dev/null) $CXX_FLAGS -O0 -o build/$REC_HASH.swapO0.so 2>/dev/null
  native_cxx "${NATIVE_PLUGIN_FLAGS[@]}" $(ls build/$REC_HASH.cpp build/$REC_HASH.p*.cpp 2>/dev/null) $CXX_FLAGS -O2 -o build/$REC_HASH.swapO2.so 2>/dev/null
...
if [ -n "$LREC" ] && [ -f "build/$LREC.cpp" ]; then
  native_cxx "${NATIVE_PLUGIN_FLAGS[@]}" $(ls build/$LREC.cpp build/$LREC.p*.cpp 2>/dev/null) $CXX_FLAGS -O0 -o build/$LREC.lsO0.so 2>/dev/null
  native_cxx "${NATIVE_PLUGIN_FLAGS[@]}" $(ls build/$LREC.cpp build/$LREC.p*.cpp 2>/dev/null) $CXX_FLAGS -O2 -o build/$LREC.lsO2.so 2>/dev/null
