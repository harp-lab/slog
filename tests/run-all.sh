...
run_harness() {
  case "$1" in
    ...
    interp)      "$CXX" -O2 -Wall -std=c++20 -pthread -fopenmp -Idaemon tests/interp-operator-tests.cpp daemon/plan.cpp daemon/plan-count.cpp daemon/query.cpp daemon/sexp.cpp daemon/runtime.cpp -o build/interp-operator-tests -lgmp && ./build/interp-operator-tests ;;
