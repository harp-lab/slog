...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("7671a42b");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  r = db->getRelation("outs");
  if (r == 0) db->addRelation("outs", 1);
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("outs"), std::array<u16,1>{0}, b));
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("flag"), std::array<u16,2>{0, 1}, b));
  r = db->getRelation("f");
  ...
  // (crule (pre (let __tconst2VEe4 constef2d127de37b942baad06145) (let __tconst5Nj37 const4e07408562bedb8b60ce05c1) (let __tconst50Pp8 const990c23868b965814b117cf9a) (let __tconst3f849 const6b86b273ff34fce19d6b804e) (let __tconst9MVN10 const9f3becba6e4b1583b3e19441) (let __tconst9MPF11 constd4735e3a265e16eee03f5971)) (once) (body) (head (emit f (0 1) __tconst3f849 __tconst3f849) (emit f (0 1) __tconst9MPF11 __tconst2VEe4) (emit f (0 1) __tconst5Nj37 __tconst3f849) (emit flag (0 1) __tconst3f849 __tconst50Pp8) (emit flag (0 1) __tconst9MPF11 __tconst9MVN10)) neg_const.slog:9 #f)
  ...
  public:
    ...
    {
      head_rel[0] = db->getRelation("f");
      std::vector<u16> ord1({0, 1});
      slog::Relation* readrel2 = db->getRelation("f");
      head_index[0] = readrel2->getIndex(ord1, false);
      head_rel[1] = db->getRelation("f");
      std::vector<u16> ord3({0, 1});
      slog::Relation* readrel4 = db->getRelation("f");
      head_index[1] = readrel4->getIndex(ord3, false);
      head_rel[2] = db->getRelation("f");
      std::vector<u16> ord5({0, 1});
      slog::Relation* readrel6 = db->getRelation("f");
      head_index[2] = readrel6->getIndex(ord5, false);
      head_rel[3] = db->getRelation("flag");
      std::vector<u16> ord7({0, 1});
      slog::Relation* readrel8 = db->getRelation("flag");
      head_index[3] = readrel8->getIndex(ord7, false);
      head_rel[4] = db->getRelation("flag");
      std::vector<u16> ord9({0, 1});
      slog::Relation* readrel10 = db->getRelation("flag");
    ...
    virtual bool work()
    {
      ...
      u64 v_c2 = v_const990c23868b965814b117cf9a;
      u64 v_c3 = v_const6b86b273ff34fce19d6b804e;
      u64 v_c4 = v_const9f3becba6e4b1583b3e19441;
      ...
      ++_fires;
      slog::emit<2>(head_rel[0], head_index[0], newbatch[0], std::array<u64,2>{v_c3, v_c3}, std::array<u16,2>{0, 1});
      slog::emit<2>(head_rel[1], head_index[1], newbatch[1], std::array<u64,2>{v_c5, v_c0}, std::array<u16,2>{0, 1});
      slog::emit<2>(head_rel[2], head_index[2], newbatch[2], std::array<u64,2>{v_c1, v_c3}, std::array<u16,2>{0, 1});
      slog::emit<2>(head_rel[3], head_index[3], newbatch[3], std::array<u64,2>{v_c3, v_c2}, std::array<u16,2>{0, 1});
      slog::emit<2>(head_rel[4], head_index[4], newbatch[4], std::array<u64,2>{v_c5, v_c4}, std::array<u16,2>{0, 1});
      ...
      if (_fires) db->bumpFires("neg_const.slog:9", "once", _fires);
