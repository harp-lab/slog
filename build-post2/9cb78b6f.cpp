...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("9cb78b6f");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("fav"), std::array<u16,2>{1, 0}, b));
  ...
  // (crule (pre (let __tconst2gVz8 const4bc9274d8758b867e8c071a6)) (probe _enum (1 0) 1 __tconst2gVz8 __t4ii45) (body (join fav (1 0) 1 __t4ii45 n)) (head (emit redfans (0) n)) enum_typed_col.slog:14 #f)
  class ReadTask1 : public slog::Task
  {
    ...
    slog::Index** favindex0;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("redfans");
      std::vector<u16> ord2({0});
      slog::Relation* readrel3 = db->getRelation("redfans");
      ...
      slog::Relation* readrel7 = db->getRelation("fav");
      favindex0 = readrel7->getIndex(ord6, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::join_probe_sliced<2,1>(driver_index, std::array<u64,2>{v_c0, 0}, _sc, _rkey, _hr, [&](const std::array<u64,2>& m8) {
        ...
        slog::join_probe<2,1>(favindex0, std::array<u64,2>{v_c1, 0}, [&](const std::array<u64,2>& m9) {
          u64 v_c2 = m9[1];
          ++_fires;
      ...
      if (_fires) db->bumpFires("enum_typed_col.slog:14", "all:_enum", _fires);
