...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("92e04a9e");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<5>(db, db->getRelation("$sup35494x8x0x0x1"), std::array<u16,5>{0, 1, 2, 3, 4}, b));
  // (crule (pre (let __tconst6je736 const4e07408562bedb8b60ce05c1) (let __tconst9vxV37 constd4735e3a265e16eee03f5971)) (probe ack (1 2 0) 2 __tconst9vxV37 __tconst6je736 __t6u1r38) (body (join ack_ans (0 1) 1 __t6u1r38 r)) (head (emit res (0 1 2) __tconst9vxV37 __tconst6je736 r)) dem_ack.slog:12 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel3 = db->getRelation("res");
      head_index[0] = readrel3->getIndex(ord2, false);
      std::vector<u16> ord4({1, 2, 0});
      slog::Relation* readrel5 = db->getRelation("ack");
      driver_index = readrel5->getIndex(ord4, true);
      std::vector<u16> ord6({0, 1});
      slog::Relation* readrel7 = db->getRelation("ack_ans");
      ack_ansindex0 = readrel7->getIndex(ord6, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::join_probe_sliced<3,2>(driver_index, std::array<u64,3>{v_c1, v_c0, 0}, _sc, _rkey, _hr, [&](const std::array<u64,3>& m8) {
        u64 v_c2 = m8[2];
        if (buckethash(v_c2) != bucket) return;
        slog::join_probe<2,1>(ack_ansindex0, std::array<u64,2>{v_c2, 0}, [&](const std::array<u64,2>& m9) {
          u64 v_c3 = m9[1];
          ++_fires;
          slog::emit<3>(head_rel[0], head_index[0], newbatch[0], std::array<u64,3>{v_c1, v_c0, v_c3}, std::array<u16,3>{0, 1, 2});
        });
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("dem_ack.slog:12", "all:ack", _fires);
  ...
  // (crule (pre (let __tconst2aCL46 const4e07408562bedb8b60ce05c1)) (probe ack (1 2 0) 1 __tconst2aCL46 dup2KN2143 __t14cO47) (body (eq __tconst2aCL46 dup2KN2143) (join ack_ans (0 1) 1 __t14cO47 r)) (head (emit res (0 1 2) __tconst2aCL46 __tconst2aCL46 r)) dem_ack.slog:13 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel13 = db->getRelation("res");
      head_index[0] = readrel13->getIndex(ord12, false);
      std::vector<u16> ord14({1, 2, 0});
      slog::Relation* readrel15 = db->getRelation("ack");
      driver_index = readrel15->getIndex(ord14, true);
      std::vector<u16> ord16({0, 1});
      slog::Relation* readrel17 = db->getRelation("ack_ans");
      ack_ansindex10 = readrel17->getIndex(ord16, false);
    ...
    virtual bool work()
    {
      ...
      slog::SliceCtx _sc{_slice_deadline, &db->runStopFlag()};
      std::array<u64,3> _rkey = resume_key; bool _hr = has_resume;
      bool _done = slog::join_probe_sliced<3,1>(driver_index, std::array<u64,3>{v_c4, 0, 0}, _sc, _rkey, _hr, [&](const std::array<u64,3>& m18) {
        u64 v_c5 = m18[1];
        u64 v_c6 = m18[2];
        if (buckethash(v_c5) != bucket) return;
        if (v_c4 != v_c5) return;
        slog::join_probe<2,1>(ack_ansindex10, std::array<u64,2>{v_c6, 0}, [&](const std::array<u64,2>& m19) {
          u64 v_c3 = m19[1];
          ++_fires;
          slog::emit<3>(head_rel[0], head_index[0], newbatch[0], std::array<u64,3>{v_c4, v_c4, v_c3}, std::array<u16,3>{0, 1, 2});
        });
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("dem_ack.slog:13", "all:ack", _fires);
