...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("01d1c362");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  // (crule (pre) (scan build __t1kQP8 n) (body (join more (0 1) 1 __t1kQP8 s2)) (head (emit more_depth (0) n)) enum_union_stack.slog:15 #f)
  class ReadTask1 : public slog::Task
  {
    ...
    slog::Index** moreindex0;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("more_depth");
      std::vector<u16> ord2({0});
      slog::Relation* readrel3 = db->getRelation("more_depth");
      head_index[0] = readrel3->getIndex(ord2, false);
      outer_rel = db->getRelation("build");
      std::vector<u16> ord4({0, 1});
      slog::Relation* readrel5 = db->getRelation("more");
      moreindex0 = readrel5->getIndex(ord4, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        u64 v_c1 = _t[1];
        slog::join_probe<2,1>(moreindex0, std::array<u64,2>{v_c0, 0}, [&](const std::array<u64,2>& m6) {
          u64 v_c2 = m6[1];
          ++_fires;
      ...
      if (_fires) db->bumpFires("enum_union_stack.slog:15", "all:build", _fires);
  ...
  // (crule (pre (let __tconst2YCx9 conste433800a79f2d3be41014577)) (probe _enum (1 0) 1 __tconst2YCx9 __t3Y9Q1) (body (join build (0 1) 1 __t3Y9Q1 n)) (head (emit halt_depth (0) n)) enum_union_stack.slog:14 #f)
  class ReadTask8 : public slog::Task
  {
    ...
    slog::Index** buildindex7;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("halt_depth");
      std::vector<u16> ord9({0});
      slog::Relation* readrel10 = db->getRelation("halt_depth");
      ...
      slog::Relation* readrel14 = db->getRelation("build");
      buildindex7 = readrel14->getIndex(ord13, false);
  
    }
    ReadTask8(slog::Database* _db, u16 _b) : db(_db), bucket(_b) { bind(_db); }
    virtual bool work()
    {
      u64 v_c3 = v_conste433800a79f2d3be41014577;
      ...
      bool _done = slog::join_probe_sliced<2,1>(driver_index, std::array<u64,2>{v_c3, 0}, _sc, _rkey, _hr, [&](const std::array<u64,2>& m15) {
        u64 v_c4 = m15[1];
        if (buckethash(v_c4) != bucket) return;
        slog::join_probe<2,1>(buildindex7, std::array<u64,2>{v_c4, 0}, [&](const std::array<u64,2>& m16) {
          u64 v_c1 = m16[1];
          ++_fires;
          slog::emit<1>(head_rel[0], head_index[0], newbatch[0], std::array<u64,1>{v_c1}, std::array<u16,1>{0});
        });
      ...
      if (_fires) db->bumpFires("enum_union_stack.slog:14", "all:_enum", _fires);
  ...
  s->addReadRel("build");
  s->addReadRel("more");
  s->addDynamicRel("halt_depth");
  s->addDynamicRel("more_depth");
  d->push(s);
  d->continueRun();
}
