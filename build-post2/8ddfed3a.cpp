...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("8ddfed3a");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  // (crule (pre) (scan smt_vcode C V) (body (join smt_check (0 1) 0 __t9SxG21 F)) (head (mkstruct smt_qsat (1 0) __4uia70 F)) smt.slog:69 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel3 = db->getRelation("smt_check");
      smt_checkindex0 = readrel3->getIndex(ord2, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        u64 v_c1 = _t[1];
        slog::join_all<2>(smt_checkindex0, [&](const std::array<u64,2>& m4) {
  ...
  // (crule (pre) (scan smt_bad_formula __erre78pr93 __errf7rdj94 __errf8ni795) (body) (head (emit error (0) __erre78pr93)) <internal>:1 #f)
  ...
  // (crule (pre) (scan smt_vcode C V) (body (join smt_core (0 1) 0 __t53nm24 F)) (head (mkstruct smt_qcore (1 0) __5NHc69 F)) smt.slog:106 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel11 = db->getRelation("smt_core");
      smt_coreindex8 = readrel11->getIndex(ord10, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        u64 v_c1 = _t[1];
        slog::join_all<2>(smt_coreindex8, [&](const std::array<u64,2>& m12) {
          u64 v_c7 = m12[0]; u64 v_c3 = m12[1];
          ++_fires;
          slog::emit_struct<2>(head_rel[0], newbatch[0], std::array<u64,1>{v_c3}, std::array<u16,2>{1, 0});
        });
      });
  ...
  // (crule (pre) (scan smt_vcode C V) (body (join smt_model (0 1) 0 __t7bdF65 F)) (head (mkstruct smt_qmodel (1 0) __1bgK75 F)) smt.slog:93 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel16 = db->getRelation("smt_model");
      smt_modelindex13 = readrel16->getIndex(ord15, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        u64 v_c1 = _t[1];
        slog::join_all<2>(smt_modelindex13, [&](const std::array<u64,2>& m17) {
          u64 v_c8 = m17[0]; u64 v_c3 = m17[1];
          ++_fires;
          slog::emit_struct<2>(head_rel[0], newbatch[0], std::array<u64,1>{v_c3}, std::array<u16,2>{1, 0});
        });
      });
