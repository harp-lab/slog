...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("ae5916b1");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  r = db->getRelation("temp8hr677");
  if (r == 0) db->addTempRelation("temp8hr677", 2);
  else if (r->getArity() != 2) slog::fatal("Temp rel arity mismatch.");
  r = db->getRelation("temp7pLu80");
  if (r == 0) db->addTempRelation("temp7pLu80", 3);
  else if (r->getArity() != 3) slog::fatal("Temp rel arity mismatch.");
  r = db->getRelation("temp7IkN79");
  if (r == 0) db->addTempRelation("temp7IkN79", 4);
  else if (r->getArity() != 4) slog::fatal("Temp rel arity mismatch.");
  r = db->getRelation("temp5HZO81");
  if (r == 0) db->addTempRelation("temp5HZO81", 3);
  else if (r->getArity() != 3) slog::fatal("Temp rel arity mismatch.");
  r = db->getRelation("temp1nWM78");
  if (r == 0) db->addTempRelation("temp1nWM78", 2);
  else if (r->getArity() != 2) slog::fatal("Temp rel arity mismatch.");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("bind"), std::array<u16,2>{1, 0}, b));
  ...
  // (crule (pre) (scan type_mismatch __erre4MDZ64 __errf0o2V65 __errf9HwC66 __errf2t5M67 __errf5RbR68) (body) (head (emit error (0) __erre4MDZ64)) <internal>:1 #f)
  ...
  // (crule (pre) (scan nan_result __erre1z4w57 __errf7AJr58 __errf08mN59 __errf5gdM60) (body) (head (emit error (0) __erre1z4w57)) <internal>:1 #f)
  ...
  // (crule (pre) (scan mpz_table_overflow __erre3fq274 __errf9o3K75 __errf4Hf776) (body) (head (emit error (0) __erre3fq274)) <internal>:1 #f)
  ...
  // (crule (pre (let __t9XNZ16 (one))) (scan bind S A) (body) (head (emit-temp temp5HZO81 A S __t9XNZ16)) lat_count.slog:24 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("temp5HZO81");
    ...
    virtual bool work()
    {
      u64 v_c12 = _prim_one(db);
      if (v_c12 == slog_error) { slog::emit_pending_error(db, "lat_count.slog:24"); return true; }
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        ...
        u64 v_c14 = _t[1];
        ++_fires;
        slog::emit_temp<3>(head_rel[0], newbatch[0], std::array<u64,3>{v_c14, v_c13, v_c12});
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("lat_count.slog:24", "all:bind", _fires);
  ...
  // (crule (pre (let __t6Shj1 (inf))) (scan bind S2 A) (body (join bind (1 0) 1 A S1) (neq S1 S2)) (head (emit-temp temp1nWM78 A __t6Shj1)) lat_count.slog:22 #f)
  class ReadTask11 : public slog::Task
  {
    ...
    slog::Index** bindindex10;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("temp1nWM78");
      outer_rel = db->getRelation("bind");
      std::vector<u16> ord12({1, 0});
      slog::Relation* readrel13 = db->getRelation("bind");
      bindindex10 = readrel13->getIndex(ord12, false);
    ...
    virtual bool work()
    {
      ...
      if (v_c15 == slog_error) { slog::emit_pending_error(db, "lat_count.slog:22"); return true; }
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c16 = _t[0];
        u64 v_c14 = _t[1];
        slog::join_probe<2,1>(bindindex10, std::array<u64,2>{v_c14, 0}, [&](const std::array<u64,2>& m14) {
          u64 v_c17 = m14[1];
      ...
      if (_fires) db->bumpFires("lat_count.slog:22", "all:bind", _fires);
  ...
  // (crule (pre) (scan temp5HZO81 A S __t9XNZ16) (body) (head (emit-lat mucnt S A __t9XNZ16)) lat_count.slog:24 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("mucnt");
      outer_rel = db->getRelation("temp5HZO81");
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        ...
        u64 v_c12 = _t[2];
        ++_fires;
        slog::emit_temp<3>(head_rel[0], newbatch[0], std::array<u64,3>{v_c13, v_c14, v_c12});
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("lat_count.slog:24", "delta:temp5HZO81", _fires);
  ...
  // (crule (pre) (scan toint_range __erre5wXe61 __errf2wqZ62 __errf8Tgj63) (body) (head (emit error (0) __erre5wXe61)) <internal>:1 #f)
  ...
  // (crule (pre) (scan temp1nWM78 A __t6Shj1) (body) (head (emit-lat mu A __t6Shj1)) lat_count.slog:22 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      outer_rel = db->getRelation("temp1nWM78");
    ...
    virtual bool work()
    {
      ...
      if (_fires) db->bumpFires("lat_count.slog:22", "delta:temp1nWM78", _fires);
  ...
  // (crule (pre (let __t5hAV3 (one))) (scan bind S A) (body) (head (emit-temp temp8hr677 A __t5hAV3)) lat_count.slog:21 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("temp8hr677");
      outer_rel = db->getRelation("bind");
    ...
    virtual bool work()
    {
      ...
      if (v_c21 == slog_error) { slog::emit_pending_error(db, "lat_count.slog:21"); return true; }
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        ...
        u64 v_c14 = _t[1];
        ++_fires;
        slog::emit_temp<2>(head_rel[0], newbatch[0], std::array<u64,2>{v_c14, v_c21});
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("lat_count.slog:21", "all:bind", _fires);
  ...
  // (crule (pre) (scan div_by_zero __erre3YG247 __errf0Qpl48 __errf8WAz49) (body) (head (emit error (0) __erre3YG247)) <internal>:1 #f)
  ...
  // (crule (pre) (scan mpz_overflow __erre6P2C69 __errf1ZRO70 __errf930G71 __errf4obc72 __errf58BZ73) (body) (head (emit error (0) __erre6P2C69)) <internal>:1 #f)
  ...
  // (crule (pre) (scan temp7IkN79 A C S2 __t9Z3R4) (body (let __t6STO5 (cplus C __t9Z3R4))) (head (emit-temp temp7pLu80 A S2 __t6STO5)) lat_count.slog:25 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("temp7pLu80");
      outer_rel = db->getRelation("temp7IkN79");
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        ...
        u64 v_c16 = _t[2];
        u64 v_c31 = _t[3];
        u64 v_c32 = _prim_cplus(db, v_c30, v_c31);
        if (v_c32 == slog_error) { slog::emit_pending_error(db, "lat_count.slog:25"); return; }
        ++_fires;
        slog::emit_temp<3>(head_rel[0], newbatch[0], std::array<u64,3>{v_c14, v_c16, v_c32});
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("lat_count.slog:25", "delta:temp7IkN79", _fires);
  ...
  // (crule (pre (let __t9Z3R4 (one))) (scan mucnt S A C) (body (join step (0 1) 1 S S2)) (head (emit-temp temp7IkN79 A C S2 __t9Z3R4)) lat_count.slog:25 #f)
  class ReadTask29 : public slog::Task
  {
    ...
    slog::Index** stepindex28;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("temp7IkN79");
      outer_rel = db->getRelation("mucnt");
      std::vector<u16> ord30({0, 1});
      slog::Relation* readrel31 = db->getRelation("step");
      stepindex28 = readrel31->getIndex(ord30, false);
    ...
    virtual bool work()
    {
      ...
      if (v_c31 == slog_error) { slog::emit_pending_error(db, "lat_count.slog:25"); return true; }
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        ...
        u64 v_c14 = _t[1];
        u64 v_c30 = _t[2];
        slog::join_probe<2,1>(stepindex28, std::array<u64,2>{v_c13, 0}, [&](const std::array<u64,2>& m32) {
          u64 v_c16 = m32[1];
          ++_fires;
          slog::emit_temp<4>(head_rel[0], newbatch[0], std::array<u64,4>{v_c14, v_c30, v_c16, v_c31});
        });
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("lat_count.slog:25", "delta:mucnt", _fires);
  ...
  // (crule (pre) (scan modulo_by_zero __erre89QS50 __errf0pnw51 __errf6yjj52) (body) (head (emit error (0) __erre89QS50)) <internal>:1 #f)
  ...
  // (crule (pre) (scan int_overflow __erre4Dlv53 __errf96r754 __errf914Q55 __errf4ovy56) (body) (head (emit error (0) __erre4Dlv53)) <internal>:1 #f)
  ...
  // (crule (pre) (scan temp8hr677 A __t5hAV3) (body) (head (emit-lat mu A __t5hAV3)) lat_count.slog:21 #f)
  ...
  public:
    ...
    {
      head_rel[0] = db->getRelation("mu");
      outer_rel = db->getRelation("temp8hr677");
    ...
    virtual bool work()
    {
      ...
      if (_fires) db->bumpFires("lat_count.slog:21", "delta:temp8hr677", _fires);
  ...
  // (crule (pre) (scan temp7pLu80 A S2 __t6STO5) (body) (head (emit-lat mucnt S2 A __t6STO5)) lat_count.slog:25 #f)
  ...
  public:
    ...
    {
      head_rel[0] = db->getRelation("mucnt");
      outer_rel = db->getRelation("temp7pLu80");
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        ...
        u64 v_c32 = _t[2];
        ++_fires;
        slog::emit_temp<3>(head_rel[0], newbatch[0], std::array<u64,3>{v_c16, v_c14, v_c32});
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("lat_count.slog:25", "delta:temp7pLu80", _fires);
  ...
  s->addReadRel("step");
  s->addReadRel("temp1nWM78");
  s->addReadRel("temp5HZO81");
  s->addReadRel("temp7IkN79");
  s->addReadRel("temp7pLu80");
  s->addReadRel("temp8hr677");
  ...
  s->addDynamicRel("mu");
  s->addDynamicRel("mucnt");
  s->addDynamicRel("nan_result");
  s->addDynamicRel("temp1nWM78");
  s->addDynamicRel("temp5HZO81");
  s->addDynamicRel("temp7IkN79");
  s->addDynamicRel("temp7pLu80");
  s->addDynamicRel("temp8hr677");
