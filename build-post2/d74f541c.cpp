...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("d74f541c");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("r"), std::array<u16,2>{0, 1}, b));
  ...
  // (crule (pre) (scan r a b) (body (join r (0 1) 0 c d)) (head (emit r (0 1) a d)) stat_selfcart.slog:26 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel3 = db->getRelation("r");
      head_index[0] = readrel3->getIndex(ord2, false);
      outer_rel = db->getRelation("r");
      std::vector<u16> ord4({0, 1});
      slog::Relation* readrel5 = db->getRelation("r");
      rindex0 = readrel5->getIndex(ord4, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        u64 v_c1 = _t[1];
        slog::join_all<2>(rindex0, [&](const std::array<u64,2>& m6) {
          u64 v_c2 = m6[0]; u64 v_c3 = m6[1];
          ++_fires;
      ...
      if (_fires) db->bumpFires("stat_selfcart.slog:26", "delta:r", _fires);
  ...
  // (crule (pre (let __tconst5Ahr4 const4e07408562bedb8b60ce05c1) (let __tconst6uEH3 const4b227777d4dd1fc61c6f884f) (let __tconst8qQk2 const6b86b273ff34fce19d6b804e) (let __tconst3hXj1 constd4735e3a265e16eee03f5971)) (once) (body) (head (emit r (0 1) __tconst5Ahr4 __tconst6uEH3) (emit r (0 1) __tconst8qQk2 __tconst3hXj1)) stat_selfcart.slog:25 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel9 = db->getRelation("r");
      head_index[0] = readrel9->getIndex(ord8, false);
      head_rel[1] = db->getRelation("r");
      std::vector<u16> ord10({0, 1});
      slog::Relation* readrel11 = db->getRelation("r");
    ...
    virtual bool work()
    {
      ...
      slog::emit<2>(head_rel[1], head_index[1], newbatch[1], std::array<u64,2>{v_c6, v_c7}, std::array<u16,2>{0, 1});
  
        head_rel[0]->sendBatch(newbatch[0]);
        head_rel[1]->sendBatch(newbatch[1]);
  
      if (_fires) db->bumpFires("stat_selfcart.slog:25", "once", _fires);
  ...
  // (crule (pre) (scan r c d) (body (join-old r (0 1) 0 (0 1) a b)) (head (emit r (0 1) a d)) stat_selfcart.slog:26 #f)
  class ReadTask14 : public slog::Task
  {
    ...
    slog::Index** rindex12;  slog::Index** rdelta13;
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel16 = db->getRelation("r");
      head_index[0] = readrel16->getIndex(ord15, false);
      outer_rel = db->getRelation("r");
      std::vector<u16> ord17({0, 1});
      slog::Relation* readrel18 = db->getRelation("r");
      rindex12 = readrel18->getIndex(ord17, false);
      std::vector<u16> ord19({0, 1});
      slog::Relation* readrel20 = db->getRelation("r");
      rdelta13 = readrel20->getIndex(ord19, true);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c2 = _t[0];
        u64 v_c3 = _t[1];
        slog::join_all_old<2>(rindex12, rdelta13, [&](const std::array<u64,2>& m21) {
          u64 v_c0 = m21[0]; u64 v_c1 = m21[1];
          ++_fires;
          slog::emit<2>(head_rel[0], head_index[0], newbatch[0], std::array<u64,2>{v_c0, v_c3}, std::array<u16,2>{0, 1});
        });
      });
