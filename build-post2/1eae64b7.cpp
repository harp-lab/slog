...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("1eae64b7");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  r = db->getRelation("temp5Q6773");
  if (r == 0) db->addTempRelation("temp5Q6773", 2);
  ...
  r = db->getRelation("more_depth");
  if (r == 0) db->addRelation("more_depth", 1);
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("more_depth"), std::array<u16,1>{0}, b));
  r = db->getRelation("more");
  if (r == 0) db->addStruct("more", 2);
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternStructTask<2>(db, db->getRelation("more"), std::array<u16,2>{1, 0}, b));
  ...
  r = db->getRelation("halt_depth");
  if (r == 0) db->addRelation("halt_depth", 1);
  else if (r->getArity() != 1)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("halt_depth");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("halt_depth"), std::array<u16,1>{0}, b));
  ...
  r = db->getRelation("build");
  if (r == 0) db->addRelation("build", 2);
  else if (r->getArity() != 2)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("build");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("build"), std::array<u16,2>{0, 1}, b));
  ...
  // (crule (pre) (scan mpz_overflow __erre5vMn65 __errf2nyT66 __errf9v7U67 __errf122R68 __errf0fBc69) (body) (head (emit error (0) __erre5vMn65)) <internal>:1 #f)
  ...
  // (crule (pre) (scan mpz_table_overflow __erre4ypv70 __errf98O471 __errf99H072) (body) (head (emit error (0) __erre4ypv70)) <internal>:1 #f)
  ...
  // (crule (pre) (scan int_overflow __erre6SNv49 __errf46O450 __errf3kxL51 __errf6wbq52) (body) (head (emit error (0) __erre6SNv49)) <internal>:1 #f)
  ...
  // (crule (pre) (scan toint_range __erre6yf557 __errf0oDM58 __errf2lha59) (body) (head (emit error (0) __erre6yf557)) <internal>:1 #f)
  ...
  // (crule (pre) (scan type_mismatch __erre2WPs60 __errf9XeM61 __errf6zBx62 __errf4NA463 __errf6IL864) (body) (head (emit error (0) __erre2WPs60)) <internal>:1 #f)
  ...
  // (crule (pre) (scan div_by_zero __erre424643 __errf9aH144 __errf0ZU145) (body) (head (emit error (0) __erre424643)) <internal>:1 #f)
  ...
  // (crule (pre (let __tconst9VMN5 const4e07408562bedb8b60ce05c1) (let __tconst47HR6 const6b86b273ff34fce19d6b804e)) (scan build s n) (body (cmp lt n __tconst9VMN5) (let __t5vxQ7 (_0002b n __tconst47HR6))) (head (emit-temp temp5Q6773 __t5vxQ7 s) (mkstruct more (1 0) __t0loK4 s)) enum_union_stack.slog:13 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("temp5Q6773");
      head_rel[1] = db->getRelation("more");
      outer_rel = db->getRelation("build");
  
    }
    ReadTask18(slog::Database* _db, u16 _b) : db(_db), bucket(_b) { bind(_db); }
    virtual bool work()
    {
      u64 v_c23 = v_const4e07408562bedb8b60ce05c1;
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c25 = _t[0];
        u64 v_c26 = _t[1];
        u64 v_c27 = _prim_lt(db, v_c26, v_c23);
        if (v_c27 == slog_error) { slog::emit_pending_error(db, "enum_union_stack.slog:13"); return; }
        if (!v_c27) return;
        u64 v_c28 = _prim__0002b(db, v_c26, v_c24);
        if (v_c28 == slog_error) { slog::emit_pending_error(db, "enum_union_stack.slog:13"); return; }
        ++_fires;
      ...
      if (_fires) db->bumpFires("enum_union_stack.slog:13", "delta:build", _fires);
  ...
  // (crule (pre) (scan temp5Q6773 __t5vxQ7 s) (body (join more (1 0) 1 s __t0loK4)) (head (emit build (0 1) __t0loK4 __t5vxQ7)) enum_union_stack.slog:13 #f)
  class ReadTask21 : public slog::Task
  {
    ...
    slog::Index** moreindex20;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("build");
      std::vector<u16> ord22({0, 1});
      slog::Relation* readrel23 = db->getRelation("build");
      head_index[0] = readrel23->getIndex(ord22, false);
      outer_rel = db->getRelation("temp5Q6773");
      std::vector<u16> ord24({1, 0});
      slog::Relation* readrel25 = db->getRelation("more");
      moreindex20 = readrel25->getIndex(ord24, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c28 = _t[0];
        u64 v_c25 = _t[1];
        slog::join_probe<2,1>(moreindex20, std::array<u64,2>{v_c25, 0}, [&](const std::array<u64,2>& m26) {
          u64 v_c29 = m26[1];
          ++_fires;
          slog::emit<2>(head_rel[0], head_index[0], newbatch[0], std::array<u64,2>{v_c29, v_c28}, std::array<u16,2>{0, 1});
        });
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("enum_union_stack.slog:13", "delta:temp5Q6773", _fires);
  ...
  // (crule (pre (let __tconst17ms10 conste433800a79f2d3be41014577) (let __tconst51UD2 const5feceb66ffc86f38d952786c)) (probe _enum (1 0) 1 __tconst17ms10 __t6Xde3) (body) (head (emit build (0 1) __t6Xde3 __tconst51UD2)) enum_union_stack.slog:10 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel29 = db->getRelation("build");
    ...
    virtual bool work()
    {
      u64 v_c30 = v_conste433800a79f2d3be41014577;
      ...
      bool _done = slog::join_probe_sliced<2,1>(driver_index, std::array<u64,2>{v_c30, 0}, _sc, _rkey, _hr, [&](const std::array<u64,2>& m32) {
        u64 v_c32 = m32[1];
        if (buckethash(v_c32) != bucket) return;
        ++_fires;
        slog::emit<2>(head_rel[0], head_index[0], newbatch[0], std::array<u64,2>{v_c32, v_c31}, std::array<u16,2>{0, 1});
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("enum_union_stack.slog:10", "all:_enum", _fires);
  ...
  // (crule (pre) (scan nan_result __erre1Nlw53 __errf9w0X54 __errf4qFr55 __errf7Q7I56) (body) (head (emit error (0) __erre1Nlw53)) <internal>:1 #f)
  ...
  // (crule (pre) (scan modulo_by_zero __erre1bKO46 __errf1irF47 __errf7ImD48) (body) (head (emit error (0) __erre1bKO46)) <internal>:1 #f)
  ...
  s->addReadRel("temp5Q6773");
  s->addReadRel("toint_range");
  s->addReadRel("type_mismatch");
  s->addDynamicRel("build");
  s->addDynamicRel("div_by_zero");
  s->addDynamicRel("error");
  s->addDynamicRel("int_overflow");
  s->addDynamicRel("modulo_by_zero");
  s->addDynamicRel("more");
  s->addDynamicRel("mpz_overflow");
  s->addDynamicRel("mpz_table_overflow");
  s->addDynamicRel("nan_result");
  s->addDynamicRel("temp5Q6773");
