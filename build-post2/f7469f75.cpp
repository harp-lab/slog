...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("f7469f75");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  r = db->getRelation("marker");
  if (r == 0) db->addRelation("marker", 1);
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("marker"), std::array<u16,1>{0}, b));
  ...
  // (crule (pre (let __tconst585n8 const6b86b273ff34fce19d6b804e)) (once) (body) (head (emit marker (0) __tconst585n8)) lat_run_main.slog:9 #f)
  ...
  public:
    ...
    {
      head_rel[0] = db->getRelation("marker");
      std::vector<u16> ord1({0});
      slog::Relation* readrel2 = db->getRelation("marker");
    ...
    virtual bool work()
    {
      ...
      if (_fires) db->bumpFires("lat_run_main.slog:9", "once", _fires);
