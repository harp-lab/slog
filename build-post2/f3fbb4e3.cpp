...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("f3fbb4e3");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  // (crule (pre) (scan a X) (body (absent b (0) 1 X)) (head (emit onlya (0) X)) neg_diff.slog:11 #f)
  ...
  public:
    ...
    {
      head_rel[0] = db->getRelation("onlya");
      std::vector<u16> ord2({0});
      slog::Relation* readrel3 = db->getRelation("onlya");
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        if (!slog::absent_probe<1,1>(bindex0, std::array<u64,1>{v_c0})) return;
        ++_fires;
      ...
      if (_fires) db->bumpFires("neg_diff.slog:11", "all:a", _fires);
