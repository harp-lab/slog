...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("6d5cce77");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  r = db->getRelation("thing");
  if (r == 0) db->addRelation("thing", 1);
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("thing"), std::array<u16,1>{0}, b));
  ...
  r = db->getRelation("incremented");
  if (r == 0) db->addRelation("incremented", 1);
  else if (r->getArity() != 1)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("incremented");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("incremented"), std::array<u16,1>{0}, b));
  ...
  // (crule (pre (let __tconst7bjV2 const7f2a66c4f14264a9066b1c36) (let __tconst67k21 const6f4b6612125fb3a0daecd279)) (once) (body) (head (emit thing (0) __tconst7bjV2) (emit thing (0) __tconst67k21)) err_typemix.slog:6 #f)
  ...
  public:
    ...
    {
      head_rel[0] = db->getRelation("thing");
      std::vector<u16> ord1({0});
      slog::Relation* readrel2 = db->getRelation("thing");
      head_index[0] = readrel2->getIndex(ord1, false);
      head_rel[1] = db->getRelation("thing");
      std::vector<u16> ord3({0});
      slog::Relation* readrel4 = db->getRelation("thing");
    ...
    virtual bool work()
    {
      u64 v_c0 = v_const7f2a66c4f14264a9066b1c36;
      u64 v_c1 = v_const6f4b6612125fb3a0daecd279;
      ...
      if (_fires) db->bumpFires("err_typemix.slog:6", "once", _fires);
