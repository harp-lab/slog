...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("583f1e35");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("rm"), std::array<u16,1>{0}, b));
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("lm"), std::array<u16,1>{0}, b));
  ...
  // (crule (pre) (scan lm __t1bK33) (body (join Nd (0 1 2) 1 __t1bK33 l ____5ZuX4)) (head (emit lm (0) l)) frozen_fact.slog:14 #f)
  ...
  public:
    ...
    {
      head_rel[0] = db->getRelation("lm");
      std::vector<u16> ord2({0});
      slog::Relation* readrel3 = db->getRelation("lm");
      head_index[0] = readrel3->getIndex(ord2, false);
      outer_rel = db->getRelation("lm");
      std::vector<u16> ord4({0, 1, 2});
      slog::Relation* readrel5 = db->getRelation("Nd");
      Ndindex0 = readrel5->getIndex(ord4, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        slog::join_probe<3,1>(Ndindex0, std::array<u64,3>{v_c0, 0, 0}, [&](const std::array<u64,3>& m6) {
          u64 v_c1 = m6[1]; u64 v_c2 = m6[2];
          ++_fires;
      ...
      if (_fires) db->bumpFires("frozen_fact.slog:14", "delta:lm", _fires);
  ...
  // (crule (pre) (scan t t0) (body) (head (emit lm (0) t0)) frozen_fact.slog:13 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel9 = db->getRelation("lm");
    ...
    virtual bool work()
    {
      ...
      if (_fires) db->bumpFires("frozen_fact.slog:13", "all:t", _fires);
  ...
  // (crule (pre) (scan t t0) (body) (head (emit rm (0) t0)) frozen_fact.slog:16 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("rm");
      std::vector<u16> ord11({0});
      slog::Relation* readrel12 = db->getRelation("rm");
    ...
    virtual bool work()
    {
      ...
      if (_fires) db->bumpFires("frozen_fact.slog:16", "all:t", _fires);
  ...
  // (crule (pre) (scan rm __t76E55) (body (join Nd (0 1 2) 1 __t76E55 ____2sBt6 r)) (head (emit rm (0) r)) frozen_fact.slog:17 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel16 = db->getRelation("rm");
      head_index[0] = readrel16->getIndex(ord15, false);
      outer_rel = db->getRelation("rm");
      std::vector<u16> ord17({0, 1, 2});
      slog::Relation* readrel18 = db->getRelation("Nd");
      Ndindex13 = readrel18->getIndex(ord17, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c4 = _t[0];
        slog::join_probe<3,1>(Ndindex13, std::array<u64,3>{v_c4, 0, 0}, [&](const std::array<u64,3>& m19) {
          u64 v_c5 = m19[1]; u64 v_c6 = m19[2];
          ++_fires;
      ...
      if (_fires) db->bumpFires("frozen_fact.slog:17", "delta:rm", _fires);
