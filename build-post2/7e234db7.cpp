...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("7e234db7");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  r = db->getRelation("temp7D2P376");
  if (r == 0) db->addTempRelation("temp7D2P376", 1);
  else if (r->getArity() != 1) slog::fatal("Temp rel arity mismatch.");
  r = db->getRelation("temp2UrR375");
  if (r == 0) db->addTempRelation("temp2UrR375", 1);
  else if (r->getArity() != 1) slog::fatal("Temp rel arity mismatch.");
  ...
  // (crule (pre) (scan check_err ID ES) (body) (head (emit-temp temp2UrR375 ES) (mkstruct lany (1 0) __t6MlS95 ES)) verify.slog:83 #f)
  ...
  public:
    ...
    {
      head_rel[0] = db->getRelation("temp2UrR375");
      head_rel[1] = db->getRelation("lany");
      outer_rel = db->getRelation("check_err");
    ...
    virtual bool work()
    {
      ...
      if (_fires) db->bumpFires("verify.slog:83", "all:check_err", _fires);
  ...
  // (crule (pre) (scan check_err ID ES) (body) (head (emit-temp temp7D2P376 ES) (mkstruct lany (1 0) __t6yF533 ES)) verify.slog:79 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("temp7D2P376");
      head_rel[1] = db->getRelation("lany");
      outer_rel = db->getRelation("check_err");
    ...
    virtual bool work()
    {
      ...
      if (_fires) db->bumpFires("verify.slog:79", "all:check_err", _fires);
  ...
  // (crule (pre) (scan temp2UrR375 ES) (body (join lany (1 0) 1 ES __t6MlS95)) (head (mkstruct smt_check (1 0) __9hMJ132 __t6MlS95)) verify.slog:83 #f)
  class ReadTask3 : public slog::Task
  {
    ...
    slog::Index** lanyindex2;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("smt_check");
      outer_rel = db->getRelation("temp2UrR375");
      std::vector<u16> ord4({1, 0});
      slog::Relation* readrel5 = db->getRelation("lany");
      lanyindex2 = readrel5->getIndex(ord4, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c1 = _t[0];
        slog::join_probe<2,1>(lanyindex2, std::array<u64,2>{v_c1, 0}, [&](const std::array<u64,2>& m6) {
          u64 v_c2 = m6[1];
          ++_fires;
      ...
      if (_fires) db->bumpFires("verify.slog:83", "delta:temp2UrR375", _fires);
  ...
  // (crule (pre) (scan temp7D2P376 ES) (body (join lany (1 0) 1 ES __t6yF533)) (head (mkstruct smt_check (1 0) __2nJw144 __t6yF533)) verify.slog:79 #f)
  class ReadTask8 : public slog::Task
  {
    ...
    slog::Index** lanyindex7;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("smt_check");
      outer_rel = db->getRelation("temp7D2P376");
      std::vector<u16> ord9({1, 0});
      slog::Relation* readrel10 = db->getRelation("lany");
      lanyindex7 = readrel10->getIndex(ord9, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c1 = _t[0];
        slog::join_probe<2,1>(lanyindex7, std::array<u64,2>{v_c1, 0}, [&](const std::array<u64,2>& m11) {
          u64 v_c3 = m11[1];
          ++_fires;
      ...
      if (_fires) db->bumpFires("verify.slog:79", "delta:temp7D2P376", _fires);
  ...
  s->addReadRel("check_err");
  s->addReadRel("lany");
  s->addReadRel("temp2UrR375");
  s->addReadRel("temp7D2P376");
  s->addDynamicRel("lany");
  s->addDynamicRel("smt_check");
  s->addDynamicRel("temp2UrR375");
  s->addDynamicRel("temp7D2P376");
  d->push(s);
  d->continueRun();
}
