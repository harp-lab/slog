...
u64 v_const3f2485fdd1ec0c7e7f57dd40;
u64 v_constda920246a08a91bf4f7e1b10;


extern "C" void slog_plugin(slog::Daemon* d)
{
  slog::Database* db = d->db();
  slog::Stratum* s = d->beginStratum("5b8f3126");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  v_const3f2485fdd1ec0c7e7f57dd40 = db->encodeString("lat_cjoin.slog:19");
  v_constda920246a08a91bf4f7e1b10 = db->encodeString("mc");
  r = db->getRelation("usz");
  if (r == 0) db->addRelation("usz", 2);
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("usz"), std::array<u16,2>{0, 1}, b));
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("sc"), std::array<u16,1>{0}, b));
  r = db->getRelation("sb");
  if (r == 0) db->addRelation("sb", 2);
  else if (r->getArity() != 2)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("sb");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::LatticeInternTask<2>(db, db->getRelation("sb"), std::array<u16,2>{0, 1}, b));
  r = db->getRelation("sa");
  if (r == 0) db->addRelation("sa", 2);
  else if (r->getArity() != 2)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("sa");
  r->setLattice(LAT_EXTERN, false, 0, false, 0, "set-int", db->collections());
    ...
    r->addMapIndex<2>(ord54);
    for (u16 b = 0; b < 32; ++b)
      s->addTask(phase_write, new slog::MapWriteTask<2>(db, r, std::array<u16,2>{0, 1}, b), true);
  

  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::LatticeInternTask<2>(db, db->getRelation("sa"), std::array<u16,2>{0, 1}, b));
  ...
  r->setLattice(LAT_EXTERN, false, 0, false, 0, "map-int-min-int", db->collections());
    ...
    r->addMapIndex<2>(ord63);
    for (u16 b = 0; b < 32; ++b)
      s->addTask(phase_write, new slog::MapWriteTask<2>(db, r, std::array<u16,2>{0, 1}, b), true);
  

  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::LatticeInternTask<2>(db, db->getRelation("mc"), std::array<u16,2>{0, 1}, b));
  r = db->getRelation("mb");
  if (r == 0) db->addRelation("mb", 2);
  else if (r->getArity() != 2)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("mb");
  r->setLattice(LAT_EXTERN, false, 0, false, 0, "map-int-min-int", db->collections());
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::LatticeInternTask<2>(db, db->getRelation("mb"), std::array<u16,2>{0, 1}, b));
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::LatticeInternTask<2>(db, db->getRelation("ma"), std::array<u16,2>{0, 1}, b));
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<3>(db, db->getRelation("got"), std::array<u16,3>{0, 1, 2}, b));
  ...
  // (crule (pre) (scan type_mismatch __erre3WjW163 __errf6KvX164 __errf6pRH165 __errf0ZRz166 __errf0kyK167) (body) (head (emit error (0) __erre3WjW163)) <internal>:1 #f)
  ...
  // (crule (pre) (scan modulo_by_zero __erre7wcF149 __errf099h150 __errf8xkO151) (body) (head (emit error (0) __erre7wcF149)) <internal>:1 #f)
  ...
  // (crule (pre) (scan nan_result __erre9S7n156 __errf1jVP157 __errf2uMJ158 __errf9fS3159) (body) (head (emit error (0) __erre9S7n156)) <internal>:1 #f)
  ...
  // (crule (pre) (scan sa k a) (body (join-lat sb (0 1) 1 k b) (cjoin __t0OkC21 (set int) a b) (let __t6G0j22 (csize __t0OkC21))) (head (emit sc (0) __t6G0j22)) lat_cjoin.slog:35 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("sc");
      std::vector<u16> ord11({0});
      slog::Relation* readrel12 = db->getRelation("sc");
      head_index[0] = readrel12->getIndex(ord11, false);
      outer_rel = db->getRelation("sa");
      std::vector<u16> ord13({0, 1});
      slog::Relation* readrel14 = db->getRelation("sb");
      sbindex9 = readrel14->getIndex(ord13, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c12 = _t[0];
        u64 v_c13 = _t[1];
        slog::join_probe_lat<1,1>(sbindex9, std::array<u64,1>{v_c12}, [&](const std::array<u64,1>& m15, u64 val16) {
          u64 v_c14 = val16;
          static const slog::LatSpec* spec17 = slog::parseLatSpecToken("set-int");
          if (spec17 == nullptr) slog::fatal("cjoin: malformed spec token");
          u64 v_c15 = db->collections()->merge_spec(v_c13, v_c14, spec17);
          u64 v_c16 = _prim_csize(db, v_c15);
          if (v_c16 == slog_error) { slog::emit_pending_error(db, "lat_cjoin.slog:35"); return; }
          ++_fires;
      ...
      if (_fires) db->bumpFires("lat_cjoin.slog:35", "all:sa", _fires);
  ...
  // (crule (pre) (scan mpz_table_overflow __erre2XoO173 __errf2Qo0174 __errf3wqr175) (body) (head (emit error (0) __erre2XoO173)) <internal>:1 #f)
  ...
  // (crule (pre) (scan mpz_overflow __erre4RLg168 __errf9V6Q169 __errf1Pw3170 __errf60U4171 __errf3M1q172) (body) (head (emit error (0) __erre4RLg168)) <internal>:1 #f)
  ...
  // (crule (pre) (scan toint_range __erre561c160 __errf8eaM161 __errf8MVr162) (body) (head (emit error (0) __erre561c160)) <internal>:1 #f)
  ...
  // (crule (pre) (scan malformed_deduction __erre6ZW1141 __errf18wd142 __errf9WXa143 __errf8Vhn144 __errf9bw4145) (body) (head (emit error (0) __erre6ZW1141)) <internal>:1 #f)
  ...
  // (crule (pre) (scan mc k z) (body) (head (emit-lat ma k z)) lat_cjoin.slog:20 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("ma");
      outer_rel = db->getRelation("mc");
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        ...
        u64 v_c33 = _t[1];
        ++_fires;
        slog::emit_temp<2>(head_rel[0], newbatch[0], std::array<u64,2>{v_c12, v_c33});
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("lat_cjoin.slog:20", "delta:mc", _fires);
  ...
  // (crule (pre) (scan div_by_zero __erre8qew146 __errf5AFD147 __errf6PZa148) (body) (head (emit error (0) __erre8qew146)) <internal>:1 #f)
  ...
  // (crule (pre (let __trid2YiH43 const3f2485fdd1ec0c7e7f57dd40) (let __trel0xJV44 constda920246a08a91bf4f7e1b10) (let __tcol15jb45 const6b86b273ff34fce19d6b804e)) (scan ma k x) (body (join-lat mb (0 1) 1 k y) (cjoin __t4nFN39 (map int (min int)) x y)) (head (tycheck __t4nFN39 (accept cnode) __trid2YiH43 __trel0xJV44 __tcol15jb45 (1 2 3 4 0)) (emit-lat mc k __t4nFN39)) lat_cjoin.slog:19 #f)
  class ReadTask35 : public slog::Task
  {
    ...
    slog::Index** mbindex34;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("malformed_deduction");
      head_rel[1] = db->getRelation("mc");
      outer_rel = db->getRelation("ma");
      std::vector<u16> ord36({0, 1});
      slog::Relation* readrel37 = db->getRelation("mb");
      mbindex34 = readrel37->getIndex(ord36, false);
  
    }
    ReadTask35(slog::Database* _db, u16 _b) : db(_db), bucket(_b) { bind(_db); }
    virtual bool work()
    {
      u64 v_c37 = v_const3f2485fdd1ec0c7e7f57dd40;
      u64 v_c38 = v_constda920246a08a91bf4f7e1b10;
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c12 = _t[0];
        u64 v_c40 = _t[1];
        slog::join_probe_lat<1,1>(mbindex34, std::array<u64,1>{v_c12}, [&](const std::array<u64,1>& m38, u64 val39) {
          u64 v_c41 = val39;
          static const slog::LatSpec* spec40 = slog::parseLatSpecToken("map-int-min-int");
          if (spec40 == nullptr) slog::fatal("cjoin: malformed spec token");
          u64 v_c42 = db->collections()->merge_spec(v_c40, v_c41, spec40);
          ++_fires;
          if (!(is_cnode(v_c42)))
          {
            slog::emit_struct<5>(head_rel[0], newbatch[0], std::array<u64,4>{v_c37, v_c38, v_c39, v_c42}, std::array<u16,5>{1, 2, 3, 4, 0});
            return;
          }
          slog::emit_temp<2>(head_rel[1], newbatch[1], std::array<u64,2>{v_c12, v_c42});
        });
      });
      ...
      if (_fires) db->bumpFires("lat_cjoin.slog:19", "delta:ma", _fires);
  ...
  // (crule (pre (let __tconst3aTz24 const19581e27de7ced00ff1ce50b) (let __tconst30UL25 constd4735e3a265e16eee03f5971) (let __tconst0G2326 constef2d127de37b942baad06145) (let __tconst0LdN27 const6b86b273ff34fce19d6b804e) (let __t9DFA28 (cmap)) (let __t35ti29 (cput __t9DFA28 __tconst0LdN27 __tconst0G2326)) (let __t10Hx30 (cput __t35ti29 __tconst30UL25 __tconst3aTz24))) (once) (body) (head (emit-lat ma __tconst0LdN27 __t10Hx30)) lat_cjoin.slog:14 #f)
  ...
  public:
    ...
    virtual bool work()
    {
      u64 v_c43 = v_const19581e27de7ced00ff1ce50b;
      ...
      u64 v_c47 = _prim_cmap(db);
      if (v_c47 == slog_error) { slog::emit_pending_error(db, "lat_cjoin.slog:14"); return true; }
      u64 v_c48 = _prim_cput(db, v_c47, v_c46, v_c45);
      if (v_c48 == slog_error) { slog::emit_pending_error(db, "lat_cjoin.slog:14"); return true; }
      u64 v_c49 = _prim_cput(db, v_c48, v_c44, v_c43);
      if (v_c49 == slog_error) { slog::emit_pending_error(db, "lat_cjoin.slog:14"); return true; }
      ...
      if (_fires) db->bumpFires("lat_cjoin.slog:14", "once", _fires);
  ...
  // (crule (pre) (scan int_overflow __erre5yNR152 __errf0G9J153 __errf0tW0154 __errf9yd8155) (body) (head (emit error (0) __erre5yNR152)) <internal>:1 #f)
