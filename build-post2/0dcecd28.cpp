...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("0dcecd28");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  // (crule (pre) (scan cand P) (body (absent seen (0) 1 P)) (head (emit fresh (0) P)) neg_struct_key.slog:13 #f)
  class ReadTask1 : public slog::Task
  {
    ...
    slog::Index** seenindex0;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("fresh");
      std::vector<u16> ord2({0});
      slog::Relation* readrel3 = db->getRelation("fresh");
      head_index[0] = readrel3->getIndex(ord2, false);
      outer_rel = db->getRelation("cand");
      std::vector<u16> ord4({0});
      slog::Relation* readrel5 = db->getRelation("seen");
      seenindex0 = readrel5->getIndex(ord4, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        if (!slog::absent_probe<1,1>(seenindex0, std::array<u64,1>{v_c0})) return;
        ++_fires;
      ...
      if (_fires) db->bumpFires("neg_struct_key.slog:13", "all:cand", _fires);
