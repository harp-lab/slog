...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("d3751771");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  r = db->getRelation("type_mismatch");
      std::vector<u16> ord3({1, 2, 3, 4, 0});
    
    r->addIndex<5>(ord3, false);
    for (u16 b = 0; b < 32; ++b)
      s->addTask(phase_write, new slog::WriteTask<5>(db, r, std::array<u16,5>{1, 2, 3, 4, 0}, false, b), true);
      std::vector<u16> ord4({0, 1, 2, 3, 4});
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("ok"), std::array<u16,1>{0}, b));
  r = db->getRelation("nope2");
  if (r == 0) db->addRelation("nope2", 1);
  else if (r->getArity() != 1)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("nope2");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("nope2"), std::array<u16,1>{0}, b));
  r = db->getRelation("nope");
  if (r == 0) db->addRelation("nope", 1);
  else if (r->getArity() != 1)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("nope");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("nope"), std::array<u16,1>{0}, b));
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("go"), std::array<u16,1>{0}, b));
  ...
  // (crule (pre (let __tconst13Wo1 const6b86b273ff34fce19d6b804e)) (once) (body) (head (emit go (0) __tconst13Wo1)) int_caps.slog:11 #f)
  ...
  public:
    ...
    {
      head_rel[0] = db->getRelation("go");
      std::vector<u16> ord1({0});
      slog::Relation* readrel2 = db->getRelation("go");
    ...
    virtual bool work()
    {
      ...
      if (_fires) db->bumpFires("int_caps.slog:11", "once", _fires);
