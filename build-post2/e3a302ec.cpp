...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("e3a302ec");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  r = db->getRelation("allans");
  if (r == 0) db->addRelation("allans", 1);
  else if (r->getArity() != 1)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("allans");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("allans"), std::array<u16,1>{0}, b));
  ...
  // (crule (pre) (scan sq_ans __t1H3x1 r) (body (join sq (0 1) 1 __t1H3x1 n)) (head (emit got (0 1) n r)) dem_escape.slog:16 #f)
  ...
  public:
    ...
    {
      head_rel[0] = db->getRelation("got");
      std::vector<u16> ord2({0, 1});
      slog::Relation* readrel3 = db->getRelation("got");
      head_index[0] = readrel3->getIndex(ord2, false);
      outer_rel = db->getRelation("sq_ans");
      std::vector<u16> ord4({0, 1});
      slog::Relation* readrel5 = db->getRelation("sq");
      sqindex0 = readrel5->getIndex(ord4, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        u64 v_c1 = _t[1];
        slog::join_probe<2,1>(sqindex0, std::array<u64,2>{v_c0, 0}, [&](const std::array<u64,2>& m6) {
          u64 v_c2 = m6[1];
          ++_fires;
      ...
      if (_fires) db->bumpFires("dem_escape.slog:16", "all:sq_ans", _fires);
  ...
  // (crule (pre) (scan sq_ans d r) (body) (head (emit allans (0) r)) dem_escape.slog:17 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("allans");
      std::vector<u16> ord8({0});
      slog::Relation* readrel9 = db->getRelation("allans");
    ...
    virtual bool work()
    {
      ...
      if (_fires) db->bumpFires("dem_escape.slog:17", "all:sq_ans", _fires);
