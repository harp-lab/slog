...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("fe08e82d");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  r = db->getRelation("temp6FOP153");
  if (r == 0) db->addTempRelation("temp6FOP153", 2);
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("out"), std::array<u16,1>{0}, b));
  ...
  r = db->getRelation("count_ans");
  if (r == 0) db->addRelation("count_ans", 2);
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("count_ans"), std::array<u16,2>{0, 1}, b));
  r = db->getRelation("count");
  if (r == 0) db->addStruct("count", 2);
  else if (r->getArity() != 2)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("count");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternStructTask<2>(db, db->getRelation("count"), std::array<u16,2>{1, 0}, b));
  ...
  r = db->getRelation("$sup4873x11x0x0x0");
  if (r == 0) db->addRelation("$sup4873x11x0x0x0", 3);
  else if (r->getArity() != 3)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("$sup4873x11x0x0x0");
      std::vector<u16> ord75({1, 0, 2});
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<3>(db, db->getRelation("$sup4873x11x0x0x0"), std::array<u16,3>{1, 0, 2}, b));
  // (crule (pre) (scan div_by_zero __erre1dxB123 __errf0eez124 __errf8Nmk125) (body) (head (emit error (0) __erre1dxB123)) <internal>:1 #f)
  ...
  // (crule (pre) (scan mpz_overflow __erre6dwB145 __errf5PJp146 __errf5DvR147 __errf5jAM148 __errf6X54149) (body) (head (emit error (0) __erre6dwB145)) <internal>:1 #f)
  ...
  // (crule (pre) (scan temp6FOP153 __t8J3g3 __t8fX92) (body) (head (emit count_ans (0 1) __t8J3g3 __t8fX92)) stat_dem.slog:12 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("count_ans");
      std::vector<u16> ord7({0, 1});
      slog::Relation* readrel8 = db->getRelation("count_ans");
      head_index[0] = readrel8->getIndex(ord7, false);
      outer_rel = db->getRelation("temp6FOP153");
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        ...
        u64 v_c9 = _t[1];
        ++_fires;
        slog::emit<2>(head_rel[0], head_index[0], newbatch[0], std::array<u64,2>{v_c8, v_c9}, std::array<u16,2>{0, 1});
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("stat_dem.slog:12", "delta:temp6FOP153", _fires);
  ...
  // (crule (pre) (scan modulo_by_zero __erre4K5s126 __errf5P7f127 __errf8ViD128) (body) (head (emit error (0) __erre4K5s126)) <internal>:1 #f)
  ...
  // (crule (pre) (scan int_overflow __erre2dhP129 __errf8AK4130 __errf0zUv131 __errf3D7s132) (body) (head (emit error (0) __erre2dhP129)) <internal>:1 #f)
  ...
  // (crule (pre) (scan mpz_table_overflow __erre5szh150 __errf3dcG151 __errf5qBM152) (body) (head (emit error (0) __erre5szh150)) <internal>:1 #f)
  ...
  // (crule (pre) (scan type_mismatch __erre5Zs1140 __errf7GUl141 __errf4BLd142 __errf23eT143 __errf49KS144) (body) (head (emit error (0) __erre5Zs1140)) <internal>:1 #f)
  ...
  // (crule (pre) (scan nan_result __erre56Ze133 __errf7g5s134 __errf8F8r135 __errf4bEJ136) (body) (head (emit error (0) __erre56Ze133)) <internal>:1 #f)
  ...
  // (crule (pre) (scan toint_range __erre8xpO137 __errf5MIy138 __errf5VP2139) (body) (head (emit error (0) __erre8xpO137)) <internal>:1 #f)
  ...
  // (crule (pre (let __tconst4t1A4 const5feceb66ffc86f38d952786c) (let __tconst5FFO5 const6b86b273ff34fce19d6b804e)) (scan count_ans __t60oP7 __v0) (body (join count (0 1) 1 __t60oP7 __t8ldR6) (join $sup4873x11x0x0x0 (1 0 2) 1 __t8ldR6 __t8J3g3 n) (cmp gt n __tconst4t1A4) (join count (0 1) 2 __t8J3g3 n) (let chk9lLj154 (_0002d n __tconst5FFO5)) (eq __t8ldR6 chk9lLj154) (let __t8fX92 (_0002b __tconst5FFO5 __v0))) (head (emit-temp temp6FOP153 __t8J3g3 __t8fX92)) stat_dem.slog:12 #f)
  class ReadTask30 : public slog::Task
  {
    ...
    slog::Index** countindex27;  slog::Index** $sup4873x11x0x0x0index28;  slog::Index** countindex29;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("temp6FOP153");
      outer_rel = db->getRelation("count_ans");
      std::vector<u16> ord31({0, 1});
      slog::Relation* readrel32 = db->getRelation("count");
      countindex27 = readrel32->getIndex(ord31, false);
      std::vector<u16> ord33({1, 0, 2});
      slog::Relation* readrel34 = db->getRelation("$sup4873x11x0x0x0");
      $sup4873x11x0x0x0index28 = readrel34->getIndex(ord33, false);
      std::vector<u16> ord35({0, 1});
      slog::Relation* readrel36 = db->getRelation("count");
      countindex29 = readrel36->getIndex(ord35, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c34 = _t[0];
        u64 v_c35 = _t[1];
        slog::join_probe<2,1>(countindex27, std::array<u64,2>{v_c34, 0}, [&](const std::array<u64,2>& m37) {
          u64 v_c36 = m37[1];
          slog::join_probe<3,1>($sup4873x11x0x0x0index28, std::array<u64,3>{v_c36, 0, 0}, [&](const std::array<u64,3>& m38) {
            u64 v_c8 = m38[1]; u64 v_c37 = m38[2];
            u64 v_c38 = _prim_gt(db, v_c37, v_c32);
            if (v_c38 == slog_error) { slog::emit_pending_error(db, "stat_dem.slog:12"); return; }
            if (!v_c38) return;
            slog::join_probe<2,2>(countindex29, std::array<u64,2>{v_c8, v_c37}, [&](const std::array<u64,2>& m40) {
              u64 v_c39 = _prim__0002d(db, v_c37, v_c33);
              if (v_c39 == slog_error) { slog::emit_pending_error(db, "stat_dem.slog:12"); return; }
              if (v_c36 != v_c39) return;
              u64 v_c9 = _prim__0002b(db, v_c33, v_c35);
              if (v_c9 == slog_error) { slog::emit_pending_error(db, "stat_dem.slog:12"); return; }
              ++_fires;
              slog::emit_temp<2>(head_rel[0], newbatch[0], std::array<u64,2>{v_c8, v_c9});
            });
          });
        });
      });
      ...
      if (_fires) db->bumpFires("stat_dem.slog:12", "delta:count_ans", _fires);
  ...
  // (crule (pre (let __tconst24GU16 const5feceb66ffc86f38d952786c)) (probe count (1 0) 1 __tconst24GU16 __t6o8D17) (body) (head (emit count_ans (0 1) __t6o8D17 __tconst24GU16)) stat_dem.slog:11 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel43 = db->getRelation("count_ans");
      head_index[0] = readrel43->getIndex(ord42, false);
      std::vector<u16> ord44({1, 0});
      slog::Relation* readrel45 = db->getRelation("count");
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::join_probe_sliced<2,1>(driver_index, std::array<u64,2>{v_c40, 0}, _sc, _rkey, _hr, [&](const std::array<u64,2>& m46) {
        u64 v_c41 = m46[1];
      ...
      if (_fires) db->bumpFires("stat_dem.slog:11", "all:count", _fires);
  ...
  s->addReadRel("$sup4873x11x0x0x0");
  s->addReadRel("count");
  s->addReadRel("count_ans");
  ...
  s->addReadRel("temp6FOP153");
  s->addReadRel("toint_range");
  s->addReadRel("type_mismatch");
  s->addDynamicRel("count_ans");
  ...
  s->addDynamicRel("temp6FOP153");
  s->addDynamicRel("toint_range");
  s->addDynamicRel("type_mismatch");
  s->addAccelRel("count_ans");
  d->push(s);
  d->continueRun();
}
