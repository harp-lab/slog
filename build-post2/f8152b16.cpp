...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("f8152b16");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  if (r == 0) db->addStruct("malformed_deduction", 5);
    ...
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("malformed_deduction");
      std::vector<u16> ord148({1, 2, 3, 4, 0});
    
    r->addIndex<5>(ord148, false);
    for (u16 b = 0; b < 32; ++b)
      s->addTask(phase_write, new slog::WriteTask<5>(db, r, std::array<u16,5>{1, 2, 3, 4, 0}, false, b), true);
      std::vector<u16> ord149({0, 1, 2, 3, 4});
    
    r->addIndex<5>(ord149, false);
    for (u16 b = 0; b < 32; ++b)
      s->addTask(phase_write, new slog::WriteTask<5>(db, r, std::array<u16,5>{0, 1, 2, 3, 4}, false, b), false);
  ...
  // (crule (pre) (scan malformed_deduction __erre77Bw186 __errf2eVg187 __errf4xS7188 __errf3984189 __errf5pWb190) (body) (head (emit error (0) __erre77Bw186)) <internal>:1 #f)
  ...
  // (crule (pre) (scan toint_range __erre8DxQ205 __errf0QO1206 __errf9sHw207) (body) (head (emit error (0) __erre8DxQ205)) <internal>:1 #f)
  ...
  // (crule (pre (let __tconst7fLj66 consteb8bf0d80db323992f6b634a)) (probe _enum (1 0) 1 __tconst7fLj66 __t5wG927) (body (join smt_check_ans (1 0) 1 __t5wG927 __t4hTl28) (join smt_check (0 1) 1 __t4hTl28 F)) (head (emit smt_unknown (0) F)) smt.slog:79 #f)
  class ReadTask8 : public slog::Task
  {
    ...
    slog::Index** smt_check_ansindex6;  slog::Index** smt_checkindex7;
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel10 = db->getRelation("smt_unknown");
      ...
      slog::Relation* readrel14 = db->getRelation("smt_check_ans");
      smt_check_ansindex6 = readrel14->getIndex(ord13, false);
      std::vector<u16> ord15({0, 1});
      slog::Relation* readrel16 = db->getRelation("smt_check");
      smt_checkindex7 = readrel16->getIndex(ord15, false);
  
    }
    ReadTask8(slog::Database* _db, u16 _b) : db(_db), bucket(_b) { bind(_db); }
    virtual bool work()
    {
      u64 v_c8 = v_consteb8bf0d80db323992f6b634a;
      ...
      bool _done = slog::join_probe_sliced<2,1>(driver_index, std::array<u64,2>{v_c8, 0}, _sc, _rkey, _hr, [&](const std::array<u64,2>& m17) {
        u64 v_c9 = m17[1];
        if (buckethash(v_c9) != bucket) return;
        slog::join_probe<2,1>(smt_check_ansindex6, std::array<u64,2>{v_c9, 0}, [&](const std::array<u64,2>& m18) {
          u64 v_c10 = m18[1];
          slog::join_probe<2,1>(smt_checkindex7, std::array<u64,2>{v_c10, 0}, [&](const std::array<u64,2>& m19) {
            u64 v_c11 = m19[1];
            ++_fires;
  ...
  // (crule (pre) (scan reach_has s a) (body) (head (emit final (0 1) s a)) smt_rhas.slog:39 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel22 = db->getRelation("final");
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        ...
        u64 v_c13 = _t[1];
        ++_fires;
        slog::emit<2>(head_rel[0], head_index[0], newbatch[0], std::array<u64,2>{v_c12, v_c13}, std::array<u16,2>{0, 1});
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("smt_rhas.slog:39", "delta:reach_has", _fires);
  ...
  // (crule (pre) (scan reach s r) (body (let __t0Y9219 (csize r))) (head (emit rsize (0 1) s __t0Y9219)) smt_rhas.slog:38 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("rsize");
      std::vector<u16> ord24({0, 1});
      slog::Relation* readrel25 = db->getRelation("rsize");
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c12 = _t[0];
        u64 v_c14 = _t[1];
        u64 v_c15 = _prim_csize(db, v_c14);
        if (v_c15 == slog_error) { slog::emit_pending_error(db, "smt_rhas.slog:38"); return; }
        ++_fires;
        slog::emit<2>(head_rel[0], head_index[0], newbatch[0], std::array<u64,2>{v_c12, v_c15}, std::array<u16,2>{0, 1});
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("smt_rhas.slog:38", "all:reach", _fires);
  ...
  // (crule (pre (let __tconst8o7Y23 constfa94099f784d91500e1b95df) (let __trid1ZGE63 constbf14e8e2740173c2e99e36a3) (let __trel5oEU64 const7b4b6d235bc6fbc8c9ba5801) (let __tcol4lHB65 constd4735e3a265e16eee03f5971)) (scan smt_vcode __t2unH22 V) (body (join smt_qmodel_ans (0 1) 0 __t9UX425 M0) (join smt_qmodel (0 1) 1 __t9UX425 F) (join smt_model (1 0) 1 F __t0r0a20) (letp chk4aME221 (cget M0 __tconst8o7Y23)) (eq __t2unH22 chk4aME221) (let __t0N8P24 (cdel M0 __tconst8o7Y23))) (head (tycheck __t0N8P24 (accept cnode) __trid1ZGE63 __trel5oEU64 __tcol4lHB65 (1 2 3 4 0)) (emit smt_model_ans (0 1 2) __t0r0a20 V __t0N8P24)) smt.slog:93 #f)
  class ReadTask29 : public slog::Task
  {
    ...
    slog::Index** smt_qmodel_ansindex26;  slog::Index** smt_qmodelindex27;  slog::Index** smt_modelindex28;
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel31 = db->getRelation("smt_model_ans");
      head_index[1] = readrel31->getIndex(ord30, false);
      outer_rel = db->getRelation("smt_vcode");
      std::vector<u16> ord32({0, 1});
      slog::Relation* readrel33 = db->getRelation("smt_qmodel_ans");
      smt_qmodel_ansindex26 = readrel33->getIndex(ord32, false);
      std::vector<u16> ord34({0, 1});
      slog::Relation* readrel35 = db->getRelation("smt_qmodel");
      smt_qmodelindex27 = readrel35->getIndex(ord34, false);
      std::vector<u16> ord36({1, 0});
      slog::Relation* readrel37 = db->getRelation("smt_model");
      smt_modelindex28 = readrel37->getIndex(ord36, false);
  
    }
    ReadTask29(slog::Database* _db, u16 _b) : db(_db), bucket(_b) { bind(_db); }
    virtual bool work()
    {
      u64 v_c16 = v_constfa94099f784d91500e1b95df;
      u64 v_c17 = v_constbf14e8e2740173c2e99e36a3;
      u64 v_c18 = v_const7b4b6d235bc6fbc8c9ba5801;
      u64 v_c19 = v_constd4735e3a265e16eee03f5971;
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c20 = _t[0];
        u64 v_c21 = _t[1];
        slog::join_all<2>(smt_qmodel_ansindex26, [&](const std::array<u64,2>& m38) {
          u64 v_c22 = m38[0]; u64 v_c23 = m38[1];
          slog::join_probe<2,1>(smt_qmodelindex27, std::array<u64,2>{v_c22, 0}, [&](const std::array<u64,2>& m39) {
            u64 v_c11 = m39[1];
            slog::join_probe<2,1>(smt_modelindex28, std::array<u64,2>{v_c11, 0}, [&](const std::array<u64,2>& m40) {
              u64 v_c24 = m40[1];
              bool ok41 = true;
              u64 v_c25 = _prim_cget(db, v_c23, v_c16, &ok41);
              if (!ok41) return;
              if (v_c20 != v_c25) return;
              u64 v_c26 = _prim_cdel(db, v_c23, v_c16);
              if (v_c26 == slog_error) { slog::emit_pending_error(db, "smt.slog:93"); return; }
              ++_fires;
              if (!(is_cnode(v_c26)))
              {
                slog::emit_struct<5>(head_rel[0], newbatch[0], std::array<u64,4>{v_c17, v_c18, v_c19, v_c26}, std::array<u16,5>{1, 2, 3, 4, 0});
                return;
              }
              slog::emit<3>(head_rel[1], head_index[1], newbatch[1], std::array<u64,3>{v_c24, v_c21, v_c26}, std::array<u16,3>{0, 1, 2});
            });
          });
        });
      });
  ...
  // (crule (pre (let __tconst7e0460 constab76c7d069d6f33dc950fb33)) (probe _enum (1 0) 1 __tconst7e0460 __t0dpz15) (body (join smt_check_ans (1 0) 1 __t0dpz15 __t2NuC16) (join smt_check (0 1) 1 __t2NuC16 F)) (head (emit smt_sat (0) F)) smt.slog:77 #f)
  class ReadTask44 : public slog::Task
  {
    ...
    std::array<u64,2> resume_key{};
    bool has_resume = false;
    slog::Index** smt_check_ansindex42;  slog::Index** smt_checkindex43;
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel46 = db->getRelation("smt_sat");
      ...
      slog::Relation* readrel50 = db->getRelation("smt_check_ans");
      smt_check_ansindex42 = readrel50->getIndex(ord49, false);
      std::vector<u16> ord51({0, 1});
      slog::Relation* readrel52 = db->getRelation("smt_check");
      smt_checkindex43 = readrel52->getIndex(ord51, false);
  
    }
    ReadTask44(slog::Database* _db, u16 _b) : db(_db), bucket(_b) { bind(_db); }
    virtual bool work()
    {
      u64 v_c27 = v_constab76c7d069d6f33dc950fb33;
      ...
      slog::SliceCtx _sc{_slice_deadline, &db->runStopFlag()};
      std::array<u64,2> _rkey = resume_key; bool _hr = has_resume;
      bool _done = slog::join_probe_sliced<2,1>(driver_index, std::array<u64,2>{v_c27, 0}, _sc, _rkey, _hr, [&](const std::array<u64,2>& m53) {
        u64 v_c28 = m53[1];
        if (buckethash(v_c28) != bucket) return;
        slog::join_probe<2,1>(smt_check_ansindex42, std::array<u64,2>{v_c28, 0}, [&](const std::array<u64,2>& m54) {
          u64 v_c29 = m54[1];
          slog::join_probe<2,1>(smt_checkindex43, std::array<u64,2>{v_c29, 0}, [&](const std::array<u64,2>& m55) {
            u64 v_c11 = m55[1];
            ++_fires;
            slog::emit<1>(head_rel[0], head_index[0], newbatch[0], std::array<u64,1>{v_c11}, std::array<u16,1>{0});
          });
  ...
  // (crule (pre) (scan int_overflow __erre817l197 __errf7mZD198 __errf2uYp199 __errf5HDa200) (body) (head (emit error (0) __erre817l197)) <internal>:1 #f)
  ...
  // (crule (pre (let __tconst3gmO72 consta221beeaebe583607e193be7)) (probe _enum (1 0) 1 __tconst3gmO72 __t3iYW38) (body (join smt_check_ans (1 0) 1 __t3iYW38 __t4wZV39) (join smt_check (0 1) 1 __t4wZV39 F)) (head (emit smt_unsat (0) F)) smt.slog:78 #f)
  class ReadTask61 : public slog::Task
  {
    ...
    std::array<u64,2> resume_key{};
    bool has_resume = false;
    slog::Index** smt_check_ansindex59;  slog::Index** smt_checkindex60;
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel63 = db->getRelation("smt_unsat");
      ...
      slog::Relation* readrel67 = db->getRelation("smt_check_ans");
      smt_check_ansindex59 = readrel67->getIndex(ord66, false);
      std::vector<u16> ord68({0, 1});
      slog::Relation* readrel69 = db->getRelation("smt_check");
      smt_checkindex60 = readrel69->getIndex(ord68, false);
  
    }
    ReadTask61(slog::Database* _db, u16 _b) : db(_db), bucket(_b) { bind(_db); }
    virtual bool work()
    {
      u64 v_c34 = v_consta221beeaebe583607e193be7;
      ...
      slog::SliceCtx _sc{_slice_deadline, &db->runStopFlag()};
      std::array<u64,2> _rkey = resume_key; bool _hr = has_resume;
      bool _done = slog::join_probe_sliced<2,1>(driver_index, std::array<u64,2>{v_c34, 0}, _sc, _rkey, _hr, [&](const std::array<u64,2>& m70) {
        u64 v_c35 = m70[1];
        if (buckethash(v_c35) != bucket) return;
        slog::join_probe<2,1>(smt_check_ansindex59, std::array<u64,2>{v_c35, 0}, [&](const std::array<u64,2>& m71) {
          u64 v_c36 = m71[1];
          slog::join_probe<2,1>(smt_checkindex60, std::array<u64,2>{v_c36, 0}, [&](const std::array<u64,2>& m72) {
            u64 v_c11 = m72[1];
            ++_fires;
            slog::emit<1>(head_rel[0], head_index[0], newbatch[0], std::array<u64,1>{v_c11}, std::array<u16,1>{0});
          });
  ...
  // (crule (pre) (scan type_mismatch __erre69zt208 __errf76At209 __errf1npt210 __errf6omb211 __errf7j8c212) (body) (head (emit error (0) __erre69zt208)) <internal>:1 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel75 = db->getRelation("error");
      head_index[0] = readrel75->getIndex(ord74, false);
  ...
  // (crule (pre) (scan modulo_by_zero __erre4ez2194 __errf7KdH195 __errf7qcr196) (body) (head (emit error (0) __erre4ez2194)) <internal>:1 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel78 = db->getRelation("error");
  ...
  // (crule (pre) (scan mpz_table_overflow __erre0nRT218 __errf3ES1219 __errf3Dd0220) (body) (head (emit error (0) __erre0nRT218)) <internal>:1 #f)
  ...
  // (crule (pre) (scan nan_result __erre0u8K201 __errf2vz4202 __errf3dQF203 __errf5l08204) (body) (head (emit error (0) __erre0u8K201)) <internal>:1 #f)
  ...
  // (crule (pre (let __tconst8LIb12 constfa94099f784d91500e1b95df) (let __trid7jEA73 constd2c28e4f541b3051f713f2a3) (let __trel9Qsn74 const356829ca6c3df0a0d1117410) (let __tcol9HrA75 constd4735e3a265e16eee03f5971)) (scan smt_vcode __t4oxS11 V) (body (join smt_qcore_ans (0 1) 0 __t5ywP14 M0) (join smt_qcore (0 1) 1 __t5ywP14 F) (join smt_core (1 0) 1 F __t0S6G9) (letp chk2bUJ222 (cget M0 __tconst8LIb12)) (eq __t4oxS11 chk2bUJ222) (let __t21C413 (cdel M0 __tconst8LIb12))) (head (tycheck __t21C413 (accept cnode) __trid7jEA73 __trel9Qsn74 __tcol9HrA75 (1 2 3 4 0)) (emit smt_core_ans (0 1 2) __t0S6G9 V __t21C413)) smt.slog:106 #f)
  class ReadTask88 : public slog::Task
  {
    ...
    slog::Index** smt_qcore_ansindex85;  slog::Index** smt_qcoreindex86;  slog::Index** smt_coreindex87;
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel90 = db->getRelation("smt_core_ans");
      head_index[1] = readrel90->getIndex(ord89, false);
      outer_rel = db->getRelation("smt_vcode");
      std::vector<u16> ord91({0, 1});
      slog::Relation* readrel92 = db->getRelation("smt_qcore_ans");
      smt_qcore_ansindex85 = readrel92->getIndex(ord91, false);
      std::vector<u16> ord93({0, 1});
      slog::Relation* readrel94 = db->getRelation("smt_qcore");
      smt_qcoreindex86 = readrel94->getIndex(ord93, false);
      std::vector<u16> ord95({1, 0});
      slog::Relation* readrel96 = db->getRelation("smt_core");
      smt_coreindex87 = readrel96->getIndex(ord95, false);
  
    }
    ReadTask88(slog::Database* _db, u16 _b) : db(_db), bucket(_b) { bind(_db); }
    virtual bool work()
    {
      u64 v_c52 = v_constfa94099f784d91500e1b95df;
      u64 v_c53 = v_constd2c28e4f541b3051f713f2a3;
      u64 v_c54 = v_const356829ca6c3df0a0d1117410;
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c56 = _t[0];
        u64 v_c21 = _t[1];
        slog::join_all<2>(smt_qcore_ansindex85, [&](const std::array<u64,2>& m97) {
          u64 v_c57 = m97[0]; u64 v_c23 = m97[1];
          slog::join_probe<2,1>(smt_qcoreindex86, std::array<u64,2>{v_c57, 0}, [&](const std::array<u64,2>& m98) {
            u64 v_c11 = m98[1];
            slog::join_probe<2,1>(smt_coreindex87, std::array<u64,2>{v_c11, 0}, [&](const std::array<u64,2>& m99) {
              u64 v_c58 = m99[1];
              bool ok100 = true;
              u64 v_c59 = _prim_cget(db, v_c23, v_c52, &ok100);
              if (!ok100) return;
              if (v_c56 != v_c59) return;
              u64 v_c60 = _prim_cdel(db, v_c23, v_c52);
              if (v_c60 == slog_error) { slog::emit_pending_error(db, "smt.slog:106"); return; }
              ++_fires;
              if (!(is_cnode(v_c60)))
              {
                slog::emit_struct<5>(head_rel[0], newbatch[0], std::array<u64,4>{v_c53, v_c54, v_c55, v_c60}, std::array<u16,5>{1, 2, 3, 4, 0});
                return;
              }
              slog::emit<3>(head_rel[1], head_index[1], newbatch[1], std::array<u64,3>{v_c58, v_c21, v_c60}, std::array<u16,3>{0, 1, 2});
            });
          });
        });
      });
  ...
  // (crule (pre) (scan div_by_zero __erre0TGX191 __errf4gA4192 __errf28yX193) (body) (head (emit error (0) __erre0TGX191)) <internal>:1 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel103 = db->getRelation("error");
  ...
  // (crule (pre) (scan mpz_overflow __erre4Si8213 __errf6acb214 __errf6EfS215 __errf5vLo216 __errf82Vq217) (body) (head (emit error (0) __erre4Si8213)) <internal>:1 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel106 = db->getRelation("error");
