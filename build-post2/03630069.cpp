...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("03630069");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternStructTask<3>(db, db->getRelation("tag"), std::array<u16,3>{2, 1, 0}, b));
  ...
  // (crule (pre (let __tconst9Tay8 constc48b5b1a9776c84602de2306)) (probe _enum (1 0) 1 __tconst9Tay8 __t1TS45) (body (join tag (2 1 0) 1 __t1TS45 n __t6Ph06) (join out (0) 1 __t6Ph06)) (head (emit oks (0) n)) enum_struct_field.slog:16 #f)
  class ReadTask2 : public slog::Task
  {
    ...
    slog::Index** tagindex0;  slog::Index** outindex1;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("oks");
      std::vector<u16> ord3({0});
      slog::Relation* readrel4 = db->getRelation("oks");
      ...
      slog::Relation* readrel8 = db->getRelation("tag");
      tagindex0 = readrel8->getIndex(ord7, false);
      std::vector<u16> ord9({0});
      slog::Relation* readrel10 = db->getRelation("out");
      outindex1 = readrel10->getIndex(ord9, false);
  
    }
    ReadTask2(slog::Database* _db, u16 _b) : db(_db), bucket(_b) { bind(_db); }
    virtual bool work()
    {
      u64 v_c0 = v_constc48b5b1a9776c84602de2306;
      ...
      bool _done = slog::join_probe_sliced<2,1>(driver_index, std::array<u64,2>{v_c0, 0}, _sc, _rkey, _hr, [&](const std::array<u64,2>& m11) {
        ...
        slog::join_probe<3,1>(tagindex0, std::array<u64,3>{v_c1, 0, 0}, [&](const std::array<u64,3>& m12) {
          u64 v_c2 = m12[1]; u64 v_c3 = m12[2];
          slog::join_probe<1,1>(outindex1, std::array<u64,1>{v_c3}, [&](const std::array<u64,1>& m13) {
            ++_fires;
      ...
      if (_fires) db->bumpFires("enum_struct_field.slog:16", "all:_enum", _fires);
