...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("62ad086a");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  r = db->getRelation("nothing");
  if (r == 0) db->addRelation("nothing", 1);
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("nothing"), std::array<u16,1>{0}, b));
  r = db->getRelation("nog");
  if (r == 0) db->addRelation("nog", 1);
  else if (r->getArity() != 1)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("nog");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("nog"), std::array<u16,1>{0}, b));
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("g"), std::array<u16,2>{0, 1}, b));
  ...
  r = db->getRelation("emptyflag");
  if (r == 0) db->addRelation("emptyflag", 1);
  else if (r->getArity() != 1)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("emptyflag");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("emptyflag"), std::array<u16,1>{0}, b));
  ...
  // (crule (pre (let __tconst4x6A15 const4e07408562bedb8b60ce05c1) (absent h (0) 0)) (once) (body) (head (emit emptyflag (0) __tconst4x6A15)) neg_wild.slog:32 #f)
  ...
  public:
    ...
    {
      head_rel[0] = db->getRelation("emptyflag");
      std::vector<u16> ord2({0});
      slog::Relation* readrel3 = db->getRelation("emptyflag");
      head_index[0] = readrel3->getIndex(ord2, false);
      std::vector<u16> ord4({0});
      slog::Relation* readrel5 = db->getRelation("h");
      hindex0 = readrel5->getIndex(ord4, false);
    ...
    virtual bool work()
    {
      ...
      if (!slog::absent_probe<1,0>(hindex0, std::array<u64,1>{0})) return true;
      ...
      if (_fires) db->bumpFires("neg_wild.slog:32", "once", _fires);
  ...
  // (crule (pre (let __tconst9Wok13 constd4735e3a265e16eee03f5971) (absent nothing (0) 0)) (once) (body) (head (emit emptyflag (0) __tconst9Wok13)) neg_wild.slog:27 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel9 = db->getRelation("emptyflag");
      head_index[0] = readrel9->getIndex(ord8, false);
      std::vector<u16> ord10({0});
      slog::Relation* readrel11 = db->getRelation("nothing");
      nothingindex6 = readrel11->getIndex(ord10, false);
    ...
    virtual bool work()
    {
      ...
      if (!slog::absent_probe<1,0>(nothingindex6, std::array<u64,1>{0})) return true;
      ...
      if (_fires) db->bumpFires("neg_wild.slog:27", "once", _fires);
  ...
  // (crule (pre (let __tconst2DBb12 const8c1f1046219ddd216a023f79) (let __tconst4Knk11 const6b86b273ff34fce19d6b804e) (absent h (0) 1 __tconst2DBb12)) (once) (body) (head (emit emptyflag (0) __tconst4Knk11)) neg_wild.slog:22 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel15 = db->getRelation("emptyflag");
      head_index[0] = readrel15->getIndex(ord14, false);
      std::vector<u16> ord16({0});
      slog::Relation* readrel17 = db->getRelation("h");
      hindex12 = readrel17->getIndex(ord16, false);
  
    }
    ReadTask13(slog::Database* _db, u16 _b) : db(_db), bucket(_b) { bind(_db); }
    virtual bool work()
    {
      u64 v_c2 = v_const8c1f1046219ddd216a023f79;
      u64 v_c3 = v_const6b86b273ff34fce19d6b804e;
      if (!slog::absent_probe<1,1>(hindex12, std::array<u64,1>{v_c2})) return true;
      ...
      if (_fires) db->bumpFires("neg_wild.slog:22", "once", _fires);
  ...
  // (crule (pre) (scan h X) (body (absent g (0 1) 1 X)) (head (emit nog (0) X)) neg_wild.slog:16 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("nog");
      std::vector<u16> ord20({0});
      slog::Relation* readrel21 = db->getRelation("nog");
      head_index[0] = readrel21->getIndex(ord20, false);
      outer_rel = db->getRelation("h");
      std::vector<u16> ord22({0, 1});
      slog::Relation* readrel23 = db->getRelation("g");
      gindex18 = readrel23->getIndex(ord22, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c4 = _t[0];
        if (!slog::absent_probe<2,1>(gindex18, std::array<u64,2>{v_c4, 0})) return;
        ++_fires;
      ...
      if (_fires) db->bumpFires("neg_wild.slog:16", "all:h", _fires);
  ...
  s->addReadRel("g");
  s->addReadRel("h");
  s->addReadRel("nothing");
  s->addDynamicRel("emptyflag");
  s->addDynamicRel("nog");
  d->push(s);
  d->continueRun();
}
