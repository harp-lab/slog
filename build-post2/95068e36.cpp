...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("95068e36");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("query"), std::array<u16,1>{0}, b));
  ...
  r = db->getRelation("fibres");
  if (r == 0) db->addRelation("fibres", 2);
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("fibres"), std::array<u16,2>{0, 1}, b));
  ...
  // (crule (pre) (scan query n) (body (join fib (1 0) 1 n __t061B11) (join fib_ans (0 1) 1 __t061B11 r)) (head (emit fibres (0 1) n r)) dem_fib.slog:14 #f)
  class ReadTask2 : public slog::Task
  {
    ...
    slog::Index** fibindex0;  slog::Index** fib_ansindex1;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("fibres");
      std::vector<u16> ord3({0, 1});
      slog::Relation* readrel4 = db->getRelation("fibres");
      head_index[0] = readrel4->getIndex(ord3, false);
      outer_rel = db->getRelation("query");
      std::vector<u16> ord5({1, 0});
      slog::Relation* readrel6 = db->getRelation("fib");
      fibindex0 = readrel6->getIndex(ord5, false);
      std::vector<u16> ord7({0, 1});
      slog::Relation* readrel8 = db->getRelation("fib_ans");
      fib_ansindex1 = readrel8->getIndex(ord7, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        slog::join_probe<2,1>(fibindex0, std::array<u64,2>{v_c0, 0}, [&](const std::array<u64,2>& m9) {
          u64 v_c1 = m9[1];
          slog::join_probe<2,1>(fib_ansindex1, std::array<u64,2>{v_c1, 0}, [&](const std::array<u64,2>& m10) {
            u64 v_c2 = m10[1];
            ++_fires;
      ...
      if (_fires) db->bumpFires("dem_fib.slog:14", "all:query", _fires);
