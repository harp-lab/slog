...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("78a8c974");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  // (crule (pre) (scan edge X Y) (body) (head (emit node (0) Y) (emit node (0) X)) shape_selfloop.slog:34 #f)
  ...
  public:
    ...
    virtual bool work()
    {
      ...
      if (_fires) db->bumpFires("shape_selfloop.slog:34", "all:edge", _fires);
  ...
  // (crule (pre) (scan edge X dup7gWs22) (body (eq X dup7gWs22)) (head (emit selfd (0) X)) shape_selfloop.slog:32 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("selfd");
      std::vector<u16> ord6({0});
      slog::Relation* readrel7 = db->getRelation("selfd");
    ...
    virtual bool work()
    {
      ...
      if (_fires) db->bumpFires("shape_selfloop.slog:32", "all:edge", _fires);
  ...
  // (crule (pre) (scan tri X dup3hXw20 dup4rCZ21) (body (eq X dup4rCZ21) (eq X dup3hXw20)) (head (emit allsame (0) X)) shape_selfloop.slog:40 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("allsame");
      std::vector<u16> ord9({0});
      slog::Relation* readrel10 = db->getRelation("allsame");
    ...
    virtual bool work()
    {
      ...
      if (_fires) db->bumpFires("shape_selfloop.slog:40", "all:tri", _fires);
  ...
  s->addReadRel("edge");
  s->addReadRel("tri");
  s->addDynamicRel("allsame");
  s->addDynamicRel("node");
  s->addDynamicRel("selfd");
  d->push(s);
  d->continueRun();
}
