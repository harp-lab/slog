...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("ba91f909");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  if (r == 0) db->addRelation("$sup7183x46x0x0x1", 8);
    ...
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("$sup7183x46x0x0x1");
      std::vector<u16> ord76({0, 1, 2, 3, 4, 5, 6, 7});
    
    r->addIndex<8>(ord76, false);
    for (u16 b = 0; b < 32; ++b)
  ...
  // (crule (pre) (scan mpz_table_overflow __erre9A94207 __errf3X9e208 __errf95GV209) (body) (head (emit error (0) __erre9A94207)) <internal>:1 #f)
  ...
  // (crule (pre) (scan toint_range __erre8nSu194 __errf2W1X195 __errf4mO4196) (body) (head (emit error (0) __erre8nSu194)) <internal>:1 #f)
  ...
  // (crule (pre) (scan int_overflow __erre0E8H186 __errf7HFr187 __errf8WWj188 __errf26ba189) (body) (head (emit error (0) __erre0E8H186)) <internal>:1 #f)
  ...
  // (crule (pre) (scan nan_result __erre3JWm190 __errf3Aiv191 __errf7jWX192 __errf6PBJ193) (body) (head (emit error (0) __erre3JWm190)) <internal>:1 #f)
  ...
  // (crule (pre) (scan div_by_zero __erre4SbP180 __errf4bG4181 __errf9p8V182) (body) (head (emit error (0) __erre4SbP180)) <internal>:1 #f)
  ...
  // (crule (pre) (scan type_mismatch __erre9XyK197 __errf0Zqp198 __errf3MWP199 __errf1Q59200 __errf1ryO201) (body) (head (emit error (0) __erre9XyK197)) <internal>:1 #f)
  ...
  // (crule (pre (let __t8wZt45 (inf))) (probe mu (1 0) 1 __t8wZt45 X) (body) (head (emit polyvariant (0) X)) 0cfa-counting.slog:82 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("polyvariant");
      std::vector<u16> ord19({0});
      slog::Relation* readrel20 = db->getRelation("polyvariant");
    ...
    virtual bool work()
    {
      ...
      if (v_c22 == slog_error) { slog::emit_pending_error(db, "0cfa-counting.slog:82"); return true; }
      ...
      if (_fires) db->bumpFires("0cfa-counting.slog:82", "all:mu", _fires);
  ...
  // (crule (pre (let __t0Pfc46 (one))) (probe mu (1 0) 1 __t0Pfc46 X) (body) (head (emit singleton (0) X)) 0cfa-counting.slog:81 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("singleton");
      std::vector<u16> ord25({0});
      slog::Relation* readrel26 = db->getRelation("singleton");
      head_index[0] = readrel26->getIndex(ord25, false);
      std::vector<u16> ord27({1, 0});
      slog::Relation* readrel28 = db->getRelation("mu");
    ...
    virtual bool work()
    {
      u64 v_c24 = _prim_one(db);
      if (v_c24 == slog_error) { slog::emit_pending_error(db, "0cfa-counting.slog:81"); return true; }
      ...
      slog::SliceCtx _sc{_slice_deadline, &db->runStopFlag()};
      std::array<u64,2> _rkey = resume_key; bool _hr = has_resume;
      bool _done = slog::join_probe_sliced<2,1>(driver_index, std::array<u64,2>{v_c24, 0}, _sc, _rkey, _hr, [&](const std::array<u64,2>& m29) {
        u64 v_c23 = m29[1];
        if (buckethash(v_c23) != bucket) return;
        ++_fires;
        slog::emit<1>(head_rel[0], head_index[0], newbatch[0], std::array<u64,1>{v_c23}, std::array<u16,1>{0});
      ...
      if (_fires) db->bumpFires("0cfa-counting.slog:81", "all:mu", _fires);
  ...
  // (crule (pre) (scan modulo_by_zero __erre3fpT183 __errf9JM9184 __errf23LZ185) (body) (head (emit error (0) __erre3fpT183)) <internal>:1 #f)
  ...
  // (crule (pre) (scan mpz_overflow __erre6XjK202 __errf3Luh203 __errf7Ptp204 __errf4jJ4205 __errf2YVC206) (body) (head (emit error (0) __erre6XjK202)) <internal>:1 #f)
  ...
  s->addDynamicRel("polyvariant");
  s->addDynamicRel("singleton");
