...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("c0eef9be");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  r = db->getRelation("temp6IKj208");
  if (r == 0) db->addTempRelation("temp6IKj208", 1);
  ...
  // (crule (pre) (scan modulo_by_zero __erre4pDZ181 __errf65PS182 __errf734m183) (body) (head (emit error (0) __erre4pDZ181)) <internal>:1 #f)
  ...
  // (crule (pre) (scan type_mismatch __erre51no195 __errf8w3I196 __errf3jrM197 __errf5xIH198 __errf5o5S199) (body) (head (emit error (0) __erre51no195)) <internal>:1 #f)
  ...
  // (crule (pre) (scan mpz_table_overflow __erre2Gwz205 __errf0ayp206 __errf2qCb207) (body) (head (emit error (0) __erre2Gwz205)) <internal>:1 #f)
  ...
  // (crule (pre (let _00024sqc2RRw33 const6b86b273ff34fce19d6b804e) (let _00024sqc8nJ734 const5feceb66ffc86f38d952786c) (let _00024sqc7Q0G35 const6b86b273ff34fce19d6b804e) (let _00024sqc3Olq36 const5feceb66ffc86f38d952786c)) (scan map __t2c8t26 f _00024seq0) (body (letp _00024sql2AFa31 (aslst _00024seq0)) (let _00024sqn691u32 (llen _00024sql2AFa31)) (cmp ge _00024sqn691u32 _00024sqc2RRw33) (letp x (lref _00024sql2AFa31 _00024sqc8nJ734)) (let _00024sqp1XxS37 (_0002d _00024sqn691u32 _00024sqc3Olq36)) (let xs (lslice _00024sql2AFa31 _00024sqc7Q0G35 _00024sqp1XxS37))) (head (emit $sup94786x12x0x0x0 (0 1 2 3 4) _00024seq0 __t2c8t26 f x xs)) dem_brackets.slog:13 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("$sup94786x12x0x0x0");
      std::vector<u16> ord10({0, 1, 2, 3, 4});
      slog::Relation* readrel11 = db->getRelation("$sup94786x12x0x0x0");
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        ...
        u64 v_c17 = _t[2];
        bool ok12 = true;
        u64 v_c18 = _prim_aslst(db, v_c17, &ok12);
        if (!ok12) return;
        u64 v_c19 = _prim_llen(db, v_c18);
        if (v_c19 == slog_error) { slog::emit_pending_error(db, "dem_brackets.slog:13"); return; }
        u64 v_c20 = _prim_ge(db, v_c19, v_c11);
        if (v_c20 == slog_error) { slog::emit_pending_error(db, "dem_brackets.slog:13"); return; }
        if (!v_c20) return;
        bool ok14 = true;
        u64 v_c21 = _prim_lref(db, v_c18, v_c12, &ok14);
        if (!ok14) return;
        u64 v_c22 = _prim__0002d(db, v_c19, v_c14);
        if (v_c22 == slog_error) { slog::emit_pending_error(db, "dem_brackets.slog:13"); return; }
        u64 v_c23 = _prim_lslice(db, v_c18, v_c13, v_c22);
        if (v_c23 == slog_error) { slog::emit_pending_error(db, "dem_brackets.slog:13"); return; }
        ++_fires;
        slog::emit<5>(head_rel[0], head_index[0], newbatch[0], std::array<u64,5>{v_c17, v_c15, v_c16, v_c21, v_c23}, std::array<u16,5>{0, 1, 2, 3, 4});
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("dem_brackets.slog:13", "delta:map", _fires);
  ...
  // (crule (pre (let __tconst0MDw71 const933b575c961e931c43e84b05)) (probe _enum (1 0) 1 __tconst0MDw71 __t5diZ19) (body (join input (0) 0 l)) (head (mkstruct map (1 2 0) __8IVP70 __t5diZ19 l)) dem_brackets.slog:21 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      inputindex15 = readrel20->getIndex(ord19, false);
  
    }
    ReadTask16(slog::Database* _db, u16 _b) : db(_db), bucket(_b) { bind(_db); }
    virtual bool work()
    {
      u64 v_c24 = v_const933b575c961e931c43e84b05;
      ...
      bool _done = slog::join_probe_sliced<2,1>(driver_index, std::array<u64,2>{v_c24, 0}, _sc, _rkey, _hr, [&](const std::array<u64,2>& m21) {
        u64 v_c25 = m21[1];
        if (buckethash(v_c25) != bucket) return;
        slog::join_all<1>(inputindex15, [&](const std::array<u64,1>& m22) {
          u64 v_c26 = m22[0];
          ++_fires;
          slog::emit_struct<3>(head_rel[0], newbatch[0], std::array<u64,2>{v_c25, v_c26}, std::array<u16,3>{1, 2, 0});
        });
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("dem_brackets.slog:21", "all:_enum", _fires);
  ...
  // (crule (pre) (scan malformed_deduction __erre4KWk173 __errf0W2R174 __errf1sQv175 __errf6arK176 __errf1FQK177) (body) (head (emit error (0) __erre4KWk173)) <internal>:1 #f)
  ...
  // (crule (pre (let __trid39KP59 const6723d20422fb29be7fe89ccb) (let __trel3kxK60 const33dffa18ca3f56970d9625f0) (let __tcol9jbh61 const5feceb66ffc86f38d952786c) (let __trel011J62 const33dffa18ca3f56970d9625f0) (let __tcol79HP63 const6b86b273ff34fce19d6b804e)) (scan $sup94786x12x0x0x0 _00024seq0 __d0 f x xs) (body) (head (tycheck f (accept (struct addk) (struct _enum)) __trid39KP59 __trel3kxK60 __tcol9jbh61 (1 2 3 4 0)) (tycheck xs (accept seq) __trid39KP59 __trel011J62 __tcol79HP63 (1 2 3 4 0)) (mkstruct map (1 2 0) __1ETt58 f xs)) dem_brackets.slog:13 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      sid26 = db->getRelation("addk")->getStructId();
  
    }
    ReadTask28(slog::Database* _db, u16 _b) : db(_db), bucket(_b) { bind(_db); }
    virtual bool work()
    {
      u64 v_c32 = v_const6723d20422fb29be7fe89ccb;
      u64 v_c33 = v_const33dffa18ca3f56970d9625f0;
      u64 v_c34 = v_const5feceb66ffc86f38d952786c;
      u64 v_c35 = v_const33dffa18ca3f56970d9625f0;
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        ...
        u64 v_c23 = _t[4];
        ++_fires;
        if (!((is_struct(v_c16) && (decode_struct_id(v_c16) == sid26 || decode_struct_id(v_c16) == sid27))))
        {
          slog::emit_struct<5>(head_rel[0], newbatch[0], std::array<u64,4>{v_c32, v_c33, v_c34, v_c16}, std::array<u16,5>{1, 2, 3, 4, 0});
          return;
        }
        if (!(is_seq(v_c23)))
        {
          slog::emit_struct<5>(head_rel[1], newbatch[1], std::array<u64,4>{v_c32, v_c35, v_c36, v_c23}, std::array<u16,5>{1, 2, 3, 4, 0});
          return;
        }
        slog::emit_struct<3>(head_rel[2], newbatch[2], std::array<u64,2>{v_c16, v_c23}, std::array<u16,3>{1, 2, 0});
      });
      ...
      if (_fires) db->bumpFires("dem_brackets.slog:13", "delta:$sup94786x12x0x0x0", _fires);
  ...
  // (crule (pre) (scan mpz_overflow __erre93a8200 __errf2vJf201 __errf4SEn202 __errf7RcA203 __errf1B2m204) (body) (head (emit error (0) __erre93a8200)) <internal>:1 #f)
  ...
  // (crule (pre) (scan int_overflow __erre3qfH184 __errf7hYe185 __errf0ixa186 __errf1xxb187) (body) (head (emit error (0) __erre3qfH184)) <internal>:1 #f)
  ...
  // (crule (pre (let __tconst1Zou29 const4a44dc15364204a80fe80e90)) (scan temp6IKj208 l) (body (join addk (1 0) 1 __tconst1Zou29 __t5n6p30)) (head (mkstruct map (1 2 0) __6oVO55 __t5n6p30 l)) dem_brackets.slog:20 #f)
  class ReadTask36 : public slog::Task
  {
    ...
    slog::Index** addkindex35;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("map");
      outer_rel = db->getRelation("temp6IKj208");
      std::vector<u16> ord37({1, 0});
      slog::Relation* readrel38 = db->getRelation("addk");
      addkindex35 = readrel38->getIndex(ord37, false);
  
    }
    ReadTask36(slog::Database* _db, u16 _b) : db(_db), bucket(_b) { bind(_db); }
    virtual bool work()
    {
      u64 v_c47 = v_const4a44dc15364204a80fe80e90;
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c26 = _t[0];
        slog::join_probe<2,1>(addkindex35, std::array<u64,2>{v_c47, 0}, [&](const std::array<u64,2>& m39) {
          u64 v_c48 = m39[1];
          ++_fires;
          slog::emit_struct<3>(head_rel[0], newbatch[0], std::array<u64,2>{v_c48, v_c26}, std::array<u16,3>{1, 2, 0});
        });
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("dem_brackets.slog:20", "delta:temp6IKj208", _fires);
  ...
  // (crule (pre) (scan nan_result __erre3Nju188 __errf5b8K189 __errf0fgA190 __errf3yk8191) (body) (head (emit error (0) __erre3Nju188)) <internal>:1 #f)
  ...
  // (crule (pre (let __tconst1Zou29 const4a44dc15364204a80fe80e90)) (scan input l) (body) (head (emit-temp temp6IKj208 l) (mkstruct addk (1 0) __t5n6p30 __tconst1Zou29)) dem_brackets.slog:20 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("temp6IKj208");
    ...
    virtual bool work()
    {
      ...
      if (_fires) db->bumpFires("dem_brackets.slog:20", "all:input", _fires);
  ...
  // (crule (pre) (scan toint_range __erre2wVp192 __errf84mB193 __errf4EoY194) (body) (head (emit error (0) __erre2wVp192)) <internal>:1 #f)
  ...
  // (crule (pre) (scan div_by_zero __erre5Mij178 __errf4tjK179 __errf642m180) (body) (head (emit error (0) __erre5Mij178)) <internal>:1 #f)
  ...
  s->addReadRel("temp6IKj208");
  s->addReadRel("toint_range");
  s->addReadRel("type_mismatch");
  s->addDynamicRel("$seq_at");
  s->addDynamicRel("$sup94786x12x0x0x0");
