...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("def89cc7");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<3>(db, db->getRelation("d"), std::array<u16,3>{0, 1, 2}, b));
  r = db->getRelation("c");
  if (r == 0) db->addRelation("c", 2);
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("c"), std::array<u16,2>{0, 1}, b));
  r = db->getRelation("b");
  ...
  // (crule (pre (let __tconst2S7G4 const6b86b273ff34fce19d6b804e) (let __tconst6Cm16 constd4735e3a265e16eee03f5971) (let __tconst77UZ7 const4b227777d4dd1fc61c6f884f) (let __tconst78vQ8 const4e07408562bedb8b60ce05c1)) (once) (body) (head (emit a (0 1) __tconst2S7G4 __tconst6Cm16) (emit a (0 1) __tconst6Cm16 __tconst78vQ8) (emit a (0 1) __tconst78vQ8 __tconst77UZ7)) strat_diamond.slog:9 #f)
  ...
  public:
    ...
    virtual bool work()
    {
      ...
      slog::emit<2>(head_rel[1], head_index[1], newbatch[1], std::array<u64,2>{v_c1, v_c3}, std::array<u16,2>{0, 1});
      ...
      if (_fires) db->bumpFires("strat_diamond.slog:9", "once", _fires);
