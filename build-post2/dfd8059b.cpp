...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("dfd8059b");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  // (crule (pre (let __tconst0fa513 const4e07408562bedb8b60ce05c1)) (probe count (1 0) 1 __tconst0fa513 __t6FS214) (body (join count_ans (0 1) 1 __t6FS214 __v0)) (head (emit out (0) __v0)) stat_dem.slog:15 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel3 = db->getRelation("out");
      head_index[0] = readrel3->getIndex(ord2, false);
      std::vector<u16> ord4({1, 0});
      slog::Relation* readrel5 = db->getRelation("count");
      driver_index = readrel5->getIndex(ord4, true);
      std::vector<u16> ord6({0, 1});
      slog::Relation* readrel7 = db->getRelation("count_ans");
      count_ansindex0 = readrel7->getIndex(ord6, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::join_probe_sliced<2,1>(driver_index, std::array<u64,2>{v_c0, 0}, _sc, _rkey, _hr, [&](const std::array<u64,2>& m8) {
        ...
        slog::join_probe<2,1>(count_ansindex0, std::array<u64,2>{v_c1, 0}, [&](const std::array<u64,2>& m9) {
          u64 v_c2 = m9[1];
          ++_fires;
      ...
      if (_fires) db->bumpFires("stat_dem.slog:15", "all:count", _fires);
