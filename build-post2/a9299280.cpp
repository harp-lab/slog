...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("a9299280");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  r = db->getRelation("temp0XdP8");
  if (r == 0) db->addTempRelation("temp0XdP8", 2);
  ...
  // (crule (pre) (scan in X Y) (body) (head (emit-temp temp0XdP8 X Y) (mkstruct pair (1 2 0) __t93WH7 X Y)) structs.slog:10 #f)
  ...
  public:
    ...
    {
      head_rel[0] = db->getRelation("temp0XdP8");
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        ...
        slog::emit_struct<3>(head_rel[1], newbatch[1], std::array<u64,2>{v_c0, v_c1}, std::array<u16,3>{1, 2, 0});
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
        head_rel[1]->sendBatch(newbatch[1]);
  
      if (_fires) db->bumpFires("structs.slog:10", "all:in", _fires);
  ...
  // (crule (pre) (scan temp0XdP8 X Y) (body (join pair (1 2 0) 2 X Y __t93WH7)) (head (emit out (0) __t93WH7)) structs.slog:10 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      outer_rel = db->getRelation("temp0XdP8");
      std::vector<u16> ord5({1, 2, 0});
      slog::Relation* readrel6 = db->getRelation("pair");
      pairindex1 = readrel6->getIndex(ord5, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        u64 v_c1 = _t[1];
        slog::join_probe<3,2>(pairindex1, std::array<u64,3>{v_c0, v_c1, 0}, [&](const std::array<u64,3>& m7) {
          u64 v_c2 = m7[2];
          ++_fires;
      ...
      if (_fires) db->bumpFires("structs.slog:10", "delta:temp0XdP8", _fires);
  ...
  s->addReadRel("in");
  s->addReadRel("pair");
  s->addReadRel("temp0XdP8");
  s->addDynamicRel("out");
  s->addDynamicRel("pair");
  s->addDynamicRel("temp0XdP8");
  d->push(s);
  d->continueRun();
}
