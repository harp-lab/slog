...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("01f1ba9a");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  // (crule (pre) (scan smt_bad_formula __erre8igD139 __errf09Xm140 __errf63B5141) (body) (head (emit error (0) __erre8igD139)) <internal>:1 #f)
  ...
  // (crule (pre) (scan smt_vcode C V) (body (join smt_core (0 1) 0 __t88qf4 F)) (head (mkstruct smt_qcore (1 0) __3fhM63 F)) smt.slog:106 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel6 = db->getRelation("smt_core");
      smt_coreindex3 = readrel6->getIndex(ord5, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c3 = _t[0];
        u64 v_c4 = _t[1];
        slog::join_all<2>(smt_coreindex3, [&](const std::array<u64,2>& m7) {
          u64 v_c5 = m7[0]; u64 v_c6 = m7[1];
          ++_fires;
  ...
  // (crule (pre) (scan smt_vcode C V) (body (join smt_check (0 1) 0 __t9kLE1 F)) (head (mkstruct smt_qsat (1 0) __3hYC62 F)) smt.slog:69 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      smt_checkindex8 = readrel11->getIndex(ord10, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c3 = _t[0];
        u64 v_c4 = _t[1];
        slog::join_all<2>(smt_checkindex8, [&](const std::array<u64,2>& m12) {
          u64 v_c7 = m12[0]; u64 v_c6 = m12[1];
          ++_fires;
          slog::emit_struct<2>(head_rel[0], newbatch[0], std::array<u64,1>{v_c6}, std::array<u16,2>{1, 0});
        });
      });
  ...
  // (crule (pre) (scan smt_vcode C V) (body (join smt_model (0 1) 0 __t6wzg5 F)) (head (mkstruct smt_qmodel (1 0) __2FIF46 F)) smt.slog:93 #f)
