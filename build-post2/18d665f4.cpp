...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("18d665f4");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  if (r == 0) db->addStruct("lite", 4);
    ...
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("lite");
      std::vector<u16> ord81({1, 2, 3, 0});
    
    r->addIndex<4>(ord81, false);
    for (u16 b = 0; b < 32; ++b)
      s->addTask(phase_write, new slog::WriteTask<4>(db, r, std::array<u16,4>{1, 2, 3, 0}, false, b), true);
      std::vector<u16> ord82({0, 1, 2, 3});
  ...
  if (r == 0) db->addStruct("int_overflow", 4);
    ...
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("int_overflow");
      std::vector<u16> ord102({1, 2, 3, 0});
    
    r->addIndex<4>(ord102, false);
    for (u16 b = 0; b < 32; ++b)
      s->addTask(phase_write, new slog::WriteTask<4>(db, r, std::array<u16,4>{1, 2, 3, 0}, false, b), true);
      std::vector<u16> ord103({0, 1, 2, 3});
  ...
  // (crule (pre) (scan div_by_zero __erre7KUI345 __errf1moU346 __errf2aly347) (body) (head (emit error (0) __erre7KUI345)) <internal>:1 #f)
  ...
  // (crule (pre) (scan mpz_overflow __erre8Iyl367 __errf26lD368 __errf0Rdv369 __errf6dDm370 __errf6WRs371) (body) (head (emit error (0) __erre8Iyl367)) <internal>:1 #f)
  ...
  // (crule (pre) (scan toint_range __erre2QMu359 __errf5dol360 __errf5U2o361) (body) (head (emit error (0) __erre2QMu359)) <internal>:1 #f)
  ...
  // (crule (pre (let __t1U8X8 (cmap))) (scan perr ID E) (body (let __t8yog9 (cins __t1U8X8 E))) (head (emit-lat check_err ID __t8yog9)) verify.slog:74 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("check_err");
      outer_rel = db->getRelation("perr");
  
    }
    ReadTask9(slog::Database* _db, u16 _b) : db(_db), bucket(_b) { bind(_db); }
    virtual bool work()
    {
      u64 v_c11 = _prim_cmap(db);
      if (v_c11 == slog_error) { slog::emit_pending_error(db, "verify.slog:74"); return true; }
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c12 = _t[0];
        u64 v_c13 = _t[1];
        u64 v_c14 = _prim_cins(db, v_c11, v_c13);
        if (v_c14 == slog_error) { slog::emit_pending_error(db, "verify.slog:74"); return; }
        ++_fires;
        slog::emit_temp<2>(head_rel[0], newbatch[0], std::array<u64,2>{v_c12, v_c14});
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("verify.slog:74", "all:perr", _fires);
  ...
  // (crule (pre) (scan modulo_by_zero __erre2vnr348 __errf7K1A349 __errf0ZBm350) (body) (head (emit error (0) __erre2vnr348)) <internal>:1 #f)
  ...
  // (crule (pre) (scan int_overflow __erre9ohk351 __errf14Pl352 __errf26tO353 __errf1PAj354) (body) (head (emit error (0) __erre9ohk351)) <internal>:1 #f)
  ...
  // (crule (pre) (scan type_mismatch __erre3gIR362 __errf1NeV363 __errf5tN7364 __errf0wPz365 __errf0MGW366) (body) (head (emit error (0) __erre3gIR362)) <internal>:1 #f)
  ...
  // (crule (pre) (scan nan_result __erre0Gwn355 __errf4lDU356 __errf90Gp357 __errf3piB358) (body) (head (emit error (0) __erre0Gwn355)) <internal>:1 #f)
  ...
  // (crule (pre) (scan mpz_table_overflow __erre2BrW372 __errf2TFy373 __errf1uY6374) (body) (head (emit error (0) __erre2BrW372)) <internal>:1 #f)
