...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("599408ce");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  // (crule (pre) (scan b Y) (body (join a (0) 0 X)) (head (emit c (0 1) X Y)) cartesian.slog:11 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel3 = db->getRelation("c");
      head_index[0] = readrel3->getIndex(ord2, false);
      outer_rel = db->getRelation("b");
      std::vector<u16> ord4({0});
      slog::Relation* readrel5 = db->getRelation("a");
      aindex0 = readrel5->getIndex(ord4, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        slog::join_all<1>(aindex0, [&](const std::array<u64,1>& m6) {
          u64 v_c1 = m6[0];
          ++_fires;
      ...
      if (_fires) db->bumpFires("cartesian.slog:11", "all:b", _fires);
