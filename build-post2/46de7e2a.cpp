...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("46de7e2a");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  r = db->getRelation("remm");
  if (r == 0) db->addRelation("remm", 3);
  else if (r->getArity() != 3)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("remm");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<3>(db, db->getRelation("remm"), std::array<u16,3>{0, 1, 2}, b));
  r = db->getRelation("quot");
  if (r == 0) db->addRelation("quot", 3);
  else if (r->getArity() != 3)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("quot");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<3>(db, db->getRelation("quot"), std::array<u16,3>{0, 1, 2}, b));
  r = db->getRelation("nrem");
  if (r == 0) db->addRelation("nrem", 3);
  else if (r->getArity() != 3)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("nrem");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<3>(db, db->getRelation("nrem"), std::array<u16,3>{0, 1, 2}, b));
  r = db->getRelation("nquot");
  if (r == 0) db->addRelation("nquot", 3);
  else if (r->getArity() != 3)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("nquot");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<3>(db, db->getRelation("nquot"), std::array<u16,3>{0, 1, 2}, b));
  r = db->getRelation("nan_result");
  if (r == 0) db->addStruct("nan_result", 4);
  else if (r->getArity() != 4)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("nan_result");
      std::vector<u16> ord19({1, 2, 3, 0});
    
    r->addIndex<4>(ord19, false);
    for (u16 b = 0; b < 32; ++b)
      s->addTask(phase_write, new slog::WriteTask<4>(db, r, std::array<u16,4>{1, 2, 3, 0}, false, b), true);
      std::vector<u16> ord20({0, 1, 2, 3});
  ...
  r = db->getRelation("drem");
  if (r == 0) db->addRelation("drem", 3);
  else if (r->getArity() != 3)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("drem");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<3>(db, db->getRelation("drem"), std::array<u16,3>{0, 1, 2}, b));
  r = db->getRelation("dquot");
  if (r == 0) db->addRelation("dquot", 3);
  else if (r->getArity() != 3)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("dquot");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<3>(db, db->getRelation("dquot"), std::array<u16,3>{0, 1, 2}, b));
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("d"), std::array<u16,2>{0, 1}, b));
  ...
  // (crule (pre (let __tconst3uR016 const7902699be42c8a8e46fbbb45) (let __tconst2Ydb17 const4b227777d4dd1fc61c6f884f) (let __tconst8OvN18 const19581e27de7ced00ff1ce50b) (let __tconst8RWu19 constd4735e3a265e16eee03f5971) (let __tconst76EH20 const2c624232cdd221771294dfbb) (let __tconst6TDO21 constef2d127de37b942baad06145) (let __tconst7Rxh22 const6b86b273ff34fce19d6b804e)) (once) (body) (head (emit d (0 1) __tconst3uR016 __tconst8RWu19) (emit d (0 1) __tconst8OvN18 __tconst2Ydb17) (emit d (0 1) __tconst76EH20 __tconst8RWu19) (emit d (0 1) __tconst7Rxh22 __tconst6TDO21)) prim2_divmod.slog:13 #f)
  ...
  public:
    ...
    {
      head_rel[0] = db->getRelation("d");
      std::vector<u16> ord1({0, 1});
      slog::Relation* readrel2 = db->getRelation("d");
      head_index[0] = readrel2->getIndex(ord1, false);
      head_rel[1] = db->getRelation("d");
      std::vector<u16> ord3({0, 1});
      slog::Relation* readrel4 = db->getRelation("d");
      head_index[1] = readrel4->getIndex(ord3, false);
      head_rel[2] = db->getRelation("d");
      std::vector<u16> ord5({0, 1});
      slog::Relation* readrel6 = db->getRelation("d");
      head_index[2] = readrel6->getIndex(ord5, false);
      head_rel[3] = db->getRelation("d");
      std::vector<u16> ord7({0, 1});
      slog::Relation* readrel8 = db->getRelation("d");
    ...
    virtual bool work()
    {
      ...
      u64 v_c2 = v_const19581e27de7ced00ff1ce50b;
      u64 v_c3 = v_constd4735e3a265e16eee03f5971;
      u64 v_c4 = v_const2c624232cdd221771294dfbb;
      ...
      slog::emit<2>(head_rel[1], head_index[1], newbatch[1], std::array<u64,2>{v_c2, v_c1}, std::array<u16,2>{0, 1});
      slog::emit<2>(head_rel[2], head_index[2], newbatch[2], std::array<u64,2>{v_c4, v_c3}, std::array<u16,2>{0, 1});
      slog::emit<2>(head_rel[3], head_index[3], newbatch[3], std::array<u64,2>{v_c6, v_c5}, std::array<u16,2>{0, 1});
      ...
      if (_fires) db->bumpFires("prim2_divmod.slog:13", "once", _fires);
