...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("de135c6e");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  // (crule (pre (let __tconst9MFD12 const990c23868b965814b117cf9a)) (scan f X Y) (body (absent flag (0 1) 2 X __tconst9MFD12)) (head (emit outs (0) X)) neg_const.slog:19 #f)
  ...
  public:
    ...
    {
      head_rel[0] = db->getRelation("outs");
      std::vector<u16> ord2({0});
      slog::Relation* readrel3 = db->getRelation("outs");
      head_index[0] = readrel3->getIndex(ord2, false);
      outer_rel = db->getRelation("f");
      std::vector<u16> ord4({0, 1});
      slog::Relation* readrel5 = db->getRelation("flag");
      flagindex0 = readrel5->getIndex(ord4, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c1 = _t[0];
        u64 v_c2 = _t[1];
        if (!slog::absent_probe<2,2>(flagindex0, std::array<u64,2>{v_c1, v_c0})) return;
        ++_fires;
      ...
      if (_fires) db->bumpFires("neg_const.slog:19", "all:f", _fires);
  ...
  // (crule (pre (let __tconst9fEQ1 const6b86b273ff34fce19d6b804e)) (scan f X Y) (body (absent f (0 1) 2 X __tconst9fEQ1)) (head (emit out (0) X)) neg_const.slog:13 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel9 = db->getRelation("out");
      head_index[0] = readrel9->getIndex(ord8, false);
      outer_rel = db->getRelation("f");
      std::vector<u16> ord10({0, 1});
      slog::Relation* readrel11 = db->getRelation("f");
      findex6 = readrel11->getIndex(ord10, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c1 = _t[0];
        u64 v_c2 = _t[1];
        if (!slog::absent_probe<2,2>(findex6, std::array<u64,2>{v_c1, v_c3})) return;
        ++_fires;
        slog::emit<1>(head_rel[0], head_index[0], newbatch[0], std::array<u64,1>{v_c1}, std::array<u16,1>{0});
      });
      ...
      if (_fires) db->bumpFires("neg_const.slog:13", "all:f", _fires);
