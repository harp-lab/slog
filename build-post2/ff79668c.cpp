...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("ff79668c");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  if (r == 0) db->addRelation("result", 2);
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("result"), std::array<u16,2>{0, 1}, b));
  ...
  r = db->getRelation("inc_ans");
  if (r == 0) db->addRelation("inc_ans", 2);
  else if (r->getArity() != 2)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("inc_ans");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("inc_ans"), std::array<u16,2>{0, 1}, b));
  r = db->getRelation("inc");
  if (r == 0) db->addStruct("inc", 2);
  else if (r->getArity() != 2)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("inc");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternStructTask<2>(db, db->getRelation("inc"), std::array<u16,2>{1, 0}, b));
  ...
  // (crule (pre (let __tconst3Zx86 const4a44dc15364204a80fe80e90) (let __tconst2TBl5 const4e07408562bedb8b60ce05c1)) (once) (body) (head (emit start (0) __tconst3Zx86) (emit start (0) __tconst2TBl5)) dem_chain.slog:10 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel2 = db->getRelation("start");
      head_index[0] = readrel2->getIndex(ord1, false);
      head_rel[1] = db->getRelation("start");
      std::vector<u16> ord3({0});
      slog::Relation* readrel4 = db->getRelation("start");
    ...
    virtual bool work()
    {
      ...
      if (_fires) db->bumpFires("dem_chain.slog:10", "once", _fires);
