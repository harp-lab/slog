...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("b18703c7");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  r = db->getRelation("two");
  if (r == 0) db->addRelation("two", 2);
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("two"), std::array<u16,2>{0, 1}, b));
  ...
  if (r == 0) db->addRelation("start", 1);
  else if (r->getArity() != 1)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("start");
      std::vector<u16> ord12({0});
    
    r->addIndex<1>(ord12, false);
    for (u16 b = 0; b < 32; ++b)
      s->addTask(phase_write, new slog::WriteTask<1>(db, r, std::array<u16,1>{0}, false, b), true);
  

  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("start"), std::array<u16,1>{0}, b));
  ...
  r = db->getRelation("rev");
  if (r == 0) db->addRelation("rev", 2);
  else if (r->getArity() != 2)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("rev");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("rev"), std::array<u16,2>{0, 1}, b));
  ...
  // (crule (pre (let __tconst8KyS2 const6b86b273ff34fce19d6b804e) (let __tconst2W7R4 constd4735e3a265e16eee03f5971) (let __tconst77eE5 const4b227777d4dd1fc61c6f884f) (let __tconst8Y3B6 const4e07408562bedb8b60ce05c1)) (once) (body) (head (emit edge (0 1) __tconst8KyS2 __tconst2W7R4) (emit edge (0 1) __tconst2W7R4 __tconst8Y3B6) (emit edge (0 1) __tconst8Y3B6 __tconst77eE5)) shape_rev.slog:17 #f)
  ...
  public:
    ...
    virtual bool work()
    {
      ...
      if (_fires) db->bumpFires("shape_rev.slog:17", "once", _fires);
