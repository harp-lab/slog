...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("0c40c6f7");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  r = db->getRelation("to4");
  if (r == 0) db->addRelation("to4", 2);
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("to4"), std::array<u16,2>{0, 1}, b));
  ...
  r = db->getRelation("hubdist");
  if (r == 0) db->addRelation("hubdist", 2);
  else if (r->getArity() != 2)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("hubdist");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("hubdist"), std::array<u16,2>{0, 1}, b));
  r = db->getRelation("hub");
  if (r == 0) db->addRelation("hub", 1);
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("hub"), std::array<u16,1>{0}, b));
  ...
  r = db->getRelation("dist");
  if (r == 0) db->addRelation("dist", 3);
  else if (r->getArity() != 3)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("dist");
  r->setLattice(LAT_MIN, true, s32_encode(0), false, 0, "min-int-floor-0", db->collections());
      std::vector<u16> ord42({0, 1, 2});
    
    r->addMapIndex<3>(ord42);
    for (u16 b = 0; b < 32; ++b)
      s->addTask(phase_write, new slog::MapWriteTask<3>(db, r, std::array<u16,3>{0, 1, 2}, b), true);
  

  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::LatticeInternTask<3>(db, db->getRelation("dist"), std::array<u16,3>{0, 1, 2}, b));
  r = db->getRelation("cheap");
  if (r == 0) db->addRelation("cheap", 2);
  else if (r->getArity() != 2)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("cheap");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("cheap"), std::array<u16,2>{0, 1}, b));
  ...
  // (crule (pre (let __tconst0oQT1 const4a44dc15364204a80fe80e90) (let __tconst2JoO4 constef2d127de37b942baad06145) (let __tconst638g10 constad57366865126e55649ecb23) (let __tconst5Hoa15 const4e07408562bedb8b60ce05c1) (let __tconst1HGC16 constf5ca38f748a1d6eaf726b8a4) (let __tconst4jGc19 const4523540f1504cd17100c4835) (let __tconst2OPs20 const4b227777d4dd1fc61c6f884f) (let __tconst317h21 const6b86b273ff34fce19d6b804e) (let __tconst3Yi222 constd4735e3a265e16eee03f5971)) (once) (body) (head (emit edge (0 1 2) __tconst317h21 __tconst3Yi222 __tconst0oQT1) (emit edge (0 1 2) __tconst3Yi222 __tconst5Hoa15 __tconst2JoO4) (emit edge (0 1 2) __tconst5Hoa15 __tconst317h21 __tconst3Yi222) (emit edge (0 1 2) __tconst317h21 __tconst5Hoa15 __tconst638g10) (emit edge (0 1 2) __tconst5Hoa15 __tconst2OPs20 __tconst317h21) (emit edge (0 1 2) __tconst3Yi222 __tconst2OPs20 __tconst1HGC16) (emit edge (0 1 2) __tconst317h21 __tconst2OPs20 __tconst4jGc19) (emit hub (0) __tconst3Yi222)) lat_sssp_extract.slog:15 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      head_rel[7] = db->getRelation("hub");
      std::vector<u16> ord15({0});
      slog::Relation* readrel16 = db->getRelation("hub");
    ...
    virtual bool work()
    {
      ...
      u64 v_c4 = v_constf5ca38f748a1d6eaf726b8a4;
      u64 v_c5 = v_const4523540f1504cd17100c4835;
      u64 v_c6 = v_const4b227777d4dd1fc61c6f884f;
      ...
      ++_fires;
      slog::emit<3>(head_rel[0], head_index[0], newbatch[0], std::array<u64,3>{v_c7, v_c8, v_c0}, std::array<u16,3>{0, 1, 2});
      slog::emit<3>(head_rel[1], head_index[1], newbatch[1], std::array<u64,3>{v_c8, v_c3, v_c1}, std::array<u16,3>{0, 1, 2});
      slog::emit<3>(head_rel[2], head_index[2], newbatch[2], std::array<u64,3>{v_c3, v_c7, v_c8}, std::array<u16,3>{0, 1, 2});
      slog::emit<3>(head_rel[3], head_index[3], newbatch[3], std::array<u64,3>{v_c7, v_c3, v_c2}, std::array<u16,3>{0, 1, 2});
      slog::emit<3>(head_rel[4], head_index[4], newbatch[4], std::array<u64,3>{v_c3, v_c6, v_c7}, std::array<u16,3>{0, 1, 2});
      slog::emit<3>(head_rel[5], head_index[5], newbatch[5], std::array<u64,3>{v_c8, v_c6, v_c4}, std::array<u16,3>{0, 1, 2});
      slog::emit<3>(head_rel[6], head_index[6], newbatch[6], std::array<u64,3>{v_c7, v_c6, v_c5}, std::array<u16,3>{0, 1, 2});
      slog::emit<1>(head_rel[7], head_index[7], newbatch[7], std::array<u64,1>{v_c8}, std::array<u16,1>{0});
      ...
      if (_fires) db->bumpFires("lat_sssp_extract.slog:15", "once", _fires);
