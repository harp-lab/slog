...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("8357b9de");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  // (crule (pre) (scan circles __t92Vc10) (body (join circle (0 1) 1 __t92Vc10 r)) (head (emit circ_r (0) r)) enum_union_direct.slog:19 #f)
  ...
  public:
    ...
    {
      head_rel[0] = db->getRelation("circ_r");
      std::vector<u16> ord2({0});
      slog::Relation* readrel3 = db->getRelation("circ_r");
      head_index[0] = readrel3->getIndex(ord2, false);
      outer_rel = db->getRelation("circles");
    ...
    virtual bool work()
    {
      ...
      if (_fires) db->bumpFires("enum_union_direct.slog:19", "all:circles", _fires);
  ...
  s->addReadRel("circle");
  s->addReadRel("circles");
  s->addDynamicRel("circ_r");
  d->push(s);
  d->continueRun();
}
