...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("1df538b7");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  r = db->getRelation("temp0dNe15");
  if (r == 0) db->addTempRelation("temp0dNe15", 2);
  ...
  // (crule (pre) (scan in n s) (body) (head (emit-temp temp0dNe15 n s) (mkstruct tag (1 2 0) __t00uM7 n s)) enum_struct_field.slog:15 #f)
  ...
  public:
    ...
    {
      head_rel[0] = db->getRelation("temp0dNe15");
    ...
    virtual bool work()
    {
      ...
      if (_fires) db->bumpFires("enum_struct_field.slog:15", "all:in", _fires);
  ...
  // (crule (pre) (scan temp0dNe15 n s) (body (join tag (1 2 0) 2 n s __t00uM7)) (head (emit out (0) __t00uM7)) enum_struct_field.slog:15 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      outer_rel = db->getRelation("temp0dNe15");
      std::vector<u16> ord5({1, 2, 0});
      slog::Relation* readrel6 = db->getRelation("tag");
      tagindex1 = readrel6->getIndex(ord5, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        u64 v_c1 = _t[1];
        slog::join_probe<3,2>(tagindex1, std::array<u64,3>{v_c0, v_c1, 0}, [&](const std::array<u64,3>& m7) {
          u64 v_c2 = m7[2];
          ++_fires;
      ...
      if (_fires) db->bumpFires("enum_struct_field.slog:15", "delta:temp0dNe15", _fires);
  ...
  s->addReadRel("in");
  s->addReadRel("tag");
  s->addReadRel("temp0dNe15");
  s->addDynamicRel("out");
  s->addDynamicRel("tag");
  s->addDynamicRel("temp0dNe15");
  d->push(s);
  d->continueRun();
}
