...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("91cfd988");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<3>(db, db->getRelation("tri"), std::array<u16,3>{0, 1, 2}, b));
  ...
  r = db->getRelation("selfj");
  if (r == 0) db->addRelation("selfj", 1);
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("selfj"), std::array<u16,1>{0}, b));
  r = db->getRelation("selfd");
  if (r == 0) db->addRelation("selfd", 1);
  else if (r->getArity() != 1)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("selfd");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("selfd"), std::array<u16,1>{0}, b));
  ...
  r = db->getRelation("dup");
  if (r == 0) db->addRelation("dup", 2);
  else if (r->getArity() != 2)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("dup");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("dup"), std::array<u16,2>{0, 1}, b));
  ...
  r = db->getRelation("allsame");
  if (r == 0) db->addRelation("allsame", 1);
  else if (r->getArity() != 1)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("allsame");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("allsame"), std::array<u16,1>{0}, b));
  ...
  // (crule (pre (let __tconst7Acn4 const6b86b273ff34fce19d6b804e) (let __tconst1QXT8 constd4735e3a265e16eee03f5971) (let __tconst1DoQ9 const4b227777d4dd1fc61c6f884f) (let __tconst2Krj10 const4e07408562bedb8b60ce05c1) (let __tconst5MXJ14 conste7f6c011776e8db7cd330b54) (let __tconst03pN16 constef2d127de37b942baad06145) (let __tconst48Dx18 const2c624232cdd221771294dfbb) (let __tconst2q5r19 const7902699be42c8a8e46fbbb45)) (once) (body) (head (emit edge (0 1) __tconst7Acn4 __tconst7Acn4) (emit edge (0 1) __tconst7Acn4 __tconst1QXT8) (emit edge (0 1) __tconst1QXT8 __tconst1QXT8) (emit edge (0 1) __tconst1QXT8 __tconst2Krj10) (emit edge (0 1) __tconst2Krj10 __tconst1DoQ9) (emit tri (0 1 2) __tconst03pN16 __tconst03pN16 __tconst03pN16) (emit tri (0 1 2) __tconst03pN16 __tconst03pN16 __tconst5MXJ14) (emit tri (0 1 2) __tconst2q5r19 __tconst48Dx18 __tconst2q5r19)) shape_selfloop.slog:22 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      head_rel[5] = db->getRelation("tri");
      std::vector<u16> ord11({0, 1, 2});
      slog::Relation* readrel12 = db->getRelation("tri");
      head_index[5] = readrel12->getIndex(ord11, false);
      head_rel[6] = db->getRelation("tri");
      std::vector<u16> ord13({0, 1, 2});
      slog::Relation* readrel14 = db->getRelation("tri");
      head_index[6] = readrel14->getIndex(ord13, false);
      head_rel[7] = db->getRelation("tri");
      std::vector<u16> ord15({0, 1, 2});
      slog::Relation* readrel16 = db->getRelation("tri");
    ...
    virtual bool work()
    {
      ...
      u64 v_c6 = v_const2c624232cdd221771294dfbb;
      ...
      ++_fires;
      slog::emit<2>(head_rel[0], head_index[0], newbatch[0], std::array<u64,2>{v_c0, v_c0}, std::array<u16,2>{0, 1});
      slog::emit<2>(head_rel[1], head_index[1], newbatch[1], std::array<u64,2>{v_c0, v_c1}, std::array<u16,2>{0, 1});
      slog::emit<2>(head_rel[2], head_index[2], newbatch[2], std::array<u64,2>{v_c1, v_c1}, std::array<u16,2>{0, 1});
      slog::emit<2>(head_rel[3], head_index[3], newbatch[3], std::array<u64,2>{v_c1, v_c3}, std::array<u16,2>{0, 1});
      slog::emit<2>(head_rel[4], head_index[4], newbatch[4], std::array<u64,2>{v_c3, v_c2}, std::array<u16,2>{0, 1});
      slog::emit<3>(head_rel[5], head_index[5], newbatch[5], std::array<u64,3>{v_c5, v_c5, v_c5}, std::array<u16,3>{0, 1, 2});
      slog::emit<3>(head_rel[6], head_index[6], newbatch[6], std::array<u64,3>{v_c5, v_c5, v_c4}, std::array<u16,3>{0, 1, 2});
      slog::emit<3>(head_rel[7], head_index[7], newbatch[7], std::array<u64,3>{v_c7, v_c6, v_c7}, std::array<u16,3>{0, 1, 2});
      ...
      if (_fires) db->bumpFires("shape_selfloop.slog:22", "once", _fires);
