...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("6ddee383");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("obs"), std::array<u16,2>{1, 0}, b));
  ...
  // (crule (pre (let __tconst9gzu18 const076de6e730df0b4c3860877a)) (probe _enum (1 0) 1 __tconst9gzu18 __tconst94HP9) (body (join obs (1 0) 1 __tconst94HP9 n)) (head (emit saw_false (0) n)) enum_bools_mix.slog:19 #f)
  class ReadTask1 : public slog::Task
  {
    ...
    slog::Index** obsindex0;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("saw_false");
      std::vector<u16> ord2({0});
      slog::Relation* readrel3 = db->getRelation("saw_false");
      ...
      obsindex0 = readrel7->getIndex(ord6, false);
  
    }
    ReadTask1(slog::Database* _db, u16 _b) : db(_db), bucket(_b) { bind(_db); }
    virtual bool work()
    {
      u64 v_c0 = v_const076de6e730df0b4c3860877a;
      ...
      bool _done = slog::join_probe_sliced<2,1>(driver_index, std::array<u64,2>{v_c0, 0}, _sc, _rkey, _hr, [&](const std::array<u64,2>& m8) {
        ...
        slog::join_probe<2,1>(obsindex0, std::array<u64,2>{v_c1, 0}, [&](const std::array<u64,2>& m9) {
          u64 v_c2 = m9[1];
          ++_fires;
      ...
      if (_fires) db->bumpFires("enum_bools_mix.slog:19", "all:_enum", _fires);
  ...
  // (crule (pre (let __tconst3Xqx12 const18d10c7d2b4b04aaf04254d1)) (probe _enum (1 0) 1 __tconst3Xqx12 __tconst9bkR10) (body (join obs (1 0) 1 __tconst9bkR10 n)) (head (emit saw_true (0) n)) enum_bools_mix.slog:18 #f)
  class ReadTask11 : public slog::Task
  {
    ...
    std::array<u64,2> resume_key{};
    bool has_resume = false;
    slog::Index** obsindex10;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("saw_true");
      std::vector<u16> ord12({0});
      slog::Relation* readrel13 = db->getRelation("saw_true");
      ...
      slog::Relation* readrel17 = db->getRelation("obs");
      obsindex10 = readrel17->getIndex(ord16, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::join_probe_sliced<2,1>(driver_index, std::array<u64,2>{v_c3, 0}, _sc, _rkey, _hr, [&](const std::array<u64,2>& m18) {
        ...
        slog::join_probe<2,1>(obsindex10, std::array<u64,2>{v_c4, 0}, [&](const std::array<u64,2>& m19) {
          u64 v_c2 = m19[1];
          ++_fires;
          slog::emit<1>(head_rel[0], head_index[0], newbatch[0], std::array<u64,1>{v_c2}, std::array<u16,1>{0});
        });
      ...
      if (_fires) db->bumpFires("enum_bools_mix.slog:18", "all:_enum", _fires);
  ...
  // (crule (pre (let __tconst0PYt13 const4bc9274d8758b867e8c071a6)) (probe _enum (1 0) 1 __tconst0PYt13 __t1b4F11) (body (join obs (1 0) 1 __t1b4F11 n)) (head (emit saw_red (0) n)) enum_bools_mix.slog:20 #f)
  class ReadTask21 : public slog::Task
  {
    ...
    std::array<u64,2> resume_key{};
    bool has_resume = false;
    slog::Index** obsindex20;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("saw_red");
      std::vector<u16> ord22({0});
      slog::Relation* readrel23 = db->getRelation("saw_red");
      ...
      slog::Relation* readrel27 = db->getRelation("obs");
      obsindex20 = readrel27->getIndex(ord26, false);
  
    }
    ReadTask21(slog::Database* _db, u16 _b) : db(_db), bucket(_b) { bind(_db); }
    virtual bool work()
    {
      u64 v_c5 = v_const4bc9274d8758b867e8c071a6;
      ...
      slog::SliceCtx _sc{_slice_deadline, &db->runStopFlag()};
      std::array<u64,2> _rkey = resume_key; bool _hr = has_resume;
      bool _done = slog::join_probe_sliced<2,1>(driver_index, std::array<u64,2>{v_c5, 0}, _sc, _rkey, _hr, [&](const std::array<u64,2>& m28) {
        u64 v_c6 = m28[1];
        if (buckethash(v_c6) != bucket) return;
        slog::join_probe<2,1>(obsindex20, std::array<u64,2>{v_c6, 0}, [&](const std::array<u64,2>& m29) {
          u64 v_c2 = m29[1];
          ++_fires;
          slog::emit<1>(head_rel[0], head_index[0], newbatch[0], std::array<u64,1>{v_c2}, std::array<u16,1>{0});
        });
      ...
      if (_fires) db->bumpFires("enum_bools_mix.slog:20", "all:_enum", _fires);
  ...
  s->addReadRel("_enum");
  s->addReadRel("obs");
  s->addDynamicRel("saw_false");
  s->addDynamicRel("saw_red");
  s->addDynamicRel("saw_true");
  d->push(s);
  d->continueRun();
}
