...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("38d6dca8");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  r = db->getRelation("tsz");
  if (r == 0) db->addRelation("tsz", 3);
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<3>(db, db->getRelation("tsz"), std::array<u16,3>{0, 1, 2}, b));
  ...
  r = db->getRelation("tags_at");
  if (r == 0) db->addRelation("tags_at", 3);
  else if (r->getArity() != 3)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("tags_at");
  r->setLattice(LAT_EXTERN, false, 0, false, 0, "set-int", db->collections());
    ...
    r->addMapIndex<3>(ord32);
    for (u16 b = 0; b < 32; ++b)
      s->addTask(phase_write, new slog::MapWriteTask<3>(db, r, std::array<u16,3>{0, 1, 2}, b), true);
  

  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::LatticeInternTask<3>(db, db->getRelation("tags_at"), std::array<u16,3>{0, 1, 2}, b));
  r = db->getRelation("tags");
  if (r == 0) db->addRelation("tags", 2);
  else if (r->getArity() != 2)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("tags");
  r->setLattice(LAT_EXTERN, false, 0, false, 0, "map-int-set-int", db->collections());
      std::vector<u16> ord33({0, 1});
    
    r->addMapIndex<2>(ord33);
    for (u16 b = 0; b < 32; ++b)
      s->addTask(phase_write, new slog::MapWriteTask<2>(db, r, std::array<u16,2>{0, 1}, b, db->getRelation("tags_at"), true), true);
  

  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::LatticeInternTask<2>(db, db->getRelation("tags"), std::array<u16,2>{0, 1}, b, db->getRelation("tags_at"), true));
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<3>(db, db->getRelation("final"), std::array<u16,3>{0, 1, 2}, b));
  ...
  r = db->getRelation("bound");
  if (r == 0) db->addRelation("bound", 3);
  else if (r->getArity() != 3)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("bound");
  r->setLattice(LAT_MIN, false, 0, false, 0, "min-int", db->collections());
    ...
    r->addMapIndex<3>(ord53);
    for (u16 b = 0; b < 32; ++b)
      s->addTask(phase_write, new slog::MapWriteTask<3>(db, r, std::array<u16,3>{0, 1, 2}, b), true);
  

  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::LatticeInternTask<3>(db, db->getRelation("bound"), std::array<u16,3>{0, 1, 2}, b));
  r = db->getRelation("best_at");
  if (r == 0) db->addRelation("best_at", 3);
  else if (r->getArity() != 3)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("best_at");
  r->setLattice(LAT_MIN, false, 0, false, 0, "min-int", db->collections());
    ...
    r->addMapIndex<3>(ord54);
    for (u16 b = 0; b < 32; ++b)
      s->addTask(phase_write, new slog::MapWriteTask<3>(db, r, std::array<u16,3>{0, 1, 2}, b), true);
  

  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::LatticeInternTask<3>(db, db->getRelation("best_at"), std::array<u16,3>{0, 1, 2}, b));
  ...
  r->setLattice(LAT_EXTERN, false, 0, false, 0, "map-int-min-int", db->collections());
    ...
    r->addMapIndex<2>(ord55);
    for (u16 b = 0; b < 32; ++b)
      s->addTask(phase_write, new slog::MapWriteTask<2>(db, r, std::array<u16,2>{0, 1}, b, db->getRelation("best_at"), true), true);
  

  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::LatticeInternTask<2>(db, db->getRelation("best"), std::array<u16,2>{0, 1}, b, db->getRelation("best_at"), true));
  ...
  // (crule (pre) (scan tags_at a k s) (body (let __t7Lr21 (csize s))) (head (emit tsz (0 1 2) a k __t7Lr21)) lat_rat.slog:46 #f)
  ...
  public:
    ...
    {
      head_rel[0] = db->getRelation("tsz");
      std::vector<u16> ord1({0, 1, 2});
      slog::Relation* readrel2 = db->getRelation("tsz");
      head_index[0] = readrel2->getIndex(ord1, false);
      outer_rel = db->getRelation("tags_at");
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        ...
        u64 v_c1 = _t[1];
        u64 v_c2 = _t[2];
        u64 v_c3 = _prim_csize(db, v_c2);
        if (v_c3 == slog_error) { slog::emit_pending_error(db, "lat_rat.slog:46"); return; }
        ++_fires;
      ...
      if (_fires) db->bumpFires("lat_rat.slog:46", "delta:tags_at", _fires);
  ...
  // (crule (pre) (scan int_overflow __erre17x3123 __errf26Xb124 __errf56at125 __errf5HY6126) (body) (head (emit error (0) __erre17x3123)) <internal>:1 #f)
  ...
  // (crule (pre) (scan mpz_table_overflow __erre6C77144 __errf0E7G145 __errf8tdg146) (body) (head (emit error (0) __erre6C77144)) <internal>:1 #f)
  ...
  // (crule (pre) (scan modulo_by_zero __erre6hQy120 __errf84Lt121 __errf8Hwm122) (body) (head (emit error (0) __erre6hQy120)) <internal>:1 #f)
  ...
  // (crule (pre) (scan type_mismatch __erre8vEX134 __errf4k7v135 __errf1aFm136 __errf4Mme137 __errf3wBq138) (body) (head (emit error (0) __erre8vEX134)) <internal>:1 #f)
  ...
  // (crule (pre) (scan nan_result __erre5P8z127 __errf9Oea128 __errf1Hoo129 __errf2ax2130) (body) (head (emit error (0) __erre5P8z127)) <internal>:1 #f)
  ...
  // (crule (pre) (scan div_by_zero __erre0jiG117 __errf0Ggo118 __errf3LrQ119) (body) (head (emit error (0) __erre0jiG117)) <internal>:1 #f)
  ...
  // (crule (pre) (scan mpz_overflow __erre9Hhn139 __errf3Gai140 __errf9aUy141 __errf2AaB142 __errf2jlt143) (body) (head (emit error (0) __erre9Hhn139)) <internal>:1 #f)
  ...
  // (crule (pre) (scan toint_range __erre3OSC131 __errf2jau132 __errf80uh133) (body) (head (emit error (0) __erre3OSC131)) <internal>:1 #f)
  ...
  s->addReadRel("tags_at");
  s->addReadRel("toint_range");
  s->addReadRel("type_mismatch");
  s->addDynamicRel("best_at");
