...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("f3cf7782");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  if (r == 0) db->addRelation("same", 1);
  else if (r->getArity() != 1)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("same");
      std::vector<u16> ord15({0});
    
    r->addIndex<1>(ord15, false);
    for (u16 b = 0; b < 32; ++b)
      s->addTask(phase_write, new slog::WriteTask<1>(db, r, std::array<u16,1>{0}, false, b), true);
  

  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("same"), std::array<u16,1>{0}, b));
  r = db->getRelation("s2");
  if (r == 0) db->addRelation("s2", 1);
  else if (r->getArity() != 1)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("s2");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("s2"), std::array<u16,1>{0}, b));
  r = db->getRelation("s1");
  if (r == 0) db->addRelation("s1", 1);
  else if (r->getArity() != 1)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("s1");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("s1"), std::array<u16,1>{0}, b));
  r = db->getRelation("pt");
  ...
  r = db->getRelation("diff");
  if (r == 0) db->addRelation("diff", 2);
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("diff"), std::array<u16,2>{0, 1}, b));
  r = db->getRelation("b");
  if (r == 0) db->addRelation("b", 2);
  else if (r->getArity() != 2)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("b");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("b"), std::array<u16,2>{0, 1}, b));
  r = db->getRelation("a");
  if (r == 0) db->addRelation("a", 2);
  else if (r->getArity() != 2)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("a");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("a"), std::array<u16,2>{0, 1}, b));
  ...
  // (crule (pre (let __tconst3xft5 const4b227777d4dd1fc61c6f884f) (let __tconst7KZx6 const4e07408562bedb8b60ce05c1) (let __tconst7HJl7 constd4735e3a265e16eee03f5971) (let __tconst0HBX8 const6b86b273ff34fce19d6b804e) (let __tconst8Rfx9 conste7f6c011776e8db7cd330b54) (let __tconst2N3F10 constef2d127de37b942baad06145)) (once) (body) (head (emit a (0 1) __tconst0HBX8 __tconst7HJl7) (emit a (0 1) __tconst7KZx6 __tconst3xft5) (emit b (0 1) __tconst0HBX8 __tconst7HJl7) (emit b (0 1) __tconst2N3F10 __tconst8Rfx9)) sdeep_dedup.slog:15 #f)
  ...
  public:
    ...
    virtual bool work()
    {
      ...
      slog::emit<2>(head_rel[1], head_index[1], newbatch[1], std::array<u64,2>{v_c1, v_c0}, std::array<u16,2>{0, 1});
      slog::emit<2>(head_rel[2], head_index[2], newbatch[2], std::array<u64,2>{v_c3, v_c2}, std::array<u16,2>{0, 1});
      slog::emit<2>(head_rel[3], head_index[3], newbatch[3], std::array<u64,2>{v_c5, v_c4}, std::array<u16,2>{0, 1});
      ...
      if (_fires) db->bumpFires("sdeep_dedup.slog:15", "once", _fires);
