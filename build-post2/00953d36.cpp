...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("00953d36");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("lamp"), std::array<u16,2>{1, 0}, b));
  ...
  // (crule (pre (let __tconst1YXC12 const990c23868b965814b117cf9a)) (probe _enum (1 0) 1 __tconst1YXC12 __t5Ax21) (body (join lamp (1 0) 1 __t5Ax21 n)) (head (emit lit (0) n)) enum_union_nullary.slog:15 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel3 = db->getRelation("lit");
      ...
      lampindex0 = readrel7->getIndex(ord6, false);
  
    }
    ReadTask1(slog::Database* _db, u16 _b) : db(_db), bucket(_b) { bind(_db); }
    virtual bool work()
    {
      u64 v_c0 = v_const990c23868b965814b117cf9a;
      ...
      bool _done = slog::join_probe_sliced<2,1>(driver_index, std::array<u64,2>{v_c0, 0}, _sc, _rkey, _hr, [&](const std::array<u64,2>& m8) {
        ...
        slog::join_probe<2,1>(lampindex0, std::array<u64,2>{v_c1, 0}, [&](const std::array<u64,2>& m9) {
          u64 v_c2 = m9[1];
          ++_fires;
      ...
      if (_fires) db->bumpFires("enum_union_nullary.slog:15", "all:_enum", _fires);
  ...
  // (crule (pre (let __tconst4NCf11 const9f3becba6e4b1583b3e19441)) (probe _enum (1 0) 1 __tconst4NCf11 __t6a4n8) (body (join lamp (1 0) 1 __t6a4n8 n)) (head (emit dark (0) n)) enum_union_nullary.slog:16 #f)
  class ReadTask11 : public slog::Task
  {
    ...
    std::array<u64,2> resume_key{};
    bool has_resume = false;
    slog::Index** lampindex10;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("dark");
      std::vector<u16> ord12({0});
      slog::Relation* readrel13 = db->getRelation("dark");
      ...
      slog::Relation* readrel17 = db->getRelation("lamp");
      lampindex10 = readrel17->getIndex(ord16, false);
  
    }
    ReadTask11(slog::Database* _db, u16 _b) : db(_db), bucket(_b) { bind(_db); }
    virtual bool work()
    {
      u64 v_c3 = v_const9f3becba6e4b1583b3e19441;
      ...
      bool _done = slog::join_probe_sliced<2,1>(driver_index, std::array<u64,2>{v_c3, 0}, _sc, _rkey, _hr, [&](const std::array<u64,2>& m18) {
        ...
        slog::join_probe<2,1>(lampindex10, std::array<u64,2>{v_c4, 0}, [&](const std::array<u64,2>& m19) {
          u64 v_c2 = m19[1];
          ++_fires;
          slog::emit<1>(head_rel[0], head_index[0], newbatch[0], std::array<u64,1>{v_c2}, std::array<u16,1>{0});
        });
      ...
      if (_fires) db->bumpFires("enum_union_nullary.slog:16", "all:_enum", _fires);
