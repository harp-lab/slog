...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("81739a51");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  // (crule (pre) (scan ref __t4uHx24 x) (body) (head (emit freevar (0 1) x __t4uHx24)) freevars.slog:9 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel2 = db->getRelation("freevar");
    ...
    virtual bool work()
    {
      ...
      if (_fires) db->bumpFires("freevars.slog:9", "all:ref", _fires);
  ...
  // (crule (pre) (scan freevar x ea) (body (join app (2 0 1) 1 ea __t0FZq26 ef)) (head (emit freevar (0 1) x __t0FZq26)) freevars.slog:13 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel6 = db->getRelation("freevar");
      ...
      appindex3 = readrel8->getIndex(ord7, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c1 = _t[0];
        u64 v_c2 = _t[1];
        slog::join_probe<3,1>(appindex3, std::array<u64,3>{v_c2, 0, 0}, [&](const std::array<u64,3>& m9) {
  ...
  // (crule (pre) (scan freevar x ef) (body (join app (1 2 0) 1 ef ea __t5irL25)) (head (emit freevar (0 1) x __t5irL25)) freevars.slog:13 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel15 = db->getRelation("app");
      appindex10 = readrel15->getIndex(ord14, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c1 = _t[0];
        u64 v_c4 = _t[1];
        slog::join_probe<3,1>(appindex10, std::array<u64,3>{v_c4, 0, 0}, [&](const std::array<u64,3>& m16) {
          u64 v_c2 = m16[1]; u64 v_c5 = m16[2];
          ++_fires;
          slog::emit<2>(head_rel[0], head_index[0], newbatch[0], std::array<u64,2>{v_c1, v_c5}, std::array<u16,2>{0, 1});
        });
      });
  ...
  // (crule (pre) (scan freevar y eb) (body (join lambda (2 1 0) 1 eb x __t0MxM23) (neq x y)) (head (emit freevar (0 1) y __t0MxM23)) freevars.slog:18 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel20 = db->getRelation("freevar");
      head_index[0] = readrel20->getIndex(ord19, false);
      outer_rel = db->getRelation("freevar");
      std::vector<u16> ord21({2, 1, 0});
      slog::Relation* readrel22 = db->getRelation("lambda");
      lambdaindex17 = readrel22->getIndex(ord21, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c6 = _t[0];
        u64 v_c7 = _t[1];
        slog::join_probe<3,1>(lambdaindex17, std::array<u64,3>{v_c7, 0, 0}, [&](const std::array<u64,3>& m23) {
          u64 v_c1 = m23[1]; u64 v_c8 = m23[2];
          if (v_c1 == v_c6) return;
          ++_fires;
