...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("7bf19ac1");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  r = db->getRelation("dub_ans");
  if (r == 0) db->addRelation("dub_ans", 2);
  else if (r->getArity() != 2)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("dub_ans");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("dub_ans"), std::array<u16,2>{0, 1}, b));
  r = db->getRelation("dub");
  if (r == 0) db->addStruct("dub", 2);
  else if (r->getArity() != 2)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("dub");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternStructTask<2>(db, db->getRelation("dub"), std::array<u16,2>{1, 0}, b));
  ...
  r = db->getRelation("asked");
  if (r == 0) db->addRelation("asked", 1);
  else if (r->getArity() != 1)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("asked");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("asked"), std::array<u16,1>{0}, b));
  ...
  // (crule (pre) (scan edge n ____4cQg2) (body (join dub (1 0) 1 n __t0QTI1) (join dub_ans (0 1) 1 __t0QTI1 __v0)) (head (emit out (0 1) n __v0)) rule_noarrow.slog:20 #f)
  class ReadTask2 : public slog::Task
  {
    ...
    slog::Index** dubindex0;  slog::Index** dub_ansindex1;
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel6 = db->getRelation("dub");
      dubindex0 = readrel6->getIndex(ord5, false);
      std::vector<u16> ord7({0, 1});
      slog::Relation* readrel8 = db->getRelation("dub_ans");
      dub_ansindex1 = readrel8->getIndex(ord7, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        u64 v_c1 = _t[1];
        slog::join_probe<2,1>(dubindex0, std::array<u64,2>{v_c0, 0}, [&](const std::array<u64,2>& m9) {
          u64 v_c2 = m9[1];
          slog::join_probe<2,1>(dub_ansindex1, std::array<u64,2>{v_c2, 0}, [&](const std::array<u64,2>& m10) {
            u64 v_c3 = m10[1];
            ++_fires;
      ...
      if (_fires) db->bumpFires("rule_noarrow.slog:20", "all:edge", _fires);
