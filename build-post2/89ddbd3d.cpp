...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("89ddbd3d");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  r = db->getRelation("labb");
  if (r == 0) db->addRelation("labb", 2);
  else if (r->getArity() != 2)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("labb");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("labb"), std::array<u16,2>{0, 1}, b));
  r = db->getRelation("laba");
  if (r == 0) db->addRelation("laba", 2);
  else if (r->getArity() != 2)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("laba");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("laba"), std::array<u16,2>{0, 1}, b));
  ...
  r = db->getRelation("classify_ans");
  if (r == 0) db->addRelation("classify_ans", 2);
  else if (r->getArity() != 2)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("classify_ans");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("classify_ans"), std::array<u16,2>{0, 1}, b));
  r = db->getRelation("classify");
  if (r == 0) db->addStruct("classify", 2);
  else if (r->getArity() != 2)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("classify");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternStructTask<2>(db, db->getRelation("classify"), std::array<u16,2>{1, 0}, b));
  ...
  // (crule (pre (let __tconst86uW7 constd4735e3a265e16eee03f5971) (let __tconst0j5T6 constb85d38dc71a8027700bebd3c) (let __tconst55ad5 const6b86b273ff34fce19d6b804e) (let __tconst2kF84 const49e9fcfb5617aad332d56d58)) (once) (body) (head (emit labb (0 1) __tconst86uW7 __tconst0j5T6) (emit laba (0 1) __tconst55ad5 __tconst2kF84)) dem_or.slog:7 #f)
  ...
  public:
    ...
    {
      head_rel[0] = db->getRelation("labb");
      std::vector<u16> ord1({0, 1});
      slog::Relation* readrel2 = db->getRelation("labb");
      head_index[0] = readrel2->getIndex(ord1, false);
      head_rel[1] = db->getRelation("laba");
      std::vector<u16> ord3({0, 1});
      slog::Relation* readrel4 = db->getRelation("laba");
    ...
    virtual bool work()
    {
      ...
      u64 v_c1 = v_constb85d38dc71a8027700bebd3c;
      u64 v_c2 = v_const6b86b273ff34fce19d6b804e;
      u64 v_c3 = v_const49e9fcfb5617aad332d56d58;
      ...
      slog::emit<2>(head_rel[1], head_index[1], newbatch[1], std::array<u64,2>{v_c2, v_c3}, std::array<u16,2>{0, 1});
  
        head_rel[0]->sendBatch(newbatch[0]);
        head_rel[1]->sendBatch(newbatch[1]);
  
      if (_fires) db->bumpFires("dem_or.slog:7", "once", _fires);
  ...
  // (crule (pre (let __tconst9gJ43 const4e07408562bedb8b60ce05c1) (let __tconst0bLo2 constd4735e3a265e16eee03f5971) (let __tconst9SCT1 const6b86b273ff34fce19d6b804e)) (once) (body) (head (emit probe (0) __tconst9gJ43) (emit probe (0) __tconst0bLo2) (emit probe (0) __tconst9SCT1)) dem_or.slog:14 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel9 = db->getRelation("probe");
    ...
    virtual bool work()
    {
      ...
      slog::emit<1>(head_rel[2], head_index[2], newbatch[2], std::array<u64,1>{v_c6}, std::array<u16,1>{0});
  
        head_rel[0]->sendBatch(newbatch[0]);
        head_rel[1]->sendBatch(newbatch[1]);
        head_rel[2]->sendBatch(newbatch[2]);
  
      if (_fires) db->bumpFires("dem_or.slog:14", "once", _fires);
  ...
  s->addDynamicRel("laba");
  s->addDynamicRel("labb");
