...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("184c38ef");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  if (r == 0) db->addStruct("nan_result", 4);
    ...
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("nan_result");
      std::vector<u16> ord21({1, 2, 3, 0});
    
    r->addIndex<4>(ord21, false);
    for (u16 b = 0; b < 32; ++b)
      s->addTask(phase_write, new slog::WriteTask<4>(db, r, std::array<u16,4>{1, 2, 3, 0}, false, b), true);
      std::vector<u16> ord22({0, 1, 2, 3});
  ...
  // (crule (pre (let __tconst94uz2 const4a44dc15364204a80fe80e90) (let __tconst0YMR5 constef2d127de37b942baad06145) (let __tconst8ujR11 constad57366865126e55649ecb23) (let __tconst7Jix16 const4e07408562bedb8b60ce05c1) (let __tconst8MsJ17 constf5ca38f748a1d6eaf726b8a4) (let __tconst6kKg19 constd4735e3a265e16eee03f5971) (let __tconst4JsQ20 const4523540f1504cd17100c4835) (let __tconst3qU421 const4b227777d4dd1fc61c6f884f) (let __tconst4J6m22 const6b86b273ff34fce19d6b804e)) (once) (body) (head (emit edge (0 1 2) __tconst4J6m22 __tconst6kKg19 __tconst94uz2) (emit edge (0 1 2) __tconst6kKg19 __tconst7Jix16 __tconst0YMR5) (emit edge (0 1 2) __tconst7Jix16 __tconst4J6m22 __tconst6kKg19) (emit edge (0 1 2) __tconst4J6m22 __tconst7Jix16 __tconst8ujR11) (emit edge (0 1 2) __tconst7Jix16 __tconst3qU421 __tconst4J6m22) (emit edge (0 1 2) __tconst6kKg19 __tconst3qU421 __tconst8MsJ17) (emit edge (0 1 2) __tconst4J6m22 __tconst3qU421 __tconst4JsQ20)) lat_sssp.slog:11 #f)
  ...
  public:
    ...
    virtual bool work()
    {
      ...
      u64 v_c6 = v_const4523540f1504cd17100c4835;
      u64 v_c7 = v_const4b227777d4dd1fc61c6f884f;
      ...
      ++_fires;
      slog::emit<3>(head_rel[0], head_index[0], newbatch[0], std::array<u64,3>{v_c8, v_c5, v_c0}, std::array<u16,3>{0, 1, 2});
      slog::emit<3>(head_rel[1], head_index[1], newbatch[1], std::array<u64,3>{v_c5, v_c3, v_c1}, std::array<u16,3>{0, 1, 2});
      slog::emit<3>(head_rel[2], head_index[2], newbatch[2], std::array<u64,3>{v_c3, v_c8, v_c5}, std::array<u16,3>{0, 1, 2});
      slog::emit<3>(head_rel[3], head_index[3], newbatch[3], std::array<u64,3>{v_c8, v_c3, v_c2}, std::array<u16,3>{0, 1, 2});
      slog::emit<3>(head_rel[4], head_index[4], newbatch[4], std::array<u64,3>{v_c3, v_c7, v_c8}, std::array<u16,3>{0, 1, 2});
      slog::emit<3>(head_rel[5], head_index[5], newbatch[5], std::array<u64,3>{v_c5, v_c7, v_c4}, std::array<u16,3>{0, 1, 2});
      slog::emit<3>(head_rel[6], head_index[6], newbatch[6], std::array<u64,3>{v_c8, v_c7, v_c6}, std::array<u16,3>{0, 1, 2});
      ...
      if (_fires) db->bumpFires("lat_sssp.slog:11", "once", _fires);
