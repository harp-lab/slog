...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("e3c50e5e");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  if (r == 0) db->addRelation("$sup56712x93x0x0x0", 5);
    ...
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("$sup56712x93x0x0x0");
      std::vector<u16> ord173({0, 1, 2, 3, 4});
  ...
  // (crule (pre) (scan int_overflow __erre0UDn1271 __errf8YIj1272 __errf74jQ1273 __errf6DDT1274) (body) (head (emit error (0) __erre0UDn1271)) <internal>:1 #f)
  ...
  // (crule (pre (let __t8Bh2133 (one))) (probe target_count (1 0) 1 __t8Bh2133 site) (body) (head (emit monomorphic_site (0) site)) analysis.slog:26 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("monomorphic_site");
      std::vector<u16> ord4({0});
      slog::Relation* readrel5 = db->getRelation("monomorphic_site");
      head_index[0] = readrel5->getIndex(ord4, false);
      std::vector<u16> ord6({1, 0});
      slog::Relation* readrel7 = db->getRelation("target_count");
    ...
    virtual bool work()
    {
      ...
      if (v_c4 == slog_error) { slog::emit_pending_error(db, "analysis.slog:26"); return true; }
      ...
      bool _done = slog::join_probe_sliced<2,1>(driver_index, std::array<u64,2>{v_c4, 0}, _sc, _rkey, _hr, [&](const std::array<u64,2>& m8) {
        u64 v_c5 = m8[1];
      ...
      if (_fires) db->bumpFires("analysis.slog:26", "all:target_count", _fires);
  ...
  // (crule (pre) (scan type_mismatch __erre4JVg1282 __errf5jDt1283 __errf2FkR1284 __errf4Ktg1285 __errf1IUT1286) (body) (head (emit error (0) __erre4JVg1282)) <internal>:1 #f)
  ...
  // (crule (pre) (scan mpz_table_overflow __erre9AJi1292 __errf1ajf1293 __errf1b001294) (body) (head (emit error (0) __erre9AJi1292)) <internal>:1 #f)
  ...
  // (crule (pre) (scan modulo_by_zero __erre1A7k1268 __errf5Zkm1269 __errf55ci1270) (body) (head (emit error (0) __erre1A7k1268)) <internal>:1 #f)
  ...
  // (crule (pre) (scan singleton_flow x l) (body (join unique_binding (0 1) 2 x l) (join binding (0 1 2) 2 x l v)) (head (emit must_value (0 1 2) x l v)) counting.slog:45 #f)
  class ReadTask20 : public slog::Task
  {
    ...
    slog::Index** unique_bindingindex18;  slog::Index** bindingindex19;
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel22 = db->getRelation("must_value");
      head_index[0] = readrel22->getIndex(ord21, false);
      outer_rel = db->getRelation("singleton_flow");
      std::vector<u16> ord23({0, 1});
      slog::Relation* readrel24 = db->getRelation("unique_binding");
      unique_bindingindex18 = readrel24->getIndex(ord23, false);
      std::vector<u16> ord25({0, 1, 2});
      slog::Relation* readrel26 = db->getRelation("binding");
      bindingindex19 = readrel26->getIndex(ord25, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c17 = _t[0];
        u64 v_c18 = _t[1];
        slog::join_probe<2,2>(unique_bindingindex18, std::array<u64,2>{v_c17, v_c18}, [&](const std::array<u64,2>& m27) {
          slog::join_probe<3,2>(bindingindex19, std::array<u64,3>{v_c17, v_c18, 0}, [&](const std::array<u64,3>& m28) {
            u64 v_c19 = m28[2];
            ++_fires;
            slog::emit<3>(head_rel[0], head_index[0], newbatch[0], std::array<u64,3>{v_c17, v_c18, v_c19}, std::array<u16,3>{0, 1, 2});
          });
        });
      });
  ...
  // (crule (pre (let __t9i17359 (inf))) (probe target_count (1 0) 1 __t9i17359 site) (body) (head (emit polymorphic_site (0) site)) analysis.slog:27 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("polymorphic_site");
      std::vector<u16> ord30({0});
      slog::Relation* readrel31 = db->getRelation("polymorphic_site");
      head_index[0] = readrel31->getIndex(ord30, false);
      std::vector<u16> ord32({1, 0});
      slog::Relation* readrel33 = db->getRelation("target_count");
      driver_index = readrel33->getIndex(ord32, true);
    ...
    virtual bool work()
    {
      ...
      if (v_c20 == slog_error) { slog::emit_pending_error(db, "analysis.slog:27"); return true; }
      ...
      slog::SliceCtx _sc{_slice_deadline, &db->runStopFlag()};
      std::array<u64,2> _rkey = resume_key; bool _hr = has_resume;
      bool _done = slog::join_probe_sliced<2,1>(driver_index, std::array<u64,2>{v_c20, 0}, _sc, _rkey, _hr, [&](const std::array<u64,2>& m34) {
        u64 v_c5 = m34[1];
        if (buckethash(v_c5) != bucket) return;
        ++_fires;
        slog::emit<1>(head_rel[0], head_index[0], newbatch[0], std::array<u64,1>{v_c5}, std::array<u16,1>{0});
      ...
      if (_fires) db->bumpFires("analysis.slog:27", "all:target_count", _fires);
  ...
  // (crule (pre) (scan mpz_overflow __erre42TO1287 __errf9W7M1288 __errf1UnA1289 __errf3we51290 __errf2U541291) (body) (head (emit error (0) __erre42TO1287)) <internal>:1 #f)
  ...
  // (crule (pre) (scan nan_result __erre3Eex1275 __errf4sFr1276 __errf2Rf61277 __errf4Rhs1278) (body) (head (emit error (0) __erre3Eex1275)) <internal>:1 #f)
  ...
  // (crule (pre) (scan toint_range __erre3OS71279 __errf7htc1280 __errf5Zb31281) (body) (head (emit error (0) __erre3OS71279)) <internal>:1 #f)
  ...
  // (crule (pre) (scan div_by_zero __erre6Z4F1265 __errf7Aks1266 __errf0YxW1267) (body) (head (emit error (0) __erre6Z4F1265)) <internal>:1 #f)
  ...
  s->addDynamicRel("monomorphic_site");
  s->addDynamicRel("mpz_overflow");
  s->addDynamicRel("mpz_table_overflow");
  s->addDynamicRel("must_value");
  s->addDynamicRel("nan_result");
  s->addDynamicRel("polymorphic_site");
