...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("507b0b45");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  // (crule (pre) (scan edge x y) (body) (head (emit path (0 1) x y)) stat_chain.slog:12 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel2 = db->getRelation("path");
    ...
    virtual bool work()
    {
      ...
      if (_fires) db->bumpFires("stat_chain.slog:12", "all:edge", _fires);
  ...
  // (crule (pre) (scan path x y) (body (join edge (0 1) 1 y z)) (head (emit path (0 1) x z)) stat_chain.slog:13 #f)
  ...
  public:
    ...
    virtual bool work()
    {
      ...
      if (_fires) db->bumpFires("stat_chain.slog:13", "delta:path", _fires);
