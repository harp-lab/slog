...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("5b4fb998");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  r = db->getRelation("peak");
  if (r == 0) db->addRelation("peak", 2);
  else if (r->getArity() != 2)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("peak");
  r->setLattice(LAT_MAX, false, 0, false, 0, "max-int", db->collections());
      std::vector<u16> ord14({0, 1});
    
    r->addMapIndex<2>(ord14);
    for (u16 b = 0; b < 32; ++b)
      s->addTask(phase_write, new slog::MapWriteTask<2>(db, r, std::array<u16,2>{0, 1}, b), true);
  

  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::LatticeInternTask<2>(db, db->getRelation("peak"), std::array<u16,2>{0, 1}, b));
  r = db->getRelation("nan_result");
  if (r == 0) db->addStruct("nan_result", 4);
  else if (r->getArity() != 4)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("nan_result");
      std::vector<u16> ord15({1, 2, 3, 0});
    
    r->addIndex<4>(ord15, false);
    for (u16 b = 0; b < 32; ++b)
      s->addTask(phase_write, new slog::WriteTask<4>(db, r, std::array<u16,4>{1, 2, 3, 0}, false, b), true);
      std::vector<u16> ord16({0, 1, 2, 3});
  ...
  r = db->getRelation("low");
  if (r == 0) db->addRelation("low", 2);
  else if (r->getArity() != 2)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("low");
  r->setLattice(LAT_MIN, false, 0, false, 0, "min-int", db->collections());
    ...
    r->addMapIndex<2>(ord25);
    for (u16 b = 0; b < 32; ++b)
      s->addTask(phase_write, new slog::MapWriteTask<2>(db, r, std::array<u16,2>{0, 1}, b), true);
  

  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::LatticeInternTask<2>(db, db->getRelation("low"), std::array<u16,2>{0, 1}, b));
  ...
  // (crule (pre (let __tconst23tf9 const4e07408562bedb8b60ce05c1) (let __tconst3xHS8 const5feceb66ffc86f38d952786c) (let __tconst6Nhk7 constd4735e3a265e16eee03f5971) (let __tconst9QCq6 const7902699be42c8a8e46fbbb45) (let __tconst0wYA5 const6b86b273ff34fce19d6b804e) (let __tconst5ewU4 constef2d127de37b942baad06145)) (once) (body) (head (emit seed (0 1) __tconst23tf9 __tconst3xHS8) (emit seed (0 1) __tconst6Nhk7 __tconst9QCq6) (emit seed (0 1) __tconst0wYA5 __tconst5ewU4)) int_lat.slog:18 #f)
  ...
  public:
    ...
    virtual bool work()
    {
      ...
      u64 v_c3 = v_const7902699be42c8a8e46fbbb45;
      ...
      slog::emit<2>(head_rel[2], head_index[2], newbatch[2], std::array<u64,2>{v_c4, v_c5}, std::array<u16,2>{0, 1});
  
        head_rel[0]->sendBatch(newbatch[0]);
        head_rel[1]->sendBatch(newbatch[1]);
        head_rel[2]->sendBatch(newbatch[2]);
  
      if (_fires) db->bumpFires("int_lat.slog:18", "once", _fires);
