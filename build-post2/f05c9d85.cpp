...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("f05c9d85");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  r = db->getRelation("tonum");
  if (r == 0) db->addRelation("tonum", 2);
  ...
  r = db->getRelation("tonum");
    ...
    r->addIndex<2>(ord16, true);
    for (u16 b = 0; b < 32; ++b)
      s->addTask(phase_write, new slog::WriteTask<2>(db, r, std::array<u16,2>{1, 0}, true, b), false);
  

  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("tonum"), std::array<u16,2>{0, 1}, b));
  ...
  r = db->getRelation("succ");
  if (r == 0) db->addStruct("succ", 2);
  else if (r->getArity() != 2)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("succ");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternStructTask<2>(db, db->getRelation("succ"), std::array<u16,2>{1, 0}, b));
  ...
  r = db->getRelation("pred");
  if (r == 0) db->addRelation("pred", 2);
  else if (r->getArity() != 2)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("pred");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("pred"), std::array<u16,2>{0, 1}, b));
  ...
  r = db->getRelation("isthree");
  if (r == 0) db->addRelation("isthree", 1);
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("isthree"), std::array<u16,1>{0}, b));
  ...
  // (crule (pre (let __tconst834P10 const4e07408562bedb8b60ce05c1)) (probe tonum (1 0) 1 __tconst834P10 N) (body) (head (emit isthree (0) N)) sdeep_recnest.slog:17 #f)
  ...
  public:
    ...
    {
      head_rel[0] = db->getRelation("isthree");
      std::vector<u16> ord1({0});
      slog::Relation* readrel2 = db->getRelation("isthree");
      head_index[0] = readrel2->getIndex(ord1, false);
      std::vector<u16> ord3({1, 0});
      slog::Relation* readrel4 = db->getRelation("tonum");
    ...
    virtual bool work()
    {
      ...
      if (_fires) db->bumpFires("sdeep_recnest.slog:17", "all:tonum", _fires);
  ...
  // (crule (pre) (scan tonum __t3kF62 ____4PdZ3) (body (join succ (0 1) 1 __t3kF62 M)) (head (emit pred (0 1) __t3kF62 M)) sdeep_recnest.slog:20 #f)
  class ReadTask7 : public slog::Task
  {
    ...
    slog::Index** succindex6;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("pred");
      std::vector<u16> ord8({0, 1});
      slog::Relation* readrel9 = db->getRelation("pred");
      head_index[0] = readrel9->getIndex(ord8, false);
      outer_rel = db->getRelation("tonum");
      std::vector<u16> ord10({0, 1});
      slog::Relation* readrel11 = db->getRelation("succ");
      succindex6 = readrel11->getIndex(ord10, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c2 = _t[0];
        u64 v_c3 = _t[1];
        slog::join_probe<2,1>(succindex6, std::array<u64,2>{v_c2, 0}, [&](const std::array<u64,2>& m12) {
          u64 v_c4 = m12[1];
          ++_fires;
      ...
      if (_fires) db->bumpFires("sdeep_recnest.slog:20", "all:tonum", _fires);
  ...
  s->addReadRel("succ");
  s->addReadRel("tonum");
  s->addDynamicRel("isthree");
  s->addDynamicRel("pred");
  d->push(s);
  d->continueRun();
}
