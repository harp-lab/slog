...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("b11103b6");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  // (crule (pre (let __tconst4gFD12 const5feceb66ffc86f38d952786c) (let __tconst6uNz13 const43e6e2bebb3774311cf0c867)) (probe parse (1 2 0) 2 __tconst6uNz13 __tconst4gFD12 __t46uV14) (body (join parse_ans (0 1) 1 __t46uV14 n) (join len (0) 1 n)) (head (emit accepted (0) n)) dem_parse.slog:29 #f)
  class ReadTask2 : public slog::Task
  {
    ...
    slog::Index** parse_ansindex0;  slog::Index** lenindex1;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("accepted");
      std::vector<u16> ord3({0});
      slog::Relation* readrel4 = db->getRelation("accepted");
      head_index[0] = readrel4->getIndex(ord3, false);
      std::vector<u16> ord5({1, 2, 0});
      slog::Relation* readrel6 = db->getRelation("parse");
      driver_index = readrel6->getIndex(ord5, true);
      std::vector<u16> ord7({0, 1});
      slog::Relation* readrel8 = db->getRelation("parse_ans");
      parse_ansindex0 = readrel8->getIndex(ord7, false);
      std::vector<u16> ord9({0});
      slog::Relation* readrel10 = db->getRelation("len");
      lenindex1 = readrel10->getIndex(ord9, false);
    ...
    virtual bool work()
    {
      ...
      u64 v_c1 = v_const43e6e2bebb3774311cf0c867;
      ...
      bool _done = slog::join_probe_sliced<3,2>(driver_index, std::array<u64,3>{v_c1, v_c0, 0}, _sc, _rkey, _hr, [&](const std::array<u64,3>& m11) {
        u64 v_c2 = m11[2];
        if (buckethash(v_c2) != bucket) return;
        slog::join_probe<2,1>(parse_ansindex0, std::array<u64,2>{v_c2, 0}, [&](const std::array<u64,2>& m12) {
          u64 v_c3 = m12[1];
          slog::join_probe<1,1>(lenindex1, std::array<u64,1>{v_c3}, [&](const std::array<u64,1>& m13) {
            ++_fires;
      ...
      if (_fires) db->bumpFires("dem_parse.slog:29", "all:parse", _fires);
