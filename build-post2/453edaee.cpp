...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("453edaee");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  // (crule (pre) (scan edge X Y) (body) (head (emit reach (0 1) X Y)) strat_selfdown.slog:16 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel2 = db->getRelation("reach");
    ...
    virtual bool work()
    {
      ...
      if (_fires) db->bumpFires("strat_selfdown.slog:16", "all:edge", _fires);
  ...
  // (crule (pre) (scan reach X Y) (body (join edge (0 1) 1 Y Z)) (head (emit reach (0 1) X Z)) strat_selfdown.slog:17 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel6 = db->getRelation("reach");
      ...
      edgeindex3 = readrel8->getIndex(ord7, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        u64 v_c1 = _t[1];
        slog::join_probe<2,1>(edgeindex3, std::array<u64,2>{v_c1, 0}, [&](const std::array<u64,2>& m9) {
          u64 v_c2 = m9[1];
          ++_fires;
      ...
      if (_fires) db->bumpFires("strat_selfdown.slog:17", "delta:reach", _fires);
