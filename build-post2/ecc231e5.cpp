...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("ecc231e5");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  r = db->getRelation("temp6Yq496");
  if (r == 0) db->addTempRelation("temp6Yq496", 2);
  ...
  r = db->getRelation("mincost_ans");
  if (r == 0) db->addRelation("mincost_ans", 2);
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::LatticeInternTask<2>(db, db->getRelation("mincost_ans"), std::array<u16,2>{0, 1}, b));
  r = db->getRelation("mincost");
  if (r == 0) db->addStruct("mincost", 2);
  else if (r->getArity() != 2)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("mincost");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternStructTask<2>(db, db->getRelation("mincost"), std::array<u16,2>{1, 0}, b));
  ...
  r = db->getRelation("goal");
  if (r == 0) db->addRelation("goal", 1);
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("goal"), std::array<u16,1>{0}, b));
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<3>(db, db->getRelation("edge"), std::array<u16,3>{1, 0, 2}, b));
  ...
  if (r == 0) db->addRelation("answer", 1);
  else if (r->getArity() != 1)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("answer");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("answer"), std::array<u16,1>{0}, b));
  ...
  // (crule (pre) (scan type_mismatch __erre91tI83 __errf87q184 __errf8p5S85 __errf2llx86 __errf096K87) (body) (head (emit error (0) __erre91tI83)) <internal>:1 #f)
  ...
  // (crule (pre) (scan mpz_overflow __erre61A988 __errf40gC89 __errf1sbs90 __errf8qyv91 __errf46K992) (body) (head (emit error (0) __erre61A988)) <internal>:1 #f)
  ...
  // (crule (pre) (scan mpz_table_overflow __erre1zOx93 __errf6Fpt94 __errf48O295) (body) (head (emit error (0) __erre1zOx93)) <internal>:1 #f)
  ...
  // (crule (pre) (scan nan_result __erre9a5D76 __errf091277 __errf7oN178 __errf3QJd79) (body) (head (emit error (0) __erre9a5D76)) <internal>:1 #f)
  ...
  // (crule (pre) (scan temp6Yq496 __t3qLB29 __t7RJm30) (body) (head (emit-lat mincost_ans __t7RJm30 __t3qLB29)) lat_demand.slog:20 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("mincost_ans");
      outer_rel = db->getRelation("temp6Yq496");
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        ...
        u64 v_c18 = _t[1];
        ++_fires;
        slog::emit_temp<2>(head_rel[0], newbatch[0], std::array<u64,2>{v_c18, v_c17});
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("lat_demand.slog:20", "delta:temp6Yq496", _fires);
  ...
  // (crule (pre) (scan int_overflow __erre9Od172 __errf4w4F73 __errf1OUI74 __errf6dBH75) (body) (head (emit error (0) __erre9Od172)) <internal>:1 #f)
  ...
  // (crule (pre (let __tconst85xG1 const5feceb66ffc86f38d952786c)) (scan goal G) (body (join mincost (1 0) 1 G __t0DY22)) (head (emit-lat mincost_ans __t0DY22 __tconst85xG1)) lat_demand.slog:19 #f)
  class ReadTask17 : public slog::Task
  {
    ...
    slog::Index** mincostindex16;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("mincost_ans");
      outer_rel = db->getRelation("goal");
      std::vector<u16> ord18({1, 0});
      slog::Relation* readrel19 = db->getRelation("mincost");
      mincostindex16 = readrel19->getIndex(ord18, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c24 = _t[0];
        slog::join_probe<2,1>(mincostindex16, std::array<u64,2>{v_c24, 0}, [&](const std::array<u64,2>& m20) {
          u64 v_c25 = m20[1];
          ++_fires;
          slog::emit_temp<2>(head_rel[0], newbatch[0], std::array<u64,2>{v_c25, v_c23});
        });
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("lat_demand.slog:19", "all:goal", _fires);
  ...
  // (crule (pre) (scan mincost_ans __t83T831 __v0) (body (join mincost (0 1) 1 __t83T831 M) (join edge (1 0 2) 1 M N W) (join mincost (1 0) 1 N __t7RJm30) (let __t3qLB29 (_0002b W __v0))) (head (emit-temp temp6Yq496 __t3qLB29 __t7RJm30)) lat_demand.slog:20 #f)
  class ReadTask24 : public slog::Task
  {
    ...
    slog::Index** mincostindex21;  slog::Index** edgeindex22;  slog::Index** mincostindex23;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("temp6Yq496");
      outer_rel = db->getRelation("mincost_ans");
      std::vector<u16> ord25({0, 1});
      slog::Relation* readrel26 = db->getRelation("mincost");
      mincostindex21 = readrel26->getIndex(ord25, false);
      std::vector<u16> ord27({1, 0, 2});
      slog::Relation* readrel28 = db->getRelation("edge");
      edgeindex22 = readrel28->getIndex(ord27, false);
      std::vector<u16> ord29({1, 0});
      slog::Relation* readrel30 = db->getRelation("mincost");
      mincostindex23 = readrel30->getIndex(ord29, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c26 = _t[0];
        u64 v_c27 = _t[1];
        slog::join_probe<2,1>(mincostindex21, std::array<u64,2>{v_c26, 0}, [&](const std::array<u64,2>& m31) {
          u64 v_c28 = m31[1];
          slog::join_probe<3,1>(edgeindex22, std::array<u64,3>{v_c28, 0, 0}, [&](const std::array<u64,3>& m32) {
            u64 v_c29 = m32[1]; u64 v_c30 = m32[2];
            slog::join_probe<2,1>(mincostindex23, std::array<u64,2>{v_c29, 0}, [&](const std::array<u64,2>& m33) {
              u64 v_c18 = m33[1];
              u64 v_c17 = _prim__0002b(db, v_c30, v_c27);
              if (v_c17 == slog_error) { slog::emit_pending_error(db, "lat_demand.slog:20"); return; }
              ++_fires;
      ...
      if (_fires) db->bumpFires("lat_demand.slog:20", "delta:mincost_ans", _fires);
  ...
  // (crule (pre) (scan toint_range __erre4D1x80 __errf4WjL81 __errf6Qks82) (body) (head (emit error (0) __erre4D1x80)) <internal>:1 #f)
  ...
  // (crule (pre) (scan div_by_zero __erre5OnQ66 __errf6qWw67 __errf49ym68) (body) (head (emit error (0) __erre5OnQ66)) <internal>:1 #f)
  ...
  // (crule (pre) (scan modulo_by_zero __erre6Z7l69 __errf0UlE70 __errf69kn71) (body) (head (emit error (0) __erre6Z7l69)) <internal>:1 #f)
  ...
  s->addReadRel("mincost");
  s->addReadRel("mincost_ans");
  s->addReadRel("modulo_by_zero");
  s->addReadRel("mpz_overflow");
  s->addReadRel("mpz_table_overflow");
  s->addReadRel("nan_result");
  s->addReadRel("temp6Yq496");
  s->addReadRel("toint_range");
  s->addReadRel("type_mismatch");
  s->addDynamicRel("div_by_zero");
  s->addDynamicRel("error");
  s->addDynamicRel("int_overflow");
  s->addDynamicRel("mincost_ans");
  s->addDynamicRel("modulo_by_zero");
  s->addDynamicRel("mpz_overflow");
  s->addDynamicRel("mpz_table_overflow");
  s->addDynamicRel("nan_result");
  s->addDynamicRel("temp6Yq496");
  s->addDynamicRel("toint_range");
  s->addDynamicRel("type_mismatch");
  s->addAccelRel("mincost_ans");
  d->push(s);
  d->continueRun();
}
