...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("30345530");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  // (crule (pre) (scan edge n ____38PT7) (body) (head (mkstruct dub (1 0) __61Pt12 n)) rule_noarrow.slog:20 #f)
  ...
  // (crule (pre) (scan path y z) (body (join edge (1 0) 1 y x)) (head (emit path (0 1) x z)) rule_noarrow.slog:11 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      edgeindex1 = readrel6->getIndex(ord5, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c2 = _t[0];
        u64 v_c3 = _t[1];
        slog::join_probe<2,1>(edgeindex1, std::array<u64,2>{v_c2, 0}, [&](const std::array<u64,2>& m7) {
          u64 v_c4 = m7[1];
          ++_fires;
      ...
      if (_fires) db->bumpFires("rule_noarrow.slog:11", "delta:path", _fires);
  ...
  // (crule (pre) (scan edge x y) (body) (head (emit path (0 1) x y)) rule_noarrow.slog:10 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel10 = db->getRelation("path");
    ...
    virtual bool work()
    {
      ...
      if (_fires) db->bumpFires("rule_noarrow.slog:10", "all:edge", _fires);
