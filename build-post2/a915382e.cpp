...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("a915382e");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  // (crule (pre) (scan smt_vcode C V) (body (join smt_core (0 1) 0 __t5oyh106 F)) (head (mkstruct smt_qcore (1 0) __1yrW267 F)) smt.slog:106 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel3 = db->getRelation("smt_core");
      smt_coreindex0 = readrel3->getIndex(ord2, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        u64 v_c1 = _t[1];
        slog::join_all<2>(smt_coreindex0, [&](const std::array<u64,2>& m4) {
          u64 v_c2 = m4[0]; u64 v_c3 = m4[1];
          ++_fires;
  ...
  // (crule (pre) (scan smt_vcode C V) (body (join smt_check (0 1) 0 __t7MEw103 F)) (head (mkstruct smt_qsat (1 0) __85qL262 F)) smt.slog:69 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel8 = db->getRelation("smt_check");
      smt_checkindex5 = readrel8->getIndex(ord7, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        u64 v_c1 = _t[1];
        slog::join_all<2>(smt_checkindex5, [&](const std::array<u64,2>& m9) {
          u64 v_c4 = m9[0]; u64 v_c3 = m9[1];
          ++_fires;
          slog::emit_struct<2>(head_rel[0], newbatch[0], std::array<u64,1>{v_c3}, std::array<u16,2>{1, 0});
        });
      });
  ...
  // (crule (pre) (scan smt_vcode C V) (body (join smt_model (0 1) 0 __t07vR261 F)) (head (mkstruct smt_qmodel (1 0) __4quW282 F)) smt.slog:93 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel13 = db->getRelation("smt_model");
      smt_modelindex10 = readrel13->getIndex(ord12, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        u64 v_c1 = _t[1];
        slog::join_all<2>(smt_modelindex10, [&](const std::array<u64,2>& m14) {
          u64 v_c5 = m14[0]; u64 v_c3 = m14[1];
          ++_fires;
          slog::emit_struct<2>(head_rel[0], newbatch[0], std::array<u64,1>{v_c3}, std::array<u16,2>{1, 0});
        });
      });
  ...
  // (crule (pre) (scan smt_bad_formula __erre2Nis411 __errf6nCL412 __errf05v8413) (body) (head (emit error (0) __erre2Nis411)) <internal>:1 #f)
