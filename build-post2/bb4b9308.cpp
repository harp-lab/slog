...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("bb4b9308");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("parent"), std::array<u16,2>{1, 0}, b));
  ...
  // (crule (pre) (scan parent Y Z) (body (join parent (1 0) 1 Y X)) (head (emit grandparent (0 1) X Z)) grandparent.slog:10 #f)
  class ReadTask1 : public slog::Task
  {
    ...
    slog::Index** parentindex0;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("grandparent");
      std::vector<u16> ord2({0, 1});
      slog::Relation* readrel3 = db->getRelation("grandparent");
      head_index[0] = readrel3->getIndex(ord2, false);
      outer_rel = db->getRelation("parent");
      std::vector<u16> ord4({1, 0});
      slog::Relation* readrel5 = db->getRelation("parent");
      parentindex0 = readrel5->getIndex(ord4, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        u64 v_c1 = _t[1];
        slog::join_probe<2,1>(parentindex0, std::array<u64,2>{v_c0, 0}, [&](const std::array<u64,2>& m6) {
          u64 v_c2 = m6[1];
          ++_fires;
      ...
      if (_fires) db->bumpFires("grandparent.slog:10", "all:parent", _fires);
