...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("f6acdcab");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  r = db->getRelation("temp13uY170");
  if (r == 0) db->addTempRelation("temp13uY170", 3);
  ...
  // (crule (pre) (scan compose __t8QaG20 f g) (body) (head (emit-temp temp13uY170 __t8QaG20 f g) (mkstruct _lam54764x22x18 (1 2 0) __t2Nhy19 f g)) dem_lambda.slog:23 #f)
  ...
  public:
    ...
    {
      head_rel[0] = db->getRelation("temp13uY170");
      head_rel[1] = db->getRelation("_lam54764x22x18");
      outer_rel = db->getRelation("compose");
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        ...
        slog::emit_struct<3>(head_rel[1], newbatch[1], std::array<u64,2>{v_c1, v_c2}, std::array<u16,3>{1, 2, 0});
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
        head_rel[1]->sendBatch(newbatch[1]);
  
      if (_fires) db->bumpFires("dem_lambda.slog:23", "all:compose", _fires);
  ...
  // (crule (pre) (scan temp13uY170 __t8QaG20 f g) (body (join _lam54764x22x18 (1 2 0) 2 f g __t2Nhy19)) (head (emit compose_ans (0 1) __t8QaG20 __t2Nhy19)) dem_lambda.slog:23 #f)
  class ReadTask2 : public slog::Task
  {
    ...
    slog::Index** _lam54764x22x18index1;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("compose_ans");
      std::vector<u16> ord3({0, 1});
      slog::Relation* readrel4 = db->getRelation("compose_ans");
      head_index[0] = readrel4->getIndex(ord3, false);
      outer_rel = db->getRelation("temp13uY170");
      std::vector<u16> ord5({1, 2, 0});
      slog::Relation* readrel6 = db->getRelation("_lam54764x22x18");
      _lam54764x22x18index1 = readrel6->getIndex(ord5, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        ...
        u64 v_c1 = _t[1];
        u64 v_c2 = _t[2];
        slog::join_probe<3,2>(_lam54764x22x18index1, std::array<u64,3>{v_c1, v_c2, 0}, [&](const std::array<u64,3>& m7) {
          u64 v_c3 = m7[2];
          ++_fires;
      ...
      if (_fires) db->bumpFires("dem_lambda.slog:23", "delta:temp13uY170", _fires);
  ...
  s->addReadRel("temp13uY170");
  s->addDynamicRel("$seq_at");
  s->addDynamicRel("_lam54764x22x18");
  s->addDynamicRel("compose_ans");
  s->addDynamicRel("temp13uY170");
  d->push(s);
  d->continueRun();
}
