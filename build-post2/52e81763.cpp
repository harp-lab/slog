...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("52e81763");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  // (crule (pre) (scan path X Y) (body (join edge (0 1) 1 Y Z)) (head (emit path (0 1) X Z)) shape_rev.slog:24 #f)
  ...
  public:
    ...
    virtual bool work()
    {
      ...
      if (_fires) db->bumpFires("shape_rev.slog:24", "delta:path", _fires);
  ...
  // (crule (pre) (scan edge X Y) (body) (head (emit rev (0 1) Y X) (emit start (0) X)) shape_rev.slog:26 #f)
  ...
  public:
    ...
    virtual bool work()
    {
      ...
      if (_fires) db->bumpFires("shape_rev.slog:26", "all:edge", _fires);
  ...
  // (crule (pre) (scan edge Y Z) (body (join edge (1 0) 1 Y X)) (head (emit two (0 1) X Z)) shape_rev.slog:28 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("two");
      std::vector<u16> ord14({0, 1});
      slog::Relation* readrel15 = db->getRelation("two");
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c1 = _t[0];
        u64 v_c2 = _t[1];
        slog::join_probe<2,1>(edgeindex12, std::array<u64,2>{v_c1, 0}, [&](const std::array<u64,2>& m18) {
          u64 v_c0 = m18[1];
          ++_fires;
          slog::emit<2>(head_rel[0], head_index[0], newbatch[0], std::array<u64,2>{v_c0, v_c2}, std::array<u16,2>{0, 1});
        });
      ...
      if (_fires) db->bumpFires("shape_rev.slog:28", "all:edge", _fires);
  ...
  // (crule (pre) (scan edge X Y) (body) (head (emit path (0 1) X Y)) shape_rev.slog:22 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel21 = db->getRelation("path");
    ...
    virtual bool work()
    {
      ...
      if (_fires) db->bumpFires("shape_rev.slog:22", "all:edge", _fires);
