...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("e13c9fb5");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  r = db->getRelation("onlya");
  if (r == 0) db->addRelation("onlya", 1);
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("onlya"), std::array<u16,1>{0}, b));
  ...
  // (crule (pre (let __tconst75UG1 const6b86b273ff34fce19d6b804e) (let __tconst6u4P3 const4e07408562bedb8b60ce05c1) (let __tconst8kBO5 constd4735e3a265e16eee03f5971) (let __tconst5rqM6 const4b227777d4dd1fc61c6f884f) (let __tconst5Fpa7 constef2d127de37b942baad06145)) (once) (body) (head (emit a (0) __tconst75UG1) (emit a (0) __tconst8kBO5) (emit a (0) __tconst6u4P3) (emit a (0) __tconst5rqM6) (emit b (0) __tconst8kBO5) (emit b (0) __tconst5rqM6) (emit b (0) __tconst5Fpa7)) neg_diff.slog:7 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      head_rel[2] = db->getRelation("a");
      std::vector<u16> ord5({0});
      slog::Relation* readrel6 = db->getRelation("a");
      head_index[2] = readrel6->getIndex(ord5, false);
      head_rel[3] = db->getRelation("a");
      std::vector<u16> ord7({0});
      slog::Relation* readrel8 = db->getRelation("a");
      head_index[3] = readrel8->getIndex(ord7, false);
      head_rel[4] = db->getRelation("b");
      std::vector<u16> ord9({0});
      slog::Relation* readrel10 = db->getRelation("b");
      head_index[4] = readrel10->getIndex(ord9, false);
      head_rel[5] = db->getRelation("b");
      std::vector<u16> ord11({0});
      slog::Relation* readrel12 = db->getRelation("b");
      head_index[5] = readrel12->getIndex(ord11, false);
      head_rel[6] = db->getRelation("b");
      std::vector<u16> ord13({0});
      slog::Relation* readrel14 = db->getRelation("b");
    ...
    virtual bool work()
    {
      ...
      slog::emit<1>(head_rel[4], head_index[4], newbatch[4], std::array<u64,1>{v_c2}, std::array<u16,1>{0});
      ...
      if (_fires) db->bumpFires("neg_diff.slog:7", "once", _fires);
