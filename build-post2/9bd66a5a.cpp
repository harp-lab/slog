...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("9bd66a5a");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  // (crule (pre) (scan node d) (body (join entry (0) 0 e) (neq d e)) (head (emit nd (0 1) d e)) domtree.slog:42 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel3 = db->getRelation("nd");
      head_index[0] = readrel3->getIndex(ord2, false);
      outer_rel = db->getRelation("node");
      std::vector<u16> ord4({0});
      slog::Relation* readrel5 = db->getRelation("entry");
      entryindex0 = readrel5->getIndex(ord4, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        slog::join_all<1>(entryindex0, [&](const std::array<u64,1>& m6) {
          u64 v_c1 = m6[0];
          if (v_c0 == v_c1) return;
          ++_fires;
      ...
      if (_fires) db->bumpFires("domtree.slog:42", "all:node", _fires);
  ...
  // (crule (pre) (scan nd d p) (body (join edge (0 1) 1 p n) (neq n d)) (head (emit nd (0 1) d n)) domtree.slog:43 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel10 = db->getRelation("nd");
      ...
      edgeindex7 = readrel12->getIndex(ord11, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        u64 v_c2 = _t[1];
        slog::join_probe<2,1>(edgeindex7, std::array<u64,2>{v_c2, 0}, [&](const std::array<u64,2>& m13) {
          u64 v_c3 = m13[1];
          if (v_c3 == v_c0) return;
          ++_fires;
      ...
      if (_fires) db->bumpFires("domtree.slog:43", "delta:nd", _fires);
