...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("3ff81455");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  // (crule (pre) (scan unique_closure cv) (body (join call_target (2 0 1 3) 1 cv site c l) (join monomorphic_site (0) 1 site)) (head (emit inlinable_call (0 1) site cv)) analysis.slog:32 #f)
  class ReadTask2 : public slog::Task
  {
    ...
    slog::Index** call_targetindex0;  slog::Index** monomorphic_siteindex1;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("inlinable_call");
      std::vector<u16> ord3({0, 1});
      slog::Relation* readrel4 = db->getRelation("inlinable_call");
      head_index[0] = readrel4->getIndex(ord3, false);
      outer_rel = db->getRelation("unique_closure");
      std::vector<u16> ord5({2, 0, 1, 3});
      slog::Relation* readrel6 = db->getRelation("call_target");
      call_targetindex0 = readrel6->getIndex(ord5, false);
      std::vector<u16> ord7({0});
      slog::Relation* readrel8 = db->getRelation("monomorphic_site");
      monomorphic_siteindex1 = readrel8->getIndex(ord7, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        slog::join_probe<4,1>(call_targetindex0, std::array<u64,4>{v_c0, 0, 0, 0}, [&](const std::array<u64,4>& m9) {
          u64 v_c1 = m9[1]; u64 v_c2 = m9[2]; u64 v_c3 = m9[3];
          slog::join_probe<1,1>(monomorphic_siteindex1, std::array<u64,1>{v_c1}, [&](const std::array<u64,1>& m10) {
            ++_fires;
      ...
      if (_fires) db->bumpFires("analysis.slog:32", "all:unique_closure", _fires);
  ...
  s->addReadRel("monomorphic_site");
  s->addReadRel("unique_closure");
  s->addDynamicRel("$seq_at");
  s->addDynamicRel("$seq_atr");
  s->addDynamicRel("inlinable_call");
  d->push(s);
  d->continueRun();
}
