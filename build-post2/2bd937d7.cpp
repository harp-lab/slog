...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("2bd937d7");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  // (crule (pre) (scan parent Y Z) (body (join parent (1 0) 1 Y X)) (head (emit gp (0 1) X Z)) shape_twice.slog:18 #f)
  ...
  public:
    ...
    {
      head_rel[0] = db->getRelation("gp");
      std::vector<u16> ord2({0, 1});
      slog::Relation* readrel3 = db->getRelation("gp");
    ...
    virtual bool work()
    {
      ...
      if (_fires) db->bumpFires("shape_twice.slog:18", "all:parent", _fires);
  ...
  // (crule (pre) (scan parent Z W) (body (join parent (1 0) 1 Z Y) (join parent (1 0) 1 Y X)) (head (emit ggp (0 1) X W)) shape_twice.slog:20 #f)
  class ReadTask9 : public slog::Task
  {
    ...
    slog::Index** parentindex7;  slog::Index** parentindex8;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("ggp");
      std::vector<u16> ord10({0, 1});
      slog::Relation* readrel11 = db->getRelation("ggp");
      head_index[0] = readrel11->getIndex(ord10, false);
      outer_rel = db->getRelation("parent");
      std::vector<u16> ord12({1, 0});
      slog::Relation* readrel13 = db->getRelation("parent");
      parentindex7 = readrel13->getIndex(ord12, false);
      std::vector<u16> ord14({1, 0});
      slog::Relation* readrel15 = db->getRelation("parent");
      parentindex8 = readrel15->getIndex(ord14, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c1 = _t[0];
        u64 v_c3 = _t[1];
        slog::join_probe<2,1>(parentindex7, std::array<u64,2>{v_c1, 0}, [&](const std::array<u64,2>& m16) {
          u64 v_c0 = m16[1];
          slog::join_probe<2,1>(parentindex8, std::array<u64,2>{v_c0, 0}, [&](const std::array<u64,2>& m17) {
            u64 v_c2 = m17[1];
            ++_fires;
      ...
      if (_fires) db->bumpFires("shape_twice.slog:20", "all:parent", _fires);
