...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("02d10256");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  // (crule (pre) (scan edge X Y) (body) (head (emit path (0 1) X Y)) accel_chain.slog:8 #f)
  ...
  public:
    ...
    virtual bool work()
    {
      ...
      if (_fires) db->bumpFires("accel_chain.slog:8", "all:edge", _fires);
  ...
  // (crule (pre) (scan path X Y) (body (join edge (0 1) 1 Y Z)) (head (emit path (0 1) X Z)) accel_chain.slog:9 #f)
  ...
  public:
    ...
    virtual bool work()
    {
      ...
      if (_fires) db->bumpFires("accel_chain.slog:9", "delta:path", _fires);
