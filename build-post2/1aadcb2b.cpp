...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("1aadcb2b");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  r = db->getRelation("temp92p57");
  if (r == 0) db->addTempRelation("temp92p57", 2);
  else if (r->getArity() != 2) slog::fatal("Temp rel arity mismatch.");
  r = db->getRelation("temp2LMT8");
  if (r == 0) db->addTempRelation("temp2LMT8", 2);
  else if (r->getArity() != 2) slog::fatal("Temp rel arity mismatch.");
  ...
  r = db->getRelation("foo");
  if (r == 0) db->addStruct("foo", 3);
  else if (r->getArity() != 3)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("foo");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternStructTask<3>(db, db->getRelation("foo"), std::array<u16,3>{1, 2, 0}, b));
  ...
  r = db->getRelation("bar");
  if (r == 0) db->addStruct("bar", 2);
  else if (r->getArity() != 2)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("bar");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternStructTask<2>(db, db->getRelation("bar"), std::array<u16,2>{1, 0}, b));
  ...
  // (crule (pre) (scan in X Y) (body) (head (emit-temp temp92p57 X Y) (mkstruct bar (1 0) __t2mSE1 X)) nested.slog:10 #f)
  ...
  public:
    ...
    {
      head_rel[0] = db->getRelation("temp92p57");
      head_rel[1] = db->getRelation("bar");
    ...
    virtual bool work()
    {
      ...
      if (_fires) db->bumpFires("nested.slog:10", "all:in", _fires);
  ...
  // (crule (pre) (scan temp2LMT8 Y __t2mSE1) (body (join foo (1 2 0) 2 __t2mSE1 Y __t0TDO2)) (head (emit out (0) __t0TDO2)) nested.slog:10 #f)
  class ReadTask2 : public slog::Task
  {
    ...
    slog::Index** fooindex1;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("out");
      std::vector<u16> ord3({0});
      slog::Relation* readrel4 = db->getRelation("out");
      head_index[0] = readrel4->getIndex(ord3, false);
      outer_rel = db->getRelation("temp2LMT8");
      std::vector<u16> ord5({1, 2, 0});
      slog::Relation* readrel6 = db->getRelation("foo");
      fooindex1 = readrel6->getIndex(ord5, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c1 = _t[0];
        u64 v_c2 = _t[1];
        slog::join_probe<3,2>(fooindex1, std::array<u64,3>{v_c2, v_c1, 0}, [&](const std::array<u64,3>& m7) {
          u64 v_c3 = m7[2];
          ++_fires;
      ...
      if (_fires) db->bumpFires("nested.slog:10", "delta:temp2LMT8", _fires);
  ...
  // (crule (pre) (scan temp92p57 X Y) (body (join bar (1 0) 1 X __t2mSE1)) (head (emit-temp temp2LMT8 Y __t2mSE1) (mkstruct foo (1 2 0) __t0TDO2 __t2mSE1 Y)) nested.slog:10 #f)
  class ReadTask9 : public slog::Task
  {
    ...
    slog::Index** barindex8;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("temp2LMT8");
      head_rel[1] = db->getRelation("foo");
      outer_rel = db->getRelation("temp92p57");
      std::vector<u16> ord10({1, 0});
      slog::Relation* readrel11 = db->getRelation("bar");
      barindex8 = readrel11->getIndex(ord10, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        u64 v_c1 = _t[1];
        slog::join_probe<2,1>(barindex8, std::array<u64,2>{v_c0, 0}, [&](const std::array<u64,2>& m12) {
          u64 v_c2 = m12[1];
          ++_fires;
          slog::emit_temp<2>(head_rel[0], newbatch[0], std::array<u64,2>{v_c1, v_c2});
      ...
      if (_fires) db->bumpFires("nested.slog:10", "delta:temp92p57", _fires);
  ...
  s->addReadRel("foo");
  s->addReadRel("in");
  s->addReadRel("temp2LMT8");
  s->addReadRel("temp92p57");
  s->addDynamicRel("bar");
  s->addDynamicRel("foo");
  s->addDynamicRel("out");
  s->addDynamicRel("temp2LMT8");
  s->addDynamicRel("temp92p57");
  d->push(s);
  d->continueRun();
}
