...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("b23ec9fe");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  if (r == 0) db->addStruct("nan_result", 4);
    ...
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("nan_result");
      std::vector<u16> ord10({1, 2, 3, 0});
    
    r->addIndex<4>(ord10, false);
    for (u16 b = 0; b < 32; ++b)
      s->addTask(phase_write, new slog::WriteTask<4>(db, r, std::array<u16,4>{1, 2, 3, 0}, false, b), true);
      std::vector<u16> ord11({0, 1, 2, 3});
  ...
  // (crule (pre (let __tconst6iLI7 const7902699be42c8a8e46fbbb45)) (once) (body) (head (emit grow (0) __tconst6iLI7)) strat_run_main.slog:11 #f)
  ...
  public:
    ...
    {
      head_rel[0] = db->getRelation("grow");
      std::vector<u16> ord1({0});
      slog::Relation* readrel2 = db->getRelation("grow");
    ...
    virtual bool work()
    {
      ...
      if (_fires) db->bumpFires("strat_run_main.slog:11", "once", _fires);
