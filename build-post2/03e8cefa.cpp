...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("v"), std::array<u16,1>{0}, b));
  ...
  r = db->getRelation("sums");
  if (r == 0) db->addRelation("sums", 3);
  else if (r->getArity() != 3)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("sums");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<3>(db, db->getRelation("sums"), std::array<u16,3>{0, 1, 2}, b));
  ...
  r = db->getRelation("small");
  if (r == 0) db->addRelation("small", 1);
  else if (r->getArity() != 1)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("small");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("small"), std::array<u16,1>{0}, b));
  ...
  r = db->getRelation("mx2");
  if (r == 0) db->addRelation("mx2", 3);
  else if (r->getArity() != 3)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("mx2");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<3>(db, db->getRelation("mx2"), std::array<u16,3>{0, 1, 2}, b));
  ...
  r = db->getRelation("ltv");
  if (r == 0) db->addRelation("ltv", 2);
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("ltv"), std::array<u16,2>{0, 1}, b));
  ...
  // (crule (pre (let __tconst3TTM2 constb8736b999909049671d0ea07) (let __tconst7whR1 constd4735e3a265e16eee03f5971)) (once) (body) (head (emit v (0) __tconst3TTM2) (emit v (0) __tconst7whR1)) prim2_mixed.slog:11 #f)
  ...
  public:
    ...
    {
      head_rel[0] = db->getRelation("v");
      std::vector<u16> ord1({0});
      slog::Relation* readrel2 = db->getRelation("v");
      head_index[0] = readrel2->getIndex(ord1, false);
      head_rel[1] = db->getRelation("v");
      std::vector<u16> ord3({0});
      slog::Relation* readrel4 = db->getRelation("v");
    ...
    virtual bool work()
    {
      u64 v_c0 = v_constb8736b999909049671d0ea07;
      ...
      if (_fires) db->bumpFires("prim2_mixed.slog:11", "once", _fires);
