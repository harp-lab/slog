...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("919aa89a");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  // (crule (pre (let __tconst3xjj8 constad57366865126e55649ecb23) (let __tconst7EYk10 const27badc983df1780b60c2b3fa) (let __tconst6Z1J13 const6b86b273ff34fce19d6b804e) (let __tconst4YfV14 const4e07408562bedb8b60ce05c1) (let __tconst2i1B15 constd4735e3a265e16eee03f5971)) (once) (body) (head (emit bind (0 1) __tconst6Z1J13 __tconst3xjj8) (emit bind (0 1) __tconst2i1B15 __tconst3xjj8) (emit bind (0 1) __tconst4YfV14 __tconst7EYk10) (emit step (0 1) __tconst6Z1J13 __tconst2i1B15) (emit step (0 1) __tconst2i1B15 __tconst4YfV14)) lat_count.slog:17 #f)
  ...
  public:
    ...
    {
      head_rel[0] = db->getRelation("bind");
      std::vector<u16> ord1({0, 1});
      slog::Relation* readrel2 = db->getRelation("bind");
      head_index[0] = readrel2->getIndex(ord1, false);
      head_rel[1] = db->getRelation("bind");
      std::vector<u16> ord3({0, 1});
      slog::Relation* readrel4 = db->getRelation("bind");
      head_index[1] = readrel4->getIndex(ord3, false);
      head_rel[2] = db->getRelation("bind");
      std::vector<u16> ord5({0, 1});
      slog::Relation* readrel6 = db->getRelation("bind");
      head_index[2] = readrel6->getIndex(ord5, false);
      head_rel[3] = db->getRelation("step");
      std::vector<u16> ord7({0, 1});
      slog::Relation* readrel8 = db->getRelation("step");
      head_index[3] = readrel8->getIndex(ord7, false);
      head_rel[4] = db->getRelation("step");
      std::vector<u16> ord9({0, 1});
      slog::Relation* readrel10 = db->getRelation("step");
    ...
    virtual bool work()
    {
      u64 v_c0 = v_constad57366865126e55649ecb23;
      u64 v_c1 = v_const27badc983df1780b60c2b3fa;
      ...
      slog::emit<2>(head_rel[1], head_index[1], newbatch[1], std::array<u64,2>{v_c4, v_c0}, std::array<u16,2>{0, 1});
      slog::emit<2>(head_rel[2], head_index[2], newbatch[2], std::array<u64,2>{v_c3, v_c1}, std::array<u16,2>{0, 1});
      ...
      if (_fires) db->bumpFires("lat_count.slog:17", "once", _fires);
