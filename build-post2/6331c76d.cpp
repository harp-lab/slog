...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("6331c76d");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  // (crule (pre) (scan shapes s) (body (join center (1 0) 1 s __t16aR14) (join center_ans (0 1) 1 __t16aR14 __t2Agj15) (join pt (0 1 2) 1 __t2Agj15 x ____2ZqE16)) (head (emit centerx (0 1 2) s __t2Agj15 x)) dem_alias.slog:23 #f)
  class ReadTask3 : public slog::Task
  {
    ...
    slog::Index** centerindex0;  slog::Index** center_ansindex1;  slog::Index** ptindex2;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("centerx");
      std::vector<u16> ord4({0, 1, 2});
      slog::Relation* readrel5 = db->getRelation("centerx");
      head_index[0] = readrel5->getIndex(ord4, false);
      outer_rel = db->getRelation("shapes");
      std::vector<u16> ord6({1, 0});
      slog::Relation* readrel7 = db->getRelation("center");
      centerindex0 = readrel7->getIndex(ord6, false);
      std::vector<u16> ord8({0, 1});
      slog::Relation* readrel9 = db->getRelation("center_ans");
      center_ansindex1 = readrel9->getIndex(ord8, false);
      std::vector<u16> ord10({0, 1, 2});
      slog::Relation* readrel11 = db->getRelation("pt");
      ptindex2 = readrel11->getIndex(ord10, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        slog::join_probe<2,1>(centerindex0, std::array<u64,2>{v_c0, 0}, [&](const std::array<u64,2>& m12) {
          u64 v_c1 = m12[1];
          slog::join_probe<2,1>(center_ansindex1, std::array<u64,2>{v_c1, 0}, [&](const std::array<u64,2>& m13) {
            u64 v_c2 = m13[1];
            slog::join_probe<3,1>(ptindex2, std::array<u64,3>{v_c2, 0, 0}, [&](const std::array<u64,3>& m14) {
              u64 v_c3 = m14[1]; u64 v_c4 = m14[2];
              ++_fires;
      ...
      if (_fires) db->bumpFires("dem_alias.slog:23", "all:shapes", _fires);
  ...
  // (crule (pre) (scan shapes s) (body (join center (1 0) 1 s __t0bd436) (join center_ans (0 1) 1 __t0bd436 __t7zVd35) (join pt (0 1 2) 1 __t7zVd35 ____2JAr37 y)) (head (emit centery (0 1) s y)) dem_alias.slog:27 #f)
  class ReadTask18 : public slog::Task
  {
    ...
    slog::Index** centerindex15;  slog::Index** center_ansindex16;  slog::Index** ptindex17;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("centery");
      std::vector<u16> ord19({0, 1});
      slog::Relation* readrel20 = db->getRelation("centery");
      head_index[0] = readrel20->getIndex(ord19, false);
      outer_rel = db->getRelation("shapes");
      std::vector<u16> ord21({1, 0});
      slog::Relation* readrel22 = db->getRelation("center");
      centerindex15 = readrel22->getIndex(ord21, false);
      std::vector<u16> ord23({0, 1});
      slog::Relation* readrel24 = db->getRelation("center_ans");
      center_ansindex16 = readrel24->getIndex(ord23, false);
      std::vector<u16> ord25({0, 1, 2});
      slog::Relation* readrel26 = db->getRelation("pt");
      ptindex17 = readrel26->getIndex(ord25, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        slog::join_probe<2,1>(centerindex15, std::array<u64,2>{v_c0, 0}, [&](const std::array<u64,2>& m27) {
          u64 v_c5 = m27[1];
          slog::join_probe<2,1>(center_ansindex16, std::array<u64,2>{v_c5, 0}, [&](const std::array<u64,2>& m28) {
            u64 v_c6 = m28[1];
            slog::join_probe<3,1>(ptindex17, std::array<u64,3>{v_c6, 0, 0}, [&](const std::array<u64,3>& m29) {
              u64 v_c7 = m29[1]; u64 v_c8 = m29[2];
              ++_fires;
      ...
      if (_fires) db->bumpFires("dem_alias.slog:27", "all:shapes", _fires);
  ...
  s->addReadRel("center");
  s->addReadRel("center_ans");
  s->addReadRel("pt");
  s->addReadRel("shapes");
  s->addDynamicRel("centerx");
  s->addDynamicRel("centery");
  d->push(s);
  d->continueRun();
}
