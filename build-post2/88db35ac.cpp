...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("88db35ac");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  if (r == 0) db->addRelation("$sup15869x25x0x0x1", 8);
    ...
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("$sup15869x25x0x0x1");
      std::vector<u16> ord47({0, 1, 2, 3, 4, 5, 6, 7});
    
    r->addIndex<8>(ord47, false);
    for (u16 b = 0; b < 32; ++b)
      s->addTask(phase_write, new slog::WriteTask<8>(db, r, std::array<u16,8>{0, 1, 2, 3, 4, 5, 6, 7}, false, b), true);
  

  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<8>(db, db->getRelation("$sup15869x25x0x0x1"), std::array<u16,8>{0, 1, 2, 3, 4, 5, 6, 7}, b));
  ...
  // (crule (pre) (scan program e) (body (join eval (1 0) 1 e __t2h7R29) (join eval_ans (0 1) 1 __t2h7R29 v)) (head (emit result (0) v)) 0cfa-demand.slog:37 #f)
  class ReadTask2 : public slog::Task
  {
    ...
    slog::Index** evalindex0;  slog::Index** eval_ansindex1;
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      eval_ansindex1 = readrel8->getIndex(ord7, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        slog::join_probe<2,1>(evalindex0, std::array<u64,2>{v_c0, 0}, [&](const std::array<u64,2>& m9) {
          u64 v_c1 = m9[1];
          slog::join_probe<2,1>(eval_ansindex1, std::array<u64,2>{v_c1, 0}, [&](const std::array<u64,2>& m10) {
            u64 v_c2 = m10[1];
            ++_fires;
