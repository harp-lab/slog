...
u64 v_const19d9457176393d4f09549620;
...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("83b6dc2d");
  if (s == nullptr) return;
  slog::Relation* r;
  v_const5feceb66ffc86f38d952786c = s32_encode(0);
  v_const19d9457176393d4f09549620 = db->encodeString("!");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("w"), std::array<u16,2>{0, 1}, b));
  ...
  r = db->getRelation("suf");
  if (r == 0) db->addRelation("suf", 2);
  else if (r->getArity() != 2)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("suf");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("suf"), std::array<u16,2>{0, 1}, b));
  ...
  r = db->getRelation("pre");
  if (r == 0) db->addRelation("pre", 2);
  else if (r->getArity() != 2)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("pre");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("pre"), std::array<u16,2>{0, 1}, b));
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<3>(db, db->getRelation("cat"), std::array<u16,3>{0, 1, 2}, b));
  r = db->getRelation("bang");
  if (r == 0) db->addRelation("bang", 2);
  else if (r->getArity() != 2)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("bang");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("bang"), std::array<u16,2>{0, 1}, b));
  ...
  // (crule (pre) (scan div_by_zero __erre6iCS47 __errf1IVo48 __errf1tfy49) (body) (head (emit error (0) __erre6iCS47)) <internal>:1 #f)
  ...
  // (crule (pre (let __tconst3Fk64 const19d9457176393d4f09549620)) (scan w A B) (body (let __t2FJD5 (_0002b A __tconst3Fk64))) (head (emit bang (0 1) A __t2FJD5)) prim2_str.slog:21 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("bang");
      std::vector<u16> ord4({0, 1});
      slog::Relation* readrel5 = db->getRelation("bang");
    ...
    virtual bool work()
    {
      u64 v_c3 = v_const19d9457176393d4f09549620;
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c4 = _t[0];
        u64 v_c5 = _t[1];
        u64 v_c6 = _prim__0002b(db, v_c4, v_c3);
        if (v_c6 == slog_error) { slog::emit_pending_error(db, "prim2_str.slog:21"); return; }
        ++_fires;
      ...
      if (_fires) db->bumpFires("prim2_str.slog:21", "all:w", _fires);
  ...
  // (crule (pre) (scan int_overflow __erre7vzL53 __errf9G0Z54 __errf8tpn55 __errf8sd656) (body) (head (emit error (0) __erre7vzL53)) <internal>:1 #f)
  ...
  // (crule (pre) (scan type_mismatch __erre3L1V64 __errf0Dgk65 __errf8LOU66 __errf0tiu67 __errf1ADW68) (body) (head (emit error (0) __erre3L1V64)) <internal>:1 #f)
  ...
  // (crule (pre (let __tconst38h16 const4fc82b26aecb47d2868c4efb) (let __tconst1eXO7 conste7f6c011776e8db7cd330b54)) (scan t S) (body (let __t02lR8 (substr S __tconst1eXO7 __tconst38h16))) (head (emit suf (0 1) S __t02lR8)) prim2_str.slog:24 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("suf");
      std::vector<u16> ord13({0, 1});
      slog::Relation* readrel14 = db->getRelation("suf");
    ...
    virtual bool work()
    {
      u64 v_c16 = v_const4fc82b26aecb47d2868c4efb;
      u64 v_c17 = v_conste7f6c011776e8db7cd330b54;
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c18 = _t[0];
        u64 v_c19 = _prim_substr(db, v_c18, v_c17, v_c16);
        if (v_c19 == slog_error) { slog::emit_pending_error(db, "prim2_str.slog:24"); return; }
        ++_fires;
      ...
      if (_fires) db->bumpFires("prim2_str.slog:24", "all:t", _fires);
  ...
  // (crule (pre (let __tconst3F6D1 constef2d127de37b942baad06145) (let __tconst3Zah2 const5feceb66ffc86f38d952786c)) (scan t S) (body (let __t5c923 (substr S __tconst3Zah2 __tconst3F6D1))) (head (emit pre (0 1) S __t5c923)) prim2_str.slog:23 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("pre");
      std::vector<u16> ord16({0, 1});
      slog::Relation* readrel17 = db->getRelation("pre");
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c18 = _t[0];
        u64 v_c22 = _prim_substr(db, v_c18, v_c21, v_c20);
        if (v_c22 == slog_error) { slog::emit_pending_error(db, "prim2_str.slog:23"); return; }
        ++_fires;
        slog::emit<2>(head_rel[0], head_index[0], newbatch[0], std::array<u64,2>{v_c18, v_c22}, std::array<u16,2>{0, 1});
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("prim2_str.slog:23", "all:t", _fires);
  ...
  // (crule (pre) (scan modulo_by_zero __erre1WRX50 __errf9Zka51 __errf7oxZ52) (body) (head (emit error (0) __erre1WRX50)) <internal>:1 #f)
  ...
  // (crule (pre) (scan nan_result __erre1aSi57 __errf5Ghe58 __errf7srR59 __errf0nON60) (body) (head (emit error (0) __erre1aSi57)) <internal>:1 #f)
  ...
  // (crule (pre) (scan w A B) (body (let __t6yNB16 (_0002b A B))) (head (emit cat (0 1 2) A B __t6yNB16)) prim2_str.slog:20 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel26 = db->getRelation("cat");
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c4 = _t[0];
        u64 v_c5 = _t[1];
        u64 v_c30 = _prim__0002b(db, v_c4, v_c5);
        if (v_c30 == slog_error) { slog::emit_pending_error(db, "prim2_str.slog:20"); return; }
        ++_fires;
        slog::emit<3>(head_rel[0], head_index[0], newbatch[0], std::array<u64,3>{v_c4, v_c5, v_c30}, std::array<u16,3>{0, 1, 2});
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("prim2_str.slog:20", "all:w", _fires);
  ...
  // (crule (pre) (scan mpz_overflow __erre8Dig69 __errf3e6l70 __errf71Fq71 __errf6og872 __errf3kBQ73) (body) (head (emit error (0) __erre8Dig69)) <internal>:1 #f)
  ...
  // (crule (pre) (scan toint_range __erre5BXB61 __errf66EI62 __errf3JtZ63) (body) (head (emit error (0) __erre5BXB61)) <internal>:1 #f)
  ...
  // (crule (pre) (scan mpz_table_overflow __erre6Lt574 __errf3Xj975 __errf8ILA76) (body) (head (emit error (0) __erre6Lt574)) <internal>:1 #f)
