...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("86123bf4");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  r = db->getRelation("type_mismatch");
      std::vector<u16> ord19({1, 2, 3, 4, 0});
    
    r->addIndex<5>(ord19, false);
    for (u16 b = 0; b < 32; ++b)
      s->addTask(phase_write, new slog::WriteTask<5>(db, r, std::array<u16,5>{1, 2, 3, 4, 0}, false, b), true);
      std::vector<u16> ord20({0, 1, 2, 3, 4});
  ...
  r = db->getRelation("fixp");
  if (r == 0) db->addRelation("fixp", 2);
  else if (r->getArity() != 2)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("fixp");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("fixp"), std::array<u16,2>{0, 1}, b));
  ...
  r = db->getRelation("dbl");
  if (r == 0) db->addRelation("dbl", 2);
  else if (r->getArity() != 2)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("dbl");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("dbl"), std::array<u16,2>{0, 1}, b));
  r = db->getRelation("consec");
  if (r == 0) db->addRelation("consec", 2);
  else if (r->getArity() != 2)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("consec");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("consec"), std::array<u16,2>{0, 1}, b));
  ...
  // (crule (pre (let __tconst6m7x13 const4b227777d4dd1fc61c6f884f) (let __tconst5UP117 conste7f6c011776e8db7cd330b54) (let __tconst2FU118 constef2d127de37b942baad06145) (let __tconst19iA19 const5feceb66ffc86f38d952786c) (let __tconst521N20 const6b86b273ff34fce19d6b804e) (let __tconst3svN21 constd4735e3a265e16eee03f5971) (let __tconst9HRR22 const4e07408562bedb8b60ce05c1)) (once) (body) (head (emit p (0 1) __tconst19iA19 __tconst521N20) (emit p (0 1) __tconst521N20 __tconst3svN21) (emit p (0 1) __tconst3svN21 __tconst6m7x13) (emit p (0 1) __tconst9HRR22 __tconst9HRR22) (emit p (0 1) __tconst2FU118 __tconst5UP117) (emit n (0) __tconst19iA19) (emit n (0) __tconst521N20) (emit n (0) __tconst3svN21) (emit n (0) __tconst9HRR22)) prim2_letfilter.slog:11 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      head_rel[3] = db->getRelation("p");
      std::vector<u16> ord7({0, 1});
      slog::Relation* readrel8 = db->getRelation("p");
      head_index[3] = readrel8->getIndex(ord7, false);
      head_rel[4] = db->getRelation("p");
      std::vector<u16> ord9({0, 1});
      slog::Relation* readrel10 = db->getRelation("p");
      head_index[4] = readrel10->getIndex(ord9, false);
      head_rel[5] = db->getRelation("n");
      std::vector<u16> ord11({0});
      slog::Relation* readrel12 = db->getRelation("n");
      head_index[5] = readrel12->getIndex(ord11, false);
      head_rel[6] = db->getRelation("n");
      std::vector<u16> ord13({0});
      slog::Relation* readrel14 = db->getRelation("n");
      head_index[6] = readrel14->getIndex(ord13, false);
      head_rel[7] = db->getRelation("n");
      std::vector<u16> ord15({0});
      slog::Relation* readrel16 = db->getRelation("n");
      head_index[7] = readrel16->getIndex(ord15, false);
      head_rel[8] = db->getRelation("n");
      std::vector<u16> ord17({0});
      slog::Relation* readrel18 = db->getRelation("n");
    ...
    virtual bool work()
    {
      ...
      u64 v_c1 = v_conste7f6c011776e8db7cd330b54;
      ...
      slog::emit<2>(head_rel[1], head_index[1], newbatch[1], std::array<u64,2>{v_c4, v_c5}, std::array<u16,2>{0, 1});
      slog::emit<2>(head_rel[2], head_index[2], newbatch[2], std::array<u64,2>{v_c5, v_c0}, std::array<u16,2>{0, 1});
      slog::emit<2>(head_rel[3], head_index[3], newbatch[3], std::array<u64,2>{v_c6, v_c6}, std::array<u16,2>{0, 1});
      slog::emit<2>(head_rel[4], head_index[4], newbatch[4], std::array<u64,2>{v_c2, v_c1}, std::array<u16,2>{0, 1});
      slog::emit<1>(head_rel[5], head_index[5], newbatch[5], std::array<u64,1>{v_c3}, std::array<u16,1>{0});
      slog::emit<1>(head_rel[6], head_index[6], newbatch[6], std::array<u64,1>{v_c4}, std::array<u16,1>{0});
      slog::emit<1>(head_rel[7], head_index[7], newbatch[7], std::array<u64,1>{v_c5}, std::array<u16,1>{0});
      slog::emit<1>(head_rel[8], head_index[8], newbatch[8], std::array<u64,1>{v_c6}, std::array<u16,1>{0});
      ...
      if (_fires) db->bumpFires("prim2_letfilter.slog:11", "once", _fires);
