...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("57ee84f9");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  // (crule (pre) (scan node X) (body (join edge (0 1) 1 X dup2nPd23) (eq X dup2nPd23)) (head (emit selfj (0) X)) shape_selfloop.slog:36 #f)
  class ReadTask1 : public slog::Task
  {
    ...
    slog::Index** edgeindex0;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("selfj");
      std::vector<u16> ord2({0});
      slog::Relation* readrel3 = db->getRelation("selfj");
      ...
      edgeindex0 = readrel5->getIndex(ord4, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        slog::join_probe<2,1>(edgeindex0, std::array<u64,2>{v_c0, 0}, [&](const std::array<u64,2>& m6) {
          u64 v_c1 = m6[1];
          if (v_c0 != v_c1) return;
          ++_fires;
      ...
      if (_fires) db->bumpFires("shape_selfloop.slog:36", "all:node", _fires);
  ...
  // (crule (pre) (scan node X) (body) (head (emit dup (0 1) X X)) shape_selfloop.slog:38 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("dup");
      std::vector<u16> ord8({0, 1});
      slog::Relation* readrel9 = db->getRelation("dup");
    ...
    virtual bool work()
    {
      ...
      if (_fires) db->bumpFires("shape_selfloop.slog:38", "all:node", _fires);
