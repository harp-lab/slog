...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("483a3db6");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  r = db->getRelation("type_mismatch");
      std::vector<u16> ord8({1, 2, 3, 4, 0});
    
    r->addIndex<5>(ord8, false);
    for (u16 b = 0; b < 32; ++b)
      s->addTask(phase_write, new slog::WriteTask<5>(db, r, std::array<u16,5>{1, 2, 3, 4, 0}, false, b), true);
      std::vector<u16> ord9({0, 1, 2, 3, 4});
  ...
  r = db->getRelation("split_ans");
  if (r == 0) db->addRelation("split_ans", 3);
  else if (r->getArity() != 3)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("split_ans");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<3>(db, db->getRelation("split_ans"), std::array<u16,3>{0, 1, 2}, b));
  r = db->getRelation("split");
  if (r == 0) db->addStruct("split", 2);
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternStructTask<2>(db, db->getRelation("split"), std::array<u16,2>{1, 0}, b));
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<3>(db, db->getRelation("pairs"), std::array<u16,3>{0, 1, 2}, b));
  ...
  r = db->getRelation("demanded");
  if (r == 0) db->addRelation("demanded", 1);
  else if (r->getArity() != 1)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("demanded");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("demanded"), std::array<u16,1>{0}, b));
  ...
  // (crule (pre (let __tconst5kgx9 const4e07408562bedb8b60ce05c1) (let __tconst4lTW8 constd4735e3a265e16eee03f5971) (let __tconst5mBp7 const6b86b273ff34fce19d6b804e)) (once) (body) (head (emit num (0) __tconst5kgx9) (emit num (0) __tconst4lTW8) (emit num (0) __tconst5mBp7)) dem_multi.slog:8 #f)
  ...
  public:
    ...
    {
      head_rel[0] = db->getRelation("num");
      std::vector<u16> ord1({0});
      slog::Relation* readrel2 = db->getRelation("num");
      ...
      slog::Relation* readrel6 = db->getRelation("num");
    ...
    virtual bool work()
    {
      ...
      if (_fires) db->bumpFires("dem_multi.slog:8", "once", _fires);
  ...
  // (crule (pre (let __tconst5ZhW6 const4b227777d4dd1fc61c6f884f)) (once) (body) (head (mkstruct split (1 0) __1YBF10 __tconst5ZhW6)) dem_multi.slog:13 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("split");
    ...
    virtual bool work()
    {
      ...
      if (_fires) db->bumpFires("dem_multi.slog:13", "once", _fires);
