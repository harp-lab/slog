...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("d2061232");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("root"), std::array<u16,2>{1, 0}, b));
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<3>(db, db->getRelation("ev"), std::array<u16,3>{1, 0, 2}, b));
  ...
  // (crule (pre (let __tconst83EY65 const1f503613ae12cc0c5121fd74)) (probe _enum (1 0) 1 __tconst83EY65 __t92AV1) (body (join ev (1 0 2) 1 __t92AV1 ee w) (join root (1 0) 1 ee pname)) (head (emit res (0 1) pname w)) ex_let.slog:43 #f)
  class ReadTask2 : public slog::Task
  {
    ...
    slog::Index** evindex0;  slog::Index** rootindex1;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("res");
      std::vector<u16> ord3({0, 1});
      slog::Relation* readrel4 = db->getRelation("res");
      ...
      slog::Relation* readrel8 = db->getRelation("ev");
      evindex0 = readrel8->getIndex(ord7, false);
      std::vector<u16> ord9({1, 0});
      slog::Relation* readrel10 = db->getRelation("root");
      rootindex1 = readrel10->getIndex(ord9, false);
  
    }
    ReadTask2(slog::Database* _db, u16 _b) : db(_db), bucket(_b) { bind(_db); }
    virtual bool work()
    {
      u64 v_c0 = v_const1f503613ae12cc0c5121fd74;
      ...
      bool _done = slog::join_probe_sliced<2,1>(driver_index, std::array<u64,2>{v_c0, 0}, _sc, _rkey, _hr, [&](const std::array<u64,2>& m11) {
        u64 v_c1 = m11[1];
        if (buckethash(v_c1) != bucket) return;
        slog::join_probe<3,1>(evindex0, std::array<u64,3>{v_c1, 0, 0}, [&](const std::array<u64,3>& m12) {
          u64 v_c2 = m12[1]; u64 v_c3 = m12[2];
          slog::join_probe<2,1>(rootindex1, std::array<u64,2>{v_c2, 0}, [&](const std::array<u64,2>& m13) {
            u64 v_c4 = m13[1];
            ++_fires;
            slog::emit<2>(head_rel[0], head_index[0], newbatch[0], std::array<u64,2>{v_c4, v_c3}, std::array<u16,2>{0, 1});
          });
        });
      });
      ...
      if (_fires) db->bumpFires("ex_let.slog:43", "all:_enum", _fires);
