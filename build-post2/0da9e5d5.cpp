...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("0da9e5d5");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  // (crule (pre) (scan modulo_by_zero __erre24f166 __errf9cfX67 __errf8hx568) (body) (head (emit error (0) __erre24f166)) <internal>:1 #f)
  ...
  // (crule (pre (let __tconst08Ic20 const37171d544cfb48318189f58f) (let __tconst8uB37 const6b86b273ff34fce19d6b804e)) (scan step __t50tb6 __t27KQ5 n) (body (join _enum (0 1) 2 __t27KQ5 __tconst08Ic20) (let __t4StV8 (_0002b n __tconst8uB37))) (head (emit step_ans (0 1) __t50tb6 __t4StV8)) dem_enum.slog:8 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel6 = db->getRelation("step_ans");
      ...
      _enumindex3 = readrel8->getIndex(ord7, false);
  
    }
    ReadTask4(slog::Database* _db, u16 _b) : db(_db), bucket(_b) { bind(_db); }
    virtual bool work()
    {
      u64 v_c3 = v_const37171d544cfb48318189f58f;
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        ...
        u64 v_c6 = _t[1];
        u64 v_c7 = _t[2];
        slog::join_probe<2,2>(_enumindex3, std::array<u64,2>{v_c6, v_c3}, [&](const std::array<u64,2>& m9) {
          u64 v_c8 = _prim__0002b(db, v_c7, v_c4);
          if (v_c8 == slog_error) { slog::emit_pending_error(db, "dem_enum.slog:8"); return; }
          ++_fires;
      ...
      if (_fires) db->bumpFires("dem_enum.slog:8", "delta:step", _fires);
  ...
  // (crule (pre) (scan mpz_table_overflow __erre9UZM90 __errf83WN91 __errf1pqB92) (body) (head (emit error (0) __erre9UZM90)) <internal>:1 #f)
  ...
  // (crule (pre (let __tconst7F9226 const37171d544cfb48318189f58f) (let __tconst3dQZ28 constc44926e20290c13324a8ecdf) (let __tconst7kQu2 constef2d127de37b942baad06145)) (scan step_ans __t1d3q4 r) (body (exists _enum (1 0) 1 __tconst7F9226) (exists _enum (1 0) 1 __tconst3dQZ28) (join-old step (0 2 1) 2 (0 2 1) __t1d3q4 __tconst7kQu2 __t2eoj3) (join _enum (0 1) 2 __t2eoj3 __tconst7F9226) (join _enum (1 0) 1 __tconst3dQZ28 __t4OJ61)) (head (mkstruct step (1 2 0) __85En27 __t4OJ61 r)) dem_enum.slog:12 #f)
  class ReadTask19 : public slog::Task
  {
    ...
    slog::Index** _enumindex13;  slog::Index** _enumindex14;  slog::Index** stepindex15;  slog::Index** _enumindex16;  slog::Index** _enumindex17;  slog::Index** stepdelta18;
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      _enumindex13 = readrel21->getIndex(ord20, false);
      std::vector<u16> ord22({1, 0});
      slog::Relation* readrel23 = db->getRelation("_enum");
      _enumindex14 = readrel23->getIndex(ord22, false);
      std::vector<u16> ord24({0, 2, 1});
      slog::Relation* readrel25 = db->getRelation("step");
      stepindex15 = readrel25->getIndex(ord24, false);
      std::vector<u16> ord26({0, 2, 1});
      slog::Relation* readrel27 = db->getRelation("step");
      stepdelta18 = readrel27->getIndex(ord26, true);
      std::vector<u16> ord28({0, 1});
      slog::Relation* readrel29 = db->getRelation("_enum");
      _enumindex16 = readrel29->getIndex(ord28, false);
      std::vector<u16> ord30({1, 0});
      slog::Relation* readrel31 = db->getRelation("_enum");
      _enumindex17 = readrel31->getIndex(ord30, false);
  
    }
    ReadTask19(slog::Database* _db, u16 _b) : db(_db), bucket(_b) { bind(_db); }
    virtual bool work()
    {
      u64 v_c12 = v_const37171d544cfb48318189f58f;
      u64 v_c13 = v_constc44926e20290c13324a8ecdf;
      u64 v_c14 = v_constef2d127de37b942baad06145;
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c15 = _t[0];
        u64 v_c16 = _t[1];
        if (!slog::exists_probe<2,1>(_enumindex13, std::array<u64,2>{v_c12, 0})) return;
        if (!slog::exists_probe<2,1>(_enumindex14, std::array<u64,2>{v_c13, 0})) return;
        slog::join_probe_old<3,2>(stepindex15, stepdelta18, std::array<u64,3>{v_c15, v_c14, 0}, [&](const std::array<u64,3>& m32) {
          u64 v_c17 = m32[2];
          slog::join_probe<2,2>(_enumindex16, std::array<u64,2>{v_c17, v_c12}, [&](const std::array<u64,2>& m33) {
            slog::join_probe<2,1>(_enumindex17, std::array<u64,2>{v_c13, 0}, [&](const std::array<u64,2>& m34) {
              u64 v_c18 = m34[1];
              ++_fires;
              slog::emit_struct<3>(head_rel[0], newbatch[0], std::array<u64,2>{v_c18, v_c16}, std::array<u16,3>{1, 2, 0});
            });
          });
        });
      });
      ...
      if (_fires) db->bumpFires("dem_enum.slog:12", "delta:step_ans", _fires);
  ...
  // (crule (pre (let __tconst2ucM25 const37171d544cfb48318189f58f) (let __tconst3GQs9 constef2d127de37b942baad06145)) (probe _enum (1 0) 1 __tconst2ucM25 __t7O2710) (body) (head (mkstruct step (1 2 0) __5I6a24 __t7O2710 __tconst3GQs9)) dem_enum.slog:12 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      driver_index = readrel37->getIndex(ord36, true);
  
    }
    ReadTask35(slog::Database* _db, u16 _b) : db(_db), bucket(_b) { bind(_db); }
    virtual bool work()
    {
      u64 v_c19 = v_const37171d544cfb48318189f58f;
      ...
      bool _done = slog::join_probe_sliced<2,1>(driver_index, std::array<u64,2>{v_c19, 0}, _sc, _rkey, _hr, [&](const std::array<u64,2>& m38) {
        u64 v_c21 = m38[1];
        if (buckethash(v_c21) != bucket) return;
        ++_fires;
        slog::emit_struct<3>(head_rel[0], newbatch[0], std::array<u64,2>{v_c21, v_c20}, std::array<u16,3>{1, 2, 0});
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("dem_enum.slog:12", "all:_enum", _fires);
  ...
  // (crule (pre) (scan nan_result __erre9HUV73 __errf4Ptg74 __errf4nx075 __errf0Osw76) (body) (head (emit error (0) __erre9HUV73)) <internal>:1 #f)
  ...
  // (crule (pre) (scan div_by_zero __erre3XRE63 __errf97ur64 __errf3Jyz65) (body) (head (emit error (0) __erre3XRE63)) <internal>:1 #f)
  ...
  // (crule (pre) (scan toint_range __erre5aWi77 __errf3GjN78 __errf1msn79) (body) (head (emit error (0) __erre5aWi77)) <internal>:1 #f)
  ...
  // (crule (pre) (scan int_overflow __erre4rr569 __errf1hEs70 __errf7DT471 __errf9zRt72) (body) (head (emit error (0) __erre4rr569)) <internal>:1 #f)
  ...
  // (crule (pre) (scan mpz_overflow __erre2IH185 __errf6HQg86 __errf1Zxd87 __errf5ps488 __errf9cQO89) (body) (head (emit error (0) __erre2IH185)) <internal>:1 #f)
  ...
  // (crule (pre (let __tconst7F9226 const37171d544cfb48318189f58f) (let __tconst3dQZ28 constc44926e20290c13324a8ecdf) (let __tconst7kQu2 constef2d127de37b942baad06145)) (probe step (2 0 1) 1 __tconst7kQu2 __t1d3q4 __t2eoj3) (body (join _enum (0 1) 2 __t2eoj3 __tconst7F9226) (exists step_ans (0 1) 1 __t1d3q4) (join _enum (1 0) 1 __tconst3dQZ28 __t4OJ61) (join step_ans (0 1) 1 __t1d3q4 r)) (head (mkstruct step (1 2 0) __85En27 __t4OJ61 r)) dem_enum.slog:12 #f)
  class ReadTask58 : public slog::Task
  {
    ...
    bool has_resume = false;
    slog::Index** _enumindex54;  slog::Index** step_ansindex55;  slog::Index** _enumindex56;  slog::Index** step_ansindex57;
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel60 = db->getRelation("step");
      driver_index = readrel60->getIndex(ord59, true);
      std::vector<u16> ord61({0, 1});
      slog::Relation* readrel62 = db->getRelation("_enum");
      _enumindex54 = readrel62->getIndex(ord61, false);
      std::vector<u16> ord63({0, 1});
      slog::Relation* readrel64 = db->getRelation("step_ans");
      step_ansindex55 = readrel64->getIndex(ord63, false);
      std::vector<u16> ord65({1, 0});
      slog::Relation* readrel66 = db->getRelation("_enum");
      _enumindex56 = readrel66->getIndex(ord65, false);
      std::vector<u16> ord67({0, 1});
      slog::Relation* readrel68 = db->getRelation("step_ans");
      step_ansindex57 = readrel68->getIndex(ord67, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::join_probe_sliced<3,1>(driver_index, std::array<u64,3>{v_c14, 0, 0}, _sc, _rkey, _hr, [&](const std::array<u64,3>& m69) {
        u64 v_c15 = m69[1];
        u64 v_c17 = m69[2];
        if (buckethash(v_c15) != bucket) return;
        slog::join_probe<2,2>(_enumindex54, std::array<u64,2>{v_c17, v_c12}, [&](const std::array<u64,2>& m70) {
          if (!slog::exists_probe<2,1>(step_ansindex55, std::array<u64,2>{v_c15, 0})) return;
          slog::join_probe<2,1>(_enumindex56, std::array<u64,2>{v_c13, 0}, [&](const std::array<u64,2>& m71) {
            u64 v_c18 = m71[1];
            slog::join_probe<2,1>(step_ansindex57, std::array<u64,2>{v_c15, 0}, [&](const std::array<u64,2>& m72) {
              u64 v_c16 = m72[1];
              ++_fires;
              slog::emit_struct<3>(head_rel[0], newbatch[0], std::array<u64,2>{v_c18, v_c16}, std::array<u16,3>{1, 2, 0});
            });
      ...
      if (_fires) db->bumpFires("dem_enum.slog:12", "delta:step", _fires);
  ...
  // (crule (pre (let __tconst8vnA23 constc44926e20290c13324a8ecdf) (let __tconst5IDG13 const6b86b273ff34fce19d6b804e)) (scan step __t5y7s12 __t15bU11 n) (body (join _enum (0 1) 2 __t15bU11 __tconst8vnA23) (let __t22BQ14 (_0002d n __tconst5IDG13))) (head (emit step_ans (0 1) __t5y7s12 __t22BQ14)) dem_enum.slog:9 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel76 = db->getRelation("step_ans");
      head_index[0] = readrel76->getIndex(ord75, false);
      outer_rel = db->getRelation("step");
      std::vector<u16> ord77({0, 1});
      slog::Relation* readrel78 = db->getRelation("_enum");
      _enumindex73 = readrel78->getIndex(ord77, false);
  
    }
    ReadTask74(slog::Database* _db, u16 _b) : db(_db), bucket(_b) { bind(_db); }
    virtual bool work()
    {
      u64 v_c41 = v_constc44926e20290c13324a8ecdf;
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        ...
        u64 v_c44 = _t[1];
        u64 v_c7 = _t[2];
        slog::join_probe<2,2>(_enumindex73, std::array<u64,2>{v_c44, v_c41}, [&](const std::array<u64,2>& m79) {
          u64 v_c45 = _prim__0002d(db, v_c7, v_c42);
          if (v_c45 == slog_error) { slog::emit_pending_error(db, "dem_enum.slog:9"); return; }
          ++_fires;
      ...
      if (_fires) db->bumpFires("dem_enum.slog:9", "delta:step", _fires);
  ...
  // (crule (pre) (scan type_mismatch __erre6BLT80 __errf6c7S81 __errf4tNB82 __errf3odZ83 __errf0cm384) (body) (head (emit error (0) __erre6BLT80)) <internal>:1 #f)
