...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("d22d73cc");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  r = db->getRelation("ooo");
  if (r == 0) db->addRelation("ooo", 2);
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("ooo"), std::array<u16,2>{0, 1}, b));
  ...
  r = db->getRelation("chain");
  if (r == 0) db->addRelation("chain", 3);
  else if (r->getArity() != 3)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("chain");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<3>(db, db->getRelation("chain"), std::array<u16,3>{0, 1, 2}, b));
  r = db->getRelation("bounded");
  if (r == 0) db->addRelation("bounded", 2);
  else if (r->getArity() != 2)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("bounded");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("bounded"), std::array<u16,2>{0, 1}, b));
  ...
  // (crule (pre) (scan mpz_overflow __erre6K0x68 __errf4z2h69 __errf3q0s70 __errf3MMK71 __errf6c1672) (body) (head (emit error (0) __erre6K0x68)) <internal>:1 #f)
  ...
  // (crule (pre) (scan modulo_by_zero __erre3YZ449 __errf76dJ50 __errf5Z0z51) (body) (head (emit error (0) __erre3YZ449)) <internal>:1 #f)
  ...
  // (crule (pre) (scan toint_range __erre4QnS60 __errf6XUM61 __errf9QSx62) (body) (head (emit error (0) __erre4QnS60)) <internal>:1 #f)
  ...
  // (crule (pre (let __tconst8j3J1 const6b86b273ff34fce19d6b804e)) (scan n A) (body (let __t0RYW2 (_0002b A __tconst8j3J1)) (let __t024T3 (_0002a __t0RYW2 __t0RYW2))) (head (emit chain (0 1 2) A __t0RYW2 __t024T3)) prim2_lets.slog:16 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("chain");
      std::vector<u16> ord10({0, 1, 2});
      slog::Relation* readrel11 = db->getRelation("chain");
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c12 = _t[0];
        u64 v_c13 = _prim__0002b(db, v_c12, v_c11);
        if (v_c13 == slog_error) { slog::emit_pending_error(db, "prim2_lets.slog:16"); return; }
        u64 v_c14 = _prim__0002a(db, v_c13, v_c13);
        if (v_c14 == slog_error) { slog::emit_pending_error(db, "prim2_lets.slog:16"); return; }
        ++_fires;
        slog::emit<3>(head_rel[0], head_index[0], newbatch[0], std::array<u64,3>{v_c12, v_c13, v_c14}, std::array<u16,3>{0, 1, 2});
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("prim2_lets.slog:16", "all:n", _fires);
  ...
  // (crule (pre (let __tconst1WW014 constd4735e3a265e16eee03f5971)) (scan n A) (body (let __t607E15 (_0002b A __tconst1WW014)) (let __t8qF713 (_0002a __t607E15 __t607E15))) (head (emit ooo (0 1) A __t8qF713)) prim2_lets.slog:17 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("ooo");
      std::vector<u16> ord13({0, 1});
      slog::Relation* readrel14 = db->getRelation("ooo");
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c12 = _t[0];
        u64 v_c16 = _prim__0002b(db, v_c12, v_c15);
        if (v_c16 == slog_error) { slog::emit_pending_error(db, "prim2_lets.slog:17"); return; }
        u64 v_c17 = _prim__0002a(db, v_c16, v_c16);
        if (v_c17 == slog_error) { slog::emit_pending_error(db, "prim2_lets.slog:17"); return; }
        ++_fires;
        slog::emit<2>(head_rel[0], head_index[0], newbatch[0], std::array<u64,2>{v_c12, v_c17}, std::array<u16,2>{0, 1});
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("prim2_lets.slog:17", "all:n", _fires);
  ...
  // (crule (pre) (scan mpz_table_overflow __erre4yTe73 __errf0qdX74 __errf4kuy75) (body) (head (emit error (0) __erre4yTe73)) <internal>:1 #f)
  ...
  // (crule (pre) (scan div_by_zero __erre11vO46 __errf0CLP47 __errf9FN048) (body) (head (emit error (0) __erre11vO46)) <internal>:1 #f)
  ...
  // (crule (pre) (scan int_overflow __erre11X752 __errf0Ns153 __errf3UAq54 __errf9T6f55) (body) (head (emit error (0) __erre11X752)) <internal>:1 #f)
  ...
  // (crule (pre) (scan nan_result __erre9dKJ56 __errf3GS657 __errf757F58 __errf8YoI59) (body) (head (emit error (0) __erre9dKJ56)) <internal>:1 #f)
  ...
  // (crule (pre) (scan type_mismatch __erre4p3n63 __errf8Kap64 __errf5UmM65 __errf7uyQ66 __errf2aJy67) (body) (head (emit error (0) __erre4p3n63)) <internal>:1 #f)
  ...
  // (crule (pre (let __tconst8t7K4 const6b86b273ff34fce19d6b804e) (let __tconst8PlU7 const624b60c58c9d8bfb6ff1886c)) (scan n A) (body (let __t9Y5m5 (_0002b A __tconst8t7K4)) (let __t0gcY6 (_0002a __t9Y5m5 __t9Y5m5)) (cmp lt __t0gcY6 __tconst8PlU7)) (head (emit bounded (0 1) A __t0gcY6)) prim2_lets.slog:18 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("bounded");
      std::vector<u16> ord31({0, 1});
      slog::Relation* readrel32 = db->getRelation("bounded");
    ...
    virtual bool work()
    {
      ...
      u64 v_c38 = v_const624b60c58c9d8bfb6ff1886c;
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c12 = _t[0];
        u64 v_c39 = _prim__0002b(db, v_c12, v_c37);
        if (v_c39 == slog_error) { slog::emit_pending_error(db, "prim2_lets.slog:18"); return; }
        u64 v_c40 = _prim__0002a(db, v_c39, v_c39);
        if (v_c40 == slog_error) { slog::emit_pending_error(db, "prim2_lets.slog:18"); return; }
        u64 v_c41 = _prim_lt(db, v_c40, v_c38);
        if (v_c41 == slog_error) { slog::emit_pending_error(db, "prim2_lets.slog:18"); return; }
        if (!v_c41) return;
        ++_fires;
        slog::emit<2>(head_rel[0], head_index[0], newbatch[0], std::array<u64,2>{v_c12, v_c40}, std::array<u16,2>{0, 1});
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("prim2_lets.slog:18", "all:n", _fires);
  ...
  s->addDynamicRel("bounded");
  s->addDynamicRel("chain");
