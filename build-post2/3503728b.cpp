...
u64 v_const4819f5315cb49422b0b21be6;
...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("3503728b");
  if (s == nullptr) return;
  slog::Relation* r;
  v_const4819f5315cb49422b0b21be6 = float_encode(5.5);
  ...
  if (r == 0) db->addStruct("nan_result", 4);
    ...
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("nan_result");
      std::vector<u16> ord63({1, 2, 3, 0});
    
    r->addIndex<4>(ord63, false);
    for (u16 b = 0; b < 32; ++b)
      s->addTask(phase_write, new slog::WriteTask<4>(db, r, std::array<u16,4>{1, 2, 3, 0}, false, b), true);
      std::vector<u16> ord64({0, 1, 2, 3});
  ...
  // (crule (pre) (scan toint_range __erre4Wjw59 __errf6L7J60 __errf4OzM61) (body) (head (emit error (0) __erre4Wjw59)) <internal>:1 #f)
  ...
  // (crule (pre) (scan fp A B) (body (cmp lt A B)) (head (emit fltp (0 1) A B)) prim_cmp.slog:30 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("fltp");
      std::vector<u16> ord4({0, 1});
      slog::Relation* readrel5 = db->getRelation("fltp");
      head_index[0] = readrel5->getIndex(ord4, false);
      outer_rel = db->getRelation("fp");
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c3 = _t[0];
        u64 v_c4 = _t[1];
        u64 v_c5 = _prim_lt(db, v_c3, v_c4);
        if (v_c5 == slog_error) { slog::emit_pending_error(db, "prim_cmp.slog:30"); return; }
        if (!v_c5) return;
        ++_fires;
      ...
      if (_fires) db->bumpFires("prim_cmp.slog:30", "all:fp", _fires);
  ...
  // (crule (pre) (scan div_by_zero __erre3rbZ45 __errf4SEv46 __errf1a5j47) (body) (head (emit error (0) __erre3rbZ45)) <internal>:1 #f)
  ...
  // (crule (pre (let __tconst2m7w2 const4819f5315cb49422b0b21be6)) (scan p A B) (body (cmp lt A __tconst2m7w2)) (head (emit mixlt (0) A)) prim_cmp.slog:34 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("mixlt");
      std::vector<u16> ord11({0});
      slog::Relation* readrel12 = db->getRelation("mixlt");
    ...
    virtual bool work()
    {
      u64 v_c9 = v_const4819f5315cb49422b0b21be6;
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c3 = _t[0];
        u64 v_c4 = _t[1];
        u64 v_c10 = _prim_lt(db, v_c3, v_c9);
        if (v_c10 == slog_error) { slog::emit_pending_error(db, "prim_cmp.slog:34"); return; }
        if (!v_c10) return;
        ++_fires;
      ...
      if (_fires) db->bumpFires("prim_cmp.slog:34", "all:p", _fires);
  ...
  // (crule (pre) (scan p A B) (body (cmp gt A B)) (head (emit gtp (0 1) A B)) prim_cmp.slog:27 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("gtp");
      std::vector<u16> ord15({0, 1});
      slog::Relation* readrel16 = db->getRelation("gtp");
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c3 = _t[0];
        u64 v_c4 = _t[1];
        u64 v_c11 = _prim_gt(db, v_c3, v_c4);
        if (v_c11 == slog_error) { slog::emit_pending_error(db, "prim_cmp.slog:27"); return; }
        if (!v_c11) return;
        ++_fires;
        slog::emit<2>(head_rel[0], head_index[0], newbatch[0], std::array<u64,2>{v_c3, v_c4}, std::array<u16,2>{0, 1});
      });
      ...
      if (_fires) db->bumpFires("prim_cmp.slog:27", "all:p", _fires);
  ...
  // (crule (pre) (scan p A B) (body (cmp lt A B)) (head (emit ltp (0 1) A B)) prim_cmp.slog:25 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("ltp");
      std::vector<u16> ord19({0, 1});
      slog::Relation* readrel20 = db->getRelation("ltp");
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c3 = _t[0];
        u64 v_c4 = _t[1];
        u64 v_c12 = _prim_lt(db, v_c3, v_c4);
        if (v_c12 == slog_error) { slog::emit_pending_error(db, "prim_cmp.slog:25"); return; }
        if (!v_c12) return;
        ++_fires;
        slog::emit<2>(head_rel[0], head_index[0], newbatch[0], std::array<u64,2>{v_c3, v_c4}, std::array<u16,2>{0, 1});
      });
      ...
      if (_fires) db->bumpFires("prim_cmp.slog:25", "all:p", _fires);
  ...
  // (crule (pre) (scan modulo_by_zero __erre3n1D48 __errf1EDq49 __errf53MO50) (body) (head (emit error (0) __erre3n1D48)) <internal>:1 #f)
  ...
  // (crule (pre) (scan fp A B) (body (cmp ge A B)) (head (emit fgep (0 1) A B)) prim_cmp.slog:31 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("fgep");
      std::vector<u16> ord26({0, 1});
      slog::Relation* readrel27 = db->getRelation("fgep");
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c3 = _t[0];
        u64 v_c4 = _t[1];
        u64 v_c16 = _prim_ge(db, v_c3, v_c4);
        if (v_c16 == slog_error) { slog::emit_pending_error(db, "prim_cmp.slog:31"); return; }
        if (!v_c16) return;
        ++_fires;
        slog::emit<2>(head_rel[0], head_index[0], newbatch[0], std::array<u64,2>{v_c3, v_c4}, std::array<u16,2>{0, 1});
      });
      ...
      if (_fires) db->bumpFires("prim_cmp.slog:31", "all:fp", _fires);
  ...
  // (crule (pre) (scan int_overflow __erre0pDX51 __errf9o5I52 __errf7VPO53 __errf0tUA54) (body) (head (emit error (0) __erre0pDX51)) <internal>:1 #f)
  ...
  // (crule (pre) (scan type_mismatch __erre7foH62 __errf0ScG63 __errf2NnK64 __errf7Lgf65 __errf7Gdz66) (body) (head (emit error (0) __erre7foH62)) <internal>:1 #f)
  ...
  // (crule (pre) (scan p A B) (body (cmp ge A B)) (head (emit gep (0 1) A B)) prim_cmp.slog:28 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("gep");
      std::vector<u16> ord36({0, 1});
      slog::Relation* readrel37 = db->getRelation("gep");
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c3 = _t[0];
        u64 v_c4 = _t[1];
        u64 v_c26 = _prim_ge(db, v_c3, v_c4);
        if (v_c26 == slog_error) { slog::emit_pending_error(db, "prim_cmp.slog:28"); return; }
        if (!v_c26) return;
        ++_fires;
        slog::emit<2>(head_rel[0], head_index[0], newbatch[0], std::array<u64,2>{v_c3, v_c4}, std::array<u16,2>{0, 1});
      });
      ...
      if (_fires) db->bumpFires("prim_cmp.slog:28", "all:p", _fires);
  ...
  // (crule (pre (let __tconst7Bg31 conste7f6c011776e8db7cd330b54)) (scan p A B) (body (cmp lt A __tconst7Bg31)) (head (emit ltc (0) A)) prim_cmp.slog:33 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("ltc");
      std::vector<u16> ord40({0});
      slog::Relation* readrel41 = db->getRelation("ltc");
    ...
    virtual bool work()
    {
      u64 v_c27 = v_conste7f6c011776e8db7cd330b54;
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c3 = _t[0];
        u64 v_c4 = _t[1];
        u64 v_c28 = _prim_lt(db, v_c3, v_c27);
        if (v_c28 == slog_error) { slog::emit_pending_error(db, "prim_cmp.slog:33"); return; }
        if (!v_c28) return;
        ++_fires;
        slog::emit<1>(head_rel[0], head_index[0], newbatch[0], std::array<u64,1>{v_c3}, std::array<u16,1>{0});
      });
      ...
      if (_fires) db->bumpFires("prim_cmp.slog:33", "all:p", _fires);
  ...
  // (crule (pre) (scan mpz_overflow __erre0pnm67 __errf7s8H68 __errf8f1G69 __errf32tj70 __errf5RGl71) (body) (head (emit error (0) __erre0pnm67)) <internal>:1 #f)
  ...
  // (crule (pre) (scan nan_result __erre7wh955 __errf3trI56 __errf8Y5c57 __errf11kS58) (body) (head (emit error (0) __erre7wh955)) <internal>:1 #f)
  ...
  // (crule (pre) (scan mpz_table_overflow __erre4svN72 __errf0xGh73 __errf16J474) (body) (head (emit error (0) __erre4svN72)) <internal>:1 #f)
  ...
  // (crule (pre) (scan p A B) (body (cmp le A B)) (head (emit lep (0 1) A B)) prim_cmp.slog:26 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("lep");
      std::vector<u16> ord53({0, 1});
      slog::Relation* readrel54 = db->getRelation("lep");
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c3 = _t[0];
        u64 v_c4 = _t[1];
        u64 v_c41 = _prim_le(db, v_c3, v_c4);
        if (v_c41 == slog_error) { slog::emit_pending_error(db, "prim_cmp.slog:26"); return; }
        if (!v_c41) return;
        ++_fires;
        slog::emit<2>(head_rel[0], head_index[0], newbatch[0], std::array<u64,2>{v_c3, v_c4}, std::array<u16,2>{0, 1});
      });
      ...
      if (_fires) db->bumpFires("prim_cmp.slog:26", "all:p", _fires);
  ...
  s->addDynamicRel("fgep");
  s->addDynamicRel("fltp");
  s->addDynamicRel("gep");
  s->addDynamicRel("gtp");
