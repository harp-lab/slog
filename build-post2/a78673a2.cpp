...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("a78673a2");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  // (crule (pre) (scan reach Y X) (body (join reach (0 1) 2 X Y)) (head (emit samecycle (0 1) X Y)) strat_selfdown.slog:19 #f)
  class ReadTask1 : public slog::Task
  {
    ...
    slog::Index** reachindex0;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("samecycle");
      std::vector<u16> ord2({0, 1});
      slog::Relation* readrel3 = db->getRelation("samecycle");
      head_index[0] = readrel3->getIndex(ord2, false);
      outer_rel = db->getRelation("reach");
      std::vector<u16> ord4({0, 1});
      slog::Relation* readrel5 = db->getRelation("reach");
      reachindex0 = readrel5->getIndex(ord4, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        u64 v_c1 = _t[1];
        slog::join_probe<2,2>(reachindex0, std::array<u64,2>{v_c1, v_c0}, [&](const std::array<u64,2>& m6) {
          ++_fires;
      ...
      if (_fires) db->bumpFires("strat_selfdown.slog:19", "all:reach", _fires);
  ...
  // (crule (pre (let __tconst0vQW1 const6b86b273ff34fce19d6b804e)) (probe reach (0 1) 1 __tconst0vQW1 Y) (body) (head (emit from1 (0) Y)) strat_selfdown.slog:20 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("from1");
      std::vector<u16> ord8({0});
      slog::Relation* readrel9 = db->getRelation("from1");
      head_index[0] = readrel9->getIndex(ord8, false);
      std::vector<u16> ord10({0, 1});
      slog::Relation* readrel11 = db->getRelation("reach");
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::join_probe_sliced<2,1>(driver_index, std::array<u64,2>{v_c2, 0}, _sc, _rkey, _hr, [&](const std::array<u64,2>& m12) {
        u64 v_c0 = m12[1];
        if (buckethash(v_c0) != bucket) return;
        ++_fires;
      ...
      if (_fires) db->bumpFires("strat_selfdown.slog:20", "all:reach", _fires);
  ...
  s->addReadRel("reach");
  s->addDynamicRel("from1");
  s->addDynamicRel("samecycle");
  d->push(s);
  d->continueRun();
}
