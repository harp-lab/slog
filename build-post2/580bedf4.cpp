...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("580bedf4");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  // (crule (pre (let __tconst63ET1 const4a44dc15364204a80fe80e90) (let __tconst27Kd3 constf5ca38f748a1d6eaf726b8a4) (let __tconst1kTk5 const624b60c58c9d8bfb6ff1886c) (let __tconst7OTb7 const6b86b273ff34fce19d6b804e) (let __tconst7WPt8 constd4735e3a265e16eee03f5971) (let __tconst6Dtt9 const4e07408562bedb8b60ce05c1)) (once) (body) (head (emit g (0 1) __tconst7OTb7 __tconst63ET1) (emit g (0 1) __tconst7OTb7 __tconst27Kd3) (emit g (0 1) __tconst6Dtt9 __tconst1kTk5) (emit h (0) __tconst7OTb7) (emit h (0) __tconst7WPt8) (emit h (0) __tconst6Dtt9)) neg_wild.slog:12 #f)
  ...
  public:
    ...
    {
      head_rel[0] = db->getRelation("g");
      std::vector<u16> ord1({0, 1});
      slog::Relation* readrel2 = db->getRelation("g");
      head_index[0] = readrel2->getIndex(ord1, false);
      head_rel[1] = db->getRelation("g");
      std::vector<u16> ord3({0, 1});
      slog::Relation* readrel4 = db->getRelation("g");
      head_index[1] = readrel4->getIndex(ord3, false);
      head_rel[2] = db->getRelation("g");
      std::vector<u16> ord5({0, 1});
      slog::Relation* readrel6 = db->getRelation("g");
      head_index[2] = readrel6->getIndex(ord5, false);
      head_rel[3] = db->getRelation("h");
      std::vector<u16> ord7({0});
      slog::Relation* readrel8 = db->getRelation("h");
      head_index[3] = readrel8->getIndex(ord7, false);
      head_rel[4] = db->getRelation("h");
      std::vector<u16> ord9({0});
      slog::Relation* readrel10 = db->getRelation("h");
      head_index[4] = readrel10->getIndex(ord9, false);
      head_rel[5] = db->getRelation("h");
      std::vector<u16> ord11({0});
      slog::Relation* readrel12 = db->getRelation("h");
    ...
    virtual bool work()
    {
      ...
      u64 v_c1 = v_constf5ca38f748a1d6eaf726b8a4;
      u64 v_c2 = v_const624b60c58c9d8bfb6ff1886c;
      ...
      ++_fires;
      slog::emit<2>(head_rel[0], head_index[0], newbatch[0], std::array<u64,2>{v_c3, v_c0}, std::array<u16,2>{0, 1});
      slog::emit<2>(head_rel[1], head_index[1], newbatch[1], std::array<u64,2>{v_c3, v_c1}, std::array<u16,2>{0, 1});
      slog::emit<2>(head_rel[2], head_index[2], newbatch[2], std::array<u64,2>{v_c5, v_c2}, std::array<u16,2>{0, 1});
      ...
      if (_fires) db->bumpFires("neg_wild.slog:12", "once", _fires);
