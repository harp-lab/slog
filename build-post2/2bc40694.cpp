...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("2bc40694");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  // (crule (pre) (scan s2 P) (body (join s1 (0) 1 P)) (head (emit same (0) P)) sdeep_dedup.slog:25 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel3 = db->getRelation("same");
      head_index[0] = readrel3->getIndex(ord2, false);
      outer_rel = db->getRelation("s2");
      std::vector<u16> ord4({0});
      slog::Relation* readrel5 = db->getRelation("s1");
      s1index0 = readrel5->getIndex(ord4, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        slog::join_probe<1,1>(s1index0, std::array<u64,1>{v_c0}, [&](const std::array<u64,1>& m6) {
          ++_fires;
      ...
      if (_fires) db->bumpFires("sdeep_dedup.slog:25", "all:s2", _fires);
  ...
  // (crule (pre) (scan s2 Q) (body (join s1 (0) 0 P) (neq P Q)) (head (emit diff (0 1) P Q)) sdeep_dedup.slog:28 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("diff");
      std::vector<u16> ord9({0, 1});
      slog::Relation* readrel10 = db->getRelation("diff");
      head_index[0] = readrel10->getIndex(ord9, false);
      outer_rel = db->getRelation("s2");
      std::vector<u16> ord11({0});
      slog::Relation* readrel12 = db->getRelation("s1");
      s1index7 = readrel12->getIndex(ord11, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c1 = _t[0];
        slog::join_all<1>(s1index7, [&](const std::array<u64,1>& m13) {
          u64 v_c0 = m13[0];
          if (v_c0 == v_c1) return;
          ++_fires;
      ...
      if (_fires) db->bumpFires("sdeep_dedup.slog:28", "all:s2", _fires);
