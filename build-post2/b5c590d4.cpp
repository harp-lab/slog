...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("b5c590d4");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("sz"), std::array<u16,2>{0, 1}, b));
  ...
  r->setLattice(LAT_EXTERN, false, 0, false, 0, "set-int", db->collections());
    ...
    r->addMapIndex<2>(ord19);
    for (u16 b = 0; b < 32; ++b)
      s->addTask(phase_write, new slog::MapWriteTask<2>(db, r, std::array<u16,2>{0, 1}, b), true);
  

  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::LatticeInternTask<2>(db, db->getRelation("reach"), std::array<u16,2>{0, 1}, b));
  ...
  // (crule (pre (let __tconst7PhL17 const4a44dc15364204a80fe80e90) (let __tconst5zj622 constd4735e3a265e16eee03f5971) (let __tconst9sTr23 const7902699be42c8a8e46fbbb45) (let __tconst0OiH26 const6b86b273ff34fce19d6b804e) (let __tconst8cxg27 const4b227777d4dd1fc61c6f884f) (let __tconst9uPX28 const4e07408562bedb8b60ce05c1)) (once) (body) (head (emit edge (0 1 2) __tconst0OiH26 __tconst5zj622 __tconst7PhL17) (emit edge (0 1 2) __tconst5zj622 __tconst9uPX28 __tconst8cxg27) (emit edge (0 1 2) __tconst0OiH26 __tconst9uPX28 __tconst9sTr23) (emit edge (0 1 2) __tconst9uPX28 __tconst8cxg27 __tconst0OiH26)) lat_set.slog:17 #f)
  ...
  public:
    ...
    virtual bool work()
    {
      ...
      ++_fires;
      slog::emit<3>(head_rel[0], head_index[0], newbatch[0], std::array<u64,3>{v_c3, v_c1, v_c0}, std::array<u16,3>{0, 1, 2});
      slog::emit<3>(head_rel[1], head_index[1], newbatch[1], std::array<u64,3>{v_c1, v_c5, v_c4}, std::array<u16,3>{0, 1, 2});
      slog::emit<3>(head_rel[2], head_index[2], newbatch[2], std::array<u64,3>{v_c3, v_c5, v_c2}, std::array<u16,3>{0, 1, 2});
      slog::emit<3>(head_rel[3], head_index[3], newbatch[3], std::array<u64,3>{v_c5, v_c4, v_c3}, std::array<u16,3>{0, 1, 2});
      ...
      if (_fires) db->bumpFires("lat_set.slog:17", "once", _fires);
  ...
  // (crule (pre (let __tconst7Yj234 const6b86b273ff34fce19d6b804e) (let __tconst41Mf33 constd4735e3a265e16eee03f5971) (let __tconst5oHm32 constef2d127de37b942baad06145)) (once) (body) (head (emit edge (0 1 2) __tconst7Yj234 __tconst41Mf33 __tconst5oHm32)) lat_set.slog:18 #f)
  ...
  public:
    ...
    virtual bool work()
    {
      ...
      ++_fires;
      slog::emit<3>(head_rel[0], head_index[0], newbatch[0], std::array<u64,3>{v_c6, v_c7, v_c8}, std::array<u16,3>{0, 1, 2});
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("lat_set.slog:18", "once", _fires);
