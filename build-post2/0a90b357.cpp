...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("0a90b357");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("sg"), std::array<u16,2>{0, 1}, b));
  ...
  // (crule (pre (let __tconst8Fg94 const6b86b273ff34fce19d6b804e) (let __tconst2QVp7 constef2d127de37b942baad06145) (let __tconst7Gto8 constd4735e3a265e16eee03f5971) (let __tconst1NGL10 const4e07408562bedb8b60ce05c1) (let __tconst4q8E11 const7902699be42c8a8e46fbbb45) (let __tconst2BPr12 const4b227777d4dd1fc61c6f884f) (let __tconst4Isb13 const2c624232cdd221771294dfbb) (let __tconst4jVb14 conste7f6c011776e8db7cd330b54)) (once) (body) (head (emit edge (0 1) __tconst8Fg94 __tconst7Gto8) (emit edge (0 1) __tconst8Fg94 __tconst1NGL10) (emit edge (0 1) __tconst7Gto8 __tconst2BPr12) (emit edge (0 1) __tconst7Gto8 __tconst2QVp7) (emit edge (0 1) __tconst1NGL10 __tconst4jVb14) (emit edge (0 1) __tconst2BPr12 __tconst4q8E11) (emit edge (0 1) __tconst4jVb14 __tconst4Isb13)) sj_sg.slog:23 #f)
  ...
  public:
    ...
    virtual bool work()
    {
      ...
      u64 v_c7 = v_conste7f6c011776e8db7cd330b54;
      ...
      slog::emit<2>(head_rel[1], head_index[1], newbatch[1], std::array<u64,2>{v_c0, v_c3}, std::array<u16,2>{0, 1});
      slog::emit<2>(head_rel[2], head_index[2], newbatch[2], std::array<u64,2>{v_c2, v_c5}, std::array<u16,2>{0, 1});
      slog::emit<2>(head_rel[3], head_index[3], newbatch[3], std::array<u64,2>{v_c2, v_c1}, std::array<u16,2>{0, 1});
      slog::emit<2>(head_rel[4], head_index[4], newbatch[4], std::array<u64,2>{v_c3, v_c7}, std::array<u16,2>{0, 1});
      slog::emit<2>(head_rel[5], head_index[5], newbatch[5], std::array<u64,2>{v_c5, v_c4}, std::array<u16,2>{0, 1});
      slog::emit<2>(head_rel[6], head_index[6], newbatch[6], std::array<u64,2>{v_c7, v_c6}, std::array<u16,2>{0, 1});
      ...
      if (_fires) db->bumpFires("sj_sg.slog:23", "once", _fires);
