...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("2191609d");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  // (crule (pre) (scan holds __t96kb7) (body (join box (0 1) 1 __t96kb7 X)) (head (emit unbox (0) X)) sdeep_strata.slog:22 #f)
  class ReadTask1 : public slog::Task
  {
    ...
    slog::Index** boxindex0;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("unbox");
      std::vector<u16> ord2({0});
      slog::Relation* readrel3 = db->getRelation("unbox");
      head_index[0] = readrel3->getIndex(ord2, false);
      outer_rel = db->getRelation("holds");
      std::vector<u16> ord4({0, 1});
      slog::Relation* readrel5 = db->getRelation("box");
      boxindex0 = readrel5->getIndex(ord4, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        slog::join_probe<2,1>(boxindex0, std::array<u64,2>{v_c0, 0}, [&](const std::array<u64,2>& m6) {
          u64 v_c1 = m6[1];
          ++_fires;
      ...
      if (_fires) db->bumpFires("sdeep_strata.slog:22", "all:holds", _fires);
