...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("db545604");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<3>(db, db->getRelation("gedge"), std::array<u16,3>{0, 1, 2}, b));
  ...
  // (crule (pre (let __tconst8CEk19 const6b86b273ff34fce19d6b804e)) (once) (body) (head (emit src (0) __tconst8CEk19)) lat_rhas.slog:17 #f)
  ...
  public:
    ...
    {
      head_rel[0] = db->getRelation("src");
      std::vector<u16> ord1({0});
      slog::Relation* readrel2 = db->getRelation("src");
    ...
    virtual bool work()
    {
      ...
      if (_fires) db->bumpFires("lat_rhas.slog:17", "once", _fires);
  ...
  // (crule (pre (let __tconst6Shq9 const6b86b273ff34fce19d6b804e) (let __tconst3xvW8 conste7f6c011776e8db7cd330b54) (let __tconst4V2y7 const19581e27de7ced00ff1ce50b)) (once) (body) (head (emit gedge (0 1 2) __tconst6Shq9 __tconst3xvW8 __tconst4V2y7)) lat_rhas.slog:21 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("gedge");
      std::vector<u16> ord4({0, 1, 2});
      slog::Relation* readrel5 = db->getRelation("gedge");
    ...
    virtual bool work()
    {
      ...
      u64 v_c3 = v_const19581e27de7ced00ff1ce50b;
      ...
      ++_fires;
      slog::emit<3>(head_rel[0], head_index[0], newbatch[0], std::array<u64,3>{v_c1, v_c2, v_c3}, std::array<u16,3>{0, 1, 2});
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("lat_rhas.slog:21", "once", _fires);
  ...
  // (crule (pre (let __tconst63ZJ16 const4e07408562bedb8b60ce05c1) (let __tconst4LQW15 const4b227777d4dd1fc61c6f884f) (let __tconst9XzS14 constd4735e3a265e16eee03f5971)) (once) (body) (head (emit gedge (0 1 2) __tconst63ZJ16 __tconst4LQW15 __tconst9XzS14)) lat_rhas.slog:19 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel8 = db->getRelation("gedge");
    ...
    virtual bool work()
    {
      ...
      u64 v_c5 = v_const4b227777d4dd1fc61c6f884f;
      ...
      ++_fires;
      slog::emit<3>(head_rel[0], head_index[0], newbatch[0], std::array<u64,3>{v_c4, v_c5, v_c6}, std::array<u16,3>{0, 1, 2});
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("lat_rhas.slog:19", "once", _fires);
  ...
  // (crule (pre (let __tconst0DAv11 const6b86b273ff34fce19d6b804e) (let __tconst7ekA12 const4e07408562bedb8b60ce05c1) (let __tconst6WMv13 constd4735e3a265e16eee03f5971)) (once) (body) (head (emit edge (0 1) __tconst0DAv11 __tconst6WMv13) (emit edge (0 1) __tconst6WMv13 __tconst7ekA12)) lat_rhas.slog:18 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel13 = db->getRelation("edge");
    ...
    virtual bool work()
    {
      ...
      ++_fires;
      slog::emit<2>(head_rel[0], head_index[0], newbatch[0], std::array<u64,2>{v_c7, v_c9}, std::array<u16,2>{0, 1});
      slog::emit<2>(head_rel[1], head_index[1], newbatch[1], std::array<u64,2>{v_c9, v_c8}, std::array<u16,2>{0, 1});
  
        head_rel[0]->sendBatch(newbatch[0]);
        head_rel[1]->sendBatch(newbatch[1]);
  
      if (_fires) db->bumpFires("lat_rhas.slog:18", "once", _fires);
  ...
  // (crule (pre (let __tconst1pRC6 const6b86b273ff34fce19d6b804e) (let __tconst6HMF5 constef2d127de37b942baad06145) (let __tconst22JG4 const4b227777d4dd1fc61c6f884f)) (once) (body) (head (emit gedge (0 1 2) __tconst1pRC6 __tconst6HMF5 __tconst22JG4)) lat_rhas.slog:20 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel16 = db->getRelation("gedge");
    ...
    virtual bool work()
    {
      ...
      ++_fires;
      slog::emit<3>(head_rel[0], head_index[0], newbatch[0], std::array<u64,3>{v_c10, v_c11, v_c12}, std::array<u16,3>{0, 1, 2});
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("lat_rhas.slog:20", "once", _fires);
