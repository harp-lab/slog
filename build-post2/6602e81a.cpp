...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("6602e81a");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  // (crule (pre) (scan modulo_by_zero __erre4WYR71 __errf4X5Z72 __errf0REM73) (body) (head (emit error (0) __erre4WYR71)) <internal>:1 #f)
  ...
  // (crule (pre (let __tconst4Rnr12 constd4735e3a265e16eee03f5971)) (probe seed (0 1) 1 __tconst4Rnr12 V) (body) (head (emit-lat peak __tconst4Rnr12 V) (emit-lat low __tconst4Rnr12 V)) int_lat.slog:25 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("peak");
      head_rel[1] = db->getRelation("low");
      std::vector<u16> ord4({0, 1});
      slog::Relation* readrel5 = db->getRelation("seed");
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::join_probe_sliced<2,1>(driver_index, std::array<u64,2>{v_c3, 0}, _sc, _rkey, _hr, [&](const std::array<u64,2>& m6) {
        u64 v_c4 = m6[1];
        if (buckethash(v_c4) != bucket) return;
        ++_fires;
        slog::emit_temp<2>(head_rel[0], newbatch[0], std::array<u64,2>{v_c3, v_c4});
        slog::emit_temp<2>(head_rel[1], newbatch[1], std::array<u64,2>{v_c3, v_c4});
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
        head_rel[1]->sendBatch(newbatch[1]);
  
      if (_fires) db->bumpFires("int_lat.slog:25", "all:seed", _fires);
  ...
  // (crule (pre (let __tconst2KDR35 constd59eced1ded07f84c145592f) (let __tconst0oM736 const6b86b273ff34fce19d6b804e) (let __t7YHP37 (shl __tconst0oM736 __tconst2KDR35))) (probe seed (0 1) 1 __tconst0oM736 V) (body) (head (emit-lat peak __tconst0oM736 __t7YHP37) (emit-lat low __tconst0oM736 __t7YHP37)) int_lat.slog:24 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel9 = db->getRelation("seed");
    ...
    virtual bool work()
    {
      u64 v_c5 = v_constd59eced1ded07f84c145592f;
      u64 v_c6 = v_const6b86b273ff34fce19d6b804e;
      u64 v_c7 = _prim_shl(db, v_c6, v_c5);
      if (v_c7 == slog_error) { slog::emit_pending_error(db, "int_lat.slog:24"); return true; }
      ...
      slog::SliceCtx _sc{_slice_deadline, &db->runStopFlag()};
      std::array<u64,2> _rkey = resume_key; bool _hr = has_resume;
      bool _done = slog::join_probe_sliced<2,1>(driver_index, std::array<u64,2>{v_c6, 0}, _sc, _rkey, _hr, [&](const std::array<u64,2>& m10) {
        u64 v_c4 = m10[1];
        if (buckethash(v_c4) != bucket) return;
        ++_fires;
        slog::emit_temp<2>(head_rel[0], newbatch[0], std::array<u64,2>{v_c6, v_c7});
        slog::emit_temp<2>(head_rel[1], newbatch[1], std::array<u64,2>{v_c6, v_c7});
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
        head_rel[1]->sendBatch(newbatch[1]);
  
      if (_fires) db->bumpFires("int_lat.slog:24", "all:seed", _fires);
  ...
  // (crule (pre) (scan int_overflow __erre4Uq474 __errf0Vty75 __errf5jPp76 __errf8I6e77) (body) (head (emit error (0) __erre4Uq474)) <internal>:1 #f)
  ...
  // (crule (pre) (scan mpz_table_overflow __erre90dr95 __errf8Xck96 __errf88rL97) (body) (head (emit error (0) __erre90dr95)) <internal>:1 #f)
  ...
  // (crule (pre (let __tconst1AOx3 const6b86b273ff34fce19d6b804e)) (probe seed (0 1) 1 __tconst1AOx3 V) (body) (head (emit-lat peak __tconst1AOx3 V) (emit-lat low __tconst1AOx3 V)) int_lat.slog:23 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel19 = db->getRelation("seed");
    ...
    virtual bool work()
    {
      ...
      slog::SliceCtx _sc{_slice_deadline, &db->runStopFlag()};
      std::array<u64,2> _rkey = resume_key; bool _hr = has_resume;
      bool _done = slog::join_probe_sliced<2,1>(driver_index, std::array<u64,2>{v_c15, 0}, _sc, _rkey, _hr, [&](const std::array<u64,2>& m20) {
        u64 v_c4 = m20[1];
        if (buckethash(v_c4) != bucket) return;
        ++_fires;
        slog::emit_temp<2>(head_rel[0], newbatch[0], std::array<u64,2>{v_c15, v_c4});
        slog::emit_temp<2>(head_rel[1], newbatch[1], std::array<u64,2>{v_c15, v_c4});
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
        head_rel[1]->sendBatch(newbatch[1]);
  
      if (_fires) db->bumpFires("int_lat.slog:23", "all:seed", _fires);
  ...
  // (crule (pre (let __tconst2Av915 const4e07408562bedb8b60ce05c1) (let __tconst03Qq16 const6f4b6612125fb3a0daecd279) (let __tconst4gXk17 const4a44dc15364204a80fe80e90) (let __t63t418 (pow __tconst4gXk17 __tconst03Qq16))) (probe seed (0 1) 1 __tconst2Av915 V) (body) (head (emit-lat peak __tconst2Av915 __t63t418) (emit-lat low __tconst2Av915 __t63t418)) int_lat.slog:28 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel23 = db->getRelation("seed");
    ...
    virtual bool work()
    {
      ...
      u64 v_c17 = v_const6f4b6612125fb3a0daecd279;
      u64 v_c18 = v_const4a44dc15364204a80fe80e90;
      u64 v_c19 = _prim_pow(db, v_c18, v_c17);
      if (v_c19 == slog_error) { slog::emit_pending_error(db, "int_lat.slog:28"); return true; }
      ...
      slog::SliceCtx _sc{_slice_deadline, &db->runStopFlag()};
      std::array<u64,2> _rkey = resume_key; bool _hr = has_resume;
      bool _done = slog::join_probe_sliced<2,1>(driver_index, std::array<u64,2>{v_c16, 0}, _sc, _rkey, _hr, [&](const std::array<u64,2>& m24) {
        u64 v_c4 = m24[1];
        if (buckethash(v_c4) != bucket) return;
        ++_fires;
        slog::emit_temp<2>(head_rel[0], newbatch[0], std::array<u64,2>{v_c16, v_c19});
        slog::emit_temp<2>(head_rel[1], newbatch[1], std::array<u64,2>{v_c16, v_c19});
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
        head_rel[1]->sendBatch(newbatch[1]);
  
      if (_fires) db->bumpFires("int_lat.slog:28", "all:seed", _fires);
  ...
  // (crule (pre (let __tconst8jeO28 const4e07408562bedb8b60ce05c1) (let __tconst4T7l29 constf5ca38f748a1d6eaf726b8a4) (let __tconst3uK630 const4a44dc15364204a80fe80e90) (let __t3rcO31 (pow __tconst3uK630 __tconst4T7l29))) (probe seed (0 1) 1 __tconst8jeO28 V) (body) (head (emit-lat peak __tconst8jeO28 __t3rcO31) (emit-lat low __tconst8jeO28 __t3rcO31)) int_lat.slog:27 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel27 = db->getRelation("seed");
      driver_index = readrel27->getIndex(ord26, true);
    ...
    virtual bool work()
    {
      ...
      u64 v_c21 = v_constf5ca38f748a1d6eaf726b8a4;
      u64 v_c22 = v_const4a44dc15364204a80fe80e90;
      u64 v_c23 = _prim_pow(db, v_c22, v_c21);
      if (v_c23 == slog_error) { slog::emit_pending_error(db, "int_lat.slog:27"); return true; }
      ...
      slog::SliceCtx _sc{_slice_deadline, &db->runStopFlag()};
      std::array<u64,2> _rkey = resume_key; bool _hr = has_resume;
      bool _done = slog::join_probe_sliced<2,1>(driver_index, std::array<u64,2>{v_c20, 0}, _sc, _rkey, _hr, [&](const std::array<u64,2>& m28) {
        u64 v_c4 = m28[1];
        if (buckethash(v_c4) != bucket) return;
        ++_fires;
        slog::emit_temp<2>(head_rel[0], newbatch[0], std::array<u64,2>{v_c20, v_c23});
        slog::emit_temp<2>(head_rel[1], newbatch[1], std::array<u64,2>{v_c20, v_c23});
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
        head_rel[1]->sendBatch(newbatch[1]);
  
      if (_fires) db->bumpFires("int_lat.slog:27", "all:seed", _fires);
  ...
  // (crule (pre (let __tconst9xD521 constd4735e3a265e16eee03f5971) (let __tconst2V4k22 constd59eced1ded07f84c145592f) (let __tconst4QcT23 const6b86b273ff34fce19d6b804e) (let __t5KnR24 (shl __tconst4QcT23 __tconst2V4k22)) (let __t1IFT25 (neg __t5KnR24))) (probe seed (0 1) 1 __tconst9xD521 V) (body) (head (emit-lat peak __tconst9xD521 __t1IFT25) (emit-lat low __tconst9xD521 __t1IFT25)) int_lat.slog:26 #f)
  ...
  public:
    ...
    virtual bool work()
    {
      ...
      u64 v_c25 = v_constd59eced1ded07f84c145592f;
      u64 v_c26 = v_const6b86b273ff34fce19d6b804e;
      u64 v_c27 = _prim_shl(db, v_c26, v_c25);
      if (v_c27 == slog_error) { slog::emit_pending_error(db, "int_lat.slog:26"); return true; }
      u64 v_c28 = _prim_neg(db, v_c27);
      if (v_c28 == slog_error) { slog::emit_pending_error(db, "int_lat.slog:26"); return true; }
      ...
      slog::SliceCtx _sc{_slice_deadline, &db->runStopFlag()};
      std::array<u64,2> _rkey = resume_key; bool _hr = has_resume;
      bool _done = slog::join_probe_sliced<2,1>(driver_index, std::array<u64,2>{v_c24, 0}, _sc, _rkey, _hr, [&](const std::array<u64,2>& m32) {
        u64 v_c4 = m32[1];
        if (buckethash(v_c4) != bucket) return;
        ++_fires;
        slog::emit_temp<2>(head_rel[0], newbatch[0], std::array<u64,2>{v_c24, v_c28});
        slog::emit_temp<2>(head_rel[1], newbatch[1], std::array<u64,2>{v_c24, v_c28});
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
        head_rel[1]->sendBatch(newbatch[1]);
  
      if (_fires) db->bumpFires("int_lat.slog:26", "all:seed", _fires);
  ...
  // (crule (pre) (scan toint_range __erre2iD382 __errf2KW783 __errf1Yso84) (body) (head (emit error (0) __erre2iD382)) <internal>:1 #f)
  ...
  // (crule (pre) (scan mpz_overflow __erre0g2990 __errf0VXK91 __errf86yS92 __errf1RB793 __errf46eu94) (body) (head (emit error (0) __erre0g2990)) <internal>:1 #f)
  ...
  // (crule (pre) (scan type_mismatch __erre3Nan85 __errf14Ow86 __errf4r3D87 __errf2fwc88 __errf6i1P89) (body) (head (emit error (0) __erre3Nan85)) <internal>:1 #f)
  ...
  // (crule (pre) (scan div_by_zero __erre2Ti868 __errf5wti69 __errf8vQr70) (body) (head (emit error (0) __erre2Ti868)) <internal>:1 #f)
  ...
  // (crule (pre) (scan nan_result __erre5qED78 __errf6VSP79 __errf3fMG80 __errf7Gdv81) (body) (head (emit error (0) __erre5qED78)) <internal>:1 #f)
