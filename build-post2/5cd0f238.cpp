...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("5cd0f238");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("edge"), std::array<u16,2>{1, 0}, b));
  ...
  // (crule (pre) (scan mpz_overflow __erre6e6A284 __errf4kj1285 __errf3GPn286 __errf5rsS287 __errf7QLK288) (body) (head (emit error (0) __erre6e6A284)) <internal>:1 #f)
  ...
  // (crule (pre) (scan backedge a h) (body) (head (emit loop (0 1) h a) (emit loop (0 1) h h)) domtree.slog:89 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("loop");
      std::vector<u16> ord4({0, 1});
      slog::Relation* readrel5 = db->getRelation("loop");
      head_index[0] = readrel5->getIndex(ord4, false);
      head_rel[1] = db->getRelation("loop");
      std::vector<u16> ord6({0, 1});
      slog::Relation* readrel7 = db->getRelation("loop");
      head_index[1] = readrel7->getIndex(ord6, false);
      outer_rel = db->getRelation("backedge");
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        ...
        slog::emit<2>(head_rel[1], head_index[1], newbatch[1], std::array<u64,2>{v_c6, v_c6}, std::array<u16,2>{0, 1});
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
        head_rel[1]->sendBatch(newbatch[1]);
  
      if (_fires) db->bumpFires("domtree.slog:89", "all:backedge", _fires);
  ...
  // (crule (pre) (scan type_mismatch __erre95Ba279 __errf5kRl280 __errf8zR8281 __errf6ARG282 __errf6KPP283) (body) (head (emit error (0) __erre95Ba279)) <internal>:1 #f)
  ...
  // (crule (pre) (scan domset n s) (body (let __t5AI532 (cdel s n)) (join-lat domset (0 1) 0 d latchk_0) (eq __t5AI532 latchk_0) (neq d n)) (head (emit idom (0 1) n d)) domtree.slog:78 #f)
  class ReadTask12 : public slog::Task
  {
    ...
    slog::Index** domsetindex11;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("idom");
      std::vector<u16> ord13({0, 1});
      slog::Relation* readrel14 = db->getRelation("idom");
      head_index[0] = readrel14->getIndex(ord13, false);
      outer_rel = db->getRelation("domset");
      std::vector<u16> ord15({0, 1});
      slog::Relation* readrel16 = db->getRelation("domset");
      domsetindex11 = readrel16->getIndex(ord15, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c12 = _t[0];
        u64 v_c13 = _t[1];
        u64 v_c14 = _prim_cdel(db, v_c13, v_c12);
        if (v_c14 == slog_error) { slog::emit_pending_error(db, "domtree.slog:78"); return; }
        slog::join_all_lat<1>(domsetindex11, [&](const std::array<u64,1>& m17, u64 val18) {
          u64 v_c15 = m17[0]; u64 v_c16 = val18;
          if (v_c14 != v_c16) return;
          if (v_c15 == v_c12) return;
          ++_fires;
      ...
      if (_fires) db->bumpFires("domtree.slog:78", "all:domset", _fires);
  ...
  // (crule (pre) (scan int_overflow __erre8vB1268 __errf5229269 __errf1vWI270 __errf6BAF271) (body) (head (emit error (0) __erre8vB1268)) <internal>:1 #f)
  ...
  // (crule (pre) (scan div_by_zero __erre09Ia262 __errf71RY263 __errf2ekz264) (body) (head (emit error (0) __erre09Ia262)) <internal>:1 #f)
  ...
  // (crule (pre) (scan mpz_table_overflow __erre3RyV289 __errf9ZRm290 __errf533i291) (body) (head (emit error (0) __erre3RyV289)) <internal>:1 #f)
  ...
  // (crule (pre) (scan modulo_by_zero __erre5NgH265 __errf7OEa266 __errf7Xld267) (body) (head (emit error (0) __erre5NgH265)) <internal>:1 #f)
  ...
  // (crule (pre) (scan toint_range __erre3l4B276 __errf6Tyd277 __errf4I1z278) (body) (head (emit error (0) __erre3l4B276)) <internal>:1 #f)
  ...
  // (crule (pre) (scan loop h n) (body (neq n h) (join edge (1 0) 1 n p)) (head (emit loop (0 1) h p)) domtree.slog:90 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel37 = db->getRelation("loop");
      head_index[0] = readrel37->getIndex(ord36, false);
      outer_rel = db->getRelation("loop");
      std::vector<u16> ord38({1, 0});
      slog::Relation* readrel39 = db->getRelation("edge");
      edgeindex34 = readrel39->getIndex(ord38, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        ...
        slog::join_probe<2,1>(edgeindex34, std::array<u64,2>{v_c12, 0}, [&](const std::array<u64,2>& m40) {
          u64 v_c33 = m40[1];
          ++_fires;
          slog::emit<2>(head_rel[0], head_index[0], newbatch[0], std::array<u64,2>{v_c6, v_c33}, std::array<u16,2>{0, 1});
        });
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("domtree.slog:90", "delta:loop", _fires);
  ...
  // (crule (pre) (scan nan_result __erre0wh8272 __errf2RMh273 __errf0qdO274 __errf5yX9275) (body) (head (emit error (0) __erre0wh8272)) <internal>:1 #f)
  ...
  s->addDynamicRel("idom");
  s->addDynamicRel("int_overflow");
  s->addDynamicRel("loop");
