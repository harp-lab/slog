...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("0a2e62a2");
  if (s == nullptr) return;
  slog::Relation* r;
  r = db->getRelation("w");
  if (r == 0) db->addRelation("w", 1);
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("w"), std::array<u16,1>{0}, b));
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("len"), std::array<u16,2>{0, 1}, b));
  ...
  r = db->getRelation("catlen");
  if (r == 0) db->addRelation("catlen", 2);
  else if (r->getArity() != 2)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("catlen");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("catlen"), std::array<u16,2>{0, 1}, b));
  ...
  // (crule (pre) (scan div_by_zero __erre6py437 __errf71NC38 __errf11wT39) (body) (head (emit error (0) __erre6py437)) <internal>:1 #f)
  ...
  // (crule (pre) (scan w A) (body (let __t9RcU1 (_0002b A A)) (let __t8UyL2 (size __t9RcU1))) (head (emit catlen (0 1) __t9RcU1 __t8UyL2)) prim2_size.slog:14 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("catlen");
      std::vector<u16> ord4({0, 1});
      slog::Relation* readrel5 = db->getRelation("catlen");
      head_index[0] = readrel5->getIndex(ord4, false);
      outer_rel = db->getRelation("w");
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c3 = _t[0];
        u64 v_c4 = _prim__0002b(db, v_c3, v_c3);
        if (v_c4 == slog_error) { slog::emit_pending_error(db, "prim2_size.slog:14"); return; }
        u64 v_c5 = _prim_size(db, v_c4);
        if (v_c5 == slog_error) { slog::emit_pending_error(db, "prim2_size.slog:14"); return; }
        ++_fires;
      ...
      if (_fires) db->bumpFires("prim2_size.slog:14", "all:w", _fires);
  ...
  // (crule (pre) (scan toint_range __erre8tMt51 __errf7Bbk52 __errf5Yqj53) (body) (head (emit error (0) __erre8tMt51)) <internal>:1 #f)
  ...
  // (crule (pre) (scan int_overflow __erre7p7U43 __errf40gi44 __errf0ExW45 __errf6JLF46) (body) (head (emit error (0) __erre7p7U43)) <internal>:1 #f)
  ...
  // (crule (pre) (scan mpz_overflow __erre4N0d59 __errf3LGZ60 __errf6hJs61 __errf8olj62 __errf9QWB63) (body) (head (emit error (0) __erre4N0d59)) <internal>:1 #f)
  ...
  // (crule (pre) (scan modulo_by_zero __erre3lhA40 __errf8lte41 __errf5xtq42) (body) (head (emit error (0) __erre3lhA40)) <internal>:1 #f)
  ...
  // (crule (pre) (scan nan_result __erre6BN447 __errf4tFS48 __errf1YC449 __errf4Omu50) (body) (head (emit error (0) __erre6BN447)) <internal>:1 #f)
  ...
  // (crule (pre) (scan mpz_table_overflow __erre44cS64 __errf9YlJ65 __errf2DhO66) (body) (head (emit error (0) __erre44cS64)) <internal>:1 #f)
  ...
  // (crule (pre) (scan w A) (body (let __t9LcS6 (size A))) (head (emit len (0 1) A __t9LcS6)) prim2_size.slog:13 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("len");
      std::vector<u16> ord25({0, 1});
      slog::Relation* readrel26 = db->getRelation("len");
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c3 = _t[0];
        u64 v_c28 = _prim_size(db, v_c3);
        if (v_c28 == slog_error) { slog::emit_pending_error(db, "prim2_size.slog:13"); return; }
        ++_fires;
        slog::emit<2>(head_rel[0], head_index[0], newbatch[0], std::array<u64,2>{v_c3, v_c28}, std::array<u16,2>{0, 1});
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("prim2_size.slog:13", "all:w", _fires);
  ...
  // (crule (pre) (scan type_mismatch __erre9BUy54 __errf8i4W55 __errf20P556 __errf8Z9857 __errf32tF58) (body) (head (emit error (0) __erre9BUy54)) <internal>:1 #f)
