...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("9cc7754f");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  // (crule (pre) (scan labb n s) (body (join classify (1 0) 1 n __t9JcF8)) (head (emit classify_ans (0 1) __t9JcF8 s)) dem_or.slog:10 #f)
  class ReadTask1 : public slog::Task
  {
    ...
    slog::Index** classifyindex0;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("classify_ans");
      std::vector<u16> ord2({0, 1});
      slog::Relation* readrel3 = db->getRelation("classify_ans");
      head_index[0] = readrel3->getIndex(ord2, false);
      outer_rel = db->getRelation("labb");
      std::vector<u16> ord4({1, 0});
      slog::Relation* readrel5 = db->getRelation("classify");
      classifyindex0 = readrel5->getIndex(ord4, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        u64 v_c1 = _t[1];
        slog::join_probe<2,1>(classifyindex0, std::array<u64,2>{v_c0, 0}, [&](const std::array<u64,2>& m6) {
          u64 v_c2 = m6[1];
          ++_fires;
      ...
      if (_fires) db->bumpFires("dem_or.slog:10", "all:labb", _fires);
  ...
  // (crule (pre) (scan laba n s) (body (join classify (1 0) 1 n __t7rWd9)) (head (emit classify_ans (0 1) __t7rWd9 s)) dem_or.slog:10 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel10 = db->getRelation("classify_ans");
      head_index[0] = readrel10->getIndex(ord9, false);
      outer_rel = db->getRelation("laba");
      std::vector<u16> ord11({1, 0});
      slog::Relation* readrel12 = db->getRelation("classify");
      classifyindex7 = readrel12->getIndex(ord11, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        u64 v_c1 = _t[1];
        slog::join_probe<2,1>(classifyindex7, std::array<u64,2>{v_c0, 0}, [&](const std::array<u64,2>& m13) {
          u64 v_c3 = m13[1];
          ++_fires;
      ...
      if (_fires) db->bumpFires("dem_or.slog:10", "all:laba", _fires);
  ...
  s->addReadRel("classify");
  s->addReadRel("laba");
  s->addReadRel("labb");
  s->addDynamicRel("classify_ans");
  d->push(s);
  d->continueRun();
}
