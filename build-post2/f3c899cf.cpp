...
u64 v_const0814305f8cfb2f14744b7c7c;


extern "C" void slog_plugin(slog::Daemon* d)
{
  slog::Database* db = d->db();
  slog::Stratum* s = d->beginStratum("f3c899cf");
  if (s == nullptr) return;
  slog::Relation* r;
  v_const0814305f8cfb2f14744b7c7c = float_encode(2.4);
  r = db->getRelation("v");
  ...
  r = db->getRelation("malformed_deduction");
    ...
    for (u16 b = 0; b < 32; ++b)
      s->addTask(phase_write, new slog::WriteTask<5>(db, r, std::array<u16,5>{1, 2, 3, 4, 0}, false, b), true);
      std::vector<u16> ord69({0, 1, 2, 3, 4});
  ...
  // (crule (pre) (scan mpz_table_overflow __erre9izh63 __errf28GD64 __errf7Z5y65) (body) (head (emit error (0) __erre9izh63)) <internal>:1 #f)
  ...
  // (crule (pre) (scan div_by_zero __erre4DCK36 __errf5DnH37 __errf9IJn38) (body) (head (emit error (0) __erre4DCK36)) <internal>:1 #f)
  ...
  // (crule (pre (let __tconst2oar3 const0814305f8cfb2f14744b7c7c)) (scan v A) (body (cmp lt A __tconst2oar3)) (head (emit small (0) A)) prim2_mixed.slog:17 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("small");
      std::vector<u16> ord7({0});
      slog::Relation* readrel8 = db->getRelation("small");
      head_index[0] = readrel8->getIndex(ord7, false);
      outer_rel = db->getRelation("v");
  
    }
    ReadTask6(slog::Database* _db, u16 _b) : db(_db), bucket(_b) { bind(_db); }
    virtual bool work()
    {
      u64 v_c6 = v_const0814305f8cfb2f14744b7c7c;
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c7 = _t[0];
        u64 v_c8 = _prim_lt(db, v_c7, v_c6);
        if (v_c8 == slog_error) { slog::emit_pending_error(db, "prim2_mixed.slog:17"); return; }
        if (!v_c8) return;
        ++_fires;
      ...
      if (_fires) db->bumpFires("prim2_mixed.slog:17", "all:v", _fires);
  ...
  // (crule (pre) (scan v A) (body (join v (0) 0 B) (let __t5DXA4 (max A B))) (head (emit mx2 (0 1 2) A B __t5DXA4)) prim2_mixed.slog:16 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("mx2");
      std::vector<u16> ord12({0, 1, 2});
      slog::Relation* readrel13 = db->getRelation("mx2");
      head_index[0] = readrel13->getIndex(ord12, false);
      outer_rel = db->getRelation("v");
      std::vector<u16> ord14({0});
      slog::Relation* readrel15 = db->getRelation("v");
      vindex10 = readrel15->getIndex(ord14, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c7 = _t[0];
        slog::join_all<1>(vindex10, [&](const std::array<u64,1>& m16) {
          u64 v_c9 = m16[0];
          u64 v_c10 = _prim_max(db, v_c7, v_c9);
          if (v_c10 == slog_error) { slog::emit_pending_error(db, "prim2_mixed.slog:16"); return; }
          ++_fires;
          slog::emit<3>(head_rel[0], head_index[0], newbatch[0], std::array<u64,3>{v_c7, v_c9, v_c10}, std::array<u16,3>{0, 1, 2});
        });
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("prim2_mixed.slog:16", "all:v", _fires);
  ...
  // (crule (pre) (scan nan_result __erre7NaB46 __errf2plI47 __errf8ZA248 __errf0nwZ49) (body) (head (emit error (0) __erre7NaB46)) <internal>:1 #f)
  ...
  // (crule (pre) (scan v A) (body (join v (0) 0 B) (let __t6MK85 (_0002b A B))) (head (emit sums (0 1 2) A B __t6MK85)) prim2_mixed.slog:15 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("sums");
      std::vector<u16> ord22({0, 1, 2});
      slog::Relation* readrel23 = db->getRelation("sums");
      head_index[0] = readrel23->getIndex(ord22, false);
      outer_rel = db->getRelation("v");
      std::vector<u16> ord24({0});
      slog::Relation* readrel25 = db->getRelation("v");
      vindex20 = readrel25->getIndex(ord24, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c7 = _t[0];
        slog::join_all<1>(vindex20, [&](const std::array<u64,1>& m26) {
          u64 v_c9 = m26[0];
          u64 v_c15 = _prim__0002b(db, v_c7, v_c9);
          if (v_c15 == slog_error) { slog::emit_pending_error(db, "prim2_mixed.slog:15"); return; }
          ++_fires;
          slog::emit<3>(head_rel[0], head_index[0], newbatch[0], std::array<u64,3>{v_c7, v_c9, v_c15}, std::array<u16,3>{0, 1, 2});
        });
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("prim2_mixed.slog:15", "all:v", _fires);
  ...
  // (crule (pre) (scan mpz_overflow __erre6rW658 __errf7eWN59 __errf5s9N60 __errf9y9F61 __errf4o0L62) (body) (head (emit error (0) __erre6rW658)) <internal>:1 #f)
  ...
  // (crule (pre) (scan type_mismatch __erre4EMf53 __errf3vyb54 __errf2kTF55 __errf8xJk56 __errf7Sy357) (body) (head (emit error (0) __erre4EMf53)) <internal>:1 #f)
  ...
  // (crule (pre) (scan toint_range __erre15np50 __errf5Guw51 __errf6I0V52) (body) (head (emit error (0) __erre15np50)) <internal>:1 #f)
  ...
  // (crule (pre) (scan int_overflow __erre2Lfq42 __errf5Ksd43 __errf5Ymt44 __errf0E0D45) (body) (head (emit error (0) __erre2Lfq42)) <internal>:1 #f)
  ...
  // (crule (pre) (scan v B) (body (join v (0) 0 A) (cmp lt A B)) (head (emit ltv (0 1) A B)) prim2_mixed.slog:18 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("ltv");
      std::vector<u16> ord41({0, 1});
      slog::Relation* readrel42 = db->getRelation("ltv");
      head_index[0] = readrel42->getIndex(ord41, false);
      outer_rel = db->getRelation("v");
      std::vector<u16> ord43({0});
      slog::Relation* readrel44 = db->getRelation("v");
      vindex39 = readrel44->getIndex(ord43, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c9 = _t[0];
        slog::join_all<1>(vindex39, [&](const std::array<u64,1>& m45) {
          u64 v_c7 = m45[0];
          u64 v_c33 = _prim_lt(db, v_c7, v_c9);
          if (v_c33 == slog_error) { slog::emit_pending_error(db, "prim2_mixed.slog:18"); return; }
          if (!v_c33) return;
          ++_fires;
      ...
      if (_fires) db->bumpFires("prim2_mixed.slog:18", "all:v", _fires);
  ...
  // (crule (pre) (scan modulo_by_zero __erre1f6z39 __errf8rbn40 __errf5cok41) (body) (head (emit error (0) __erre1f6z39)) <internal>:1 #f)
  ...
  s->addDynamicRel("small");
  s->addDynamicRel("sums");
