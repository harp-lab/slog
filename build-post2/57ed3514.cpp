...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("57ed3514");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  // (crule (pre) (scan type_mismatch __erre99Xg247 __errf7uda248 __errf7JjZ249 __errf18Dw250 __errf4xD6251) (body) (head (emit error (0) __erre99Xg247)) <internal>:1 #f)
  ...
  // (crule (pre) (scan div_by_zero __erre1pDS230 __errf5Qop231 __errf4jrl232) (body) (head (emit error (0) __erre1pDS230)) <internal>:1 #f)
  ...
  // (crule (pre) (scan toint_range __erre2TBs244 __errf89PH245 __errf72jq246) (body) (head (emit error (0) __erre2TBs244)) <internal>:1 #f)
  ...
  // (crule (pre) (scan mpz_table_overflow __erre8Afq257 __errf1y1Q258 __errf4je3259) (body) (head (emit error (0) __erre8Afq257)) <internal>:1 #f)
  ...
  // (crule (pre (let one const6b86b273ff34fce19d6b804e)) (scan $seq_pos l j p) (body (let n (llen l)) (let lim (_0002d n j)) (cmp lt p lim) (let p1 (_0002b p one))) (head (emit $seq_pos (0 1 2) l j p1)) seq_split_mixed.slog:13 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("$seq_pos");
      std::vector<u16> ord13({0, 1, 2});
      slog::Relation* readrel14 = db->getRelation("$seq_pos");
      head_index[0] = readrel14->getIndex(ord13, false);
      outer_rel = db->getRelation("$seq_pos");
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        ...
        u64 v_c16 = _t[1];
        u64 v_c17 = _t[2];
        u64 v_c18 = _prim_llen(db, v_c15);
        if (v_c18 == slog_error) { slog::emit_pending_error(db, "seq_split_mixed.slog:13"); return; }
        u64 v_c19 = _prim__0002d(db, v_c18, v_c16);
        if (v_c19 == slog_error) { slog::emit_pending_error(db, "seq_split_mixed.slog:13"); return; }
        u64 v_c20 = _prim_lt(db, v_c17, v_c19);
        if (v_c20 == slog_error) { slog::emit_pending_error(db, "seq_split_mixed.slog:13"); return; }
        if (!v_c20) return;
        u64 v_c21 = _prim__0002b(db, v_c17, v_c14);
        if (v_c21 == slog_error) { slog::emit_pending_error(db, "seq_split_mixed.slog:13"); return; }
        ++_fires;
        slog::emit<3>(head_rel[0], head_index[0], newbatch[0], std::array<u64,3>{v_c15, v_c16, v_c21}, std::array<u16,3>{0, 1, 2});
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("seq_split_mixed.slog:13", "delta:$seq_pos", _fires);
  ...
  // (crule (pre) (scan modulo_by_zero __erre2p7D233 __errf9mNn234 __errf1xS2235) (body) (head (emit error (0) __erre2p7D233)) <internal>:1 #f)
  ...
  // (crule (pre) (scan mpz_overflow __erre4JpQ252 __errf8Kuy253 __errf5yXZ254 __errf4561255 __errf9udK256) (body) (head (emit error (0) __erre4JpQ252)) <internal>:1 #f)
  ...
  // (crule (pre) (scan nan_result __erre3j6O240 __errf5bA3241 __errf93yP242 __errf7Txt243) (body) (head (emit error (0) __erre3j6O240)) <internal>:1 #f)
  ...
  // (crule (pre (let z const5feceb66ffc86f38d952786c)) (scan $seq_posdem l j) (body (let n (llen l)) (cmp ge n j)) (head (emit $seq_pos (0 1 2) l j z)) seq_split_mixed.slog:13 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel27 = db->getRelation("$seq_pos");
      head_index[0] = readrel27->getIndex(ord26, false);
      outer_rel = db->getRelation("$seq_posdem");
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        ...
        u64 v_c18 = _prim_llen(db, v_c15);
        if (v_c18 == slog_error) { slog::emit_pending_error(db, "seq_split_mixed.slog:13"); return; }
        u64 v_c35 = _prim_ge(db, v_c18, v_c16);
        if (v_c35 == slog_error) { slog::emit_pending_error(db, "seq_split_mixed.slog:13"); return; }
        if (!v_c35) return;
        ++_fires;
        slog::emit<3>(head_rel[0], head_index[0], newbatch[0], std::array<u64,3>{v_c15, v_c16, v_c34}, std::array<u16,3>{0, 1, 2});
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("seq_split_mixed.slog:13", "all:$seq_posdem", _fires);
    ...
    for (u16 b = 0; b < 32; ++b)
      s->addTask(phase_read, new ReadTask25(db,b), true);
  // (crule (pre) (scan int_overflow __erre0R1q236 __errf6yR5237 __errf6065238 __errf4RMz239) (body) (head (emit error (0) __erre0R1q236)) <internal>:1 #f)
