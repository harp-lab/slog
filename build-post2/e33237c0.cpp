...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("e33237c0");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<3>(db, db->getRelation("edge"), std::array<u16,3>{0, 2, 1}, b));
  ...
  // (crule (pre) (scan path X Y L) (body (join edge (0 2 1) 2 Y L Z)) (head (emit path (0 1 2) X Z L)) labels.slog:16 #f)
  ...
  public:
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        ...
        u64 v_c1 = _t[1];
        u64 v_c2 = _t[2];
        slog::join_probe<3,2>(edgeindex0, std::array<u64,3>{v_c1, v_c2, 0}, [&](const std::array<u64,3>& m6) {
          u64 v_c3 = m6[2];
          ++_fires;
      ...
      if (_fires) db->bumpFires("labels.slog:16", "delta:path", _fires);
  ...
  // (crule (pre) (scan edge X Y L) (body) (head (emit path (0 1 2) X Y L)) labels.slog:11 #f)
  ...
  public:
    ...
    virtual bool work()
    {
      ...
      if (_fires) db->bumpFires("labels.slog:11", "all:edge", _fires);
