...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("96f06cca");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  // (crule (pre) (scan sg PX PY) (body (exists edge (0 1) 1 PX) (join edge (0 1) 1 PY Y) (join edge (0 1) 1 PX X)) (head (emit sg (0 1) X Y)) sj_sg.slog:28 #f)
  class ReadTask3 : public slog::Task
  {
    ...
    slog::Index** edgeindex0;  slog::Index** edgeindex1;  slog::Index** edgeindex2;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("sg");
      std::vector<u16> ord4({0, 1});
      slog::Relation* readrel5 = db->getRelation("sg");
      head_index[0] = readrel5->getIndex(ord4, false);
      outer_rel = db->getRelation("sg");
      std::vector<u16> ord6({0, 1});
      slog::Relation* readrel7 = db->getRelation("edge");
      edgeindex0 = readrel7->getIndex(ord6, false);
      std::vector<u16> ord8({0, 1});
      slog::Relation* readrel9 = db->getRelation("edge");
      edgeindex1 = readrel9->getIndex(ord8, false);
      std::vector<u16> ord10({0, 1});
      slog::Relation* readrel11 = db->getRelation("edge");
      edgeindex2 = readrel11->getIndex(ord10, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        u64 v_c1 = _t[1];
        if (!slog::exists_probe<2,1>(edgeindex0, std::array<u64,2>{v_c0, 0})) return;
        slog::join_probe<2,1>(edgeindex1, std::array<u64,2>{v_c1, 0}, [&](const std::array<u64,2>& m12) {
          u64 v_c2 = m12[1];
          slog::join_probe<2,1>(edgeindex2, std::array<u64,2>{v_c0, 0}, [&](const std::array<u64,2>& m13) {
            u64 v_c3 = m13[1];
            ++_fires;
      ...
      if (_fires) db->bumpFires("sj_sg.slog:28", "delta:sg", _fires);
  ...
  // (crule (pre) (scan edge P Y) (body (join edge (0 1) 1 P X)) (head (emit sg (0 1) X Y)) sj_sg.slog:27 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel17 = db->getRelation("sg");
      ...
      edgeindex14 = readrel19->getIndex(ord18, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c4 = _t[0];
        u64 v_c2 = _t[1];
        slog::join_probe<2,1>(edgeindex14, std::array<u64,2>{v_c4, 0}, [&](const std::array<u64,2>& m20) {
          u64 v_c3 = m20[1];
          ++_fires;
          slog::emit<2>(head_rel[0], head_index[0], newbatch[0], std::array<u64,2>{v_c3, v_c2}, std::array<u16,2>{0, 1});
        });
      ...
      if (_fires) db->bumpFires("sj_sg.slog:27", "all:edge", _fires);
