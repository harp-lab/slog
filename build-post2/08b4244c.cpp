...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("08b4244c");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  r = db->getRelation("temp9C3E84");
  if (r == 0) db->addTempRelation("temp9C3E84", 3);
  else if (r->getArity() != 3) slog::fatal("Temp rel arity mismatch.");
  r = db->getRelation("temp6hfj85");
  if (r == 0) db->addTempRelation("temp6hfj85", 2);
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<3>(db, db->getRelation("padd"), std::array<u16,3>{0, 1, 2}, b));
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("num"), std::array<u16,2>{1, 0}, b));
  r = db->getRelation("nan_result");
  if (r == 0) db->addStruct("nan_result", 4);
  else if (r->getArity() != 4)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("nan_result");
      std::vector<u16> ord112({1, 2, 3, 0});
  ...
  r = db->getRelation("mpz_overflow");
    ...
    for (u16 b = 0; b < 32; ++b)
      s->addTask(phase_write, new slog::WriteTask<5>(db, r, std::array<u16,5>{1, 2, 3, 4, 0}, false, b), true);
      std::vector<u16> ord117({0, 1, 2, 3, 4});
  ...
  if (r == 0) db->addStruct("malformed_deduction", 5);
    ...
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("malformed_deduction");
      std::vector<u16> ord120({1, 2, 3, 4, 0});
    
    r->addIndex<5>(ord120, false);
    for (u16 b = 0; b < 32; ++b)
      s->addTask(phase_write, new slog::WriteTask<5>(db, r, std::array<u16,5>{1, 2, 3, 4, 0}, false, b), true);
      std::vector<u16> ord121({0, 1, 2, 3, 4});
  ...
  // (crule (pre) (scan mpz_overflow __erre8HtF76 __errf09LP77 __errf38LT78 __errf6w3m79 __errf7PvD80) (body) (head (emit error (0) __erre8HtF76)) <internal>:1 #f)
  ...
  // (crule (pre) (scan temp9C3E84 B C __t09sP15) (body (join s (1 0) 1 C __t9bD913)) (head (emit padd (0 1 2) __t09sP15 B __t9bD913)) ex_peano.slog:31 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel6 = db->getRelation("padd");
      head_index[0] = readrel6->getIndex(ord5, false);
      outer_rel = db->getRelation("temp9C3E84");
      std::vector<u16> ord7({1, 0});
      slog::Relation* readrel8 = db->getRelation("s");
      sindex3 = readrel8->getIndex(ord7, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        ...
        u64 v_c6 = _t[1];
        u64 v_c7 = _t[2];
        slog::join_probe<2,1>(sindex3, std::array<u64,2>{v_c6, 0}, [&](const std::array<u64,2>& m9) {
          u64 v_c8 = m9[1];
          ++_fires;
          slog::emit<3>(head_rel[0], head_index[0], newbatch[0], std::array<u64,3>{v_c7, v_c5, v_c8}, std::array<u16,3>{0, 1, 2});
        });
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("ex_peano.slog:31", "delta:temp9C3E84", _fires);
  ...
  // (crule (pre) (scan modulo_by_zero __erre9WjI57 __errf7rMF58 __errf68dc59) (body) (head (emit error (0) __erre9WjI57)) <internal>:1 #f)
  ...
  // (crule (pre (let __tconst9uHm3 const4b227777d4dd1fc61c6f884f) (let __tconst6J4i4 const6b86b273ff34fce19d6b804e)) (scan count N) (body (cmp lt N __tconst9uHm3) (let __t7AYQ5 (_0002b N __tconst6J4i4))) (head (emit count (0) __t7AYQ5)) ex_peano.slog:22 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel15 = db->getRelation("count");
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c14 = _t[0];
        u64 v_c15 = _prim_lt(db, v_c14, v_c12);
        if (v_c15 == slog_error) { slog::emit_pending_error(db, "ex_peano.slog:22"); return; }
        if (!v_c15) return;
        u64 v_c16 = _prim__0002b(db, v_c14, v_c13);
        if (v_c16 == slog_error) { slog::emit_pending_error(db, "ex_peano.slog:22"); return; }
        ++_fires;
      ...
      if (_fires) db->bumpFires("ex_peano.slog:22", "delta:count", _fires);
  ...
  // (crule (pre) (scan padd A B C) (body (join-old s (1 0) 1 (1 0) A __t09sP15) (join-old num (1 0) 1 (1 0) __t09sP15 ____7Lz216)) (head (emit-temp temp9C3E84 B C __t09sP15) (mkstruct s (1 0) __t9bD913 C)) ex_peano.slog:31 #f)
  class ReadTask21 : public slog::Task
  {
    ...
    slog::Index** sindex17;  slog::Index** numindex18;  slog::Index** sdelta19;  slog::Index** numdelta20;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("temp9C3E84");
      head_rel[1] = db->getRelation("s");
      outer_rel = db->getRelation("padd");
      std::vector<u16> ord22({1, 0});
      slog::Relation* readrel23 = db->getRelation("s");
      sindex17 = readrel23->getIndex(ord22, false);
      std::vector<u16> ord24({1, 0});
      slog::Relation* readrel25 = db->getRelation("s");
      sdelta19 = readrel25->getIndex(ord24, true);
      ...
      numdelta20 = readrel29->getIndex(ord28, true);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        ...
        u64 v_c5 = _t[1];
        u64 v_c6 = _t[2];
        slog::join_probe_old<2,1>(sindex17, sdelta19, std::array<u64,2>{v_c17, 0}, [&](const std::array<u64,2>& m30) {
          u64 v_c7 = m30[1];
          slog::join_probe_old<2,1>(numindex18, numdelta20, std::array<u64,2>{v_c7, 0}, [&](const std::array<u64,2>& m31) {
            u64 v_c18 = m31[1];
            ++_fires;
            slog::emit_temp<3>(head_rel[0], newbatch[0], std::array<u64,3>{v_c5, v_c6, v_c7});
      ...
      if (_fires) db->bumpFires("ex_peano.slog:31", "delta:padd", _fires);
  ...
  // (crule (pre) (scan mpz_table_overflow __erre48fZ81 __errf9pBy82 __errf9rDC83) (body) (head (emit error (0) __erre48fZ81)) <internal>:1 #f)
  ...
  // (crule (pre (let __tconst3Bmb21 const20c400557af0eddc0be4d9e0) (let __tconst0cGY19 const5feceb66ffc86f38d952786c)) (probe _enum (1 0) 1 __tconst3Bmb21 __t9r2718) (body) (head (emit count (0) __tconst0cGY19) (emit num (1 0) __t9r2718 __tconst0cGY19)) ex_peano.slog:17 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel37 = db->getRelation("count");
      head_index[0] = readrel37->getIndex(ord36, false);
      head_rel[1] = db->getRelation("num");
      std::vector<u16> ord38({1, 0});
      slog::Relation* readrel39 = db->getRelation("num");
      head_index[1] = readrel39->getIndex(ord38, false);
      std::vector<u16> ord40({1, 0});
      slog::Relation* readrel41 = db->getRelation("_enum");
      driver_index = readrel41->getIndex(ord40, true);
  
    }
    ReadTask35(slog::Database* _db, u16 _b) : db(_db), bucket(_b) { bind(_db); }
    virtual bool work()
    {
      u64 v_c22 = v_const20c400557af0eddc0be4d9e0;
      ...
      bool _done = slog::join_probe_sliced<2,1>(driver_index, std::array<u64,2>{v_c22, 0}, _sc, _rkey, _hr, [&](const std::array<u64,2>& m42) {
        u64 v_c24 = m42[1];
        if (buckethash(v_c24) != bucket) return;
        ++_fires;
        slog::emit<1>(head_rel[0], head_index[0], newbatch[0], std::array<u64,1>{v_c23}, std::array<u16,1>{0});
        slog::emit<2>(head_rel[1], head_index[1], newbatch[1], std::array<u64,2>{v_c24, v_c23}, std::array<u16,2>{1, 0});
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
        head_rel[1]->sendBatch(newbatch[1]);
  
      if (_fires) db->bumpFires("ex_peano.slog:17", "all:_enum", _fires);
  ...
  // (crule (pre) (scan temp6hfj85 X __t8Iiy12) (body (join s (1 0) 1 X __t4AI19)) (head (emit num (1 0) __t4AI19 __t8Iiy12)) ex_peano.slog:25 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel46 = db->getRelation("num");
      head_index[0] = readrel46->getIndex(ord45, false);
      outer_rel = db->getRelation("temp6hfj85");
      std::vector<u16> ord47({1, 0});
      slog::Relation* readrel48 = db->getRelation("s");
      sindex43 = readrel48->getIndex(ord47, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c25 = _t[0];
        u64 v_c26 = _t[1];
        slog::join_probe<2,1>(sindex43, std::array<u64,2>{v_c25, 0}, [&](const std::array<u64,2>& m49) {
          u64 v_c27 = m49[1];
          ++_fires;
          slog::emit<2>(head_rel[0], head_index[0], newbatch[0], std::array<u64,2>{v_c27, v_c26}, std::array<u16,2>{1, 0});
        });
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("ex_peano.slog:25", "delta:temp6hfj85", _fires);
  ...
  // (crule (pre) (scan type_mismatch __erre888I71 __errf43bi72 __errf6E9l73 __errf4HbA74 __errf1LaD75) (body) (head (emit error (0) __erre888I71)) <internal>:1 #f)
  ...
  // (crule (pre) (scan s __t09sP15 A) (body (exists padd (0 1 2) 1 A) (join num (1 0) 1 __t09sP15 ____7Lz216) (join padd (0 1 2) 1 A B C)) (head (emit-temp temp9C3E84 B C __t09sP15) (mkstruct s (1 0) __t9bD913 C)) ex_peano.slog:31 #f)
  class ReadTask56 : public slog::Task
  {
    ...
    slog::Index** paddindex53;  slog::Index** numindex54;  slog::Index** paddindex55;
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("temp9C3E84");
      head_rel[1] = db->getRelation("s");
      outer_rel = db->getRelation("s");
      std::vector<u16> ord57({0, 1, 2});
      slog::Relation* readrel58 = db->getRelation("padd");
      paddindex53 = readrel58->getIndex(ord57, false);
      std::vector<u16> ord59({1, 0});
      slog::Relation* readrel60 = db->getRelation("num");
      numindex54 = readrel60->getIndex(ord59, false);
      std::vector<u16> ord61({0, 1, 2});
      slog::Relation* readrel62 = db->getRelation("padd");
      paddindex55 = readrel62->getIndex(ord61, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c7 = _t[0];
        u64 v_c17 = _t[1];
        if (!slog::exists_probe<3,1>(paddindex53, std::array<u64,3>{v_c17, 0, 0})) return;
        slog::join_probe<2,1>(numindex54, std::array<u64,2>{v_c7, 0}, [&](const std::array<u64,2>& m63) {
          u64 v_c18 = m63[1];
          slog::join_probe<3,1>(paddindex55, std::array<u64,3>{v_c17, 0, 0}, [&](const std::array<u64,3>& m64) {
            u64 v_c5 = m64[1]; u64 v_c6 = m64[2];
            ++_fires;
            slog::emit_temp<3>(head_rel[0], newbatch[0], std::array<u64,3>{v_c5, v_c6, v_c7});
            slog::emit_struct<2>(head_rel[1], newbatch[1], std::array<u64,1>{v_c6}, std::array<u16,2>{1, 0});
          });
      ...
      if (_fires) db->bumpFires("ex_peano.slog:31", "delta:s", _fires);
  ...
  // (crule (pre) (scan nan_result __erre8loe64 __errf8aXu65 __errf6AsS66 __errf2HDU67) (body) (head (emit error (0) __erre8loe64)) <internal>:1 #f)
  ...
  // (crule (pre) (scan int_overflow __erre2ghV60 __errf0eH561 __errf3bQE62 __errf91wR63) (body) (head (emit error (0) __erre2ghV60)) <internal>:1 #f)
  ...
  // (crule (pre) (scan div_by_zero __erre1xpt54 __errf8qOD55 __errf3Y5656) (body) (head (emit error (0) __erre1xpt54)) <internal>:1 #f)
  ...
  // (crule (pre (let __tconst07Tf20 const20c400557af0eddc0be4d9e0)) (scan num ____4wB52 B) (body (join _enum (1 0) 1 __tconst07Tf20 __t1bKL1)) (head (emit padd (0 1 2) __t1bKL1 B B)) ex_peano.slog:28 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel77 = db->getRelation("padd");
      ...
      _enumindex74 = readrel79->getIndex(ord78, false);
  
    }
    ReadTask75(slog::Database* _db, u16 _b) : db(_db), bucket(_b) { bind(_db); }
    virtual bool work()
    {
      u64 v_c44 = v_const20c400557af0eddc0be4d9e0;
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c45 = _t[0];
        u64 v_c5 = _t[1];
        slog::join_probe<2,1>(_enumindex74, std::array<u64,2>{v_c44, 0}, [&](const std::array<u64,2>& m80) {
          u64 v_c46 = m80[1];
          ++_fires;
          slog::emit<3>(head_rel[0], head_index[0], newbatch[0], std::array<u64,3>{v_c46, v_c5, v_c5}, std::array<u16,3>{0, 1, 2});
        });
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("ex_peano.slog:28", "delta:num", _fires);
  ...
  // (crule (pre (let __tconst45HA10 const4b227777d4dd1fc61c6f884f) (let __tconst2f4j11 const6b86b273ff34fce19d6b804e)) (scan num N X) (body (cmp lt N __tconst45HA10) (let __t8Iiy12 (_0002b N __tconst2f4j11))) (head (emit-temp temp6hfj85 X __t8Iiy12) (mkstruct s (1 0) __t4AI19 X)) ex_peano.slog:25 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("temp6hfj85");
      head_rel[1] = db->getRelation("s");
      outer_rel = db->getRelation("num");
    ...
    virtual bool work()
    {
      u64 v_c47 = v_const4b227777d4dd1fc61c6f884f;
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c14 = _t[0];
        u64 v_c25 = _t[1];
        u64 v_c49 = _prim_lt(db, v_c14, v_c47);
        if (v_c49 == slog_error) { slog::emit_pending_error(db, "ex_peano.slog:25"); return; }
        if (!v_c49) return;
        u64 v_c26 = _prim__0002b(db, v_c14, v_c48);
        if (v_c26 == slog_error) { slog::emit_pending_error(db, "ex_peano.slog:25"); return; }
        ++_fires;
      ...
      if (_fires) db->bumpFires("ex_peano.slog:25", "delta:num", _fires);
  ...
  // (crule (pre) (scan toint_range __erre9gUA68 __errf2kE769 __errf01LC70) (body) (head (emit error (0) __erre9gUA68)) <internal>:1 #f)
  ...
  // (crule (pre) (scan num ____7Lz216 __t09sP15) (body (join-old s (0 1) 1 (0 1) __t09sP15 A) (join padd (0 1 2) 1 A B C)) (head (emit-temp temp9C3E84 B C __t09sP15) (mkstruct s (1 0) __t9bD913 C)) ex_peano.slog:31 #f)
  class ReadTask89 : public slog::Task
  {
    ...
    slog::Index** sindex86;  slog::Index** paddindex87;  slog::Index** sdelta88;
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel91 = db->getRelation("s");
      sindex86 = readrel91->getIndex(ord90, false);
      std::vector<u16> ord92({0, 1});
      slog::Relation* readrel93 = db->getRelation("s");
      sdelta88 = readrel93->getIndex(ord92, true);
      std::vector<u16> ord94({0, 1, 2});
      slog::Relation* readrel95 = db->getRelation("padd");
      paddindex87 = readrel95->getIndex(ord94, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c18 = _t[0];
        u64 v_c7 = _t[1];
        slog::join_probe_old<2,1>(sindex86, sdelta88, std::array<u64,2>{v_c7, 0}, [&](const std::array<u64,2>& m96) {
          u64 v_c17 = m96[1];
          slog::join_probe<3,1>(paddindex87, std::array<u64,3>{v_c17, 0, 0}, [&](const std::array<u64,3>& m97) {
            u64 v_c5 = m97[1]; u64 v_c6 = m97[2];
            ++_fires;
            slog::emit_temp<3>(head_rel[0], newbatch[0], std::array<u64,3>{v_c5, v_c6, v_c7});
            slog::emit_struct<2>(head_rel[1], newbatch[1], std::array<u64,1>{v_c6}, std::array<u16,2>{1, 0});
          });
      ...
      if (_fires) db->bumpFires("ex_peano.slog:31", "delta:num", _fires);
  ...
  s->addReadRel("padd");
  s->addReadRel("s");
  s->addReadRel("temp6hfj85");
  s->addReadRel("temp9C3E84");
  ...
  s->addDynamicRel("s");
  s->addDynamicRel("temp6hfj85");
  s->addDynamicRel("temp9C3E84");
