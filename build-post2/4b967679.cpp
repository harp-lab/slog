...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("4b967679");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  r = db->getRelation("skew");
  if (r == 0) db->addRelation("skew", 1);
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("skew"), std::array<u16,1>{0}, b));
  r = db->getRelation("outr");
  if (r == 0) db->addRelation("outr", 2);
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("outr"), std::array<u16,2>{0, 1}, b));
  ...
  r = db->getRelation("lst");
  if (r == 0) db->addRelation("lst", 1);
  else if (r->getArity() != 1)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("lst");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("lst"), std::array<u16,1>{0}, b));
  r = db->getRelation("joined");
  if (r == 0) db->addRelation("joined", 1);
  else if (r->getArity() != 1)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("joined");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("joined"), std::array<u16,1>{0}, b));
  ...
  r = db->getRelation("fst");
  if (r == 0) db->addRelation("fst", 1);
  else if (r->getArity() != 1)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("fst");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("fst"), std::array<u16,1>{0}, b));
  ...
  // (crule (pre (let __tconst3WQU1 const4e07408562bedb8b60ce05c1) (let __tconst2juI2 constd4735e3a265e16eee03f5971) (let __tconst2kNd3 const6b86b273ff34fce19d6b804e) (let __tconst77rj4 conste7f6c011776e8db7cd330b54) (let __tconst9QqH5 constef2d127de37b942baad06145) (let __tconst3c9S6 const4b227777d4dd1fc61c6f884f) (let __tconst2ZfV8 const2c624232cdd221771294dfbb) (let __tconst6XVg9 const7902699be42c8a8e46fbbb45)) (once) (body) (head (emit tri (0 1 2) __tconst2kNd3 __tconst2juI2 __tconst3WQU1) (emit tri (0 1 2) __tconst3c9S6 __tconst9QqH5 __tconst77rj4) (emit tri (0 1 2) __tconst6XVg9 __tconst2ZfV8 __tconst6XVg9)) shape_wild.slog:21 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      head_rel[2] = db->getRelation("tri");
      std::vector<u16> ord5({0, 1, 2});
      slog::Relation* readrel6 = db->getRelation("tri");
    ...
    virtual bool work()
    {
      ...
      ++_fires;
      slog::emit<3>(head_rel[0], head_index[0], newbatch[0], std::array<u64,3>{v_c2, v_c1, v_c0}, std::array<u16,3>{0, 1, 2});
      slog::emit<3>(head_rel[1], head_index[1], newbatch[1], std::array<u64,3>{v_c5, v_c4, v_c3}, std::array<u16,3>{0, 1, 2});
      slog::emit<3>(head_rel[2], head_index[2], newbatch[2], std::array<u64,3>{v_c7, v_c6, v_c7}, std::array<u16,3>{0, 1, 2});
  
        head_rel[0]->sendBatch(newbatch[0]);
        head_rel[1]->sendBatch(newbatch[1]);
        head_rel[2]->sendBatch(newbatch[2]);
  
      if (_fires) db->bumpFires("shape_wild.slog:21", "once", _fires);
