...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("52397d3a");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  r = db->getRelation("temp97vQ132");
  if (r == 0) db->addTempRelation("temp97vQ132", 1);
  else if (r->getArity() != 1) slog::fatal("Temp rel arity mismatch.");
  r = db->getRelation("temp5HiV135");
  if (r == 0) db->addTempRelation("temp5HiV135", 1);
  else if (r->getArity() != 1) slog::fatal("Temp rel arity mismatch.");
  r = db->getRelation("temp4yGW134");
  if (r == 0) db->addTempRelation("temp4yGW134", 1);
  else if (r->getArity() != 1) slog::fatal("Temp rel arity mismatch.");
  r = db->getRelation("temp3vjJ133");
  if (r == 0) db->addTempRelation("temp3vjJ133", 1);
  else if (r->getArity() != 1) slog::fatal("Temp rel arity mismatch.");
  ...
  r = db->getRelation("malformed_deduction");
    ...
    for (u16 b = 0; b < 32; ++b)
      s->addTask(phase_write, new slog::WriteTask<5>(db, r, std::array<u16,5>{1, 2, 3, 4, 0}, false, b), true);
      std::vector<u16> ord110({0, 1, 2, 3, 4});
  ...
  r = db->getRelation("int_overflow");
    ...
    r->addIndex<4>(ord145, false);
    for (u16 b = 0; b < 32; ++b)
      s->addTask(phase_write, new slog::WriteTask<4>(db, r, std::array<u16,4>{1, 2, 3, 0}, false, b), true);
      std::vector<u16> ord146({0, 1, 2, 3});
  ...
  // (crule (pre (let __tconst0c6B49 const683276d23a1fc1d255583994)) (scan temp3vjJ133 __t5Mpc43) (body (join _enum (1 0) 1 __tconst0c6B49 __t1kZ339) (let __t4D7u44 (cins __t5Mpc43 __t1kZ339))) (head (emit-temp temp4yGW134 __t4D7u44)) smt_err.slog:13 #f)
  ...
  public:
    ...
    {
      head_rel[0] = db->getRelation("temp4yGW134");
      outer_rel = db->getRelation("temp3vjJ133");
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c1 = _t[0];
        slog::join_probe<2,1>(_enumindex0, std::array<u64,2>{v_c0, 0}, [&](const std::array<u64,2>& m4) {
          u64 v_c2 = m4[1];
          u64 v_c3 = _prim_cins(db, v_c1, v_c2);
          if (v_c3 == slog_error) { slog::emit_pending_error(db, "smt_err.slog:13"); return; }
          ++_fires;
      ...
      if (_fires) db->bumpFires("smt_err.slog:13", "delta:temp3vjJ133", _fires);
  ...
  // (crule (pre) (scan toint_range __erre43xc116 __errf5owr117 __errf7wks118) (body) (head (emit error (0) __erre43xc116)) <internal>:1 #f)
  ...
  // (crule (pre) (scan mpz_table_overflow __erre7sDc129 __errf88Yg130 __errf0GNv131) (body) (head (emit error (0) __erre7sDc129)) <internal>:1 #f)
  ...
  // (crule (pre (let __tconst3Kij40 const6b86b273ff34fce19d6b804e) (let __tconst0c6B49 const683276d23a1fc1d255583994)) (scan temp97vQ132 __t7GPl42) (body (exists ic (1 0) 1 __tconst3Kij40) (join _enum (1 0) 1 __tconst0c6B49 __t1kZ339) (join ic (1 0) 1 __tconst3Kij40 __t9wmU41) (let __t5Mpc43 (cins __t7GPl42 __t9wmU41))) (head (emit-temp temp3vjJ133 __t5Mpc43)) smt_err.slog:13 #f)
  class ReadTask14 : public slog::Task
  {
    ...
    slog::Index** icindex11;  slog::Index** _enumindex12;  slog::Index** icindex13;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("temp3vjJ133");
      outer_rel = db->getRelation("temp97vQ132");
      std::vector<u16> ord15({1, 0});
      slog::Relation* readrel16 = db->getRelation("ic");
      icindex11 = readrel16->getIndex(ord15, false);
      std::vector<u16> ord17({1, 0});
      slog::Relation* readrel18 = db->getRelation("_enum");
      _enumindex12 = readrel18->getIndex(ord17, false);
      std::vector<u16> ord19({1, 0});
      slog::Relation* readrel20 = db->getRelation("ic");
      icindex13 = readrel20->getIndex(ord19, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c11 = _t[0];
        if (!slog::exists_probe<2,1>(icindex11, std::array<u64,2>{v_c10, 0})) return;
        slog::join_probe<2,1>(_enumindex12, std::array<u64,2>{v_c0, 0}, [&](const std::array<u64,2>& m21) {
          u64 v_c2 = m21[1];
          slog::join_probe<2,1>(icindex13, std::array<u64,2>{v_c10, 0}, [&](const std::array<u64,2>& m22) {
            u64 v_c12 = m22[1];
            u64 v_c1 = _prim_cins(db, v_c11, v_c12);
            if (v_c1 == slog_error) { slog::emit_pending_error(db, "smt_err.slog:13"); return; }
            ++_fires;
      ...
      if (_fires) db->bumpFires("smt_err.slog:13", "delta:temp97vQ132", _fires);
  ...
  // (crule (pre) (scan div_by_zero __erre2K2Q102 __errf3QX8103 __errf9YPl104) (body) (head (emit error (0) __erre2K2Q102)) <internal>:1 #f)
  ...
  // (crule (pre) (scan int_overflow __erre6Ljl108 __errf3alZ109 __errf9Vt2110 __errf3HyB111) (body) (head (emit error (0) __erre6Ljl108)) <internal>:1 #f)
  ...
  // (crule (pre) (scan type_mismatch __erre86oG119 __errf9lPR120 __errf1U4W121 __errf7FM3122 __errf6qkI123) (body) (head (emit error (0) __erre86oG119)) <internal>:1 #f)
  ...
  // (crule (pre) (scan modulo_by_zero __erre6FsE105 __errf45Ts106 __errf6r0c107) (body) (head (emit error (0) __erre6FsE105)) <internal>:1 #f)
  ...
  // (crule (pre) (scan mpz_overflow __erre61dZ124 __errf6dqc125 __errf9pZS126 __errf0AxD127 __errf0LaI128) (body) (head (emit error (0) __erre61dZ124)) <internal>:1 #f)
  ...
  // (crule (pre) (scan temp5HiV135 __t4D7u44) (body (join lall (1 0) 1 __t4D7u44 __t4DwO45)) (head (mkstruct smt_check (1 0) __38AE48 __t4DwO45)) smt_err.slog:13 #f)
  class ReadTask39 : public slog::Task
  {
    ...
    slog::Index** lallindex38;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("smt_check");
      outer_rel = db->getRelation("temp5HiV135");
      std::vector<u16> ord40({1, 0});
      slog::Relation* readrel41 = db->getRelation("lall");
      lallindex38 = readrel41->getIndex(ord40, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c3 = _t[0];
        slog::join_probe<2,1>(lallindex38, std::array<u64,2>{v_c3, 0}, [&](const std::array<u64,2>& m42) {
          u64 v_c33 = m42[1];
          ++_fires;
      ...
      if (_fires) db->bumpFires("smt_err.slog:13", "delta:temp5HiV135", _fires);
  ...
  // (crule (pre) (scan nan_result __erre4P3K112 __errf3QHS113 __errf35hx114 __errf1XNE115) (body) (head (emit error (0) __erre4P3K112)) <internal>:1 #f)
  ...
  // (crule (pre (let __tconst0c6B49 const683276d23a1fc1d255583994) (let __tconst3Kij40 const6b86b273ff34fce19d6b804e) (let __t7GPl42 (cmap))) (probe _enum (1 0) 1 __tconst0c6B49 __t1kZ339) (body) (head (emit-temp temp97vQ132 __t7GPl42) (mkstruct ic (1 0) __t9wmU41 __tconst3Kij40)) smt_err.slog:13 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("temp97vQ132");
    ...
    virtual bool work()
    {
      ...
      if (v_c11 == slog_error) { slog::emit_pending_error(db, "smt_err.slog:13"); return true; }
      ...
      bool _done = slog::join_probe_sliced<2,1>(driver_index, std::array<u64,2>{v_c0, 0}, _sc, _rkey, _hr, [&](const std::array<u64,2>& m49) {
        u64 v_c2 = m49[1];
  ...
  // (crule (pre (let __tconst69NN55 consta221beeaebe583607e193be7) (let __tconst4OCO56 constab76c7d069d6f33dc950fb33) (let __tconst97Uy57 consteb8bf0d80db323992f6b634a) (let __tconst3Znq19 constd4735e3a265e16eee03f5971) (let __tconst1ds817 const6b86b273ff34fce19d6b804e) (let __tconst6Bwm15 const5feceb66ffc86f38d952786c)) (probe _enum (1 0) 1 __tconst69NN55 __t6p8A18) (body (exists _enum (1 0) 1 __tconst97Uy57) (join _enum (1 0) 1 __tconst4OCO56 __t9VX216) (join _enum (1 0) 1 __tconst97Uy57 __t9jZL14)) (head (emit smt_vcode (0 1) __tconst3Znq19 __t6p8A18) (emit smt_vcode (0 1) __tconst1ds817 __t9VX216) (emit smt_vcode (0 1) __tconst6Bwm15 __t9jZL14)) smt.slog:68 #f)
  class ReadTask53 : public slog::Task
  {
    ...
    std::array<u64,2> resume_key{};
    bool has_resume = false;
    slog::Index** _enumindex50;  slog::Index** _enumindex51;  slog::Index** _enumindex52;
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel55 = db->getRelation("smt_vcode");
      head_index[0] = readrel55->getIndex(ord54, false);
      head_rel[1] = db->getRelation("smt_vcode");
      std::vector<u16> ord56({0, 1});
      slog::Relation* readrel57 = db->getRelation("smt_vcode");
      head_index[1] = readrel57->getIndex(ord56, false);
      head_rel[2] = db->getRelation("smt_vcode");
      std::vector<u16> ord58({0, 1});
      slog::Relation* readrel59 = db->getRelation("smt_vcode");
      head_index[2] = readrel59->getIndex(ord58, false);
      std::vector<u16> ord60({1, 0});
      slog::Relation* readrel61 = db->getRelation("_enum");
      driver_index = readrel61->getIndex(ord60, true);
      std::vector<u16> ord62({1, 0});
      slog::Relation* readrel63 = db->getRelation("_enum");
      _enumindex50 = readrel63->getIndex(ord62, false);
      std::vector<u16> ord64({1, 0});
      slog::Relation* readrel65 = db->getRelation("_enum");
      _enumindex51 = readrel65->getIndex(ord64, false);
      std::vector<u16> ord66({1, 0});
      slog::Relation* readrel67 = db->getRelation("_enum");
      _enumindex52 = readrel67->getIndex(ord66, false);
  
    }
    ReadTask53(slog::Database* _db, u16 _b) : db(_db), bucket(_b) { bind(_db); }
    virtual bool work()
    {
      u64 v_c38 = v_consta221beeaebe583607e193be7;
      u64 v_c39 = v_constab76c7d069d6f33dc950fb33;
      u64 v_c40 = v_consteb8bf0d80db323992f6b634a;
      ...
      slog::SliceCtx _sc{_slice_deadline, &db->runStopFlag()};
      std::array<u64,2> _rkey = resume_key; bool _hr = has_resume;
      bool _done = slog::join_probe_sliced<2,1>(driver_index, std::array<u64,2>{v_c38, 0}, _sc, _rkey, _hr, [&](const std::array<u64,2>& m68) {
        u64 v_c44 = m68[1];
        if (buckethash(v_c44) != bucket) return;
        if (!slog::exists_probe<2,1>(_enumindex50, std::array<u64,2>{v_c40, 0})) return;
        slog::join_probe<2,1>(_enumindex51, std::array<u64,2>{v_c39, 0}, [&](const std::array<u64,2>& m69) {
          u64 v_c45 = m69[1];
          slog::join_probe<2,1>(_enumindex52, std::array<u64,2>{v_c40, 0}, [&](const std::array<u64,2>& m70) {
            u64 v_c46 = m70[1];
            ++_fires;
            slog::emit<2>(head_rel[0], head_index[0], newbatch[0], std::array<u64,2>{v_c41, v_c44}, std::array<u16,2>{0, 1});
            slog::emit<2>(head_rel[1], head_index[1], newbatch[1], std::array<u64,2>{v_c42, v_c45}, std::array<u16,2>{0, 1});
            slog::emit<2>(head_rel[2], head_index[2], newbatch[2], std::array<u64,2>{v_c43, v_c46}, std::array<u16,2>{0, 1});
          });
        });
      });
  ...
  // (crule (pre) (scan temp4yGW134 __t4D7u44) (body) (head (emit-temp temp5HiV135 __t4D7u44) (mkstruct lall (1 0) __t4DwO45 __t4D7u44)) smt_err.slog:13 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("temp5HiV135");
      head_rel[1] = db->getRelation("lall");
      outer_rel = db->getRelation("temp4yGW134");
    ...
    virtual bool work()
    {
      ...
      if (_fires) db->bumpFires("smt_err.slog:13", "delta:temp4yGW134", _fires);
  ...
  s->addReadRel("temp3vjJ133");
  s->addReadRel("temp4yGW134");
  s->addReadRel("temp5HiV135");
  s->addReadRel("temp97vQ132");
  ...
  s->addDynamicRel("temp3vjJ133");
  s->addDynamicRel("temp4yGW134");
  s->addDynamicRel("temp5HiV135");
  s->addDynamicRel("temp97vQ132");
