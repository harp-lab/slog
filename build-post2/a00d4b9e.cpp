...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("a00d4b9e");
  if (s == nullptr) return;
  slog::Relation* r;
  r = db->getRelation("unique");
  if (r == 0) db->addRelation("unique", 1);
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("unique"), std::array<u16,1>{0}, b));
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("step"), std::array<u16,2>{0, 1}, b));
  ...
  r = db->getRelation("mucnt");
  if (r == 0) db->addRelation("mucnt", 3);
  else if (r->getArity() != 3)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("mucnt");
  r->setLattice(LAT_COUNT, false, 0, false, 0, "count", db->collections());
    ...
    r->addMapIndex<3>(ord40);
    for (u16 b = 0; b < 32; ++b)
      s->addTask(phase_write, new slog::MapWriteTask<3>(db, r, std::array<u16,3>{0, 1, 2}, b), true);
  

  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::LatticeInternTask<3>(db, db->getRelation("mucnt"), std::array<u16,3>{0, 1, 2}, b));
  r = db->getRelation("mu");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("bind"), std::array<u16,2>{0, 1}, b));
  ...
  // (crule (pre) (scan toint_range __erre35AG126 __errf5ddD127 __errf8wUO128) (body) (head (emit error (0) __erre35AG126)) <internal>:1 #f)
  ...
  // (crule (pre) (scan type_mismatch __erre6qd0129 __errf6ESS130 __errf0Mto131 __errf1qKE132 __errf1fJ7133) (body) (head (emit error (0) __erre6qd0129)) <internal>:1 #f)
  ...
  // (crule (pre) (scan nan_result __erre17Sd122 __errf5EKt123 __errf67wb124 __errf9uXC125) (body) (head (emit error (0) __erre17Sd122)) <internal>:1 #f)
  ...
  // (crule (pre) (scan int_overflow __erre50Bj118 __errf5RQw119 __errf2eLI120 __errf6DQr121) (body) (head (emit error (0) __erre50Bj118)) <internal>:1 #f)
  ...
  // (crule (pre) (scan mpz_table_overflow __erre5Nor139 __errf8sqx140 __errf9PEA141) (body) (head (emit error (0) __erre5Nor139)) <internal>:1 #f)
  ...
  // (crule (pre) (scan modulo_by_zero __erre3yYo115 __errf7SAu116 __errf1xsD117) (body) (head (emit error (0) __erre3yYo115)) <internal>:1 #f)
  ...
  // (crule (pre (let __t8wCI2 (one))) (probe mu (1 0) 1 __t8wCI2 A) (body) (head (emit unique (0) A)) lat_count.slog:28 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("unique");
      std::vector<u16> ord19({0});
      slog::Relation* readrel20 = db->getRelation("unique");
      head_index[0] = readrel20->getIndex(ord19, false);
      std::vector<u16> ord21({1, 0});
      slog::Relation* readrel22 = db->getRelation("mu");
    ...
    virtual bool work()
    {
      ...
      if (v_c22 == slog_error) { slog::emit_pending_error(db, "lat_count.slog:28"); return true; }
      ...
      bool _done = slog::join_probe_sliced<2,1>(driver_index, std::array<u64,2>{v_c22, 0}, _sc, _rkey, _hr, [&](const std::array<u64,2>& m23) {
        u64 v_c23 = m23[1];
      ...
      if (_fires) db->bumpFires("lat_count.slog:28", "all:mu", _fires);
  ...
  // (crule (pre) (scan mpz_overflow __erre1ma9134 __errf8iov135 __errf7icY136 __errf799d137 __errf29p8138) (body) (head (emit error (0) __erre1ma9134)) <internal>:1 #f)
  ...
  // (crule (pre) (scan div_by_zero __erre6S3z112 __errf4WFG113 __errf7Fuo114) (body) (head (emit error (0) __erre6S3z112)) <internal>:1 #f)
