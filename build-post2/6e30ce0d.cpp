...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("6e30ce0d");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  // (crule (pre) (scan edge X Y) (body (absent blocked (0) 1 Y)) (head (emit reach (0 1) X Y)) neg_reach.slog:13 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel3 = db->getRelation("reach");
      head_index[0] = readrel3->getIndex(ord2, false);
      outer_rel = db->getRelation("edge");
      std::vector<u16> ord4({0});
      slog::Relation* readrel5 = db->getRelation("blocked");
      blockedindex0 = readrel5->getIndex(ord4, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        u64 v_c1 = _t[1];
        if (!slog::absent_probe<1,1>(blockedindex0, std::array<u64,1>{v_c1})) return;
        ++_fires;
      ...
      if (_fires) db->bumpFires("neg_reach.slog:13", "all:edge", _fires);
  ...
  // (crule (pre) (scan reach X Y) (body (join edge (0 1) 1 Y Z) (absent blocked (0) 1 Z)) (head (emit reach (0 1) X Z)) neg_reach.slog:19 #f)
  class ReadTask8 : public slog::Task
  {
    ...
    slog::Index** edgeindex6;  slog::Index** blockedindex7;
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel10 = db->getRelation("reach");
      ...
      edgeindex6 = readrel12->getIndex(ord11, false);
      std::vector<u16> ord13({0});
      slog::Relation* readrel14 = db->getRelation("blocked");
      blockedindex7 = readrel14->getIndex(ord13, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        u64 v_c1 = _t[1];
        slog::join_probe<2,1>(edgeindex6, std::array<u64,2>{v_c1, 0}, [&](const std::array<u64,2>& m15) {
          u64 v_c2 = m15[1];
          if (!slog::absent_probe<1,1>(blockedindex7, std::array<u64,1>{v_c2})) return;
          ++_fires;
      ...
      if (_fires) db->bumpFires("neg_reach.slog:19", "delta:reach", _fires);
