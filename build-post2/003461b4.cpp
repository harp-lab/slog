...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("003461b4");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  r = db->getRelation("temp59EG12");
  if (r == 0) db->addTempRelation("temp59EG12", 2);
  else if (r->getArity() != 2) slog::fatal("Temp rel arity mismatch.");
  r = db->getRelation("temp2rkR11");
  if (r == 0) db->addTempRelation("temp2rkR11", 2);
  else if (r->getArity() != 2) slog::fatal("Temp rel arity mismatch.");
  ...
  // (crule (pre) (scan temp2rkR11 X Y) (body (join pt (1 2 0) 2 X Y __t7UOY1)) (head (emit s2 (0) __t7UOY1)) sdeep_dedup.slog:22 #f)
  ...
  public:
    ...
    {
      head_rel[0] = db->getRelation("s2");
      std::vector<u16> ord2({0});
      slog::Relation* readrel3 = db->getRelation("s2");
      head_index[0] = readrel3->getIndex(ord2, false);
      outer_rel = db->getRelation("temp2rkR11");
      std::vector<u16> ord4({1, 2, 0});
      slog::Relation* readrel5 = db->getRelation("pt");
      ptindex0 = readrel5->getIndex(ord4, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        u64 v_c1 = _t[1];
        slog::join_probe<3,2>(ptindex0, std::array<u64,3>{v_c0, v_c1, 0}, [&](const std::array<u64,3>& m6) {
          u64 v_c2 = m6[2];
          ++_fires;
      ...
      if (_fires) db->bumpFires("sdeep_dedup.slog:22", "delta:temp2rkR11", _fires);
  ...
  // (crule (pre) (scan temp59EG12 X Y) (body (join pt (1 2 0) 2 X Y __t0YDz2)) (head (emit s1 (0) __t0YDz2)) sdeep_dedup.slog:21 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("s1");
      std::vector<u16> ord9({0});
      slog::Relation* readrel10 = db->getRelation("s1");
      head_index[0] = readrel10->getIndex(ord9, false);
      outer_rel = db->getRelation("temp59EG12");
      std::vector<u16> ord11({1, 2, 0});
      slog::Relation* readrel12 = db->getRelation("pt");
      ptindex7 = readrel12->getIndex(ord11, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        u64 v_c1 = _t[1];
        slog::join_probe<3,2>(ptindex7, std::array<u64,3>{v_c0, v_c1, 0}, [&](const std::array<u64,3>& m13) {
          u64 v_c3 = m13[2];
          ++_fires;
      ...
      if (_fires) db->bumpFires("sdeep_dedup.slog:21", "delta:temp59EG12", _fires);
  ...
  // (crule (pre) (scan a X Y) (body) (head (emit-temp temp59EG12 X Y) (mkstruct pt (1 2 0) __t0YDz2 X Y)) sdeep_dedup.slog:21 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("temp59EG12");
    ...
    virtual bool work()
    {
      ...
      if (_fires) db->bumpFires("sdeep_dedup.slog:21", "all:a", _fires);
  ...
  // (crule (pre) (scan b X Y) (body) (head (emit-temp temp2rkR11 X Y) (mkstruct pt (1 2 0) __t7UOY1 X Y)) sdeep_dedup.slog:22 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("temp2rkR11");
      head_rel[1] = db->getRelation("pt");
    ...
    virtual bool work()
    {
      ...
      if (_fires) db->bumpFires("sdeep_dedup.slog:22", "all:b", _fires);
  ...
  s->addReadRel("b");
  s->addReadRel("pt");
  s->addReadRel("temp2rkR11");
  s->addReadRel("temp59EG12");
  s->addDynamicRel("pt");
  s->addDynamicRel("s1");
  s->addDynamicRel("s2");
  s->addDynamicRel("temp2rkR11");
  s->addDynamicRel("temp59EG12");
  d->push(s);
  d->continueRun();
}
