...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("4c7b72d7");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  if (r == 0) db->addRelation("$sup56712x93x0x0x0", 5);
    ...
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("$sup56712x93x0x0x0");
      std::vector<u16> ord129({0, 1, 2, 3, 4});
    
    r->addIndex<5>(ord129, false);
    for (u16 b = 0; b < 32; ++b)
  ...
  if (r == 0) db->addRelation("$sup56712x91x0x0x0", 5);
    ...
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("$sup56712x91x0x0x0");
      std::vector<u16> ord130({0, 1, 2, 3, 4});
  ...
  class ReadTask2 : public slog::Task
  {
    ...
    slog::Index** unique_bindingindex0;  slog::Index** bindingindex1;
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel4 = db->getRelation("must_value");
      head_index[0] = readrel4->getIndex(ord3, false);
      outer_rel = db->getRelation("singleton_flow");
      std::vector<u16> ord5({0, 1});
      slog::Relation* readrel6 = db->getRelation("unique_binding");
      unique_bindingindex0 = readrel6->getIndex(ord5, false);
      std::vector<u16> ord7({0, 1, 2});
      slog::Relation* readrel8 = db->getRelation("binding");
      bindingindex1 = readrel8->getIndex(ord7, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        u64 v_c1 = _t[1];
        slog::join_probe<2,2>(unique_bindingindex0, std::array<u64,2>{v_c0, v_c1}, [&](const std::array<u64,2>& m9) {
          slog::join_probe<3,2>(bindingindex1, std::array<u64,3>{v_c0, v_c1, 0}, [&](const std::array<u64,3>& m10) {
            u64 v_c2 = m10[2];
            ++_fires;
