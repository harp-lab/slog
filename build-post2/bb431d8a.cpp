...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("bb431d8a");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  r = db->getRelation("cyc4");
  if (r == 0) db->addRelation("cyc4", 4);
  else if (r->getArity() != 4)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("cyc4");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<4>(db, db->getRelation("cyc4"), std::array<u16,4>{0, 1, 2, 3}, b));
  ...
  // (crule (pre) (scan edge D A) (body (exists edge (0 1) 1 A) (join edge (1 0) 1 D C) (join3 B (full edge (1 0) 1 () C B) (full edge (0 1) 1 () A B))) (head (emit cyc4 (0 1 2 3) A B C D)) sj_cyc4.slog:17 #f)
  class ReadTask4 : public slog::Task
  {
    ...
    slog::Index** edgeindex0;  slog::Index** edgeindex1;  slog::Index** edgeindex2;  slog::Index** edgeindex3;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("cyc4");
      std::vector<u16> ord5({0, 1, 2, 3});
      slog::Relation* readrel6 = db->getRelation("cyc4");
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        u64 v_c1 = _t[1];
        if (!slog::exists_probe<2,1>(edgeindex0, std::array<u64,2>{v_c1, 0})) return;
        slog::join_probe<2,1>(edgeindex1, std::array<u64,2>{v_c0, 0}, [&](const std::array<u64,2>& m15) {
          u64 v_c2 = m15[1];
          slog::join3<2,1,slog::Join3View::full,2,1,slog::Join3View::full>(edgeindex2, edgeindex2, std::array<u64,2>{v_c2, 0}, edgeindex3, edgeindex3, std::array<u64,2>{v_c1, 0}, [&](u64 cycle16) {
          u64 v_c3 = cycle16;
            ++_fires;
            slog::emit<4>(head_rel[0], head_index[0], newbatch[0], std::array<u64,4>{v_c1, v_c3, v_c2, v_c0}, std::array<u16,4>{0, 1, 2, 3});
          });
        });
      });
      ...
      if (_fires) db->bumpFires("sj_cyc4.slog:17", "all:edge", _fires);
