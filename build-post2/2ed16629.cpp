...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("2ed16629");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  r = db->getRelation("temp34LV6");
  if (r == 0) db->addTempRelation("temp34LV6", 2);
  else if (r->getArity() != 2) slog::fatal("Temp rel arity mismatch.");
  r = db->getRelation("t4");
  if (r == 0) db->addRelation("t4", 3);
  else if (r->getArity() != 3)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("t4");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<3>(db, db->getRelation("t4"), std::array<u16,3>{0, 1, 2}, b));
  r = db->getRelation("t3");
  if (r == 0) db->addRelation("t3", 1);
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("t3"), std::array<u16,1>{0}, b));
  r = db->getRelation("t2");
  if (r == 0) db->addRelation("t2", 2);
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("t2"), std::array<u16,2>{0, 1}, b));
  r = db->getRelation("t1");
  if (r == 0) db->addRelation("t1", 1);
  else if (r->getArity() != 1)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("t1");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("t1"), std::array<u16,1>{0}, b));
  r = db->getRelation("src");
  if (r == 0) db->addRelation("src", 2);
  else if (r->getArity() != 2)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("src");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("src"), std::array<u16,2>{0, 1}, b));
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternStructTask<3>(db, db->getRelation("pr"), std::array<u16,3>{1, 2, 0}, b));
  ...
  // (crule (pre) (scan src X Y) (body) (head (emit-temp temp34LV6 X Y) (emit t4 (0 1 2) X Y X) (mkstruct pr (1 2 0) __t9bZf1 X Y) (emit t2 (0 1) Y X) (emit t1 (0) X)) shape_multihead.slog:23 #f)
  ...
  public:
    ...
    {
      head_rel[0] = db->getRelation("temp34LV6");
      head_rel[1] = db->getRelation("t4");
      std::vector<u16> ord1({0, 1, 2});
      slog::Relation* readrel2 = db->getRelation("t4");
      head_index[1] = readrel2->getIndex(ord1, false);
      head_rel[2] = db->getRelation("pr");
      head_rel[3] = db->getRelation("t2");
      std::vector<u16> ord3({0, 1});
      slog::Relation* readrel4 = db->getRelation("t2");
      head_index[3] = readrel4->getIndex(ord3, false);
      head_rel[4] = db->getRelation("t1");
      std::vector<u16> ord5({0});
      slog::Relation* readrel6 = db->getRelation("t1");
      head_index[4] = readrel6->getIndex(ord5, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        ...
        slog::emit<3>(head_rel[1], head_index[1], newbatch[1], std::array<u64,3>{v_c0, v_c1, v_c0}, std::array<u16,3>{0, 1, 2});
        slog::emit_struct<3>(head_rel[2], newbatch[2], std::array<u64,2>{v_c0, v_c1}, std::array<u16,3>{1, 2, 0});
        slog::emit<2>(head_rel[3], head_index[3], newbatch[3], std::array<u64,2>{v_c1, v_c0}, std::array<u16,2>{0, 1});
        slog::emit<1>(head_rel[4], head_index[4], newbatch[4], std::array<u64,1>{v_c0}, std::array<u16,1>{0});
      });
      ...
      if (_fires) db->bumpFires("shape_multihead.slog:23", "all:src", _fires);
  ...
  // (crule (pre) (scan temp34LV6 X Y) (body (join pr (1 2 0) 2 X Y __t9bZf1)) (head (emit t3 (0) __t9bZf1)) shape_multihead.slog:23 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("t3");
      std::vector<u16> ord9({0});
      slog::Relation* readrel10 = db->getRelation("t3");
      head_index[0] = readrel10->getIndex(ord9, false);
      outer_rel = db->getRelation("temp34LV6");
      std::vector<u16> ord11({1, 2, 0});
      slog::Relation* readrel12 = db->getRelation("pr");
      prindex7 = readrel12->getIndex(ord11, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        u64 v_c1 = _t[1];
        slog::join_probe<3,2>(prindex7, std::array<u64,3>{v_c0, v_c1, 0}, [&](const std::array<u64,3>& m13) {
          u64 v_c2 = m13[2];
          ++_fires;
      ...
      if (_fires) db->bumpFires("shape_multihead.slog:23", "delta:temp34LV6", _fires);
  ...
  s->addReadRel("pr");
  s->addReadRel("src");
  s->addReadRel("temp34LV6");
  s->addDynamicRel("pr");
  s->addDynamicRel("t1");
  s->addDynamicRel("t2");
  s->addDynamicRel("t3");
  s->addDynamicRel("t4");
  s->addDynamicRel("temp34LV6");
  d->push(s);
  d->continueRun();
}
