...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("2c69ff51");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  r = db->getRelation("temp0qfw129");
  if (r == 0) db->addTempRelation("temp0qfw129", 3);
  ...
  // (crule (pre) (scan modulo_by_zero __erre239C102 __errf9Pfd103 __errf43ik104) (body) (head (emit error (0) __erre239C102)) <internal>:1 #f)
  ...
  // (crule (pre) (scan type_mismatch __erre9yMW116 __errf3bbU117 __errf1fyQ118 __errf0x4K119 __errf9TuW120) (body) (head (emit error (0) __erre9yMW116)) <internal>:1 #f)
  ...
  // (crule (pre) (scan nan_result __erre5u73109 __errf9t7s110 __errf9oLR111 __errf6y4p112) (body) (head (emit error (0) __erre5u73109)) <internal>:1 #f)
  ...
  // (crule (pre) (scan toint_range __erre7MVm113 __errf6fIq114 __errf3NPH115) (body) (head (emit error (0) __erre7MVm113)) <internal>:1 #f)
  ...
  // (crule (pre) (scan temp0qfw129 X Z __t4jLP5) (body) (head (emit-lat dist X Z __t4jLP5)) lat_negcycle.slog:16 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("dist");
      outer_rel = db->getRelation("temp0qfw129");
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        ...
        u64 v_c17 = _t[2];
        ++_fires;
        slog::emit_temp<3>(head_rel[0], newbatch[0], std::array<u64,3>{v_c15, v_c16, v_c17});
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("lat_negcycle.slog:16", "delta:temp0qfw129", _fires);
  ...
  // (crule (pre) (scan int_overflow __erre9dVu105 __errf4UhB106 __errf3qea107 __errf7rRk108) (body) (head (emit error (0) __erre9dVu105)) <internal>:1 #f)
  ...
  // (crule (pre) (scan div_by_zero __erre7OFn99 __errf9p3a100 __errf7U2Z101) (body) (head (emit error (0) __erre7OFn99)) <internal>:1 #f)
  ...
  // (crule (pre) (scan mpz_overflow __erre2njB121 __errf1laC122 __errf9Kh0123 __errf9zVe124 __errf1y84125) (body) (head (emit error (0) __erre2njB121)) <internal>:1 #f)
  ...
  // (crule (pre) (scan edge X Y W) (body) (head (emit-lat dist X Y W)) lat_negcycle.slog:15 #f)
  ...
  public:
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        ...
        u64 v_c31 = _t[2];
        ++_fires;
        slog::emit_temp<3>(head_rel[0], newbatch[0], std::array<u64,3>{v_c15, v_c30, v_c31});
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("lat_negcycle.slog:15", "all:edge", _fires);
  ...
  // (crule (pre) (scan dist X Y D) (body (join edge (0 1 2) 1 Y Z W) (let __t4jLP5 (_0002b D W))) (head (emit-temp temp0qfw129 X Z __t4jLP5)) lat_negcycle.slog:16 #f)
  class ReadTask24 : public slog::Task
  {
    ...
    slog::Index** edgeindex23;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("temp0qfw129");
      outer_rel = db->getRelation("dist");
      std::vector<u16> ord25({0, 1, 2});
      slog::Relation* readrel26 = db->getRelation("edge");
      edgeindex23 = readrel26->getIndex(ord25, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        ...
        u64 v_c30 = _t[1];
        u64 v_c32 = _t[2];
        slog::join_probe<3,1>(edgeindex23, std::array<u64,3>{v_c30, 0, 0}, [&](const std::array<u64,3>& m27) {
          u64 v_c16 = m27[1]; u64 v_c31 = m27[2];
          u64 v_c17 = _prim__0002b(db, v_c32, v_c31);
          if (v_c17 == slog_error) { slog::emit_pending_error(db, "lat_negcycle.slog:16"); return; }
          ++_fires;
          slog::emit_temp<3>(head_rel[0], newbatch[0], std::array<u64,3>{v_c15, v_c16, v_c17});
        });
      });
      ...
      if (_fires) db->bumpFires("lat_negcycle.slog:16", "delta:dist", _fires);
  ...
  // (crule (pre) (scan mpz_table_overflow __erre4tkh126 __errf04Ew127 __errf4QpK128) (body) (head (emit error (0) __erre4tkh126)) <internal>:1 #f)
  ...
  s->addReadRel("temp0qfw129");
  s->addReadRel("toint_range");
  s->addReadRel("type_mismatch");
  s->addDynamicRel("dist");
