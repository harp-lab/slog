...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("da03c61a");
  if (s == nullptr) return;
  slog::Relation* r;
  r = db->getRelation("unbox");
  if (r == 0) db->addRelation("unbox", 1);
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("unbox"), std::array<u16,1>{0}, b));
  ...
  r = db->getRelation("temp6Y6R68");
  if (r == 0) db->addTempRelation("temp6Y6R68", 1);
  ...
  r = db->getRelation("holds");
  if (r == 0) db->addRelation("holds", 1);
  else if (r->getArity() != 1)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("holds");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("holds"), std::array<u16,1>{0}, b));
  ...
  // (crule (pre) (scan n X) (body) (head (emit-temp temp6Y6R68 X) (mkstruct box (1 0) __t0YwK6 X)) sdeep_strata.slog:19 #f)
  ...
  public:
    ...
    {
      head_rel[0] = db->getRelation("temp6Y6R68");
      head_rel[1] = db->getRelation("box");
    ...
    virtual bool work()
    {
      ...
      if (_fires) db->bumpFires("sdeep_strata.slog:19", "all:n", _fires);
  ...
  // (crule (pre) (scan temp6Y6R68 X) (body (join box (1 0) 1 X __t0YwK6)) (head (emit holds (0) __t0YwK6)) sdeep_strata.slog:19 #f)
  class ReadTask2 : public slog::Task
  {
    ...
    slog::Index** boxindex1;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("holds");
      std::vector<u16> ord3({0});
      slog::Relation* readrel4 = db->getRelation("holds");
      head_index[0] = readrel4->getIndex(ord3, false);
      outer_rel = db->getRelation("temp6Y6R68");
      std::vector<u16> ord5({1, 0});
      slog::Relation* readrel6 = db->getRelation("box");
      boxindex1 = readrel6->getIndex(ord5, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        slog::join_probe<2,1>(boxindex1, std::array<u64,2>{v_c0, 0}, [&](const std::array<u64,2>& m7) {
          u64 v_c1 = m7[1];
          ++_fires;
      ...
      if (_fires) db->bumpFires("sdeep_strata.slog:19", "delta:temp6Y6R68", _fires);
  ...
  s->addReadRel("box");
  s->addReadRel("n");
  s->addReadRel("temp6Y6R68");
  s->addDynamicRel("box");
  s->addDynamicRel("holds");
  s->addDynamicRel("temp6Y6R68");
  d->push(s);
  d->continueRun();
}
