...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("51057b4e");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  r = db->getRelation("temp9uLS572");
  if (r == 0) db->addTempRelation("temp9uLS572", 2);
  else if (r->getArity() != 2) slog::fatal("Temp rel arity mismatch.");
  r = db->getRelation("temp5zLw571");
  if (r == 0) db->addTempRelation("temp5zLw571", 3);
  ...
  // (crule (pre) (scan modulo_by_zero __erre6NpJ544 __errf4ouG545 __errf570M546) (body) (head (emit error (0) __erre6NpJ544)) <internal>:1 #f)
  ...
  // (crule (pre) (scan temp5zLw571 __t6Z6a79 __t9C7a77 acc) (body (let __t9eiK78 (lcat __t9C7a77 acc))) (head (emit-temp temp9uLS572 __t6Z6a79 __t9eiK78)) list.slog:42 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("temp9uLS572");
      outer_rel = db->getRelation("temp5zLw571");
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        ...
        u64 v_c4 = _t[1];
        u64 v_c5 = _t[2];
        u64 v_c6 = _prim_lcat(db, v_c4, v_c5);
        if (v_c6 == slog_error) { slog::emit_pending_error(db, "list.slog:42"); return; }
        ++_fires;
        slog::emit_temp<2>(head_rel[0], newbatch[0], std::array<u64,2>{v_c3, v_c6});
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("list.slog:42", "delta:temp5zLw571", _fires);
  ...
  // (crule (pre) (scan mpz_table_overflow __erre5HdB568 __errf1bVg569 __errf6zgk570) (body) (head (emit error (0) __erre5HdB568)) <internal>:1 #f)
  ...
  // (crule (pre) (scan probe k) (body (join seed (0 1) 0 i l)) (head (emit $sup76318x53x0x0x0 (0 1 2) i k l)) seq_oracle.slog:54 #f)
  class ReadTask8 : public slog::Task
  {
    ...
    slog::Index** seedindex7;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("$sup76318x53x0x0x0");
      std::vector<u16> ord9({0, 1, 2});
      slog::Relation* readrel10 = db->getRelation("$sup76318x53x0x0x0");
      head_index[0] = readrel10->getIndex(ord9, false);
      outer_rel = db->getRelation("probe");
      std::vector<u16> ord11({0, 1});
      slog::Relation* readrel12 = db->getRelation("seed");
      seedindex7 = readrel12->getIndex(ord11, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c10 = _t[0];
        slog::join_all<2>(seedindex7, [&](const std::array<u64,2>& m13) {
          u64 v_c11 = m13[0]; u64 v_c12 = m13[1];
          ++_fires;
          slog::emit<3>(head_rel[0], head_index[0], newbatch[0], std::array<u64,3>{v_c11, v_c10, v_c12}, std::array<u16,3>{0, 1, 2});
        });
      });
  ...
  // (crule (pre) (scan seed i l) (body) (head (mkstruct lst_rev (1 0) __0In2357 l)) seq_oracle.slog:59 #f)
  ...
  // (crule (pre) (scan int_overflow __erre64Y5547 __errf1I9S548 __errf02Ap549 __errf9e0x550) (body) (head (emit error (0) __erre64Y5547)) <internal>:1 #f)
  ...
  // (crule (pre) (scan temp9uLS572 __t6Z6a79 __t9eiK78) (body) (head (emit lst_rev_acc_ans (0 1) __t6Z6a79 __t9eiK78)) list.slog:42 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel20 = db->getRelation("lst_rev_acc_ans");
      head_index[0] = readrel20->getIndex(ord19, false);
      outer_rel = db->getRelation("temp9uLS572");
    ...
    virtual bool work()
    {
      ...
      if (_fires) db->bumpFires("list.slog:42", "delta:temp9uLS572", _fires);
  ...
  // (crule (pre) (scan needle y) (body (join seed (0 1) 0 i l)) (head (emit $sup76318x38x0x0x0 (0 1 2) i l y)) seq_oracle.slog:39 #f)
  class ReadTask22 : public slog::Task
  {
    ...
    slog::Index** seedindex21;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("$sup76318x38x0x0x0");
      std::vector<u16> ord23({0, 1, 2});
      slog::Relation* readrel24 = db->getRelation("$sup76318x38x0x0x0");
      head_index[0] = readrel24->getIndex(ord23, false);
      outer_rel = db->getRelation("needle");
      std::vector<u16> ord25({0, 1});
      slog::Relation* readrel26 = db->getRelation("seed");
      seedindex21 = readrel26->getIndex(ord25, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c17 = _t[0];
        slog::join_all<2>(seedindex21, [&](const std::array<u64,2>& m27) {
          u64 v_c11 = m27[0]; u64 v_c12 = m27[1];
          ++_fires;
          slog::emit<3>(head_rel[0], head_index[0], newbatch[0], std::array<u64,3>{v_c11, v_c12, v_c17}, std::array<u16,3>{0, 1, 2});
        });
      });
  ...
  // (crule (pre) (scan mpz_overflow __erre7YHr563 __errf3hPg564 __errf4raL565 __errf42T9566 __errf0boH567) (body) (head (emit error (0) __erre7YHr563)) <internal>:1 #f)
  ...
  // (crule (pre) (scan needle y) (body (join seed (0 1) 0 i l)) (head (emit $sup76318x43x0x0x0 (0 1 2) i l y)) seq_oracle.slog:44 #f)
  class ReadTask32 : public slog::Task
  {
    ...
    slog::Index** seedindex31;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("$sup76318x43x0x0x0");
      std::vector<u16> ord33({0, 1, 2});
      slog::Relation* readrel34 = db->getRelation("$sup76318x43x0x0x0");
      head_index[0] = readrel34->getIndex(ord33, false);
      outer_rel = db->getRelation("needle");
      std::vector<u16> ord35({0, 1});
      slog::Relation* readrel36 = db->getRelation("seed");
      seedindex31 = readrel36->getIndex(ord35, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c17 = _t[0];
        slog::join_all<2>(seedindex31, [&](const std::array<u64,2>& m37) {
          u64 v_c11 = m37[0]; u64 v_c12 = m37[1];
          ++_fires;
          slog::emit<3>(head_rel[0], head_index[0], newbatch[0], std::array<u64,3>{v_c11, v_c12, v_c17}, std::array<u16,3>{0, 1, 2});
        });
      });
  ...
  // (crule (pre) (scan seed i l) (body) (head (mkstruct lst_len (1 0) __8EFp301 l)) seq_oracle.slog:49 #f)
  ...
  // (crule (pre) (scan probe k) (body (join seed (0 1) 0 i l)) (head (emit $sup76318x33x0x0x0 (0 1 2) i k l)) seq_oracle.slog:34 #f)
  class ReadTask40 : public slog::Task
  {
    ...
    slog::Index** seedindex39;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("$sup76318x33x0x0x0");
      std::vector<u16> ord41({0, 1, 2});
      slog::Relation* readrel42 = db->getRelation("$sup76318x33x0x0x0");
      head_index[0] = readrel42->getIndex(ord41, false);
      outer_rel = db->getRelation("probe");
      std::vector<u16> ord43({0, 1});
      slog::Relation* readrel44 = db->getRelation("seed");
      seedindex39 = readrel44->getIndex(ord43, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c10 = _t[0];
        slog::join_all<2>(seedindex39, [&](const std::array<u64,2>& m45) {
          u64 v_c11 = m45[0]; u64 v_c12 = m45[1];
          ++_fires;
          slog::emit<3>(head_rel[0], head_index[0], newbatch[0], std::array<u64,3>{v_c11, v_c10, v_c12}, std::array<u16,3>{0, 1, 2});
        });
      });
  ...
  // (crule (pre) (scan seed j b) (body (join seed (0 1) 0 i a)) (head (emit $sup76318x27x0x0x0 (0 1 2 3) a b i j)) seq_oracle.slog:28 #f)
  class ReadTask47 : public slog::Task
  {
    ...
    slog::Index** seedindex46;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("$sup76318x27x0x0x0");
      std::vector<u16> ord48({0, 1, 2, 3});
      slog::Relation* readrel49 = db->getRelation("$sup76318x27x0x0x0");
      head_index[0] = readrel49->getIndex(ord48, false);
      outer_rel = db->getRelation("seed");
      std::vector<u16> ord50({0, 1});
      slog::Relation* readrel51 = db->getRelation("seed");
      seedindex46 = readrel51->getIndex(ord50, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c23 = _t[0];
        u64 v_c24 = _t[1];
        slog::join_all<2>(seedindex46, [&](const std::array<u64,2>& m52) {
          u64 v_c11 = m52[0]; u64 v_c25 = m52[1];
          ++_fires;
          slog::emit<4>(head_rel[0], head_index[0], newbatch[0], std::array<u64,4>{v_c25, v_c24, v_c11, v_c23}, std::array<u16,4>{0, 1, 2, 3});
        });
      });
  ...
  // (crule (pre) (scan div_by_zero __erre16C4541 __errf2yF9542 __errf5tj9543) (body) (head (emit error (0) __erre16C4541)) <internal>:1 #f)
  ...
  // (crule (pre) (scan type_mismatch __erre50jC558 __errf31FE559 __errf1eoQ560 __errf1bH0561 __errf4HZI562) (body) (head (emit error (0) __erre50jC558)) <internal>:1 #f)
  ...
  // (crule (pre) (scan toint_range __erre8HnV555 __errf2yPc556 __errf6aNA557) (body) (head (emit error (0) __erre8HnV555)) <internal>:1 #f)
  ...
  // (crule (pre) (scan lst_rev_acc __t6Z6a79 l acc) (body (let __t9C7a77 (lrev l))) (head (emit-temp temp5zLw571 __t6Z6a79 __t9C7a77 acc)) list.slog:42 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("temp5zLw571");
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        ...
        u64 v_c12 = _t[1];
        u64 v_c5 = _t[2];
        u64 v_c4 = _prim_lrev(db, v_c12);
        if (v_c4 == slog_error) { slog::emit_pending_error(db, "list.slog:42"); return; }
        ++_fires;
        slog::emit_temp<3>(head_rel[0], newbatch[0], std::array<u64,3>{v_c3, v_c4, v_c5});
      });
  ...
  // (crule (pre) (scan nan_result __erre922R551 __errf8uBs552 __errf9Wk7553 __errf6uQJ554) (body) (head (emit error (0) __erre922R551)) <internal>:1 #f)
  ...
  s->addReadRel("probe");
  s->addReadRel("seed");
  s->addReadRel("temp5zLw571");
  s->addReadRel("temp9uLS572");
  s->addReadRel("toint_range");
  s->addReadRel("type_mismatch");
  s->addDynamicRel("$seq_at");
  s->addDynamicRel("$sup76318x27x0x0x0");
  s->addDynamicRel("$sup76318x33x0x0x0");
  s->addDynamicRel("$sup76318x38x0x0x0");
  s->addDynamicRel("$sup76318x43x0x0x0");
  s->addDynamicRel("$sup76318x53x0x0x0");
  ...
  s->addDynamicRel("temp5zLw571");
  s->addDynamicRel("temp9uLS572");
