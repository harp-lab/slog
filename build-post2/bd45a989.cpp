...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("bd45a989");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  // (crule (pre) (scan input l) (body (join sum (1 0) 1 l __t0uSN27) (join sum_ans (0 1) 1 __t0uSN27 r)) (head (emit total (0) r)) lst_typed.slog:20 #f)
  class ReadTask2 : public slog::Task
  {
    ...
    slog::Index** sumindex0;  slog::Index** sum_ansindex1;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("total");
      std::vector<u16> ord3({0});
      slog::Relation* readrel4 = db->getRelation("total");
      head_index[0] = readrel4->getIndex(ord3, false);
      outer_rel = db->getRelation("input");
      std::vector<u16> ord5({1, 0});
      slog::Relation* readrel6 = db->getRelation("sum");
      sumindex0 = readrel6->getIndex(ord5, false);
      std::vector<u16> ord7({0, 1});
      slog::Relation* readrel8 = db->getRelation("sum_ans");
      sum_ansindex1 = readrel8->getIndex(ord7, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        slog::join_probe<2,1>(sumindex0, std::array<u64,2>{v_c0, 0}, [&](const std::array<u64,2>& m9) {
          u64 v_c1 = m9[1];
          slog::join_probe<2,1>(sum_ansindex1, std::array<u64,2>{v_c1, 0}, [&](const std::array<u64,2>& m10) {
            u64 v_c2 = m10[1];
            ++_fires;
