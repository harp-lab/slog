...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("2cf22f51");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  // (crule (pre) (scan k X) (body (absent-lat best (0 1) 1 X)) (head (emit nobest (0) X)) neg_lat.slog:13 #f)
  class ReadTask1 : public slog::Task
  {
    ...
    slog::Index** bestindex0;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("nobest");
      std::vector<u16> ord2({0});
      slog::Relation* readrel3 = db->getRelation("nobest");
      head_index[0] = readrel3->getIndex(ord2, false);
      outer_rel = db->getRelation("k");
      std::vector<u16> ord4({0, 1});
      slog::Relation* readrel5 = db->getRelation("best");
      bestindex0 = readrel5->getIndex(ord4, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        if (!slog::absent_probe_lat<1,1>(bestindex0, std::array<u64,1>{v_c0})) return;
        ++_fires;
      ...
      if (_fires) db->bumpFires("neg_lat.slog:13", "all:k", _fires);
