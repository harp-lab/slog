...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("3b83d911");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  r = db->getRelation("temp98xv156");
  if (r == 0) db->addTempRelation("temp98xv156", 1);
  else if (r->getArity() != 1) slog::fatal("Temp rel arity mismatch.");
  r = db->getRelation("temp1A1G157");
  if (r == 0) db->addTempRelation("temp1A1G157", 1);
  else if (r->getArity() != 1) slog::fatal("Temp rel arity mismatch.");
  r = db->getRelation("temp18Hd158");
  if (r == 0) db->addTempRelation("temp18Hd158", 1);
  else if (r->getArity() != 1) slog::fatal("Temp rel arity mismatch.");
  ...
  // (crule (pre (let __tconst5vhP11 const43e6e2bebb3774311cf0c867) (let __tconst0ig910 const5feceb66ffc86f38d952786c)) (once) (body) (head (mkstruct parse (1 2 0) __2OZs95 __tconst5vhP11 __tconst0ig910)) dem_parse.slog:28 #f)
  ...
  public:
    ...
    {
      head_rel[0] = db->getRelation("parse");
  
    }
    ReadTask0(slog::Database* _db, u16 _b) : db(_db), bucket(_b) { bind(_db); }
    virtual bool work()
    {
      u64 v_c0 = v_const43e6e2bebb3774311cf0c867;
      ...
      if (_fires) db->bumpFires("dem_parse.slog:28", "once", _fires);
  ...
  // (crule (pre (let __tconst2S1X84 const57cb45de78dd46c657524688) (let __tconst2Ly286 consted725292f5f32d61535958c1) (let __tconst0utE81 const6b86b273ff34fce19d6b804e) (let __tconst0p0n83 const66804d12743756eb85fb3115)) (probe parse (1 0 2) 1 __tconst2S1X84 __t2xd285 i) (body (join tok (1 0) 1 __tconst2Ly286 j) (let __t5PJH82 (_0002b j __tconst0utE81))) (head (emit-temp temp1A1G157 __t5PJH82)) dem_parse.slog:20 #f)
  class ReadTask2 : public slog::Task
  {
    ...
    slog::Index** tokindex1;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("temp1A1G157");
      std::vector<u16> ord3({1, 0, 2});
      slog::Relation* readrel4 = db->getRelation("parse");
      driver_index = readrel4->getIndex(ord3, true);
      std::vector<u16> ord5({1, 0});
      slog::Relation* readrel6 = db->getRelation("tok");
      tokindex1 = readrel6->getIndex(ord5, false);
  
    }
    ReadTask2(slog::Database* _db, u16 _b) : db(_db), bucket(_b) { bind(_db); }
    virtual bool work()
    {
      u64 v_c2 = v_const57cb45de78dd46c657524688;
      u64 v_c3 = v_consted725292f5f32d61535958c1;
      u64 v_c4 = v_const6b86b273ff34fce19d6b804e;
      u64 v_c5 = v_const66804d12743756eb85fb3115;
      ...
      bool _done = slog::join_probe_sliced<3,1>(driver_index, std::array<u64,3>{v_c2, 0, 0}, _sc, _rkey, _hr, [&](const std::array<u64,3>& m7) {
        u64 v_c6 = m7[1];
        u64 v_c7 = m7[2];
        if (buckethash(v_c6) != bucket) return;
        slog::join_probe<2,1>(tokindex1, std::array<u64,2>{v_c3, 0}, [&](const std::array<u64,2>& m8) {
          u64 v_c8 = m8[1];
          u64 v_c9 = _prim__0002b(db, v_c8, v_c4);
          if (v_c9 == slog_error) { slog::emit_pending_error(db, "dem_parse.slog:20"); return; }
          ++_fires;
      ...
      if (_fires) db->bumpFires("dem_parse.slog:20", "delta:parse", _fires);
  ...
  // (crule (pre) (scan mpz_overflow __erre2XwG148 __errf5g8e149 __errf2oxA150 __errf5cEQ151 __errf5Ibr152) (body) (head (emit error (0) __erre2XwG148)) <internal>:1 #f)
  ...
  // (crule (pre (let __tconst9Lfy57 const57cb45de78dd46c657524688)) (scan temp98xv156 __t31f456) (body) (head (mkstruct parse (1 2 0) __6iVP91 __tconst9Lfy57 __t31f456)) dem_parse.slog:16 #f)
  ...
  public:
    ...
    {
      head_rel[0] = db->getRelation("parse");
      outer_rel = db->getRelation("temp98xv156");
  
    }
    ReadTask12(slog::Database* _db, u16 _b) : db(_db), bucket(_b) { bind(_db); }
    virtual bool work()
    {
      u64 v_c15 = v_const57cb45de78dd46c657524688;
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c16 = _t[0];
        ++_fires;
        slog::emit_struct<3>(head_rel[0], newbatch[0], std::array<u64,2>{v_c15, v_c16}, std::array<u16,3>{1, 2, 0});
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("dem_parse.slog:16", "delta:temp98xv156", _fires);
  ...
  // (crule (pre (let __tconst9GD163 const43e6e2bebb3774311cf0c867)) (scan temp18Hd158 __t0R5j62) (body) (head (mkstruct parse (1 2 0) __9kLR94 __tconst9GD163 __t0R5j62)) dem_parse.slog:25 #f)
  ...
  public:
    ...
    {
      head_rel[0] = db->getRelation("parse");
      outer_rel = db->getRelation("temp18Hd158");
  
    }
    ReadTask13(slog::Database* _db, u16 _b) : db(_db), bucket(_b) { bind(_db); }
    virtual bool work()
    {
      u64 v_c17 = v_const43e6e2bebb3774311cf0c867;
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c18 = _t[0];
        ++_fires;
        slog::emit_struct<3>(head_rel[0], newbatch[0], std::array<u64,2>{v_c17, v_c18}, std::array<u16,3>{1, 2, 0});
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("dem_parse.slog:25", "delta:temp18Hd158", _fires);
  ...
  // (crule (pre) (scan int_overflow __erre8hkj132 __errf8fCz133 __errf2GwS134 __errf2nep135) (body) (head (emit error (0) __erre8hkj132)) <internal>:1 #f)
  ...
  // (crule (pre (let __tconst0p0n83 const66804d12743756eb85fb3115)) (scan temp1A1G157 __t5PJH82) (body) (head (mkstruct parse (1 2 0) __6CmH90 __tconst0p0n83 __t5PJH82)) dem_parse.slog:20 #f)
  ...
  public:
    ...
    {
      head_rel[0] = db->getRelation("parse");
      outer_rel = db->getRelation("temp1A1G157");
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c9 = _t[0];
        ++_fires;
        slog::emit_struct<3>(head_rel[0], newbatch[0], std::array<u64,2>{v_c5, v_c9}, std::array<u16,3>{1, 2, 0});
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("dem_parse.slog:20", "delta:temp1A1G157", _fires);
  ...
  // (crule (pre) (scan type_mismatch __erre2hky143 __errf0jHh144 __errf2S0x145 __errf2CVK146 __errf9YRY147) (body) (head (emit error (0) __erre2hky143)) <internal>:1 #f)
  ...
  // (crule (pre (let __tconst54Sc58 const43e6e2bebb3774311cf0c867) (let __tconst7bl760 const7297d2085ea0adffc396d546) (let __tconst8HE555 const6b86b273ff34fce19d6b804e) (let __tconst9Lfy57 const57cb45de78dd46c657524688)) (probe parse (1 0 2) 1 __tconst54Sc58 __t8K4e59 i) (body (join tok (1 0) 1 __tconst7bl760 j) (let __t31f456 (_0002b j __tconst8HE555))) (head (emit-temp temp98xv156 __t31f456)) dem_parse.slog:16 #f)
  class ReadTask22 : public slog::Task
  {
    ...
    std::array<u64,3> resume_key{};
    bool has_resume = false;
    slog::Index** tokindex21;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("temp98xv156");
      std::vector<u16> ord23({1, 0, 2});
      slog::Relation* readrel24 = db->getRelation("parse");
      driver_index = readrel24->getIndex(ord23, true);
      std::vector<u16> ord25({1, 0});
      slog::Relation* readrel26 = db->getRelation("tok");
      tokindex21 = readrel26->getIndex(ord25, false);
  
    }
    ReadTask22(slog::Database* _db, u16 _b) : db(_db), bucket(_b) { bind(_db); }
    virtual bool work()
    {
      u64 v_c28 = v_const43e6e2bebb3774311cf0c867;
      u64 v_c29 = v_const7297d2085ea0adffc396d546;
      ...
      slog::SliceCtx _sc{_slice_deadline, &db->runStopFlag()};
      std::array<u64,3> _rkey = resume_key; bool _hr = has_resume;
      bool _done = slog::join_probe_sliced<3,1>(driver_index, std::array<u64,3>{v_c28, 0, 0}, _sc, _rkey, _hr, [&](const std::array<u64,3>& m27) {
        u64 v_c31 = m27[1];
        u64 v_c7 = m27[2];
        if (buckethash(v_c31) != bucket) return;
        slog::join_probe<2,1>(tokindex21, std::array<u64,2>{v_c29, 0}, [&](const std::array<u64,2>& m28) {
          u64 v_c8 = m28[1];
          u64 v_c16 = _prim__0002b(db, v_c8, v_c30);
          if (v_c16 == slog_error) { slog::emit_pending_error(db, "dem_parse.slog:16"); return; }
          ++_fires;
      ...
      if (_fires) db->bumpFires("dem_parse.slog:16", "delta:parse", _fires);
  ...
  // (crule (pre (let __tconst7cMJ20 const57cb45de78dd46c657524688) (let __tconst9Q4h19 const66804d12743756eb85fb3115)) (probe parse (1 0 2) 1 __tconst7cMJ20 __t6YIX21 i) (body) (head (mkstruct parse (1 2 0) __7Nfr93 __tconst9Q4h19 i)) dem_parse.slog:21 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel31 = db->getRelation("parse");
    ...
    virtual bool work()
    {
      u64 v_c32 = v_const57cb45de78dd46c657524688;
      u64 v_c33 = v_const66804d12743756eb85fb3115;
      ...
      slog::SliceCtx _sc{_slice_deadline, &db->runStopFlag()};
      std::array<u64,3> _rkey = resume_key; bool _hr = has_resume;
      bool _done = slog::join_probe_sliced<3,1>(driver_index, std::array<u64,3>{v_c32, 0, 0}, _sc, _rkey, _hr, [&](const std::array<u64,3>& m32) {
        u64 v_c34 = m32[1];
        u64 v_c7 = m32[2];
        if (buckethash(v_c34) != bucket) return;
        ++_fires;
        slog::emit_struct<3>(head_rel[0], newbatch[0], std::array<u64,2>{v_c33, v_c7}, std::array<u16,3>{1, 2, 0});
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("dem_parse.slog:21", "delta:parse", _fires);
  ...
  // (crule (pre) (scan mpz_table_overflow __erre73AM153 __errf5gRw154 __errf5KZ4155) (body) (head (emit error (0) __erre73AM153)) <internal>:1 #f)
  ...
  // (crule (pre) (scan div_by_zero __erre7KLj126 __errf2B5T127 __errf6eAA128) (body) (head (emit error (0) __erre7KLj126)) <internal>:1 #f)
  ...
  // (crule (pre (let __tconst5bdj64 const66804d12743756eb85fb3115) (let __tconst4PQw66 const2c8b6d4a8b88a4444dd3bdd5) (let __tconst1NDR67 constc9216a4bb2623d4056d2c458) (let __tconst8PDV61 const6b86b273ff34fce19d6b804e) (let __tconst9GD163 const43e6e2bebb3774311cf0c867)) (probe parse (1 0 2) 1 __tconst5bdj64 __t58vr65 i) (body (join tok (1 0) 2 __tconst4PQw66 i) (join tok (1 0) 1 __tconst1NDR67 j) (let __t0R5j62 (_0002b i __tconst8PDV61))) (head (emit-temp temp18Hd158 __t0R5j62)) dem_parse.slog:25 #f)
  class ReadTask41 : public slog::Task
  {
    ...
    std::array<u64,3> resume_key{};
    bool has_resume = false;
    slog::Index** tokindex39;  slog::Index** tokindex40;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("temp18Hd158");
      std::vector<u16> ord42({1, 0, 2});
      slog::Relation* readrel43 = db->getRelation("parse");
      driver_index = readrel43->getIndex(ord42, true);
      std::vector<u16> ord44({1, 0});
      slog::Relation* readrel45 = db->getRelation("tok");
      tokindex39 = readrel45->getIndex(ord44, false);
      std::vector<u16> ord46({1, 0});
      slog::Relation* readrel47 = db->getRelation("tok");
      tokindex40 = readrel47->getIndex(ord46, false);
  
    }
    ReadTask41(slog::Database* _db, u16 _b) : db(_db), bucket(_b) { bind(_db); }
    virtual bool work()
    {
      u64 v_c41 = v_const66804d12743756eb85fb3115;
      u64 v_c42 = v_const2c8b6d4a8b88a4444dd3bdd5;
      u64 v_c43 = v_constc9216a4bb2623d4056d2c458;
      ...
      slog::SliceCtx _sc{_slice_deadline, &db->runStopFlag()};
      std::array<u64,3> _rkey = resume_key; bool _hr = has_resume;
      bool _done = slog::join_probe_sliced<3,1>(driver_index, std::array<u64,3>{v_c41, 0, 0}, _sc, _rkey, _hr, [&](const std::array<u64,3>& m48) {
        u64 v_c45 = m48[1];
        u64 v_c7 = m48[2];
        if (buckethash(v_c45) != bucket) return;
        slog::join_probe<2,2>(tokindex39, std::array<u64,2>{v_c42, v_c7}, [&](const std::array<u64,2>& m49) {
          slog::join_probe<2,1>(tokindex40, std::array<u64,2>{v_c43, 0}, [&](const std::array<u64,2>& m50) {
            u64 v_c8 = m50[1];
            u64 v_c18 = _prim__0002b(db, v_c7, v_c44);
            if (v_c18 == slog_error) { slog::emit_pending_error(db, "dem_parse.slog:25"); return; }
            ++_fires;
      ...
      if (_fires) db->bumpFires("dem_parse.slog:25", "delta:parse", _fires);
  ...
  // (crule (pre) (scan toint_range __erre2FDn140 __errf6fDR141 __errf8oNL142) (body) (head (emit error (0) __erre2FDn140)) <internal>:1 #f)
  ...
  // (crule (pre) (scan nan_result __erre3ydT136 __errf3U5K137 __errf9eBq138 __errf6T8b139) (body) (head (emit error (0) __erre3ydT136)) <internal>:1 #f)
  ...
  // (crule (pre (let __tconst1dge88 const43e6e2bebb3774311cf0c867) (let __tconst1pS887 const5feceb66ffc86f38d952786c)) (scan len n) (body) (head (mkstruct parse (1 2 0) __5Med92 __tconst1dge88 __tconst1pS887)) dem_parse.slog:29 #f)
  ...
  public:
    ...
    {
      head_rel[0] = db->getRelation("parse");
      outer_rel = db->getRelation("len");
  
    }
    ReadTask57(slog::Database* _db, u16 _b) : db(_db), bucket(_b) { bind(_db); }
    virtual bool work()
    {
      u64 v_c53 = v_const43e6e2bebb3774311cf0c867;
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c55 = _t[0];
        ++_fires;
        slog::emit_struct<3>(head_rel[0], newbatch[0], std::array<u64,2>{v_c53, v_c54}, std::array<u16,3>{1, 2, 0});
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("dem_parse.slog:29", "all:len", _fires);
  ...
  // (crule (pre (let __tconst9ktx69 const43e6e2bebb3774311cf0c867) (let __tconst9siy68 const57cb45de78dd46c657524688)) (probe parse (1 0 2) 1 __tconst9ktx69 __t1tRd70 i) (body) (head (mkstruct parse (1 2 0) __4M1O89 __tconst9siy68 i)) dem_parse.slog:17 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel60 = db->getRelation("parse");
    ...
    virtual bool work()
    {
      u64 v_c56 = v_const43e6e2bebb3774311cf0c867;
      u64 v_c57 = v_const57cb45de78dd46c657524688;
      ...
      slog::SliceCtx _sc{_slice_deadline, &db->runStopFlag()};
      std::array<u64,3> _rkey = resume_key; bool _hr = has_resume;
      bool _done = slog::join_probe_sliced<3,1>(driver_index, std::array<u64,3>{v_c56, 0, 0}, _sc, _rkey, _hr, [&](const std::array<u64,3>& m61) {
        u64 v_c58 = m61[1];
        u64 v_c7 = m61[2];
        if (buckethash(v_c58) != bucket) return;
        ++_fires;
        slog::emit_struct<3>(head_rel[0], newbatch[0], std::array<u64,2>{v_c57, v_c7}, std::array<u16,3>{1, 2, 0});
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("dem_parse.slog:17", "delta:parse", _fires);
  ...
  // (crule (pre) (scan modulo_by_zero __erre8e50129 __errf4tnU130 __errf13Mn131) (body) (head (emit error (0) __erre8e50129)) <internal>:1 #f)
  ...
  s->addReadRel("parse");
  s->addReadRel("temp18Hd158");
  s->addReadRel("temp1A1G157");
  s->addReadRel("temp98xv156");
  ...
  s->addDynamicRel("parse");
  s->addDynamicRel("temp18Hd158");
  s->addDynamicRel("temp1A1G157");
  s->addDynamicRel("temp98xv156");
