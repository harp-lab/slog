...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("191b43ea");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("m2"), std::array<u16,1>{0}, b));
  r = db->getRelation("m1");
  if (r == 0) db->addRelation("m1", 1);
  else if (r->getArity() != 1)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("m1");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("m1"), std::array<u16,1>{0}, b));
  r = db->getRelation("m0");
  if (r == 0) db->addRelation("m0", 1);
  else if (r->getArity() != 1)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("m0");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<1>(db, db->getRelation("m0"), std::array<u16,1>{0}, b));
  ...
  // (crule (pre) (scan mpz_table_overflow __erre3N0V68 __errf2b6J69 __errf7gdn70) (body) (head (emit error (0) __erre3N0V68)) <internal>:1 #f)
  ...
  // (crule (pre) (scan toint_range __erre4eIk55 __errf2Pxn56 __errf4dXT57) (body) (head (emit error (0) __erre4eIk55)) <internal>:1 #f)
  ...
  // (crule (pre) (scan nan_result __erre9UVx51 __errf5t1A52 __errf0BWI53 __errf14JZ54) (body) (head (emit error (0) __erre9UVx51)) <internal>:1 #f)
  ...
  // (crule (pre) (scan modulo_by_zero __erre63Lp44 __errf7Tr045 __errf6USf46) (body) (head (emit error (0) __erre63Lp44)) <internal>:1 #f)
  ...
  // (crule (pre (let __tconst5Bn08 const4a44dc15364204a80fe80e90) (let __tconst8QP99 const6b86b273ff34fce19d6b804e)) (scan m2 N) (body (cmp lt N __tconst5Bn08) (let __t1EcM10 (_0002b N __tconst8QP99))) (head (emit m0 (0) __t1EcM10)) strat_mutual.slog:14 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("m0");
      std::vector<u16> ord13({0});
      slog::Relation* readrel14 = db->getRelation("m0");
      head_index[0] = readrel14->getIndex(ord13, false);
      outer_rel = db->getRelation("m2");
  
    }
    ReadTask12(slog::Database* _db, u16 _b) : db(_db), bucket(_b) { bind(_db); }
    virtual bool work()
    {
      u64 v_c13 = v_const4a44dc15364204a80fe80e90;
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        ...
        if (v_c16 == slog_error) { slog::emit_pending_error(db, "strat_mutual.slog:14"); return; }
        if (!v_c16) return;
        u64 v_c17 = _prim__0002b(db, v_c15, v_c14);
        if (v_c17 == slog_error) { slog::emit_pending_error(db, "strat_mutual.slog:14"); return; }
        ++_fires;
      ...
      if (_fires) db->bumpFires("strat_mutual.slog:14", "delta:m2", _fires);
  ...
  // (crule (pre) (scan div_by_zero __erre7y0r41 __errf4zSa42 __errf97ox43) (body) (head (emit error (0) __erre7y0r41)) <internal>:1 #f)
  ...
  // (crule (pre) (scan int_overflow __erre3GhG47 __errf0Wwi48 __errf3Tib49 __errf0hac50) (body) (head (emit error (0) __erre3GhG47)) <internal>:1 #f)
  ...
  // (crule (pre) (scan type_mismatch __erre2n9358 __errf8usP59 __errf280360 __errf9kms61 __errf8HI362) (body) (head (emit error (0) __erre2n9358)) <internal>:1 #f)
  ...
  // (crule (pre) (scan mpz_overflow __erre9mXt63 __errf75FH64 __errf33yl65 __errf30jD66 __errf511E67) (body) (head (emit error (0) __erre9mXt63)) <internal>:1 #f)
  ...
  // (crule (pre (let __tconst9Frp1 const4a44dc15364204a80fe80e90) (let __tconst35cg2 const6b86b273ff34fce19d6b804e)) (scan m1 N) (body (cmp lt N __tconst9Frp1) (let __t6LE13 (_0002b N __tconst35cg2))) (head (emit m2 (0) __t6LE13)) strat_mutual.slog:13 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("m2");
      std::vector<u16> ord29({0});
      slog::Relation* readrel30 = db->getRelation("m2");
      head_index[0] = readrel30->getIndex(ord29, false);
      outer_rel = db->getRelation("m1");
  
    }
    ReadTask28(slog::Database* _db, u16 _b) : db(_db), bucket(_b) { bind(_db); }
    virtual bool work()
    {
      u64 v_c35 = v_const4a44dc15364204a80fe80e90;
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c15 = _t[0];
        u64 v_c37 = _prim_lt(db, v_c15, v_c35);
        if (v_c37 == slog_error) { slog::emit_pending_error(db, "strat_mutual.slog:13"); return; }
        if (!v_c37) return;
        u64 v_c38 = _prim__0002b(db, v_c15, v_c36);
        if (v_c38 == slog_error) { slog::emit_pending_error(db, "strat_mutual.slog:13"); return; }
        ++_fires;
      ...
      if (_fires) db->bumpFires("strat_mutual.slog:13", "delta:m1", _fires);
  ...
  // (crule (pre (let __tconst1A1z5 const4a44dc15364204a80fe80e90) (let __tconst0KaC6 const6b86b273ff34fce19d6b804e)) (scan m0 N) (body (cmp lt N __tconst1A1z5) (let __t1lF77 (_0002b N __tconst0KaC6))) (head (emit m1 (0) __t1lF77)) strat_mutual.slog:12 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("m1");
      std::vector<u16> ord33({0});
      slog::Relation* readrel34 = db->getRelation("m1");
      head_index[0] = readrel34->getIndex(ord33, false);
      outer_rel = db->getRelation("m0");
  
    }
    ReadTask32(slog::Database* _db, u16 _b) : db(_db), bucket(_b) { bind(_db); }
    virtual bool work()
    {
      u64 v_c39 = v_const4a44dc15364204a80fe80e90;
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c15 = _t[0];
        u64 v_c41 = _prim_lt(db, v_c15, v_c39);
        if (v_c41 == slog_error) { slog::emit_pending_error(db, "strat_mutual.slog:12"); return; }
        if (!v_c41) return;
        u64 v_c42 = _prim__0002b(db, v_c15, v_c40);
        if (v_c42 == slog_error) { slog::emit_pending_error(db, "strat_mutual.slog:12"); return; }
        ++_fires;
      ...
      if (_fires) db->bumpFires("strat_mutual.slog:12", "delta:m0", _fires);
  ...
  // (crule (pre (let __tconst3ILv4 const5feceb66ffc86f38d952786c)) (once) (body) (head (emit m0 (0) __tconst3ILv4)) strat_mutual.slog:9 #f)
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel38 = db->getRelation("m0");
    ...
    virtual bool work()
    {
      ...
      if (_fires) db->bumpFires("strat_mutual.slog:9", "once", _fires);
