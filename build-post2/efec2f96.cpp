...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("efec2f96");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  r = db->getRelation("tedge");
  if (r == 0) db->addRelation("tedge", 3);
  else if (r->getArity() != 3)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("tedge");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<3>(db, db->getRelation("tedge"), std::array<u16,3>{0, 1, 2}, b));
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("sedge"), std::array<u16,2>{1, 0}, b));
  r = db->getRelation("redge");
  if (r == 0) db->addRelation("redge", 2);
  else if (r->getArity() != 2)
    slog::fatal("Relation already exists at incorrect arity.");
  r = db->getRelation("redge");
  ...
  for (u16 b = 0; b < 32; ++b)
    s->addTask(phase_intern, new slog::InternTask<2>(db, db->getRelation("redge"), std::array<u16,2>{0, 1}, b));
  ...
  // (crule (pre) (scan tedge C A W) (body (join3 B (full sedge (1 0) 1 () C B) (full redge (0 1) 1 () A B))) (head (emit tri (0 1 2) A B C)) sj_tri.slog:25 #f)
  class ReadTask2 : public slog::Task
  {
    ...
    slog::Index** sedgeindex0;  slog::Index** redgeindex1;
  ...
  public:
    void bind(slog::Database* db)
    {
      ...
      slog::Relation* readrel4 = db->getRelation("tri");
      head_index[0] = readrel4->getIndex(ord3, false);
      outer_rel = db->getRelation("tedge");
      std::vector<u16> ord5({1, 0});
      slog::Relation* readrel6 = db->getRelation("sedge");
      sedgeindex0 = readrel6->getIndex(ord5, false);
      std::vector<u16> ord7({0, 1});
      slog::Relation* readrel8 = db->getRelation("redge");
      redgeindex1 = readrel8->getIndex(ord7, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        ...
        u64 v_c1 = _t[1];
        u64 v_c2 = _t[2];
        slog::join3<2,1,slog::Join3View::full,2,1,slog::Join3View::full>(sedgeindex0, sedgeindex0, std::array<u64,2>{v_c0, 0}, redgeindex1, redgeindex1, std::array<u64,2>{v_c1, 0}, [&](u64 cycle9) {
        u64 v_c3 = cycle9;
          ++_fires;
          slog::emit<3>(head_rel[0], head_index[0], newbatch[0], std::array<u64,3>{v_c1, v_c3, v_c0}, std::array<u16,3>{0, 1, 2});
        });
      });
  
        head_rel[0]->sendBatch(newbatch[0]);
  
      if (_fires) db->bumpFires("sj_tri.slog:25", "all:tedge", _fires);
