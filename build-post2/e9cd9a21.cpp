...
extern "C" void slog_plugin(slog::Daemon* d)
{
  ...
  slog::Stratum* s = d->beginStratum("e9cd9a21");
  if (s == nullptr) return;
  slog::Relation* r;
  ...
  // (crule (pre) (scan viafact P) (body (join m (0) 1 P)) (head (emit samefact (0) P)) sdeep_share.slog:25 #f)
  ...
  public:
    ...
    {
      head_rel[0] = db->getRelation("samefact");
      std::vector<u16> ord2({0});
      slog::Relation* readrel3 = db->getRelation("samefact");
      head_index[0] = readrel3->getIndex(ord2, false);
      outer_rel = db->getRelation("viafact");
      std::vector<u16> ord4({0});
      slog::Relation* readrel5 = db->getRelation("m");
      mindex0 = readrel5->getIndex(ord4, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        slog::join_probe<1,1>(mindex0, std::array<u64,1>{v_c0}, [&](const std::array<u64,1>& m6) {
          ++_fires;
      ...
      if (_fires) db->bumpFires("sdeep_share.slog:25", "all:viafact", _fires);
  ...
  // (crule (pre) (scan computed P) (body (join given2 (0) 1 P)) (head (emit fsame (0) P)) sdeep_share.slog:30 #f)
  class ReadTask8 : public slog::Task
  {
    ...
    slog::Index** given2index7;
  
  public:
    void bind(slog::Database* db)
    {
      head_rel[0] = db->getRelation("fsame");
      std::vector<u16> ord9({0});
      slog::Relation* readrel10 = db->getRelation("fsame");
      head_index[0] = readrel10->getIndex(ord9, false);
      outer_rel = db->getRelation("computed");
      std::vector<u16> ord11({0});
      slog::Relation* readrel12 = db->getRelation("given2");
      given2index7 = readrel12->getIndex(ord11, false);
    ...
    virtual bool work()
    {
      ...
      bool _done = slog::read_delta_sliced(outer_rel, bucket, db->getThreadCount(), _sc, _rt, _ri, [&](const u64* _t) {
        u64 v_c0 = _t[0];
        slog::join_probe<1,1>(given2index7, std::array<u64,1>{v_c0}, [&](const std::array<u64,1>& m13) {
          ++_fires;
          slog::emit<1>(head_rel[0], head_index[0], newbatch[0], std::array<u64,1>{v_c0}, std::array<u16,1>{0});
        });
      });
      ...
      if (_fires) db->bumpFires("sdeep_share.slog:30", "all:computed", _fires);
  ...
  s->addReadRel("computed");
  s->addReadRel("given2");
  s->addReadRel("m");
  s->addReadRel("viafact");
  s->addDynamicRel("fsame");
  s->addDynamicRel("samefact");
  d->push(s);
  d->continueRun();
}
